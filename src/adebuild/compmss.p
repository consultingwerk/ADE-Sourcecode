...
/*
** Procedure compmss.p
** Sets things up so that the mss dictionary parts can be compiled.
**
*/
...
IF AVAILABLE DICTDB._Db THEN DO:
    IF DICTDB._Db._Db-type  = "MSS" THEN RETURN.
    DELETE DICTDB._Db.
END.

IF NOT AVAILABLE DICTDB._Db THEN DO:
    IF CAN-DO(GATEWAYS,"MSS") THEN DO:
   CREATE DICTDB._Db.                         
   ...
   RUN prodict/mss/_mss_sys.p (RECID(DICTDB._Db), 
                  INPUT-OUTPUT system).
    END.
END.
RETURN.
