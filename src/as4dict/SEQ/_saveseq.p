...
PROCEDURE Add_to_List:
   ...
   find FIRST as4dict.p__Seq where  as4dict.p__Seq._Seq-Name > p_name
      NO-ERROR.

   ins_name = (if AVAILABLE as4dict.p__Seq then as4dict.p__Seq._Seq-name else "").
...
run as4dict/_blnknam.p (INPUT p_NameHdl, INPUT "sequence", OUTPUT no_name).
...
do ON ERROR UNDO, LEAVE  ON STOP UNDO, LEAVE:
   ...
   if s_Adding then
      assign
      	 b_Sequence._Db-recid = 1
      	 b_Sequence._Seq-num = initval.
 
   assign
      b_Sequence._Seq-Name = CAPS(newname)
      b_Sequence._Seq-Init = initval
      b_Sequence._Seq-Incr = p_Incr
      b_Sequence._Cycle-Ok = (if p_Cycle then "Y" else "N")
      s_Seq_Limit = p_Limit.
