...
Date Created: 01/28/92  
     Modified: 01/1995 to run with PROGRESS/400 Data Dictionary
               ...
               03/16/99 Added allow_rep_trig variable for replication triggers
               03/30/99 Added stored procedure support
               02/07/00 Added chgpf support 91B
               10/25/00 Added allow_raw and allow_ench
...
Define {1} var s_CurrProc as char init "" NO-UNDO.
Define {1} var s_CurrParm as char init "" NO-UNDO.
...
Define {1} var s_ProcRecId  as recid    NO-UNDO.
...
Define {1} var s_ProcForNo as integer NO-UNDO.
Define {1} var s_ParmForNo AS integer NO-UNDO.
...
Define {1} var allow_rep_trig as logical NO-UNDO.
Define {1} var allow_st_proc as logical NO-UNDO.
DEFINE {1} VAR allow_chgpf   AS LOGICAL NO-UNDO.
DEFINE {1} VAR allow_raw     AS LOGICAL NO-UNDO.
DEFINE {1} VAR allow_enhdba  AS LOGICAL NO-UNDO.
...
&global-define  OBJ_PROC    7
&global-define  OBJ_PARM    8
...
Define {1} var s_Proc_Cached as logical init false NO-UNDO.
Define {1} var s_Parm_Cached as logical init false NO-UNDO.
...
Define {1} var s_win_Parm   as widget-handle NO-UNDO.
Define {1} var s_win_Proc   as widget-handle NO-UNDO.

/* To remember where each window was when it was closed, so it
   can reopen in the same place.
*/
Define {1} var s_x_Win as integer extent 8 NO-UNDO init ?.
Define {1} var s_y_Win as integer extent 8 NO-UNDO init ?.
...
Define {1} var s_Proc_ReadOnly   as logical NO-UNDO.
Define {1} var s_Parm_Readonly   as logical NO-UNDO.
...
DEFINE {1} VARIABLE dump-lib    AS LOGICAL INITIAL FALSE NO-UNDO. 
