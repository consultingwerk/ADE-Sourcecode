...
/* rtvdara.p
   Entry procedure for API to the AS/400 to work with Data Areas for receiving
...
IF data-length > 2048 THEN DO:
  ...
  RETURN.
END.
IF (start-pos + data-length - 1) > 2048 THEN DO:
  ASSIGN stat = -5.
  DELETE alias as4dict.
  RETURN.
END.
      
RUN as4dict/qdtaara.p (INPUT "R", INPUT area-name, INPUT lib-name,
