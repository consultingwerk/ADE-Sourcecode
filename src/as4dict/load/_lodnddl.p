...
/* _lodnddl.p - load .df schema definitions file */
...
out: user_env[35] = s to know that load was stopped and not to display load complete.     
*/
/* 
History:
 
     mcmann    12/18/95 Copied as4dict/load/_lod_ddl.p and modified to work with
                                           temporary tables to process load before creating the
                                           DB2/400 database. 
               06/25/97 Changed form to be view-as dialog-box   
               08/07/97 Added new DBA command for word index support D. McMann
               09/11/97 Added assignment null capable field _fld-misc2[2] when
                        AS/400 format and not specifically noted as null capable
               11/25/97 Added code to recoginze AREA and ignor V9 only.
               01/12/98 Added code to recoginze POSITION
               11/30/98 Moved p__db find 98-08-21-022
...
ASSIGN
  ...
  error_text[26] = "Unable to create word index objects, index not loaded.":t72  error_text[27] = "Incompatible translation table version - Load will be terminated":t72
...
FORM
   ...
   WITH FRAME working2 OVERLAY CENTERED BGCOLOR 8 USE-TEXT.

FORM
   "Error occured during load. Backing out - Please wait ..."
   WITH FRAME backout OVERLAY CENTERED BGCOLOR 8 USE-TEXT.
...
IF FALSE THEN FIND NEXT wtp__File.
IF FALSE THEN FIND NEXT wtp__Field.
IF FALSE THEN FIND NEXT wtp__Index.
 ...
 DO ON STOP UNDO, LEAVE:
    ...
    REPEAT ON ERROR UNDO,RETRY ON ENDKEY UNDO, LEAVE:
        ...
        IF ierror > 0 AND NOT inoerror THEN DO:
            IF table_stat < 2 THEN RUN Show_Error ("curr").
        ...
        */
        ...
        ilin[1]) THEN DO:
            ...
            IF table_stat = 0 THEN DO:
                IF AVAILABLE wfil THEN RUN "as4dict/load/_lodnfil.p".             
                IF AVAILABLE wfld THEN RUN "as4dict/load/_lodnfld.p".
                IF AVAILABLE widx THEN RUN "as4dict/load/_lodnidx.p".
	...
	ASSIGN wseq._Seq-Name = ilin[3].
            ELSE DO:
                ...
                IF NOT AVAILABLE as4dict.p__Seq THEN DO:                       
                    ASSIGN ierror = 3. /* "Try to modify unknown &2" */
        ...
        ELSE DO:
            ...
            IF iobj = "t"  THEN 
                ASSIGN scrap = ilin[3].
            ELSE IF CAN-DO("OF,ON",ilin[4]) THEN 
                ASSIGN scrap = ilin[5].
            ELSE IF CAN-DO("OF,ON",ilin[5]) THEN 
                ASSIGN scrap = ilin[6].
            ELSE IF CAN-DO("OF,ON",ilin[6]) THEN 
                ASSIGN scrap = ilin[7].
            ELSE IF CAN-DO("OF,ON",ilin[7]) THEN 
                ASSIGN scrap = ilin[8].

            IF scrap = ? THEN .
            ELSE DO:               
                IF AVAILABLE as4dict.p__Db THEN
                    FIND FIRST wtp__File WHERE TRIM(wtp__File._File-name) = scrap NO-ERROR.
            END.
            IF AVAILABLE wtp__File AND NOT AVAILABLE as4dict.p__Db THEN DO:
                FIND FIRST as4dict.p__Db.
                ASSIGN drec_db = RECID(as4dict.p__Db).
            END.
            IF scrap <> ? AND AVAILABLE wtp__File THEN 
                ASSIGN drec_file = RECID(wtp__File).
        ...
        IF iobj = "t" THEN DO: /* start file action block */
		 ...
		 "" @ widx._Index-name "" @ wseq._Seq-Name
	  	WITH FRAME working.
            ...
            ELSE DO:
                IF NOT AVAILABLE wtp__File THEN DO:
                    ASSIGN table_stat = 1
                                     ierror = 3. /* "Try to modify unknown &2" */
                    UNDO,RETRY load_loop.
                END.  
                ASSIGN table_stat = 0.
                { as4dict/load/copy_fil.i &from= wtp__File &to=wfil &all=true}
                /* Check for read-only file if not adding */
                IF SUBSTRING(wtp__file._Fil-Misc2[4],8,1) = "Y" THEN DO:  
                ...
                FOR EACH wtp__trgfl WHERE wtp__Trgfl._File-number = wtp__File._File-number:
                    CREATE wfit.
                    { as4dict/load/copy_fit.i &from= wtp__trgfl &to=wfit }
                END.
            END.  /* modify table */
        ...
        IF iobj = "f" THEN DO: /* start field action block */
            IF pfilenumber <> ? THEN
	   FIND wtp__File WHERE wtp__File._File-number = pfilenumber NO-ERROR.
            IF NOT AVAILABLE wtp__File THEN DO:
                IF table_stat > 0 THEN 
                    ASSIGN table_stat = 2
 	                    ierror = 5. /* "Try to modify &2 without file" */
                UNDO,RETRY load_loop.
             END.                           
             IF TERMINAL <> "" THEN 
	       DISPLAY wtp__File._File-name @ wfil._File-name
	  ...
	  ASSIGN  wfld._Field-name = ilin[3]    
                ...
                ELSE DO:
                    ...
                    FIND wtp__Field WHERE wtp__Field._File-number = wtp__File._File-number
                                                            AND TRIM(wtp__Field._Field-name) = ilin[3] NO-ERROR.
 
 	  IF NOT AVAILABLE wtp__Field THEN DO:
                        IF table_stat > 0 THEN 
                            ASSIGN table_stat = 2
	                            ierror = 3. /* "Try to modify unknown &2" */
                        UNDO,RETRY load_loop.
	  END.
	  { as4dict/load/copy_fld.i &from= wtp__Field &to=wfld &all=true}
	  FOR EACH wtp__Trgfd  WHERE wtp__trgfd._file-number = wtp__Field._File-number
	      ...
	      CREATE wflt.
	      { as4dict/load/copy_flt.i &from=wtp__Trgfd &to=wflt }
	  END.
               END.
        END. /* end field action block */
  
        IF iobj = "i" THEN DO: /* start index action block */
            IF NOT AVAILABLE wtp__File AND drec_file <> ? THEN
	    FIND wtp__File WHERE drec_file = RECID(wtp__File) NO-ERROR.
            IF NOT AVAILABLE wtp__File THEN DO:
                IF table_stat > 0 THEN 
                    ASSIGN table_stat = 2
	    ...
	    ASSIGN widx._Index-name = ilin[3]
            ...
            ELSE DO:
                FIND wtp__Index WHERE TRIM(wtp__Index._Index-name) = ilin[3] 
                                                        AND wtp__Index._file-number = wtp__file._file-number NO-ERROR.
                IF NOT AVAILABLE wtp__Index and imod <> "d" THEN DO:
                    IF table_stat > 0 THEN 
                        ASSIGN table_stat = 2
                                          ierror = 3. /* "Try to modify unknown &2" */
                    UNDO,RETRY load_loop.
                END.
                ELSE IF AVAILABLE (wtp__index) THEN DO:
                    IF SUBSTRING(wtp__Index._I-Misc2[4],8,1) = "Y" THEN DO:  
                        ASSIGN ierror = 48. /* Unable to modify - object is read only */
                        UNDO,RETRY load_loop.
                    END.                                    
                    { as4dict/load/copy_idx.i &from= wtp__Index &to=widx }
                    FOR EACH wtp__Idxfd WHERE wtp__Idxfd._File-number = wtp__Index._File-number
                                                                        AND wtp__Idxfd._Idx-num = wtp__Index._Idx-num:
                        CREATE wixf.
                        { as4dict/load/copy_ixf.i &from=wtp__Idxfd &to=wixf }
                    END.
    ...
    */
	  ...
	  WHEN "AREA" THEN. /* Original-area not used */
	...
	WHEN    "AS400-TYPE" OR WHEN "FOREIGN-TYPE" THEN wfld._For-Type      = iarg.
    ...
    WHEN    "MAX-GLYPHS"             THEN wfld._Fld-misc1[5]  = INTEGER(iarg).
	  ...
	  WHEN "INDEX-FIELD" OR WHEN "KEY-FIELD" THEN DO:
	       FIND wtp__Field
	           WHERE wtp__Field._File-number = pfilenumber
	                 AND TRIM(wtp__Field._Field-name) = ilin[2] NO-ERROR.
	       IF imod <> "a":u THEN ierror = 12.
	      /* "Cannot add index field to existing index" */
	       IF NOT AVAILABLE wtp__Field THEN ierror = 13. /* "Cannot find field to index" */
	       IF ierror > 0 THEN UNDO,RETRY load_loop.	     
	       CREATE wixf.
	       ASSIGN icomponent        = icomponent + 1
	                       wixf._Index-Seq   = icomponent
	                       wixf._Field-recid = RECID(wtp__Field)    
	                       wixf._Fld-number = wtp__Field._Fld-number
	                       wixf._Ascending   = "Y".
  ...
  IF stopped  THEN DO:
      run adecomm/_setcurs.p ("").
      MESSAGE "Load terminated, information being backed out."
        	   VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.         
        ASSIGN user_env[35] = "s".
   END.
   ELSE DO:  /* all but last definition-set executed */
        IF NOT (ierror > 0 AND user_env[4] BEGINS "y":u) THEN
           ...
           IF NOT xerror THEN DO:
               RUN "as4dict/load/_lodfini.p".
               RUN as4dict/load/_schsync.p.
           END.    
  ...
  IF TERMINAL <> "" THEN DO:  /* TERMINAL <> "" */
    ...
    IF  (xerror or stopped)  THEN DO:  
        user_env[35] = "s".          
        if xerror THEN
            MESSAGE  "Error occurred, information being backed out"
            VIEW-AS ALERT-BOX ERROR.    
         assign user_path = "*R":u.              
       run adecomm/_setcurs.p ("wait").  
     ...
     ELSE DO:
        ...
        IF NOT User_Env[28] BEGINS "y" AND diag_errs THEN 
            IF User_env[27] BEGINS "y" THEN 
                MESSAGE "Error(s) occurred or warnings issued during the load"
                                      "process which were not displayed to the screen."
                                      "Some definitions may not have been loaded.  Check"
                                      "error file " fil-e "in your working directory"
                    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
            ELSE
                MESSAGE "Error(s) occurred or warnings issued during the load" 
                                      "process which were not displayed to the screen. "
                                      "Some definitions may not have been loaded. " 
                    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
       END.                
   END.     /* TERMINAL <> "" */
