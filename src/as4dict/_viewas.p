...
Date Created: 08/04/92 
    Modified: 04/13/98 D. McMann added logical to statement when p_Dtype = 0.
                       98-04-13-030
...
*/
Define var ViewAs_Relevant as char extent 6 NO-UNDO init
...
Define button btn_Copy   
   ...
   &ELSE
      label "&Copy Selected Syntax" size 22 by 1.
   &ENDIF
   ...
   do:
      ...
      assign
      	 p_Viewas:CURSOR-OFFSET in frame fld_viewas = 
            INTEGER(compiler:FILE-OFFSET) - 17 - LENGTH(p_DTypeStr).
...
*/            
if p_Dtype = 0 THEN DO:             
  find b_Field where b_Field._File-number = s_TblForNo
               and b_Field._Field-name = s_CurrFld.   
                   
   IF b_field._Data-type = "character" THEN ASSIGN p_Dtype = 1.
   ELSE   IF b_field._Data-type = "date" THEN ASSIGN p_Dtype = 2.
   ELSE IF b_field._Data-type = "logical" THEN ASSIGN p_Dtype = 3. 
   ELSE IF b_field._Data-type = "integer" THEN ASSIGN p_Dtype = 4.
   ELSE   IF b_field._Data-type = "decimal" THEN ASSIGN p_Dtype = 5.
end. 
...
if p_Viewas = "" OR p_Viewas = ? then
   ...
   do ix_types = 1 to NUM-ENTRIES(ViewAs_Relevant[ix_DTypes]):
      ...
      */
      len = LENGTH(widgtype).
      if SUBSTR(p_Viewas, 9, len) = widgtype then
      do:
