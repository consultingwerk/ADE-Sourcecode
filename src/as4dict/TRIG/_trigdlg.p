...
mcmann  95/01/01    Converted to work with PROGRESS/400 Data Dictionary
mcmann  95/11/03    Fixed bug with delete not assigning value for CRC to change    
mcmann  96/07/25    Added assign of _Fil-misc2[5][1] = Y for ade maintained
mcmann  96/10/25    Corrected assignment to know if CRC should be checked. 96-03-28-021
mcmann  97/03/12    Bug 97-03-11-052 in Read-only mode dictionary was being set to
                    dirty from viewing the triggers even though the user could not
                    change.
mcmann 99/02/04     Added replcations support.                    

----------------------------------------------------------------------------*/
{as4dict/dictvar.i shared}
{as4dict/menu.i shared}
{as4dict/uivar.i shared} /* Common UI definitions */
...
Define shared buffer b_File  for as4dict.p__File.
Define shared buffer b_Field for as4dict.p__Field.
Define        buffer b_ttrig for as4dict.p__Trgfl.
Define        buffer b_ftrig for as4dict.p__Trgfd.

/* Trigger attributes for form */
Define var trig_event      as char    format "x(22)"                 
...
Define button btn_file    label "Fi&les..." SIZE 11 by 1.
...
FORM
   ...
   "Event:"       at  2 view-as TEXT
   "Procedure:"   at 30 view-as TEXT
   "Options:"     at 64 view-as TEXT
   trig_crc       at 73 LABEL "&Check CRC" 
   SKIP({&VM_WID})

   trig_event     at  2 NO-LABEL {&STDPH_FILL} SPACE(0)
   trig_proc      at 30 NO-LABEL {&STDPH_FILL} SPACE(.3)
   btn_File                        
...
DEFINE VARIABLE db2-event AS CHARACTER  NO-UNDO.

   assign
      new_trig = false
      trig_event = caps(trig_event:screen-value in frame trigedit)
      old_crc_val = ?.
   ...
   CASE trig_event:
	    WHEN "CREATE" OR WHEN "DELETE" OR WHEN "WRITE" OR WHEN "FIND" THEN
	      ASSIGN db2-event = trig_event.
	    WHEN "REPLICATION-CREATE" THEN
	      ASSIGN db2-event = "RCREAT".
	    WHEN "REPLICATION-DELETE" THEN
	      ASSIGN db2-event = "RDELET".
	    WHEN "REPLICATION-WRITE" THEN
	      ASSIGN db2-event = "RWRITE".
	  END CASE.    

      find b_ttrig where b_ttrig._File-Number = b_file._File-number
            AND b_ttrig._Event = db2-event NO-ERROR.
      if AVAILABLE b_ttrig then
	 assign
	    trig_proc      = b_ttrig._Proc-Name
	    trig_override  = 
	          (if b_ttrig._Override = "Y" then yes else no)
	    trig_crc       = (if b_ttrig._Trig-CRC = 0 OR
	                         b_ttrig._Trig-CRC = ? then no else yes)
	    old_crc_val    = b_ttrig._Trig-CRC.
      else
	 new_trig = true.
   end.
   else do:
      find b_ftrig where  b_ftrig._File-number = b_field._File-number
                                  and b_ftrig._Fld-number = b_field._Fld-number
                                  and b_ftrig._Event = trig_event NO-ERROR.
      if AVAILABLE b_ftrig then
	 assign
	    trig_proc      = b_ftrig._Proc-Name
	    trig_override  = 
	        (if b_ftrig._Override = "Y" then yes else no)
	    trig_crc       = (if b_ftrig._Trig-CRC = 0 OR
	                         b_ftrig._Trig-CRC = ? then no else yes)
...
PROCEDURE Save_Trigger:
   ...
   do ON ERROR UNDO, LEAVE  ON STOP UNDO, LEAVE:
	 ...
	 */
	 ...
	 create b_ttrig.
	 assign
	    b_ttrig._File-Number = b_File._File-number  
	    b_ttrig._File-recid = b_File._File-number
	    b_ttrig._Proc-Name = tproc
	    b_ttrig._Override = 
	       (if input frame trigedit trig_override then "Y" else "N")
	    b_ttrig._Trig-CRC = (if NOT crc then ? else new_crc_val).    
	  
	  CASE trig_event:
	    WHEN "CREATE" OR WHEN "DELETE" OR WHEN "WRITE" OR WHEN "FIND" THEN
	      ASSIGN b_ttrig._Event = trig_event.
	    WHEN "REPLICATION-CREATE" THEN
	      ASSIGN b_ttrig._Event = "RCREAT".
	    WHEN "REPLICATION-DELETE" THEN
	      ASSIGN b_ttrig._Event = "RDELET".
	    WHEN "REPLICATION-WRITE" THEN
	      ASSIGN b_ttrig._Event = "RWRITE".
	  END CASE.    
	    
	  /* Assign b_File._Fil-misc1[1] so sync will know file has changed */
	  ASSIGN b_file._Fil-Misc1[1] = b_File._Fil-Misc1[1] + 1
	 ...
	 create b_ftrig.
	 assign
	    b_ftrig._File-number = b_Field._File-number
	    b_ftrig._Fld-number = b_Field._Fld-number     
	    b_ftrig._Field-recid = b_Field._Fld-number
	    b_ftrig._Event = caps(trig_event)
	    b_ftrig._Proc-Name = tproc
	    b_ftrig._Override = 
	       (if input frame trigedit trig_override = yes then "Y" else "N")
...
do:
   ...
   IF can_update then DO:
     run Do_Ok (INPUT false, OUTPUT success).
     if NOT success then return NO-APPLY.
   END.
   ELSE
     RETURN NO-APPLY.  
end.
...
do:
   ...
   */
   ...
   do:
      ...
      */
      ASSIGN trig_event:screen-value in frame trigedit = caps(trig_event).
      return NO-APPLY.
   end.
END.
...
IF NOT s_ReadOnly and p_Obj = {&OBJ_TBL} 
         and substring(b_file._Fil-Misc2[4],1,1) = "Y"  THEN
       MESSAGE "There are DB2/400 Triggers defined for this table."
           view-as alert-box warning button ok. 

IF p_Obj = {&OBJ_TBL} and (allow_rep_trig) THEN 
    ASSIGN p_Events = p_Events + 
...
frame trigedit:title = (if p_Obj = {&OBJ_TBL} then "AS/400 Table Triggers"
					      else "AS/400 Field Triggers").
