...
FORM
  as4dict.p__File._File-name FORMAT "x(32)" LABEL "Table" AT 2 ATTR-SPACE
  SPACE(0) fil            FORMAT "x(33)" LABEL "Dump File"
  msg                     FORMAT "x(8)"  LABEL "Records" ATTR-SPACE SPACE(1)
  HEADER 
    " Dumping Data.   Press " + 
    KBLABEL("STOP") + " to terminate the dump process." format "x(66)" SKIP(1)
  WITH FRAME dumpdata NO-ATTR-SPACE USE-TEXT SCROLLABLE
  SCREEN-LINES - 8 DOWN ROW 2 CENTERED.
...
DO ON STOP UNDO, LEAVE:
  DO FOR as4dict.p__File WHILE ENTRY(1,user_env[1]) <> "" ON ERROR UNDO,NEXT:
    FIND as4dict.p__File
      WHERE as4dict.p__File._File-name = ENTRY(1,user_env[1]).
    ...
    ASSIGN
      fil  = user_env[2] + (IF lots THEN
	     (IF as4dict.p__file._Dump-name = ? THEN 
	        as4dict.p__file._File-name ELSE as4dict.p__file._Dump-name) + ".d" ELSE "")
      loop = FALSE
      recs = 0.
  
    DISPLAY as4dict.p__File._File-name fil "Dumping" @ msg WITH FRAME dumpdata.
    COLOR DISPLAY MESSAGES as4dict.p__File._File-name fil msg
      WITH FRAME dumpdata.
    ...
    IF as4dict.p__File._Prime-Index <> ? /* AND user_dbtype = "PROGRESS" */ THEN DO:
      FIND as4dict.p__Index WHERE 
            as4dict.p__Index._file-number = as4dict.p__file._file-number AND
            as4dict.p__Index._idx-num = as4dict.p__File._Prime-Index.
      IF as4dict.p__Index._Active <> "Y" THEN DO:
	FIND FIRST as4dict.p__Index WHERE 
	   as4dict.p__index._file-number = as4dict.p__File._file-number 
	   AND as4dict.p__Index._Active = "Y" NO-ERROR.
	IF NOT AVAILABLE as4dict.p__Index THEN DO:
	  DISPLAY "Error!" @ msg WITH FRAME dumpdata.
	  COLOR DISPLAY NORMAL as4dict.p__File._File-name fil msg WITH FRAME dumpdata.
	  MESSAGE
	    "Cannot dump data when all indexes for a table inactive.".
	  NEXT.
	END.
	useix = "USE-INDEX " + as4dict.p__Index._Index-name.
      END.
    END.
    ...
    message "before rundump" user_env[4] useix.
    RUN "prodict/misc/_rundump.i" (INPUT ENTRY(1, user_env[4]))
  			  VALUE(as4dict.p__File._File-name) VALUE(useix).
...
/*------------------ Trailer-INFO ------------------*/
  ...
  {prodict/dump/dmptrail.i
    &entries      = "PUT STREAM dump UNFORMATTED
                      ""filename=""   as4dict.p__file._File-name SKIP
                      ""records=""    STRING(recs,""99999999"") SKIP
...
/*------------------ Trailer-INFO ------------------*/
    ...
    COLOR DISPLAY NORMAL as4dict.p__File._File-name fil msg WITH FRAME dumpdata.
    DISPLAY as4dict.p__File._File-name fil STRING(recs,"ZZZZZZZ9") @ msg
      WITH FRAME dumpdata.
    cntr = cntr + 1.
