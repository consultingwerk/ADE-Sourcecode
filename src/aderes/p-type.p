...
/*----------------------------------------------------------------------------
  ...
  File: p-type.p - set standard page size properties
...
/* see r-define.i for information on qbf-p-cat contents and structure */
...
FORM
  ...
  TITLE "Standard Page":t32 VIEW-AS DIALOG-BOX.
...
ON HELP OF FRAME qbf-area OR CHOOSE OF qbf-help IN FRAME qbf-area
  RUN adecomm/_adehelp.p ("res":u,"CONTEXT":u,{&Report_Types_Dlg_Box},?).
...
ON GO OF FRAME qbf-area DO:
  ...
  IF qbf-admin = 1 AND qbf-d > "" THEN DO:
    ...
    RUN adecomm/_s-alert.p (INPUT-OUTPUT qbf-a,"question":u,"ok-cancel":u,
      SUBSTITUTE("The following standard page size types will be deleted from the configuration file:^^&1^^Do you want to continue?",
      qbf-t)).
    
    IF qbf-a THEN DO:
      /* store initial list */
      DO qbf-i = 1 TO EXTENT(qbf-p-cat):
        qbf-l[qbf-i] = qbf-p-cat[qbf-i].
      END.
    
      /* zap deleted entries */
      DO qbf-i = 1 TO NUM-ENTRIES(qbf-d, CHR(44)):
        qbf-c = SUBSTRING(ENTRY(qbf-i, qbf-d, CHR(44)), 1, 
                          INDEX(ENTRY(qbf-i, qbf-d, CHR(44)),"(":u) - 2,
                          "CHARACTER":u).
    ...
    IF qbf-s <> "" THEN DO:
      ...
      DO qbf-i = 1 TO NUM-ENTRIES(qbf-s):
        ...
        CASE SUBSTRING(qbf-c,1,2,"CHARACTER":u):
          WHEN "c=":u THEN qbf-rsys.qbf-format    = 
            SUBSTRING(qbf-c,3,-1,"CHARACTER":u).
          WHEN "d=":u THEN qbf-rsys.qbf-dimen     = 
            SUBSTRING(qbf-c,3,-1,"CHARACTER":u).
          WHEN "h=":u THEN qbf-rsys.qbf-page-size = 
            INTEGER(SUBSTRING(qbf-c,3,-1,"CHARACTER":u)).
          WHEN "w=":u THEN qbf-rsys.qbf-width     = 
            INTEGER(SUBSTRING(qbf-c,3,-1,"CHARACTER":u)).
        END CASE.
      END.
    END.
...
PROCEDURE build_list:
  ...
  DO qbf-i = 1 TO EXTENT(qbf-p-cat) WHILE qbf-p-cat[qbf-i] <> "":
    ...
    IF (LOOKUP("c=":u + STRING(qbf-rsys.qbf-format), qbf-p-cat[qbf-i]) > 0
       OR qbf-i = 1)
      THEN qbf-s = qbf-c.
  END.
...
PROCEDURE return_name:
  DEFINE INPUT  PARAMETER qbf_i AS INTEGER   NO-UNDO.
  DEFINE OUTPUT PARAMETER qbf_o AS CHARACTER NO-UNDO.
  ...
  ASSIGN
    qbf_c = ""
    qbf_d = "".
  DO qbf_j = 1 TO NUM-ENTRIES(qbf-p-cat[qbf_i]) WHILE qbf_c = "" OR qbf_d = "":
    IF ENTRY(qbf_j,qbf-p-cat[qbf_i]) BEGINS "c=":u THEN
      qbf_c = SUBSTRING(ENTRY(qbf_j, qbf-p-cat[qbf_i]),3,-1,"CHARACTER":u).
    ELSE
    IF ENTRY(qbf_j,qbf-p-cat[qbf_i]) BEGINS "d=":u THEN
      qbf_d = SUBSTRING(ENTRY(qbf_j, qbf-p-cat[qbf_i]),3,-1,"CHARACTER":u).
  END.
