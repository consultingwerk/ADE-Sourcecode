...
IF CAN-DO("UNIX,OS2":u,OPSYS) THEN
  OUTPUT THROUGH "quoter > qbf2.d":u.
ELSE
  OUTPUT TO "qbf1.d":u NO-ECHO.
RUN VALUE(qbf-f).
OUTPUT CLOSE.

/* JEP WIN32 7/11/97 3:52PM */
IF OPSYS = "MSDOS":u OR OPSYS = "WIN32":u THEN
  DOS SILENT "quoter qbf1.d > qbf2.d":u.
ELSE IF OPSYS = "VMS":u THEN
  VMS SILENT "PROGRESS/TOOLS=QUOTER/OUTPUT=qbf2.d qbf1.d":u.
ELSE IF OPSYS = "BTOS":u THEN
  BTOS OS-QUOTER "qbf1.d":u "qbf2.d":u.
  /*BTOS SILENT "[Sys]<Dlc>Quoter.Run" Quoter "qbf1.d > qbf2.d".*/

INPUT FROM "qbf2.d":u NO-ECHO.
...
DO WHILE TRUE:
  ...
  IF (KEYFUNCTION(LASTKEY) = "BACKSPACE":u AND LENGTH(qbf-t) > 0)
    OR (KEYFUNCTION(LASTKEY) = CHR(LASTKEY) AND LASTKEY >= 32) THEN DO:
    qbf-t = (IF KEYFUNCTION(LASTKEY) = "BACKSPACE":u
            THEN SUBSTRING(qbf-t,1,LENGTH(qbf-t) - 1)
            ELSE qbf-t + CHR(LASTKEY)).
    NEXT.
  END.

  IF CAN-DO("GO,RETURN":u,KEYFUNCTION(LASTKEY)) AND qbf-t <> "" THEN DO:
    ASSIGN
      qbf-w = TRUE
      qbf-s = 0.
  ...
  qbf-t = "".
  IF CAN-DO("CURSOR-DOWN,RETURN":u,KEYFUNCTION(LASTKEY))
  ...
  END.
  ELSE
  IF KEYFUNCTION(LASTKEY) = "CURSOR-UP":u AND qbf-r > 1 THEN DO:
    qbf-r = qbf-r - 1.
  ...
  END.
  ELSE
  IF CAN-DO("GO,PAGE-DOWN,BOTTOM-COLUMN":u,KEYFUNCTION(LASTKEY)) THEN DO:
    ASSIGN
      qbf-r = qbf-r + qbf-d
      qbf-w = TRUE.
  ...
  END.
  ELSE
  IF CAN-DO("PAGE-UP,TOP-COLUMN":u,KEYFUNCTION(LASTKEY)) THEN
    ASSIGN
      qbf-r = qbf-r - qbf-d
      qbf-w = TRUE.
  ELSE
  IF CAN-DO("HOME,MOVE":u,KEYFUNCTION(LASTKEY)) THEN DO:
    ASSIGN
