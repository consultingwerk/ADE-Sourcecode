...
DEFINE VARIABLE p_ClassType   AS CHARACTER NO-UNDO.
DEFINE VARIABLE cClassFile    AS CHARACTER NO-UNDO.
DEFINE VARIABLE cClassTmpDir  AS CHARACTER INIT ? NO-UNDO.
...
DEFINE VARIABLE lIsClass      AS LOGICAL   INIT NO NO-UNDO.
...
ASSIGN
  ...
  p_classType  = get-field("classtype":U)
  lIsClass     = (IF get-field("isClass":U) = "yes":U THEN TRUE ELSE FALSE)
...
------------------------------------------------------------------------------*/
  ...
  /* IZ1852: If it's an Open operation, send 'open-save' to _relname.p so that it
     always opens the correct file. We will call _relname.p again below
     to return the relative path*/
  IF lOpen THEN
    RUN webutil/_relname.p (p_fileName, "must-exist,open-save":U, OUTPUT cRelName).
  ELSE 
  ...
  ELSE IF lOpen THEN DO:
    ...
    IF INDEX(cValue,"R":U) eq 0 THEN DO:
      ...
      RETURN "leave":U.
    END.
    ELSE IF NOT lEditor THEN DO:
      /* when opening a file, we have to send the relative path to the client, so
...
------------------------------------------------------------------------------*/
  ...
  /* IZ1852: When compiling, send 'open-save' to webutil/_relname.p 
     or file might end up in the working directory */
  IF lCompile THEN
     RUN webutil/_relname.p (p_fileName, "must-exist,open-save":U, OUTPUT cRelName).
  ELSE
     RUN webutil/_relname.p (p_fileName, "must-exist":U, OUTPUT cRelName).
  ...
  ELSE DO:
    IF lCompile OR lCheckSection OR lCheckSyntax THEN DO:
      ASSIGN cCompileFile = (IF lCompile THEN cRelName ELSE 
                             (IF lIsClass THEN cClassFile ELSE p_tempfile)).
      ...
      /* if we created cClassFile, remove it */
      IF cClassFile <> "" AND cClassFile <> ? THEN
          RUN deleteFile( cClassFile, "#11").
      /* for a check of a .cls file, remove the cClassTmpDir directory */
      IF (lIsClass AND cClassTmpDir <> ?) THEN
      DO:
          OS-DELETE VALUE ( cClassTmpDir ) RECURSIVE NO-ERROR.
      END.
    END.
    ELSE IF lRun THEN 
    ...
    IF COMPILER:ERROR THEN DO:
      ...
      DO:
          {&OUT} "ERROR:":U + 
              (IF COMPILER:CLASS-TYPE <> ? THEN "CLASSTYPE:":U + COMPILER:CLASS-TYPE ELSE "":U) + 
              " ":U COMPILER:FILE-OFFSET cAction SKIP
            SUBSTITUTE("The following &1 occurred on the WebSpeed agent.",
                       cErrMsgs) SKIP(1).
      END.
...
------------------------------------------------------------------------------*/
  ...
  CASE p_action:
    ...
    WHEN "last":U THEN DO:
      ...
      ELSE IF ((lCheckSyntax OR lCheckSection) AND lIsClass) THEN
      DO:
        /* if this is untitled, and we don't know the classtype 
         * just re-use the existing temp_file, with a .cls extension */
        IF p_fileName BEGINS "Untitled:" AND p_ClassType = "" THEN
        DO:
            ix = R-INDEX(p_tempFile,".tmp":U).
            IF (ix > 0) THEN
                cClassFile = SUBSTRING(p_tempFile,1,ix - 1) + ".cls":U.
        END.
        ELSE
        DO:
            /* if we are checking the syntax of a .cls file, 
             * we want to generate a unique subdir for it. */
            RUN GetUniqueDir(SESSION:TEMP-DIR,"p",?,OUTPUT cClassTmpDir).
            /* create Comp_File from the directory and classtype */
            RUN GetClassCompileName(
                   cClassTmpDir,
                   (IF p_fileName BEGINS "Untitled:":U THEN ? ELSE p_fileName),
                   p_ClassType,
                   OUTPUT cClassFile).
        END.
        RUN copyFile (p_tempFile, cClassFile).
      END.
    ...
    OTHERWISE DO: 
      IF p_Action BEGINS "okToSave":U THEN DO:
        ...
        /* IZ1852: included 'open-save' or file might end up in the working directory */
        RUN webutil/_relname.p (p_fileName, "must-exist,open-save":U, OUTPUT cRelName).
        
        IF cRelName ne ? THEN DO:
          /* show relative path in the message */
          RUN webutil/_relname.p (p_fileName, "must-exist":U, OUTPUT cRelName).
        ...
        ELSE IF cRelName eq ? THEN DO:
          ...
          /* send the relative path name so that the client shows the relative path
             - it already knows the full path since it passed it in */
          RUN webutil/_relname.p (p_fileName, "must-exist":U, OUTPUT cRelName).
          {&OUT} cRelName SKIP.
