...
------------------------------------------------------------------------------*/
  ...
  IF NOT debugging-enabled THEN DO:
    DYNAMIC-FUNCTION ("logNote":U IN web-utilities-hdl, "WARNING":U,
                              SUBSTITUTE ("debug.p was requested by &1 and debugging mode not set. (Ref: &2)",
                                          REMOTE_ADDR, HTTP_REFERER)) NO-ERROR.
    DYNAMIC-FUNCTION ("ShowErrorScreen":U IN web-utilities-hdl,"Unable to find web object file 'debug.p'") NO-ERROR.  
    RETURN.
  END.
