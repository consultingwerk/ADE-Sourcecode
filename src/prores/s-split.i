...
DO qbf-h = 1 TO {&num} - 1:
  IF LENGTH({&dst}[qbf-h]) > {&len} THEN ASSIGN
    qbf-i             = R-INDEX(SUBSTRING({&dst}[qbf-h],1,{&len}),"{&chr}")
    qbf-i             = (IF qbf-i = 0 THEN {&len} ELSE qbf-i)
    {&dst}[qbf-h + 1] = TRIM(SUBSTRING({&dst}[qbf-h],qbf-i + 1))
    {&dst}[qbf-h    ] = SUBSTRING({&dst}[qbf-h],1,qbf-i).
  ELSE
    {&dst}[qbf-h + 1] = "".
END.
