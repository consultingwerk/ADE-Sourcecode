...
PROCEDURE web.output:
  ...
  /* If it had a value, then replace it.  Otherwise just put one in at the end 
     of the definition before the enclosing ">". */
  ASSIGN startPos = INDEX(fieldDef, "value=":U).
  IF startPos = 0 THEN
    ASSIGN startPos = INDEX(fieldDef, "value =":U).
    
  ASSIGN  resultString = IF startPos > 0 THEN 
  ...
  /* OK, now we have either skipped over the original value attribute setting 
     ...
     from the HTML field def or from the Progress field definition.  Note that 
     we need to convert the contents from ascii to html in case any invalid 
     characters are in the widget itself.  If the PROGRESS widget is NOT 
     enabled, just output the value. 
  */
  ...
  resultString = IF hWid:SENSITIVE EQ NO THEN (" ":U + screenValue + " ":U)
               ...
               + (IF (hWid:FORMAT eq "") or (hWid:FORMAT eq ?)
                  THEN "" ELSE " onBlur=~"WSTextVerify(this, '":U
               + hWid:FORMAT + "')~"":U ) 
               + '>').
