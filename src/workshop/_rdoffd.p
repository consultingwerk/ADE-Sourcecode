...
&if {&debug} &then
define stream debug.
output stream debug aa-rdoffd.log.
&endif
...
RUN webutil/_offsrch.p (_P._html-file, _P._fullpath, INPUT-OUTPUT p_offset).
IF p_offset = ? THEN DO:
  RUN Add-Error IN _err-hdl ("ERROR":U, ?,
    SUBSTITUTE("The offset file &1 was not found and could not be regenerated. [_rdoffd.p]",p_offset)).
  RETURN "Error".
END.
...
REPEAT:
  ...
  &if {&debug} &then
    put stream debug unformatted  
      "off-token:" off-token skip
      "off-value:" off-value skip
    
      "_htm-name:" _htm._htm-name skip
      "_u-recid:"  _htm._u-recid skip
      "_htm-type:" _htm._htm-type skip
      "_mdt-type:" _htm._mdt-type skip
      "_p-recid:"  _htm._P-recid skip(1).
  &endif
