...
/**************************************************************************
    Procedure:  _so-cuew.p


    Purpose:    Deletes the cue card for this object upon .W write (XFTR)
...
RUN adeuib/_uibinfo.p (INPUT recid-trg,   /* pi_context: of the code  */
                       ...
                       INPUT "TEMPLATE",  /* p_request: all proc. id's */
                       OUTPUT cInfo).
/* Is this a TEMPLATE? If so return. */
IF cInfo eq "yes" THEN RETURN.
 ...
 * so that the next time the .W is read in, _so-cue.w will delete it
 */
ASSIGN trg-Code = firstline + CHR(10) + "Destroy on next read */".
