...
p_DBName   - The name of the database the schema was picked from.  May 
             optionally be passed in to default the database.
...
Author:   Ross Hunter
Created:  08/10/95 
Modified: 09/22/99 adams    Refound s_ttb_db after opening query.
...
/* Refind correct s_ttb_db record, since opening the query will reposition
   the s_ttb_db record cursor. */
RUN findDb.
IF ERROR-STATUS:ERROR THEN RETURN.
...
------------------------------------------------------------------------------*/
  ...
  ELSE DO:
    ...
    REPEAT ix = 1 TO NUM-DBS:
      IF LDBNAME("DICTDB":U) = LDBNAME(ix) THEN LEAVE db-search.
      ELSE IF DBTYPE(ix) <> "PROGRESS" THEN NEXT db-search.
...
/*------------------------------------------------------------------------------
  Purpose:    Populate shared browse tbl-browse based on fields from the db 
              pointed to by repos.  Initially select the tables based on the 
              value of ivalue, or if ivalue is "" then pick the first table in 
              tbl-browse for single select lists and no table for multiselect 
              lists.
  Parameters: ivalue
  Notes:      syntax: RUN load_tables.
------------------------------------------------------------------------------*/
  ...
  ELSE DO:  /* Attempting to select from the temptables */
    ...
    IF NOT CAN-FIND(FIRST _tt-tbl) THEN DO:
      DO ix = 1 TO NUM-ENTRIES(p_TT):
        CREATE _tt-tbl.
        ASSIGN cur-ent         = ENTRY(ix, p_TT)
      ...
      END. /* Do ix = 1 to num-entries(p_TT) */
