 ...
 DO: 
   ...
   FOR EACH _BC WHERE _BC._x-recid = grQueryRecid:
     ...
     ELSE DO:
       ...
       IF NOT CAN-FIND(FIRST ryc_smartobject WHERE ryc_smartobject.object_filename = cCalcMaster) AND 
 ...
 FOR EACH _BC WHERE _BC._x-recid = RECID(b_U) AND _BC._DBNAME = "_<CALC>":U:
   ...
   DO:
     ...
     IF AVAIL ttClassAttribute AND 
       ...
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Data-Type':U,{&CHARACTER-DATA-TYPE},_BC._DATA-TYPE,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'Label':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE _BC._LABEL THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Label':U,{&CHARACTER-DATA-TYPE},_BC._LABEL,?,?,?,?,?).
     ...
     IF AVAIL ttClassAttribute AND 
       ...
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'ColumnLabel':U,{&CHARACTER-DATA-TYPE},cCalcColLabel,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
       ttClassAttribute.tAttributeLabel = 'Format':U NO-ERROR.
     IF AVAIL ttClassAttribute AND
       ttClassAttribute.tAttributeValue NE _BC._FORMAT THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Format':U,{&CHARACTER-DATA-TYPE},_BC._FORMAT,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'Help':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE _BC._HELP THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Help':U,{&CHARACTER-DATA-TYPE},_BC._HELP,?,?,?,?,?).
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE gsc_product_module THEN DO:
    ...
    IF VALID-HANDLE(hSCMTool) THEN DO:
      ...
      RUN scmGetModuleDir IN hSCMTool (INPUT gsc_product_module.product_module_code, INPUT "":U, OUTPUT cCSPRelativeDir) NO-ERROR.          
