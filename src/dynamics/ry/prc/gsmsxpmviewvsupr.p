...
/*------------------------------------------------------------------------
  ...
  File: gsmsxpmviewvsupr.p

  Description: Super Procedure for SCM Xref Product Module viewer
...
------------------------------------------------------------------------------*/
  ...
  /* Assign the value of the hidden field for owning_enityt_mnemomnic */
  assignWidgetValue('owning_entity_mnemonic':U, "GSCPM":U). 
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN hSCMForeignKeyLookup = widgetHandle('scm_foreign_key'). 
      
  IF VALID-HANDLE(ghScmTool) THEN
  DO:  
    IF CONNECTED("RTB") THEN
    DO:
      ASSIGN cBaseQueryString = "FOR EACH rtb.rtb_pmod NO-LOCK":U. 
      ...
      DYNAMIC-FUNCTION('setFieldLabel'           IN hSCMForeignKeyLookup, INPUT "SCM product module"). 
      ...
      DYNAMIC-FUNCTION('setKeyField'             IN hSCMForeignKeyLookup, INPUT "rtb_pmod.pmod"). 
      DYNAMIC-FUNCTION('setDisplayedField'       IN hSCMForeignKeyLookup, INPUT "rtb_pmod.pmod"). 
      
      /* Set query properties */
      DYNAMIC-FUNCTION('setBaseQueryString':U    IN hSCMForeignKeyLookup, INPUT cBaseQueryString).
      DYNAMIC-FUNCTION('setQueryTables'          IN hSCMForeignKeyLookup, INPUT "rtb_pmod").
      
      /* Set browser properties  */
      DYNAMIC-FUNCTION('setBrowseFields'         IN hSCMForeignKeyLookup, INPUT "rtb_pmod.pmod,rtb_pmod.description").
      DYNAMIC-FUNCTION('setColumnLabels'         IN hSCMForeignKeyLookup, INPUT "Product Module,Description").
      DYNAMIC-FUNCTION('setColumnFormat'         IN hSCMForeignKeyLookup, INPUT "X(20),X(35)").
      DYNAMIC-FUNCTION('setBrowseFieldDataTypes' IN hSCMForeignKeyLookup, INPUT "character,character").
      DYNAMIC-FUNCTION('setBrowseTitle'          IN hSCMForeignKeyLookup, INPUT "SCM product module lookup").      
    END.        
  END.
  ELSE
  DO:
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE cNewDisplayedfieldValue AS CHARACTER  NO-UNDO.
  DEFINE VARIABLE iScmSiteNumber          AS INTEGER    NO-UNDO.
  DEFINE VARIABLE iSiteInteger            AS INTEGER    NO-UNDO.

  IF phObject = widgetHandle('scm_foreign_key') THEN
  DO:
      /* As we do not want to have site number prefixes in the SCM module
         names, any module that comes back with the site number has to have the 
         site number stripped off - otherwise we end up with incorrect SCM Xref 
         data. 
      */
      IF VALID-HANDLE(ghScmTool) THEN
      DO:
          ASSIGN cNewDisplayedfieldValue = pcKeyFieldValue.
          /* check if the product module starts with a 3 digit integer */
          ASSIGN iSiteInteger = INTEGER( SUBSTRING(cNewDisplayedfieldValue,1,3) ) NO-ERROR.

          RUN scmGetSitenumber IN ghScmTool (OUTPUT iScmSiteNumber) NO-ERROR. 
          IF (iScmSiteNumber > 0 AND pcKeyFieldValue BEGINS STRING(iScmSiteNumber)) OR 
             isiteInteger > 0 THEN
          DO:
             RUN scmSitePrefixDel IN ghScmTool (INPUT-OUTPUT cNewDisplayedfieldValue) NO-ERROR.
             assignWidgetValue('scm_foreign_key':U, cNewDisplayedfieldValue).
          END.
      END.
  END.
END PROCEDURE.
