...
DEFINE VARIABLE lAnySDFEnabled    AS LOGICAL    NO-UNDO.
...
DO WHILE ghCacheObject:AVAILABLE:
    ...
    DO:        
        ...
        /* Create the object.
          ...
          */        
        ASSIGN gcCurrentObjectName = STRING(dInstanceId).
        ...
        ASSIGN dRowPosition           = ttWidget.tRow
               ...
               lAnySDFEnabled         = IF NOT lAnySDFEnabled 
                                        THEN lEnabled 
                                        ELSE lAnySDFEnabled.
    ...
    DO:
        ...
        IF lDisplayField AND ttWidget.tTableName <> "":U THEN
            ASSIGN cDisplayedFields = cDisplayedFields
                                    + (IF ttWidget.tTableName EQ "rowObject":U OR cDataSourceNames NE "":U 
                                       THEN "":U 
                                       ELSE ttWidget.tTableName + ".":U)
                                    + cWidgetName + ",":U.
        
        /* display field */
        IF lEnabled THEN
            IF ttWidget.tTableName EQ "":U THEN
                ASSIGN cEnabledObjects = cEnabledObjects + cWidgetName + ",":U.
            ELSE
                ASSIGN cEnabledFields = cEnabledFields 
                                      + (IF ttWidget.tTableName EQ "rowObject":U OR cDataSourceNames NE "":U 
                                         THEN "":U 
                                         ELSE ttWidget.tTableName + ".":U)
                                      + cWidgetName + ",":U.
        ...
        ASSIGN hField:PRIVATE-DATA = cPrivateData WHEN CAN-SET(hField, "PRIVATE-DATA":U)
               ...
               cAllFieldNames   = cAllFieldNames + hField:NAME + ",":U
               .
        ...
        /* Table Name */
        IF ttWidget.tTableName <> "":U AND cDataSourceNames = "":U THEN
        ...
        IF ttWidget.tLabel <> ? THEN
            ...
            DO:
                ASSIGN dLabelWidth = FONT-TABLE:GET-TEXT-WIDTH-PIXELS(ttWidget.tLabel + ":":U, iLabelFont) + 3.

                IF dLabelWidth > 0 THEN
                DO:
                    CREATE TEXT hLabel
                        ASSIGN FRAME        = hDefaultFrame
                               FORMAT       = "x(":U + STRING(LENGTH(ttWidget.tLabel, "CHARACTER":U) + 1) + ")":U
                               HEIGHT-CHARS = 1
                               NAME         = "LABEL_":U + REPLACE(cWidgetName, ".":U, "_":U)
                               TAB-STOP     = NO            /* Labels should never be in the tab order */
                               ROW          = hField:ROW
                               SCREEN-VALUE = ttWidget.tLabel + ":":U
                               WIDTH-PIXELS = dLabelWidth.
        ...
        /* Set the initial SENSITIVE state of the field to FALSE and let the toolbar take care of enabling and disabling the fields */
...
ASSIGN cAllFieldHandles   = RIGHT-TRIM(cAllFieldHandles, ",":U)
       ...
       cEnabledObjects    = RIGHT-TRIM(cEnabledObjects, ",":U)
...
IF cEnabledFields = "":U AND NOT lAnySDFEnabled THEN 
DO:
