 ...
 FOR EACH _BC WHERE _BC._x-recid = grFrameRecid :
  /* fetch the class attributes in case the master attributes are not found */
    ...
    IF _BC._DBNAME = "_<CALC>":U THEN
      ASSIGN cObjectName = _BC._DISP-NAME.
...
------------------------------------------------------------------------------*/
 ...
 DO:
   CASE pcLabel:
     ...
     WHEN "DisplayField":U     THEN f_U._DISPLAY         = LOGICAL(pcValue).
...
------------------------------------------------------------------------------*/
 ...
 FOR EACH b_ttObject WHERE b_ttObject.tContainerSmartObjectObj = ttObject.tSmartObjectObj:
    ...
    DO:  
       ...
       IF glSBODataSource AND NUM-ENTRIES(b_ttObject.tLogicalObjectName, ".":U) = 2 THEN
          gcSDOName = ENTRY(1, b_ttObject.tLogicalObjectName, ".":U). 
    ...
    ASSIGN gdFrameMaxWidth  = MAX(gdFrameMaxWidth, dWidgetCol + dWidgetWidth)
           gdFrameMaxHeight = MAX(gdFrameMaxHeight, dWidgetRow + dWidgetHeight).
    ...
    ASSIGN gcSettingsList = 
        ...
        "MappedFields"              + CHR(4) + CHR(3) + "UseCache":U + CHR(4).
   ...
   DO:
     ...
     IF SEARCH(_S._FILE-NAME) = ? THEN DO:
       FIND ryc_smartobject NO-LOCK
             WHERE ryc_smartobject.OBJECT_filename = _S._FILE-NAME 
               AND ryc_smartobject.customization_result_obj = 0 NO-ERROR.
       IF AVAILABLE ryc_smartobject THEN DO:
         _S._FILE-NAME = ryc_smartobject.object_path + (IF ryc_smartobject.object_path > "" THEN "/":U ELSE "") +
                            ryc_smartobject.object_filename.
         IF SEARCH(_S._FILE-NAME) = ? AND NUM-ENTRIES(_S._FILE-NAME, ".") = 1 AND 
                    ryc_smartobject.object_extension NE "":U THEN 
         DO:
           _S._FILE-NAME = ryc_smartobject.object_path +  (IF ryc_smartobject.object_path > "" THEN "/":U ELSE "") +
...
------------------------------------------------------------------------------*/
 ...
 DO:
   ...
   DO:
     ...
     IF LOOKUP(gcDynClass,"DynBrow,DynSDO,DynView":U) = 0 AND 
       ((SEARCH(_Ryobject.design_template_file) = ?) OR (SEARCH(_Ryobject.design_propsheet_file) = ?)) THEN 
     DO:
       /* Reset the cursor for user input.*/
       RUN adecomm/_setcurs.p ("").
