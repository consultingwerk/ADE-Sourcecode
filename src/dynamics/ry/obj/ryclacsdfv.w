...
/*---------------------------------------------------------------------------------
  ...
  Description:  Layout combo SDF

  Purpose:      Layout combo smartdatafield

  Parameters:   <none>

  History:
  --------
  (v:010000)    Task:        7487   UserRef:    
                Date:   16/01/2001  Author:     Anthony Swindells
...
&Scoped-Define ENABLED-OBJECTS coLayout 
&Scoped-Define DISPLAYED-OBJECTS coLayout 
...
DEFINE VARIABLE coLayout AS CHARACTER FORMAT "X(256)":U 
     LABEL "Layout" 
...
DEFINE FRAME frMain
     coLayout AT ROW 1 COL 7.6 COLON-ALIGNED
...
ON VALUE-CHANGED OF coLayout IN FRAME frMain /* Layout */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
  FIND FIRST ttComboData WHERE ttComboData.hWidget = coLayout:HANDLE.
  IF NOT AVAILABLE ttComboData THEN RETURN.

  coLayout:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = ttComboData.cListItemPairs.
...
------------------------------------------------------------------------------*/
   ASSIGN coLayout:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
------------------------------------------------------------------------------*/
   ASSIGN coLayout:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  FIND FIRST ttComboData
       WHERE ttComboData.cWidgetName = "coLayout":U
       NO-ERROR.
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH ryc_layout NO-LOCK BY ryc_layout.layout_name":U
    ttComboData.cBufferList = "ryc_layout":U
    ttComboData.cKeyFieldName = "ryc_layout.layout_name":U
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN FRAME {&FRAME-NAME} coLayout.
  IF coLayout = ".":U THEN ASSIGN coLayout = "":U.  /* remove dot for none option */
  RETURN coLayout.
...
iLookup = coLayout:LOOKUP(pcValue) IN FRAME {&FRAME-NAME}.
IF coLayout:NUM-ITEMS > 0 AND
   (iLookup = ? OR iLookup = 0) THEN ASSIGN iLookup = 1.
IF iLookup <> ? AND iLookup <> 0 THEN
DO:
    cEntry = coLayout:ENTRY(iLookup) NO-ERROR.
    IF cEntry <> ? AND NOT ERROR-STATUS:ERROR THEN
    DO:
        coLayout:SCREEN-VALUE = cEntry NO-ERROR.
    END.
    ELSE DO:
        /* blank the combo */
        coLayout:LIST-ITEM-PAIRS = coLayout:LIST-ITEM-PAIRS.
        lSuccess = FALSE.
    END.
END.
ELSE DO:
