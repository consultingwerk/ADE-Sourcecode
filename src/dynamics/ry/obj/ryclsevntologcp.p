...
&scop object-name       ryclsevntologcp.p
...
&GLOB DATA-FIELD-DEFS  "ry/obj/ryclsevnto.i"
...
------------------------------------------------------------------------------*/
    ...
    DEFINE variable hSdoEventBuffer         as handle                 no-undo.
    ...
    publish "getClassEventBuffer" from hContainerSource (output hEventBuffer).
    if not valid-handle(hEventBuffer) then
        return.
    
    {get TempTableBuffer hSdoEventBuffer}.
    if not valid-handle(hSdoEventBuffer) then
        return.
    
    hSdoEventBuffer:empty-temp-table().
    
    /* fill the SDO's temp-table */   
    ghQuery:set-buffers(hEventBuffer).
    ghQuery:query-prepare('for each ' + hEventBuffer:name).
    ghQuery:query-open().
    
    ghQuery:get-first().
    do while hEventBuffer:available:
        hSdoEventBuffer:buffer-create().
        hSdoEventBuffer:buffer-copy(hEventBuffer).
        hSdoEventBuffer:buffer-release().
...
------------------------------------------------------------------------------*/
    ...
    for each rowObjUpd where
        ...
        buffer-copy rowObjUpd to ttStoreUiEvent
            assign ttStoreUiEvent.tEventParent = 'Class'
                   ttStoreUiEvent.tEventParentObj = dClassObj.
    end.    /* A,U,C */
    
    if can-find(first ttStoreUiEvent) and valid-handle(gshRepositoryManager) then
    do:
        hAttributeBuffer = buffer ttStoreUiEvent:handle.
        hAttributeTable = ?.
        
        if not valid-handle(hRdm) then
            hRDM = {fnarg getManagerHandle 'RepositoryDesignManager'}.        
        run insertUiEvents in hRdm (input              hAttributeBuffer,
    ...
    for each rowObjUpd where
        ...
        do:
            ...
            assign ttStoreUiEvent.tEventParent = 'Class'
                   ttStoreUiEvent.tEventParentObj = dClassObj
                   ttStoreUiEvent.tEventName = rowObjUpd.tEventName.
    ...
    do:
        ...
        do:
            ...
            run removeUiEvents in hRDM (input              hAttributeBuffer,
                                        input table-handle hAttributeTable) no-error.
            if return-value ne '' or error-status:error then
            do:
                error-status:error = no.
