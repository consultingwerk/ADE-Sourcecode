...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_release Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_release table
...
&GLOB DATA-LOGIC-TABLE gsm_release
&GLOB DATA-FIELD-DEFS  "ry/obj/gsmrlfullo.i"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD CalculateReleaseTime DataLogicProcedure  _DB-REQUIRED
FUNCTION CalculateReleaseTime RETURNS CHARACTER
...
/*------------------------------------------------------------------------------
  Purpose:     When we create a new release we need to obtain a release version
...
------------------------------------------------------------------------------*/
 ...
 ASSIGN 
    dLoginCompanyObj = DECIMAL(DYNAMIC-FUNCTION("getPropertyList":U IN gshSessionManager,
                                                ...
                                                INPUT NO)) NO-ERROR.
  ASSIGN
    b_gsm_release.release_number = DYNAMIC-FUNCTION("getNextSequenceValue":U IN gshGenManager,
...
------------------------------------------------------------------------------*/
  FOR EACH gst_release_version
    WHERE gst_release_version.release_obj = b_gsm_release.release_obj:
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN
      b_gsm_release.release_date = TODAY
      b_gsm_release.release_time = TIME
      b_gsm_release.release_user = cUserLogin
      .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION CalculateReleaseTime DataLogicProcedure  _DB-REQUIRED
FUNCTION CalculateReleaseTime RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE hReleaseTime AS HANDLE NO-UNDO.

  hRowObject  = DYNAMIC-FUNCTION('getRowObject':U IN TARGET-PROCEDURE).
  hReleaseTime = hRowObject:BUFFER-FIELD('release_time':U).

  RETURN STRING(hReleaseTime:BUFFER-VALUE,"HH:MM:SS":U).
