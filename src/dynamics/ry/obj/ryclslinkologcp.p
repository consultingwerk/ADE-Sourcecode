...
&scop object-name       ryclslinkologcp.p
...
&GLOB DATA-LOGIC-TABLE ttSupportedLink
&GLOB DATA-FIELD-DEFS  "ry/obj/ryclslinko.i"
...
------------------------------------------------------------------------------*/
    ...
    DEFINE variable hSupportedLinkBuffer    as handle                 no-undo.
    DEFINE variable hSdoLinkBuffer          as handle                 no-undo.
    ...
    {get ContainerSource hContainerSource}.        
    publish "getClassLinkBuffer" from hContainerSource (output hSupportedLinkBuffer).    
    if not valid-handle(hSupportedLinkBuffer) then
        return.
    
    {get TempTableBuffer hSdoLinkBuffer}.
    if not valid-handle(hSdoLinkBuffer) then
        return.
    
    hSdoLinkBuffer:empty-temp-table().
...
------------------------------------------------------------------------------*/
    ...
    DEFINE variable dSupportedLinkObj        as decimal                   no-undo.
    ...
    do:
        error-status:error = no.
        return {aferrortxt.i 'AF' '40' '?' '?' '"Unable to obtain a handle to the Repository Design Manager"'}.
    end.    /* invalid RDM */
    ...
    for each rowObjUpd where
             ...
             rowObjUpd.RowMod = 'a' :
        run insertSupportedLink in hRdm (input (if cCustomisingClassName eq '' then cClassName else cCustomisingClassName),
                                         input rowObjUpd.LinkName,
                                         input rowObjUpd.DeactivateOnHide,
                                         input rowObjUpd.IsLinkSource,
                                         input rowObjUpd.IsLinkTarget,
                                         output dSupportedLinkObj ) no-error.
        if return-value ne '' or error-status:error then
        do:
            error-status:error = no.
            return return-value.
        end.    /* error saving link */
    end.    /* A,U,C */
    ...
    for each rowObjUpd where
         ...
         */
        if rowObjUpd.WhereStored eq cClassName or
           rowObjUpd.WhereStored eq cCustomisingClassName then
        do:
                run removeSupportedLink in hRdm (input rowObjUpd.WhereStored,
                                                 input rowObjUpd.LinkName  ) no-error.
                if return-value ne '' or error-status:error then
                do:
                    error-status:error = no.
                    return return-value.
                end.    /* error deleting link */
        end.    /* stored against this class or its custom class */
    end.    /* deleted records */
...
FUNCTION getFilterAvailable RETURNS LOGICAL
        (  ):
/*------------------------------------------------------------------------------
  Purpose:  Force the filter to always display
    Notes:
------------------------------------------------------------------------------*/
    return yes.
