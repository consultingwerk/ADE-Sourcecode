...
/*---------------------------------------------------------------------------------
  ...
  Description:  rym_customization Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the rym_customization table
...
&GLOB DATA-LOGIC-TABLE rym_customization
&GLOB DATA-FIELD-DEFS  "ry/obj/rymczfullo.i"
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "rym_customization Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST rym_customization 
              WHERE rym_customization.customization_type_obj = b_rym_customization.customization_type_obj
                AND rym_customization.customization_reference = b_rym_customization.customization_reference) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_customization.customization_type_obj) + ', ' + STRING(b_rym_customization.customization_reference)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {aferrortxt.i 'AF' '8' 'rym_customization' '' "'customization_type_obj, customization_reference, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST rym_customization 
              WHERE rym_customization.customization_type_obj = b_rym_customization.customization_type_obj
                AND rym_customization.customization_reference = b_rym_customization.customization_reference
                AND ROWID(rym_customization) <> TO-ROWID(ENTRY(1,b_rym_customization.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_customization.customization_type_obj) + ', ' + STRING(b_rym_customization.customization_reference)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_rym_customization.customization_reference) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'rym_customization' 'customization_reference' "'Customization Reference'"}.

  IF b_rym_customization.customization_result_obj = 0 OR b_rym_customization.customization_result_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'rym_customization' 'customization_result_obj' "'Customization Result Obj'"}.

  IF b_rym_customization.customization_type_obj = 0 OR b_rym_customization.customization_type_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'rym_customization' 'customization_type_obj' "'Customization Type Obj'"}.
