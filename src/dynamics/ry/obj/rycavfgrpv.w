...
"Astra 2 SmartDataField Template.

Use this template as a starting point for creating Astra 2 SmartDataField Objects. A SmartDataField object represents a single field of a SmartDataObject using a non-standard visualization of your choosing, and can be inserted into a SmartDataField."
*/
&ANALYZE-RESUME
...
&Scoped-Define ENABLED-OBJECTS coAttributeGroup coAttribute 
&Scoped-Define DISPLAYED-OBJECTS coAttributeGroup coAttribute 
...
DEFINE VARIABLE coAttribute AS CHARACTER FORMAT "X(256)":U INITIAL "0" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     DROP-DOWN-LIST
     SIZE 34.4 BY 1 NO-UNDO.

DEFINE VARIABLE coAttributeGroup AS DECIMAL FORMAT "->>>>>>>>>>>>>>>>>9.999999999":U INITIAL 0 
     VIEW-AS COMBO-BOX INNER-LINES 10
     LIST-ITEM-PAIRS "",0
     DROP-DOWN-LIST
     SIZE 29.6 BY 1.05 NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME frMain
     coAttributeGroup AT ROW 1 COL 1 NO-LABEL
     coAttribute AT ROW 1 COL 28.6 COLON-ALIGNED NO-LABEL
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL coAttribute sObject
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL coAttributeGroup sObject
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    /* store all the initial values of the attribute in a global variable */
    ASSIGN
        gcDataString = ttComboData.cListItemPairs.

    /*RUN setfilterComboValue (INPUT coAttributeGroup:SCREEN-VALUE).     */
    APPLY "VALUE-CHANGED":U TO coAttributeGroup IN FRAME {&FRAME-NAME}.
END.
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN
       coAttributeGroup:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE
       coAttribute:SENSITIVE IN FRAME {&FRAME-NAME} = FALSE.
...
------------------------------------------------------------------------------*/
   ASSIGN
       coAttributeGroup:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE
       coAttribute:SENSITIVE IN FRAME {&FRAME-NAME} = TRUE.
...
/***DIAG***/
MESSAGE {&line-number} PROGRAM-NAME(1) SKIP
    "SESSION:NUMERIC-DECIMAL-POINT " SESSION:NUMERIC-DECIMAL-POINT SKIP
    "ttComboData.cListItemPairs (attribute) " ttComboData.cListItemPairs
    VIEW-AS ALERT-BOX INFO BUTTONS OK.

IF  LENGTH(pcFilterInfo) > 0 THEN DO:
    DO iLoop = 1 TO NUM-ENTRIES(ttComboData.cListItemPairs) BY 2:
        ASSIGN
            cString = ENTRY(iLoop, ttComboData.cListItemPairs) + "," + ENTRY(iLoop + 1, ttComboData.cListItemPairs).

        IF ENTRY(1, ENTRY(2, cString, "|":U)) = pcFilterInfo THEN
            ASSIGN
                cNewListItemPairs = TRIM(cNewListItemPairs) + TRIM(ENTRY(1, cString, "|":U)) + "," + TRIM(ENTRY(2, cString)) + ",".

    END.

    ASSIGN
        ttComboData.cListItemPairs  = TRIM(cNewListItemPairs, ',').
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN
    ...
    ttComboData.cKeyFieldName = "ryc_attribute.attribute_group_obj":U
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ASSIGN FRAME {&FRAME-NAME} coAttribute.
    PUBLISH "populateChildCombo":U FROM ghSDV (INPUT coAttribute, INPUT plModified).
  END.
...
valueLoop:
DO iLoop = 1 TO NUM-ENTRIES(gcDataString) BY 2:
    IF DECIMAL(pcKeyValue) = DECIMAL(ENTRY(iLoop + 1, gcDataString)) THEN DO:
        ASSIGN
            coAttributeGroup:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ENTRY(2, ENTRY(iLoop, gcDataString), "|":U)
            plSuccess = TRUE.
...
iLookup = coAttribute:LOOKUP(STRING(pcValue)) IN FRAME {&FRAME-NAME} NO-ERROR.
...
/*ttRycAttributeValue.attribute_group_obj
ttRycAttributeValue.attribute_type_tla 
