...
/* DEFINE VARIABLE gcCurrentObjectID AS DECIMAL NO-UNDO. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table bTableWin _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      rowObject.object_filename FORMAT "X(35)":U WIDTH 15
      rowObject.object_description FORMAT "X(35)":U WIDTH 20
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-AUTO-VALIDATE NO-ROW-MARKERS SEPARATORS SIZE 45 BY 6.86 ROW-HEIGHT-CHARS .62.
...
/* Settings for THIS-PROCEDURE
   ...
   Data Source: "ry/obj/dobjectlookup.w"
   Allow: Basic,Browse
   Frames: 1
...
"rowObject.object_filename" ? ? "character" ? ? ? ? ? ? no ? no no "15" yes no no "U" "" ""
     _FldNameList[2]   > _<SDO>.rowObject.object_description
"rowObject.object_description" ? ? "character" ? ? ? ? ? ? no ? no no "20" yes no no "U" "" ""
...
DO:
  PUBLISH 'F2Pressed':U.  /* the user presses F2 key. */
END.
...
DO:
  /*  APPLY 'value-changed' TO SELF.*/   /* the user selects a row. */
END.
...
DO:
  PUBLISH 'F2Pressed':U.   /* the user presses Return key when in browser, which is 
                              considered as GO, inform the smartwindow of the event. */
END.
...
/*------------------------------------------------------------------------------
  Purpose:     get object filename (first column) and ID (last column) in the 
               currrently highlighted row in the browser.
  Parameters:  firstColumnHandle -- widget-handle of the first column (object filename)
               lastColumnHandle  -- widget-handle of the last column (object security ID)
  Notes:       
------------------------------------------------------------------------------*/
DEFINE VARIABLE firstColumnHandle AS HANDLE NO-UNDO. 
/* DEFINE VARIABLE lastColumnHandle AS HANDLE NO-UNDO. */

DEFINE VARIABLE temp AS INTEGER NO-UNDO.

ASSIGN temp = {&BROWSE-NAME}:NUM-COLUMNS IN FRAME {&FRAME-NAME}.

firstColumnHandle = {&BROWSE-NAME}:GET-BROWSE-COLUMN(1) IN FRAME {&FRAME-NAME}.
/*lastColumnHandle = {&BROWSE-NAME}:GET-BROWSE-COLUMN(temp) IN FRAME {&FRAME-NAME}.
 */
ASSIGN gcCurrentFileName = firstColumnHandle:SCREEN-VALUE.
     /*  gcCurrentObjectID = DECIMAL(lastColumnHandle:SCREEN-VALUE). */
