...
/*---------------------------------------------------------------------------------
  ...
  Description:  Attribute Combo Viewer

  Purpose:      Attribute Combo Viewer.  Can be filtered by attribute group combo -
                (rycagdcsfv.w)
  ...
  (v:010000)    Task:        6517   UserRef:    
                Date:   19/09/2000  Author:     Jenny Bond

  Update Notes: Created from Template rysttccsfv.w.  Initial coding.
...
*/
DEFINE VARIABLE gdSaveKey   AS DECIMAL NO-UNDO.
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 19 COLON-ALIGNED
...
ON VALUE-CHANGED OF coCombo IN FRAME frMain /* Attribute Label */
DO:
  {set DataModified TRUE}.
  PUBLISH "populateAttributeType":U FROM ghSDV (INPUT coCombo:SCREEN-VALUE).
END.
...
------------------------------------------------------------------------------*/
  ...
  DO:
    SUBSCRIBE TO "populateAttributeCombo":U IN ghSDV.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE populateAttributeCombo sObject 
PROCEDURE populateAttributeCombo :
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pdKey                    AS DECIMAL      NO-UNDO.
DEFINE INPUT PARAMETER plModified               AS LOGICAL      NO-UNDO.

/* If value not changed, then do nothing */
IF pdKey <> 0 AND gdSaveKey = pdKey THEN RETURN.

ASSIGN gdSaveKey = pdKey. /* reset last key value */
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  */
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH ryc_attribute_group NO-LOCK WHERE ryc_attribute_group.attribute_group_obj = " + STRING(pdKey) + ",":U +
                           "  EACH ryc_attribute NO-LOCK WHERE ryc_attribute.attribute_group_obj = ryc_attribute_group.attribute_group_obj " +
                           " BY ryc_attribute.attribute_label":U
    ttComboData.cBufferList = "ryc_attribute_group,ryc_attribute":U
    ttComboData.cKeyFieldName = "ryc_attribute.attribute_label":U
    ttComboData.cDescFieldNames = "ryc_attribute.attribute_label":U
