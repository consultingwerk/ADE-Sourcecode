...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DO:
    ...
    /* If user specified an extension, try to find it without the extension */
    IF cQueryPosition = 'NoRecordAvailable':U AND NUM-ENTRIES(cFileName,".") > 1 THEN 
    DO:
      DYNAMIC-FUNC('assignQuerySelection':U IN h_dopendialog,'object_filename':U,ENTRY(1,cFileName,"."),'EQ':U).
      DYNAMIC-FUNCTION('openQuery':U IN h_dopendialog).      
      {get QueryPosition cQueryPosition h_dopendialog}. /* any data? */

      /* If found, verify that the entered extension matches the retrieved object's extension */
      IF cQueryPosition <> "NoRecordAvailable":U THEN 
      DO:
         cExtension    = TRIM(DYNAMIC-FUNCTION("ColumnStringValue":U IN h_dopendialog,"object_extension":U)).
         IF cExtension = ENTRY(2,cFileName,".") THEN
            cFileName = ENTRY(1,cFileName,".").
         ELSE    
           cQueryPosition = "NoRecordAvailable":U.
      END.   
    END.
    ...
    ELSE DO:
       ...
       DO:
         ASSIGN
           cObjPath      = DYNAMIC-FUNCTION("ColumnStringValue":U IN h_dopendialog,"object_path":U)
           cExtension    = TRIM(DYNAMIC-FUNCTION("ColumnStringValue":U IN h_dopendialog,"object_extension":U))
           cObjPath      = trim(cObjPath) + IF cObjPath = "" THEN "" ELSE "/":U
           cFullFileName = SEARCH (cObjPath + cFileName + (IF cExtension > "" THEN "." + cExtension ELSE ""))
           NO-ERROR.
         IF cFullFileName = ? THEN
            cFullFileName = SEARCH (cObjPath + ENTRY(1,cFileName,".") + IF cExtension > "" THEN "." + cExtension ELSE "") NO-ERROR.
           
         IF cFullFileName = ? THEN /* Could not find static object, try the relative_path for backward compatibility */
            ASSIGN
              cRelPath      = DYNAMIC-FUNCTION("ColumnStringValue":U IN h_dopendialog,"relative_path":U)
              cRelPath      = trim(cRelPath) + IF cRelPath = "" THEN "" ELSE "/":U
              cFullFileName = SEARCH (cRelPath + cFileName + (IF cExtension > "" THEN "." + cExtension ELSE ""))
              NO-ERROR.     
         IF cFullFileName = ? THEN
              cFullFileName = SEARCH (cRelPath + ENTRY(1,cFileName,".") + IF cExtension > "" THEN "." + cExtension ELSE "") NO-ERROR.  
             
         IF cFullFileName = ? THEN /* Could not find static object */
         DO:     
            MESSAGE SUBSTITUTE("The static file '&1' cannot be found in your Propath.",
                               (IF cObjPath> "" THEN cObjPath ELSE cRelPath) + cFileName + (IF cExtension > "" THEN "." + cExtension ELSE "") ) SKIP
