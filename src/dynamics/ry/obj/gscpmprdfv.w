...
/*---------------------------------------------------------------------------------
  ...
  Description:  Product Module SDF by Product

  Purpose:      Product Module SDF by Product
  ...
  (v:010000)    Task:        6517   UserRef:    
                Date:   22/08/2000  Author:     Jenny Bond

  Update Notes: Created from Template rysttcmsfv.w

  (v:010001)    Task:        7412   UserRef:    
...
DEFINE VARIABLE gcProdMod   AS CHARACTER    NO-UNDO.
...
DO:
    ...
    /* When included in the object viewer, changedProductModule needs to run to 
       change the object path of the object for a changed product module */
    PUBLISH 'changedProductModule':U (INPUT coProdMod).
END.
...
DO:
    ...
    ASSIGN
        ttComboData.cListItemPairs = gcProdMod.

    RUN filterCombo (INPUT coProduct:SCREEN-VALUE). /* Filter product module combo by value of product combo */
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    ASSIGN
        gcProdMod = ttComboData.cListItemPairs.
...
valueLoop:
DO iLoop = 1 TO NUM-ENTRIES(gcProdMod, coProduct:DELIMITER IN FRAME {&FRAME-NAME}) BY 2:
    IF DECIMAL(pcKeyValue) = DECIMAL(ENTRY(iLoop + 1, gcProdMod, coProduct:DELIMITER)) THEN DO:
        ASSIGN
            coProduct:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ENTRY(2, ENTRY(iLoop, gcProdMod, coProduct:DELIMITER), "|":U)
            plSuccess = TRUE.
