...
ASSIGN cNodes = DYNAMIC-FUNCTION("getTranslatableNodes":U IN ghCallerHandle) NO-ERROR.
...
ASSIGN cTreeViewName = DYNAMIC-FUNCTION("getTreeLogicalName":U IN ghCallerHandle) NO-ERROR.
IF ERROR-STATUS:ERROR THEN
...
DEFINE VARIABLE iLoop                           AS INTEGER    NO-UNDO.
DEFINE VARIABLE hFrame                          AS HANDLE     NO-UNDO. 
DEFINE VARIABLE hTreeviewTitleFillIn            AS HANDLE     NO-UNDO.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  /* If we're in a treeview, get the correct window title */
  ASSIGN hTreeviewTitleFillIn = DYNAMIC-FUNCTION("getTitleBar":U IN ghCallerHandle) NO-ERROR.
  IF VALID-HANDLE(hTreeviewTitleFillIn) 
  THEN DO:
      ASSIGN hContainer = DYNAMIC-FUNCTION('getContainerSource' IN TARGET-PROCEDURE) NO-ERROR.
      IF VALID-HANDLE(hContainer) 
      THEN DO:
          ASSIGN hTreeview = DYNAMIC-FUNCTION('getCallerProcedure' IN hContainer) NO-ERROR.
          IF VALID-HANDLE(hTreeview) 
          THEN DO:
              ASSIGN cTreeviewName = DYNAMIC-FUNCTION('getObjectName' IN hTreeview) NO-ERROR.
              /* Add entry for treeview window title */
              CREATE ttTranslate.
              ASSIGN
                ttTranslate.dLanguageObj        = DECIMAL(DYNAMIC-FUNCTION("getDataValue":U IN hLanguage))
                ttTranslate.cObjectName         = cTreeviewName
                ...
                ttTranslate.hWidgetHandle       = hTreeview
                ttTranslate.iWidgetEntry        = 0
  ...
  ASSIGN
    ...
    ttTranslate.iWidgetEntry        = 0
    ttTranslate.cOriginalLabel      = IF VALID-HANDLE(hTreeviewTitleFillIn)
                                      THEN hTreeviewTitleFillIn:SCREEN-VALUE
                                      ELSE ghCallerWindow:TITLE    
    ttTranslate.cTranslatedLabel    = "":U  
    ttTranslate.cOriginalTooltip    = "":U  
    ttTranslate.cTranslatedTooltip  = "":U
    .  
