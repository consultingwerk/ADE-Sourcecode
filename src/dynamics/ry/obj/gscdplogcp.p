...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsc_deploy_package Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the gsc_deploy_package table
...
&GLOB DATA-LOGIC-TABLE gsc_deploy_package
&GLOB DATA-FIELD-DEFS  "ry/obj/gscdpfullo.i"
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "gsc_deploy_package Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST gsc_deploy_package 
              WHERE gsc_deploy_package.package_code = b_gsc_deploy_package.package_code) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gsc_deploy_package.package_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {aferrortxt.i 'AF' '8' 'gsc_deploy_package' '' "'package_code, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST gsc_deploy_package 
              WHERE gsc_deploy_package.package_code = b_gsc_deploy_package.package_code
                AND ROWID(gsc_deploy_package) <> TO-ROWID(ENTRY(1,b_gsc_deploy_package.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gsc_deploy_package.package_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsc_deploy_package.package_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_deploy_package' 'package_code' "'Package Code'"}.

  IF isFieldBlank(b_gsc_deploy_package.package_description) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_deploy_package' 'package_description' "'Package Description'"}.
