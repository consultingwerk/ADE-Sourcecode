...
/*---------------------------------------------------------------------------------
  ...
  Description:  gst_deployment Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the gst_deployment table
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "gst_deployment Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST gst_deployment 
              WHERE gst_deployment.deploy_package_obj = b_gst_deployment.deploy_package_obj
                AND gst_deployment.originating_site_number = b_gst_deployment.originating_site_number
                AND gst_deployment.deployment_number = b_gst_deployment.deployment_number) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gst_deployment.deploy_package_obj) + ', ' + STRING(b_gst_deployment.originating_site_number) + ', ' + STRING(b_gst_deployment.deployment_number)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {aferrortxt.i 'AF' '8' 'gst_deployment' '' "'deploy_package_obj, originating_site_number, deployment_number, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST gst_deployment 
              WHERE gst_deployment.deploy_package_obj = b_gst_deployment.deploy_package_obj
                AND gst_deployment.originating_site_number = b_gst_deployment.originating_site_number
                AND gst_deployment.deployment_number = b_gst_deployment.deployment_number
                AND ROWID(gst_deployment) <> TO-ROWID(ENTRY(1,b_gst_deployment.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gst_deployment.deploy_package_obj) + ', ' + STRING(b_gst_deployment.originating_site_number) + ', ' + STRING(b_gst_deployment.deployment_number)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
...
------------------------------------------------------------------------------*/
  ...
  IF b_gst_deployment.deploy_package_obj = 0 OR b_gst_deployment.deploy_package_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gst_deployment' 'deploy_package_obj' "'Deploy Package Obj'"}.

  IF b_gst_deployment.originating_site_number = 0 OR b_gst_deployment.originating_site_number = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gst_deployment' 'originating_site_number' "'Originating Site Number'"}.

  IF b_gst_deployment.deployment_number = 0 OR b_gst_deployment.deployment_number = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gst_deployment' 'deployment_number' "'Deployment Number'"}.

  IF b_gst_deployment.load_after_deployment_obj = 0 OR b_gst_deployment.load_after_deployment_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gst_deployment' 'load_after_deployment_obj' "'Load After Deployment Obj'"}.
