...
DO:
    ...
    ASSIGN currentObjectType = DYNAMIC-FUNCTION("getObjectTypeCodeFromDB":u IN ghRepositoryDesignManager, DECIMAL(coObjectType:SCREEN-VALUE)) 
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  REPEAT:
    ...
    ELSE DO:
      ...
      IF LOOKUP(gsc_object_type.object_type_code, "Visual,Field,Action,Base,ProgressWidget,FieldWidget,AppServer,Messaging":U) > 0
        THEN lRemove = TRUE.

      
      FIND ext_object NO-LOCK 
          WHERE Ext_object.OBJECT_type_obj = gsc_object_type.extends_object_type_obj.
      IF LOOKUP(ext_object.OBJECT_type_code, "Visual,Field,Action,Base,ProgressWidget,FieldWidget,AppServer,Messaging":U) > 0
        THEN lRemove = TRUE.
  ...
  DO:
     ...
     DO i = 1 TO NUM-ENTRIES(coObjectType:LIST-ITEM-PAIRS,coObjectType:DELIMITER) BY 2:
        ASSIGN cEntry = ENTRY(i,coObjectType:LIST-ITEM-PAIRS,coObjectType:DELIMITER)
               cEntry = TRIM(SUBSTRING(cEntry,1,R-INDEX(cEntry,"(":U) - 1)) NO-ERROR.
        IF cEntry =  cMappedObjectType THEN 
        DO:
