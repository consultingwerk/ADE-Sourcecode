...
/*---------------------------------------------------------------------------------
  ...
  Description:  Wizard SBO SmartDataField

  Purpose:      SmartDataField that displays the SDOs of an SBO in a combo box to choose
                the query_SDO_name for rym_wizard_objc and rym_wizard_fold_page.

  Parameters:   <none>

  History:
  --------
  (v:010000)    Task:    90000108   UserRef:    posse
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 19.8 COLON-ALIGNED
...
ON VALUE-CHANGED OF coCombo IN FRAME frMain /* Query SDO Name */
DO:
...
/*     SUBSCRIBE TO "getComboQuery":U IN ghSDV. */
    SUBSCRIBE TO "populateCombo":U IN ghSDV.
  END.
...
DEFINE VARIABLE iNumItems       AS INTEGER      NO-UNDO.
  ...
  cListItemPairs = '<None>,.':U.

  DO iNumItems = 1 TO NUM-ENTRIES(pcList):
    cListItemPairs = cListItemPairs + ',':U +
        ENTRY(iNumItems, pcList) + ',':U + ENTRY(iNumItems,pcList).
  END.

  coCombo:LIST-ITEM-PAIRS IN FRAME {&FRAME-NAME} = cListItemPairs.

  setDataValue(pcValue).
