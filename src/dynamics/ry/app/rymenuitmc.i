...
IF NOT lSecured
AND gsm_menu_item.item_select_type = 'Launch':U 
THEN rycso-fetch-blk: DO:

    /* Is the container we're going to launch secured? */
    IF plRYCSOSecurityExists
    THEN DO:
    ...
    IF gsm_menu_item.instance_attribute_obj > 0
    THEN DO:
...
ASSIGN ttAction.menu_item_obj      = gsm_menu_item.menu_item_obj
       ...
       ttAction.DISABLED           = IF lSecured THEN YES ELSE ttAction.DISABLED.
...
THEN DO:
    ...
    THEN DO:
        ...
        THEN DO:
            ...
            IF gsc_item_category.parent_item_category_obj <> 0 
            THEN DO:
