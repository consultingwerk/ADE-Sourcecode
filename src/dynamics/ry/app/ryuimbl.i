    ...
    */
    ...
    CASE cSDOEvent:
      ...
      DO:
        ...
        ASSIGN 
          cFilter       = get-value('_':U + cSDOName + '._filter':U)
          cSortFields   = get-value('_':U + cSDOName + '._sorting':U)
          cFilterKey    = cOverrideLogicalObjectName + ".":U + cDSName + ".filter":U
          cSortingKey   = cOverrideLogicalObjectName + ".":U + cDSName + ".sorting":U
        ...
        RUN queryFilter (INPUT cOverrideLogicalObjectName, INPUT cDSName, INPUT cSDOEvent, INPUT hDS, INPUT YES).
        ...
        RUN queryFilter (INPUT cOverrideLogicalObjectName, INPUT cDSName, INPUT cSDOEvent, INPUT hDS, INPUT NO).
...
------------------------------------------------------------------------------*/
  ...
  IF (lResult AND (NOT ERROR-STATUS:ERROR )) THEN
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttDSSaveEvents:
    ...
    DO i1 = 1 TO NUM-ENTRIES(ttDSSaveEvents.DSSaveEvent, ",":U):
      ...
      cDataDelimiter = getSavedDSDataDelimiter(cOverrideObjectName, cDSName).
      ...
      */
      ...
      DO:
        ASSIGN 
          ...
          hSDO = findSDOHandleInSBO(cOverrideObjectName, cDSName, hSBO, cSDOName)
          hDS = hSBO
          cDataName = cSBOName + "." + cSDOName.
      END.
      ELSE
      DO:
        ASSIGN 
          hSDO = getDataSourceHandle(cOverrideObjectName, cSDOName, ?)
          cSBOName = "":U
          hDS = hSDO
          cDataName = cSDOName.
      END.
      ...
      cRowsToBatch = getSavedDSProperty(cOverrideObjectName, cSDOName, 'RowsToBatch':U).
      ...
      ASSIGN
        c1 = getSDODataColumns(cOverrideObjectName, hSDO, NO)
        cParamNames  = "":U
        cDataColumns = '':U
        cLookupCols  = '':U.
      ...
      ASSIGN cDSLookupList = DYNAMIC-FUNCTION("getPropertyList":U IN gshSessionManager,
                                                 cOverrideObjectName + ".":U + cSDOName + ".lookup":U, NO).
      ...
      DO iCols = 1 TO NUM-ENTRIES(cDSLookupList, "|":U):
        ...
        DO:
          ...
          IF lDSIsSBO THEN
            ASSIGN cLookupCols = cLookupCols + cSDOName + "." + cKeyField + ',':u.
          ELSE
      ...
      DO iSDOEvents = 1 to NUM-ENTRIES(cSDOEvents, "|":U):
        ...
        CASE cSDOEvent:
          ...
          DO:
            ...
            DO iCols = 1 to NUM-ENTRIES(cDataColumns):
              ...
              DO:
                ...
                cOldVal = DYNAMIC-FUNCTION('columnProps':U IN hDS, ENTRY(iCols, cDataColumns, ",":U), 'Initial':U) NO-ERROR.
                
                cLookupParam = ENTRY(LOOKUP(ENTRY(iCols,cDataColumns, ",":U), cLookupCols, ",":U), cLookupParams, ",":U).
                RUN saveLookup(cOverrideObjectName, cLookupParam,iColENTRY,cOldVal,cOldVal,cDataDelimiter) NO-ERROR.
                ...
                DO:
                  ...
                  ERROR-STATUS:ERROR = FALSE.
                  plSuccess = handleErrors(cOverrideObjectName, cSBOName, cSDOName, cSDOEvent, RETURN-VALUE, "", "._":U + ENTRY(iCols, cDataColumns, ",":U)).
                  RETURN.
                END.
              END.
            ...
            DO:
              ...
              plSuccess = handleErrors(cOverrideObjectName, cSBOName, cSDOName, cSDOEvent, cError, "", "").
              RETURN.
            END.
            ELSE
            DO:
          ...
          DO:
            ...
            DO iCols = 1 to NUM-ENTRIES(cDataColumns):
              ...
              DO:
                cLookupParam = ENTRY(LOOKUP(ENTRY(iCols,cDataColumns, ",":U), cLookupCols, ",":U), cLookupParams, ",":U).
                RUN saveLookup(cOverrideObjectName, cLookupParam,iColENTRY + 2, cOldVal, cNewVal, cDataDelimiter) NO-ERROR.
                IF NOT ERROR-STATUS:ERROR THEN
            ...
            DO:
              ...
              IF (cConflictError = 'CONFLICT') THEN
                plSuccess = returnConflict(cOverrideObjectName, cSDOName, cDataColumns, escapeData(cNewColValues), hDS) NO-ERROR.
              ELSE
                plSuccess = handleErrors(cOverrideObjectName, cSBOName, cSDOName, cSDOEvent, cError, "", "").
              RETURN.
            END.
          ...
          DO:
            ASSIGN iColENTRY = iColENTRY + 1
                   cRowId = ENTRY(iColENTRY, get-value(cdSDOName + ".rowident":U), cDataDelimiter) NO-ERROR.
                   
            IF lDSIsSBO THEN
              cDeleteColumn = cSDOName + ".RowIdent":U.
            ELSE
        ...
        DO:
          ASSIGN cData = appendLookupData(cOverrideObjectName, cSDOName, cDataColumns, cData) NO-ERROR.
          CREATE ttCommittedData.
          ASSIGN ttCommittedData.DSName = cSDOName
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  IF (pcCommitAction = "DELETE":U) THEN
    cData = "":U.
  ELSE   
    ASSIGN cData = appendLookupData(gcLogicalObjectName, pcObjectName, pcColumnNames, pcValues) NO-ERROR.
  CREATE ttCommittedData.
  ASSIGN ttCommittedData.DSName = pcObjectName
         ttCommittedData.Data = cData.
...
------------------------------------------------------------------------------*/
  ...
  /* Append lookup data to SDO data string */
    ...
    IF (i2 > 0)  THEN
    DO:
      cFieldValue = ENTRY(i2, cRetSDOValues, cDataDelimiter).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttCommittedData:
    ...
    {&OUT} (IF iSaveSuccess = 1 THEN " '":U ELSE ",'":U) + escapeData(ttCommittedData.Data) + "'":U + '~n':U.
...
------------------------------------------------------------------------------*/
  ...
  cOut = 'app._':U + pcSDOName + '.saveconflict([':U + '~n':U.
