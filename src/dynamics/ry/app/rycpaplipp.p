...
&IF DEFINED(EXCLUDE-updateRycPage) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE updateRycPage Procedure 
PROCEDURE updateRycPage :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT-OUTPUT PARAMETER TABLE FOR ttRycPage.
...
DEFINE BUFFER bttRycPage FOR ttRycPage.

FOR EACH ttRycPage
    WHERE ttRycPage.rowMod = "D":u:
    FIND bttRycPage
        WHERE bttRycPage.rowIdent = ttRycPage.rowIdent 
        AND   bttRycPage.rowMod = "":u NO-ERROR.
    IF  AVAILABLE bttRycPage THEN
        DELETE bttRycPage.
    ELSE DO:
        /***DIAG***/
...
RUN serverCommit IN hSDO (INPUT-OUTPUT TABLE ttRycPage, OUTPUT cErrorMessage, OUTPUT cUndoRowIds) NO-ERROR.
