...
DO iClassLoop = 1 TO NUM-ENTRIES(cClassList):
  ...
  FIND FIRST gsc_object_type WHERE
             gsc_object_type.object_type_code = cClassName
             NO-LOCK NO-ERROR.
  IF NOT AVAILABLE gsc_object_type THEN
      RETURN ERROR {aferrortxt.i 'AF' '5' 'gsc_object_type' 'object_type_code' '"object type (class)"' "'Object type: ' + cClassName" }.
  ...
  DO WHILE AVAILABLE gsc_object_type:
      ...
      FOR EACH ryc_attribute_value WHERE
         ...
         IF NOT CAN-FIND(FIRST ttClassAttribute 
                         WHERE ttClassAttribute.tClassName      = cClassName AND
                               ttClassAttribute.tAttributeLabel = ryc_attribute.attribute_label ) THEN
         DO:
             CREATE ttClassAttribute.
             ASSIGN ttClassAttribute.tClassName      = cClassName
                    ...
                    ttClassAttribute.tWhereStored    = gsc_object_type.object_type_code.
      ...
      ASSIGN dParentClassObj = gsc_object_type.extends_object_type_obj.

      FIND FIRST gsc_object_type WHERE
                 gsc_object_type.object_type_obj = dParentClassObj
                 NO-LOCK NO-ERROR.
      /* If there is a class available, then this loop will repeat.
       * If not, we leave the loop.                                 */
      NEXT.
  END.    /* available gsc_object_type */
END.  /* Class Loop */
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST gsc_object_type WHERE
               ...
               NO-LOCK NO-ERROR.
    IF NOT AVAILABLE gsc_object_type THEN
        RETURN ERROR {aferrortxt.i 'AF' '5' 'gsc_object_type' 'object_type_code' '"object type (class)"' "'Object type: ' + pcClassName" }.

    DO WHILE AVAILABLE gsc_object_type:
        ASSIGN pcInheritsFromClasses = pcInheritsFromClasses + ",":U + gsc_object_type.object_type_code.
        ...
        FOR EACH ryc_ui_event WHERE
            ...
            DO:
                ...
                ASSIGN ttUiEvent.tSmartObjectObj    = 0
                       ...
                       ttUiEvent.tWhereStored       = gsc_object_type.object_type_code.
    ...
    ASSIGN pcInheritsFromClasses = LEFT-TRIM(pcInheritsFromClasses, ",":U).
    &ENDIF
