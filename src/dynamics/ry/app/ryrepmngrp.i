 ...
 DO:
   ...
   FOR EACH ryc_attribute_value 
     ...
     CASE ryc_attribute.attribute_label:
       WHEN "SuperProcedure":U THEN ASSIGN ttClass.SuperProcedures = ryc_attribute_value.character_value.
       ...
       WHEN "SuperProcedureMode":U THEN ASSIGN ttClass.SuperProcedureModes = ryc_attribute_value.character_value.
...
------------------------------------------------------------------------------*/
    ...
    IF pcRunAttribute EQ ? THEN
        ASSIGN pcRunAttribute = DYNAMIC-FUNCTION("getSessionParam":U IN TARGET-PROCEDURE, "RunAttribute":U).
    IF pcRunAttribute EQ ? THEN
    ...
    DO:
        ASSIGN hManagerHandle = DYNAMIC-FUNCTION("getManagerHandle":U IN TARGET-PROCEDURE, "CustomizationManager":U).
    ...
    DO:
         ...
         * Default this to retrieving all pages.
         */
    ...
    /*
     ...
     this willhappen though because of the fact that dinstanceId is passed in to the doObjectRetrieval() 
     call.
    */
    ...
    ELSE
    DO:
        /* If not instances or pages were requested, then get everything.
         */
         ...
         */ 
        IF NOT CAN-DO(cPageList, STRING(0)) THEN
...
------------------------------------------------------------------------------*/
    ...
    DO iLoop = 1 TO iNumFields:
        ...
        /* Skip those fields that may have been updated already (off the ttClass
           record).
         */
        IF CAN-DO("SuperHandles,SuperProcedureModes,SuperProcedures,ClassObj,InheritsFromClasses":U,
...
ACCESS_LEVEL=PRIVATE
  ...
  Notes:       * Page '0' (zero) must be specified explicitly if the all option
                 is not used.
               * Gets all links for the specified object only when page zero is
                 retrieved.
               * Token and field security are passed in to this procedure because
                 objects are secured at a container level; it is cheaper to retrieve
                 this information once per object retrieval request.
               * This API is a server-side-only API.
------------------------------------------------------------------------------*/
...
------------------------------------------------------------------------------*/
  ...
  /* Save the cached entities to the disk if auto_dump_entity_cache is anything 
     other than NO or FALSE */
  IF LOOKUP(cDumpEntity, "no,false") = 0 THEN
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN hbuff = getCacheEntityObject(INPUT pcEntities) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN
    RETURN ERROR "Error while retrieving entity information".
  ...
  DO WHILE hbuff:AVAILABLE:
    ...
    DO i = 1 TO hHandle:NUM-FIELDS:
      ...
      IF hHandle:BUFFER-FIELD(i):DATA-TYPE = "DATE" AND 
         DATE(hHandle:BUFFER-FIELD(i):INITIAL) = TODAY THEN
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 to NUM-ENTRIES(pcEntityNames):
    ASSIGN cEntityName = ENTRY(i, pcEntityNames).
...
------------------------------------------------------------------------------*/
 ...
 IF pcRenameColumnsByEntity = '':U THEN
   pcRenameColumnsByEntity = FILL({&adm-tabledelimiter},NUM-ENTRIES(pcEntities) - 1).
 ...
 /* Build a list that corresponds to ColumnsByEntity where the columns are
    ...
    Also build a list of non-cached entities so we can cahce the missing ones
    in one call later */
 DO iEntity = 1 TO NUM-ENTRIES(pcEntities): 
   ASSIGN
 ...
 DO FOR bEntity:
   ...
   DO iColumn = 1 TO NUM-ENTRIES(pcColumns):
     ...
     /* If not found in the list of qualifiedColumns then this is a calculated 
        column. If dynamic (not prepared) we get the instance properties from
        Repository. (prepared/static TTs is currently not stored in Repository)*/
     ...
     DO:   
       ...
       DO:
         ...
         RUN getInstanceProperties IN TARGET-PROCEDURE
                    (cColumn,
                     '':U,
                     INPUT-OUTPUT cAttributes,
                     OUTPUT cAttributeValues) NO-ERROR.

         IF ERROR-STATUS:ERROR THEN
         DO:
           MESSAGE  "Failed add columns to the RowObjectTable." SKIP
                     "Could not create calculated Field'" cColumn "' from Repository" SKIP(1)
                    (IF RETURN-VALUE EQ "":U THEN ERROR-STATUS:GET-MESSAGE(1) ELSE RETURN-VALUE)                
           VIEW-AS ALERT-BOX ERROR.
           RETURN FALSE.
         END.
         /* Avoid NO-ERROR and let Progress give the error 
            (Difficult to anticipate all possible errors)  */
         DO ON ERROR UNDO,RETURN FALSE:
           ASSIGN
             cDataType     = ENTRY(1,cAttributeValues,CHR(1))
             cFormat       = ENTRY(2,cAttributeValues,CHR(1))
             cDefaultValue = ENTRY(3,cAttributeValues,CHR(1))
             cLabel        = ENTRY(4,cAttributeValues,CHR(1))
             cColumnLabel  = ENTRY(5,cAttributeValues,CHR(1))
             lOk           = phRowObjectTable:ADD-NEW-FIELD
                                   (cColumn,
                                    cDataType,
                                    0,   /* Extent  */
                                    IF cFormat <> '?':U THEN cFormat ELSE ?,
                                    IF cDefaultValue <> '?':U THEN cDefaultValue ELSE ?,
                                    IF cLabel <> '?':U THEN cLabel ELSE ?,
                                    IF cColumnLabel <> '?':U THEN cLabel ELSE ?)
            . 
         END. /* do on error (add-field from repos) */
       END. /* not prepared */
     END. /* Calculated */ 
     ELSE 
     DO:
