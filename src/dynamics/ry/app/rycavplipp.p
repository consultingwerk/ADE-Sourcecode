...
/*---------------------------------------------------------------------------------
  ...
  (v:010000)    Task:        6962   UserRef:    
                Date:   04/01/2001  Author:     Jenny Bond

  Update Notes: Procedures to read/update attribute value

  (v:010100)    Task:    90000052   UserRef:    POSSE
                Date:   02/05/2001  Author:     Phil Magnay
...
&IF DEFINED(EXCLUDE-updateRycAttributeValue) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE updateRycAttributeValue Procedure 
PROCEDURE updateRycAttributeValue :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH ttRycAttributeValue
    WHERE ttRycAttributeValue.rowMod = "D":u:
    FIND bttRycAttributeValue
        WHERE bttRycAttributeValue.rowIdent = ttRycAttributeValue.rowIdent 
        AND   bttRycAttributeValue.rowMod = "":u NO-ERROR.
    IF  AVAILABLE bttRycAttributeValue THEN
        DELETE bttRycAttributeValue.
    ELSE DO:
        /***DIAG***/
...
/*
/* limit rows retrieved for efficiency */
...
RUN serverCommit IN hSDO (INPUT-OUTPUT TABLE ttRycAttributeValue, OUTPUT cErrorMessage, OUTPUT cUndoRowIds) NO-ERROR.
IF  ERROR-STATUS:ERROR OR RETURN-VALUE <> "":u  THEN DO:
    cErrorMessage = IF cErrorMessage = "":u THEN RETURN-VALUE ELSE cErrorMessage + CHR(3) + RETURN-VALUE.
