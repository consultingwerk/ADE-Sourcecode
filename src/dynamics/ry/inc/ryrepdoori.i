...
---------------------------------------------------------------------------------*/    
    ...
    DO iResultCodeLoop = 1 TO iNumberOfResultCodes:
        ...
        DO:
            ...
            DO:
                /* If we are requesting all pages, and the object has a InitPages and/or StartPage attribute,
                 * then we use these to construct a list of pages to retrieve initially.
                 */
                IF pcPageList EQ "*":U THEN
                DO:
                    ...
                    DO:
                        ...
                        ELSE
                            ASSIGN cEntry = hBufferField:INITIAL.
                        
                        IF cEntry NE ? AND NOT CAN-DO("*,", cEntry) THEN
                            ASSIGN pcPageList = cEntry.                                        
                    ...
                    IF NOT CAN-DO(pcPageList, STRING(0)) THEN
                        ASSIGN pcPageList = pcPageList + ",":U + STRING(0).
                END.    /* page list is all */
     ...
     */
    do:
        assign dContainerInstanceId = cacheObject.InstanceId.
