...
Maincontrol.prototype.framepos=function(name){
  ...
  if(!f || !f.dyntag) return;
  var from=f.dyntag.parentNode.style;
  var off=f.parent;
  f.top=(from.top).replace('px','')*1+off.top;      // Recursively storing positions for frames 
...
Maincontrol.prototype.fixMainSizes=function(){
  ...
  var IFRAME=appcontrol.activeframe.tag;
...
// App width
  ...
  dh=(dh+ih)>10?(dh+ih):10;
  TREE.style.height=dh+'px';
