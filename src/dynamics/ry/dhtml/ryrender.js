...
// Configure horizontal resize for treeview
...
function resizex(e){   // Horizontal resize of treeview pane
  window.fixEvent(e); 
...
function fixRectangles(){  // Set resize for browses
  ...
  for(var j=0;j<elem.length;j++){
    var e=elem[j];
    if(e.getAttribute('resize')!='resize') continue;
...
function fixLayout(){
  ...
  for(var i=0;i<a.length;i++){
    ...
    var grow='no';                               // fixed size objects
    if(ch.getAttribute('tabs')) layout.tabcell=a[i];
    if(ch.getAttribute('resize')) grow='yes';    // Browse = grow/lock depends
    a[i].setAttribute('grow',grow);
  }	
...
function sizeContainer(app){
  ...
  if(lay.tabcell){
    ...
    lay.tabcell.setAttribute('grow',grow);
  }
	sizePage(lay);
  if(app.frameObject.dyntag){
...
function sizeDynframe(fr,lay){
  ...
  fr.dyntag.parentNode.setAttribute('grow',lay.grow>0?'yes':'no');
}
...
//      if(min==null || min==undefined || min=='') alert('undefined='+min+'/'+r+','+i+'\n'+cell.outerHTML)
      ...
      min=(min+',').split(',');
      var gr=cell.getAttribute('grow');
      if(gr=='yes') min[0]=min[0]*1+growx;
    	if(gr!='lock') min[1]=maxy;
...
function sizePage(lay){
  ...
  function sizeRow(row){
    ...
    for(e in row.col){  // Loop through columns for row
      var cell=row.col[e];
      if(cell.getAttribute('grow')=='yes') row.grow++;   // Find grow for row     
