...
//  alert('[rylogic.js] assignwidgetvalue');
  return action(this.setWidgetPrefix(fld)+'.set|'+val);
}
...
//  alert('[rylogic.js] assignwidgetvaluelist');
  ...
  for (var i=0;i<f.length;i++){
    ...
    artn=action(this.setWidgetPrefix(f[i])+'.set|'+val);
    rtn=rtn&&artn;
  }
  return rtn;
}
...
//  alert('[rylogic.js] blankwidget');
  ...
  for (var i=0;i<f.length;i++){
    action(this.setWidgetPrefix(f[i])+'.set|');
  }
  return true;
}
...
//  alert('[rylogic.js] resetwidgetvalue');
  ...
  for (var i=0;i<f.length;i++){
    ...
    action(this.setWidgetPrefix(f[i])+'.set|'+old);
  }
  return rtn;
}
