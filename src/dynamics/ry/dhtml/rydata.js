...
//  this.childRow=this.row;   // Child-data is always synchronized at Load
...
Data.prototype.pick=function(iNew){
  with(this){
//    if(iNew>0 && (!lNav || lMod)) return false;
    ...
    for(var i=0;i<target.length;i++) target[i].refreshRow();  // positioning each Wdo
    refreshTools();
  }
}
...
Data.prototype.refreshTools=function(){ // Set appropriate button statuses
  with(this){
    ...
    // Cannot add if any parent is in add mode or has no data
    var parentAdd=false;
    var e=this;
    while(e=e.parentDS) if(e.lAdd || !e.data.length) parentAdd=true;
    ...
    this.status=
     ...
     ,filter : lNav && !lMod && !lAdd && (lDat || filterOn)
     ,find   : lOK
    };
