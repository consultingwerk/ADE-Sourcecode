  ...
  with(this){
    ...
    for(var panel in tools){
      ...
      for(var e in status){
        ...
        if(e=='filter' && this.filterOn!=hdata.filterOn){
...
Wdo.prototype.viewerMode=function(){
  with(this){
    for(var i in inputField){
      var e=inputField[i];
      var lOn=hdata.lUpd && e.getAttribute('disable')!='true';
      if(e.nodeName=='SELECT'||e.nodeName=='BUTTON'||e.type=='radio'||e.type=='checkbox') e.disabled=(lOn?null:true);
      if(e.nodeName=='TEXTAREA'||e.type=='text') e.readOnly=(lOn?null:true);
      if(e.getAttribute('tab')) e.tabIndex=(lOn?e.getAttribute('tab'):-1);
...
Wdo.prototype.initFocus=function(){    // Initialize focus
  ...
  if(el && !el.disabled && el.style.visibility!='hidden') try{
    el.focus();
  } catch(e){};
}
