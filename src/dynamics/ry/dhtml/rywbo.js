...
Wbo.prototype.setMenues=function(){
  ...
  if(!app.isPopup && !app.isFrame) loadMenu(appcontrol.menuglobal);  // Load global menues
  ...
  function loadMenu(menu){
    for(var i=0;i<menu.length;i++){
      ...
      if(tgt=='panel'){                             // Panel are rendered immediately and buttons registered
        m.innerHTML=window.toolbar.render(mm);
        if(wdo){
        ...
        if(!app.menuCache[tgt]) app.menuCache[tgt]=(tgt=='menubar'?[[0,'root']]:[]);  // Hierarchy for menubar/treeview
        if(tgt=='toolbar' && wdo!='wbo'){
          wdo=wdo.split(',');
          for(var j=0;j<wdo.length;j++){
            app.toolbar[wdo[j]]=true; // Add all wdo to toolbar object
          }
...
Wbo.prototype.process=function(){    // When being run from hidden frame
  this.committing=false;
  this.app.document.form['do'].value='';
  for(var e in app.wdo) if(app.wdo[e].hdata.mustinitdata) app.wdo[e].hdata.initdata(); // Initialize screen data
  this.datasync();
}
...
Wbo.prototype.action=function(c,prm){
  with(this){
    ...
    case 'refresh':
      for(var e in app.wdo) app.wdo[e].hdata.refreshTools();  // Refresh tools for WDOs
      return;
