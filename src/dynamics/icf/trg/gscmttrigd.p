...
TRIGGER PROCEDURE FOR DELETE OF gsc_manager_type .
...
/* gsc_manager_type describes gsm_required_manager ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST gsm_required_manager WHERE
    ...
    DO:
      ...
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "gsc_manager_type|gsm_required_manager":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscmttrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscmttrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_manager_type.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscmttrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gscmt":U, INPUT BUFFER gsc_manager_type:HANDLE, INPUT BUFFER o_gsc_manager_type:HANDLE).
