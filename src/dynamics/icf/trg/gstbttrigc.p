...
TRIGGER PROCEDURE FOR CREATE OF gst_batch_job .
...
/* gsm_status  gst_batch_job ON CHILD INSERT RESTRICT */
IF 
    ( gst_batch_job.status_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsm_status WHERE
        ...
        DO:
          ...
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gst_batch_job|gsm_status":U.
...
/* Generated by ICF ERwin Template */
/* gsm_user  gst_batch_job ON CHILD INSERT RESTRICT */
IF 
    ( gst_batch_job.user_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsm_user WHERE
        gst_batch_job.user_obj = gsm_user.user_obj)) THEN
        DO:
          /* Cannot create child because parent does not exist ! */
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gst_batch_job|gsm_user":U.
          RUN error-message (lv-errgrp, lv-errnum, lv-include).
        END.
...
ASSIGN gst_batch_job.{&TRIGGER_OBJ} = getNextObj() NO-ERROR.
IF ERROR-STATUS:ERROR THEN 
DO:
    ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 31 lv-include = "gst_batch_job|the specified object number.  Please ensure your database sequences have been set correctly":U.
    RUN error-message (lv-errgrp, lv-errnum, lv-include).
END.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "CREATE":U, INPUT "gstbt":U, INPUT BUFFER gst_batch_job:HANDLE, INPUT BUFFER o_gst_batch_job:HANDLE).
