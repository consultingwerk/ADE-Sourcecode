...
TRIGGER PROCEDURE FOR CREATE OF gsc_language_text .
...
/* gsm_category of gsc_language_text ON CHILD INSERT RESTRICT */
IF 
    ( gsc_language_text.category_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsm_category WHERE
        ...
        DO:
          ...
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gsc_language_text|gsm_category":U.
...
/* Generated by ICF ERwin Template */
/* gsc_language  gsc_language_text ON CHILD INSERT RESTRICT */
IF 
    ( gsc_language_text.language_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsc_language WHERE
        gsc_language_text.language_obj = gsc_language.language_obj)) THEN
        DO:
          /* Cannot create child because parent does not exist ! */
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gsc_language_text|gsc_language":U.
          RUN error-message (lv-errgrp, lv-errnum, lv-include).
        END.
...
ASSIGN gsc_language_text.{&TRIGGER_OBJ} = getNextObj() NO-ERROR.
IF ERROR-STATUS:ERROR THEN 
DO:
    ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 31 lv-include = "gsc_language_text|the specified object number.  Please ensure your database sequences have been set correctly":U.
    RUN error-message (lv-errgrp, lv-errnum, lv-include).
END.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "CREATE":U, INPUT "gsclt":U, INPUT BUFFER gsc_language_text:HANDLE, INPUT BUFFER o_gsc_language_text:HANDLE).
