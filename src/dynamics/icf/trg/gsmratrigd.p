...
TRIGGER PROCEDURE FOR DELETE OF gsm_range .
...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gsm_range           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gsm_range
&SCOPED-DEFINE TRIGGER_FLA gsmra
&SCOPED-DEFINE TRIGGER_OBJ range_obj


DEFINE BUFFER lb_table FOR gsm_range.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gsm_range.     /* Used for lock upgrades */

DEFINE BUFFER o_gsm_range FOR gsm_range.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_range.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_range.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmratrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_range.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_range.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmratrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_range.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_range.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmratrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmra':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmra":U, INPUT BUFFER gsm_range:HANDLE, INPUT BUFFER o_gsm_range:HANDLE).
