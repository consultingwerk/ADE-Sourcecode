...
TRIGGER PROCEDURE FOR CREATE OF gsm_flow .
...
/* gsc_product_module contains gsm_flow ON CHILD INSERT RESTRICT */
IF 
    ( gsm_flow.product_module_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsc_product_module WHERE
        ...
        DO:
          ...
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gsm_flow|gsc_product_module":U.
...
ASSIGN gsm_flow.{&TRIGGER_OBJ} = getNextObj() NO-ERROR.
IF ERROR-STATUS:ERROR THEN 
DO:
    ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 31 lv-include = "gsm_flow|the specified object number.  Please ensure your database sequences have been set correctly":U.
    RUN error-message (lv-errgrp, lv-errnum, lv-include).
END.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "CREATE":U, INPUT "gsmfw":U, INPUT BUFFER gsm_flow:HANDLE, INPUT BUFFER o_gsm_flow:HANDLE).
