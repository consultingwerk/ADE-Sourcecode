...
TRIGGER PROCEDURE FOR CREATE OF gsc_logical_service .
...
/* gsc_service_type categorizes gsc_logical_service ON CHILD INSERT RESTRICT */
IF 
    ( gsc_logical_service.service_type_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsc_service_type WHERE
        ...
        DO:
          ...
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gsc_logical_service|gsc_service_type":U.
...
ASSIGN gsc_logical_service.{&TRIGGER_OBJ} = getNextObj() NO-ERROR.
IF ERROR-STATUS:ERROR THEN 
DO:
    ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 31 lv-include = "gsc_logical_service|the specified object number.  Please ensure your database sequences have been set correctly":U.
    RUN error-message (lv-errgrp, lv-errnum, lv-include).
END.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "CREATE":U, INPUT "gscls":U, INPUT BUFFER gsc_logical_service:HANDLE, INPUT BUFFER o_gsc_logical_service:HANDLE).
