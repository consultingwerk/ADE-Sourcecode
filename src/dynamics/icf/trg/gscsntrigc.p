...
TRIGGER PROCEDURE FOR CREATE OF gsc_next_sequence .
...
/* gsc_sequence has gsc_next_sequence ON CHILD INSERT RESTRICT */
IF 
    ( gsc_next_sequence.sequence_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsc_sequence WHERE
        ...
        DO:
          ...
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "gsc_next_sequence|gsc_sequence":U.
...
ASSIGN gsc_next_sequence.{&TRIGGER_OBJ} = getNextObj() NO-ERROR.
IF ERROR-STATUS:ERROR THEN 
DO:
    ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 31 lv-include = "gsc_next_sequence|the specified object number.  Please ensure your database sequences have been set correctly":U.
    RUN error-message (lv-errgrp, lv-errnum, lv-include).
END.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "CREATE":U, INPUT "gscsn":U, INPUT BUFFER gsc_next_sequence:HANDLE, INPUT BUFFER o_gsc_next_sequence:HANDLE).
