...
TRIGGER PROCEDURE FOR WRITE OF gsc_deploy_package OLD BUFFER o_gsc_deploy_package.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER gsc_deploy_package:HANDLE).
...
IF NOT NEW gsc_deploy_package AND gsc_deploy_package.{&TRIGGER_OBJ} <> o_gsc_deploy_package.{&TRIGGER_OBJ} THEN
    DO:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "gscdp":U, INPUT BUFFER gsc_deploy_package:HANDLE, INPUT BUFFER o_gsc_deploy_package:HANDLE).
