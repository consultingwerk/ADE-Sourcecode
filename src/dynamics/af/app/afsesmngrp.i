...
------------------------------------------------------------------------------*/
  ...
  cFieldPopupMapping = DYNAMIC-FUNCTION("getFieldPopupMapping":U IN phObject) NO-ERROR.
  ...
  DO iLoop = 1 TO NUM-ENTRIES(cAllFieldHandles):
    ...
    THEN DO:
        FIND FIRST ttViewerCol
             WHERE ttViewerCol.dColumn = DYNAMIC-FUNCTION("getCol":U IN hWidget)
             NO-LOCK NO-ERROR.
        ...
        hSDFFrame = ?.
        {get ComboHandle hSDFFrame hWidget} NO-ERROR.
        IF NOT VALID-HANDLE(hSDFFrame) THEN
          {get LookupHandle hSDFFrame hWidget} NO-ERROR.
        ...
        THEN DO:
            ...
            RUN resizeSDFFrame IN TARGET-PROCEDURE
                               (INPUT hWidget,   /* The SDF procedure handle */
                                ...
                                INPUT ttViewerCol.dNewCol - DYNAMIC-FUNCTION("getCol":U IN hWidget)). /* The amount it needs to move */
            hSDFLabelHandle = DYNAMIC-FUNCTION("getLabelHandle":U IN hWidget) NO-ERROR.
    ...
    THEN DO:
        ASSIGN hSideLabel:SCREEN-VALUE = IF INDEX(hSideLabel:SCREEN-VALUE, ":":U) = 0
                                         ...
                                         NO-ERROR.
        ASSIGN hSideLabel:WIDTH  = FONT-TABLE:GET-TEXT-WIDTH-CHARS(hSideLabel:SCREEN-VALUE, hSideLabel:FONT) NO-ERROR.
        ASSIGN hSideLabel:COLUMN = (hWidget:COLUMN - hSideLabel:WIDTH) NO-ERROR.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO iLoop = 1 TO NUM-ENTRIES(cAllFieldHandles):
    ...
    /* got a valid widget to move */
    ASSIGN hSideLabel = DYNAMIC-FUNCTION("getLabelHandle":U IN phObject).   
...
------------------------------------------------------------------------------*/
  ...
  AND hParentFrame:TYPE = "FRAME":U THEN
      IF (phFrame:COLUMN + phFrame:WIDTH + pdAddCol) <= hParentFrame:WIDTH THEN DO:
          ...
          ASSIGN hSideLabel = DYNAMIC-FUNCTION("getLabelHandle":U IN phObject).   
          IF VALID-HANDLE(hSideLabel) THEN
            hSideLabel:COLUMN = IF (phFrame:COLUMN - hSideLabel:WIDTH) < 1 THEN 1 ELSE (phFrame:COLUMN - hSideLabel:WIDTH).
  END.
      ELSE
...
DEFINE INPUT PARAMETER phObject AS HANDLE NO-UNDO.
  ...
  DEFINE VARIABLE lHasDisplayField          AS LOGICAL    NO-UNDO.
  ...
  ASSIGN cResizeForTranslation = DYNAMIC-FUNCTION("getUserProperty":U IN phObject, INPUT "ResizeForTranslation":U).
  ...
  IF cAllFieldHandles = "":U 
  OR cAllFieldHandles = ? 
  AND LOOKUP("getLookupHandles":U,phObject:INTERNAL-ENTRIES) > 0 
  THEN DO:
      ASSIGN cAllFieldHandles = STRING(phObject).
      ASSIGN hSDFParentFrame = DYNAMIC-FUNCTION("getcontainersource":U IN phObject).
  ...
  THEN DO:
    ...
    DO iLoop = 1 TO NUM-ENTRIES(cAllFieldHandles):
      ...
      ASSIGN iFont = ?.
      IF NOT CAN-QUERY(hWidget,"FONT":U) 
      THEN DO:
          /* Check for SDF */
          IF CAN-QUERY(hWidget,"FILE-NAME":U) 
          THEN DO:
              ASSIGN hLabelHandle = ?
                     hLabelHandle = DYNAMIC-FUNCTION("getLabelHandle":U IN hWidget) 
                     NO-ERROR.

              IF VALID-HANDLE(hLabelHandle) 
              AND CAN-QUERY(hLabelHandle,"FONT":U) THEN
      ...
      IF NOT CAN-QUERY(hWidget,"LABEL":U) 
      THEN DO:
          ASSIGN cLabelText = "":U.
          /* Check for SDF */
          IF CAN-QUERY(hWidget,"FILE-NAME":U) THEN 
              ASSIGN cLabelText = DYNAMIC-FUNCTION("getFieldLabel":U IN hWidget) NO-ERROR.
          ASSIGN dNewLabelLength = FONT-TABLE:GET-TEXT-WIDTH-CHARS(cLabelText + " :   ":U, iFont).
      END.
      ELSE else-blk: DO:

          IF hWidget:TYPE = "radio-set":U 
          THEN DO:
              ASSIGN cLabelText      = "":U
                     dNewLabelLength = 0.
              LEAVE else-blk.
      ...
      AND CAN-QUERY(hWidget, "column":U) 
      ...
      ELSE DO:
          ...
          THEN DO:
              ASSIGN dColumn = ?
                     dColumn = DYNAMIC-FUNCTION("getCol":U IN hWidget) 
                     NO-ERROR.
  ...
  FOR EACH ttViewerCol:
    ...
    IF ttViewerCol.dColumn < ttViewerCol.dNewCol 
    THEN DO:
  ...
  IF dMinFrameWidth > (SESSION:WIDTH - 5) 
  THEN DO:
      RUN afmessagep IN TARGET-PROCEDURE 
  ...
  /* Invoking getFieldLabel and invoking getDisplayField are used to determine whether phObject
     is a lookup/combo or another SDF.  This is a hack, instanceOf cannot be used 
     to determine the object type for SDFs on static viewers and INTERNAL-ENTRIES cannot 
     be used because adm prototypes may be excluded from the object.  This needs to 
     be resolved in the future. */
  ASSIGN cField = DYNAMIC-FUNCTION("getFieldLabel":U IN phObject) NO-ERROR.
  IF NOT ERROR-STATUS:ERROR AND ERROR-STATUS:NUM-MESSAGES = 0 THEN
      ASSIGN lHasFieldLabel = TRUE.

  ASSIGN cField = DYNAMIC-FUNCTION("getDisplayField":U IN phObject) NO-ERROR.
  IF NOT ERROR-STATUS:ERROR AND ERROR-STATUS:NUM-MESSAGES = 0 THEN
      ASSIGN lHasDisplayField = TRUE.

  /* Need to resize frame to fit new labels */
  IF CAN-FIND(FIRST ttViewerCol WHERE ttViewerCol.dColumn <> ttViewerCol.dNewCol) 
  THEN DO:
  ...
  FOR EACH ttTranslate:
    ...
    CASE ttTranslate.cWidgetType:
      ...
      THEN DO:
          IF ttTranslate.cTranslatedLabel NE "":U 
          AND ttTranslate.cTranslatedLabel NE ? 
          THEN DO:
              IF ttTranslate.iWidgetEntry EQ 0 
              THEN DO:
      ...
      THEN DO:
          ...
          IF NOT ttTranslate.hWidgetHandle:HORIZONTAL 
          THEN DO:
              DO iRadioLoop = 1 TO NUM-ENTRIES(cRadioButtons) BY 2:
                  ASSIGN cRadioOption = ENTRY(iRadioLoop, cRadioButtons).
      ...
      OTHERWISE DO:
          ...
          ELSE 
              IF ttTranslate.cTranslatedLabel <> "":U 
              AND INDEX(ttTranslate.cObjectName, ":":U) <> 0 
              AND ttTranslate.cOriginalLabel = "nolabel":U 
              THEN DO:          
          ...
          IF ttTranslate.cTranslatedTooltip <> "":U 
          THEN DO:
  ...
  AND cTranslatedTextsAndValues <> "":U 
  THEN DO:
...
DEFINE VARIABLE cSDFFieldLabel       AS CHARACTER NO-UNDO.
...
ASSIGN hDataSource      = DYNAMIC-FUNCTION("getDataSource":U IN phObject)
       cDisplayedFields = DYNAMIC-FUNCTION("getAllFieldHandles":U IN phObject) NO-ERROR.
...
IF VALID-HANDLE(phFrame) AND phFrame:NAME <> "FolderFrame":U THEN DO:
  ...
  IF pcAction = "setup":U THEN DO:
    ...
    ASSIGN cContainerName = DYNAMIC-FUNCTION('getLogicalObjectName':U IN hRealContainer) NO-ERROR.
    ...
    DO: /* for toolbars and SDF's - use container name for translations */
      ...
      ASSIGN cObjectName = DYNAMIC-FUNCTION('getLogicalObjectName':U IN phContainer) NO-ERROR.
    ...
    DO: /* otherwise use object name for translations */
      ...
      ASSIGN cObjectName = DYNAMIC-FUNCTION('getLogicalObjectName':U IN phObject) NO-ERROR.
      IF cObjectName = "":U OR cObjectName = ? THEN
        ASSIGN cObjectName = phObject:FILE-NAME.
  ...
  cSDFFieldLabel = DYNAMIC-FUNCTION("getFieldLabel":U IN phObject) NO-ERROR.
  IF NOT ERROR-STATUS:ERROR AND ERROR-STATUS:NUM-MESSAGES = 0 THEN
    lHasFieldLabel = TRUE.
  IF pcAction = "setup":U AND hWidget = ? AND INDEX(cObjectName, ":":U) <> 0 AND 
     lObjectTranslated NE YES AND lHasFieldLabel THEN
  DO:
    CREATE ttTranslate.
    ASSIGN
  ...
  DO:
      IF cObjectName = ? OR cObjectName = "":U 
      THEN DO:
  ...
  DO WHILE VALID-HANDLE (hWidget):
    ...
    AND hWidget:TYPE = "FRAME":U THEN
      cFieldSecurity = DYNAMIC-FUNCTION("setSecurityForDynObjects":U IN TARGET-PROCEDURE,hWidget,cSecuredFields,cDisplayedFields,cFieldSecurity,phObject).
  ...
  IF pcAction = "Setup":U 
  AND CAN-DO("browse,text,button,fill-in,selection-list,editor,combo-box,radio-set,slider,toggle-box":U, hWidget:TYPE)
  THEN DO:
      ...
      ELSE DO:
          /* Fields in an SBO will be qualified with an SDO name.  If so, remove the qualifier and pretend it's *
           * just a normal field for security purposes.  Also, remove a table qualifier if present.             */     
          IF NUM-ENTRIES(cFieldName, ".":U) = 2 THEN
      ...
      AND CAN-FIND(FIRST ttTranslate
                   WHERE ttTranslate.dLanguageObj = 0
                     ...
                     AND ttTranslate.cWidgetName  = cFieldName) 
      THEN DO:
          ASSIGN hWidget = hWidget:NEXT-SIBLING.
          NEXT widget-walk.
      END.
      ...
      THEN DO:
          ...
          AND CAN-QUERY(hWidget, "LIST-ITEM-PAIRS":U)
          THEN DO:
              ASSIGN cListItems = hWidget:LIST-ITEM-PAIRS
                     cDelimiter = hWidget:DELIMITER.

              IF cListItems NE ? AND NUM-ENTRIES(cListItems, cDelimiter) GE 2 
              THEN DO iRadioLoop = 1 TO NUM-ENTRIES(cListItems, cDelimiter) BY 2:
                  CREATE ttTranslate.
          ...
          THEN DO:
              ...
              IF INDEX(cObjectName, ":":U) <> 0 AND ttTranslate.cOriginalLabel = "":U 
              THEN DO:
                  ASSIGN hLabel = ?
                         hLabel = DYNAMIC-FUNCTION("getLabelHandle":U IN phObject) 
                         NO-ERROR.
          ...
          THEN DO: 
              ...
              DO iBrowseLoop = 1 TO hWidget:NUM-COLUMNS:
                  ...
                  THEN DO:
                      ...
                      IF iEntry > 0 AND NUM-ENTRIES(cSecuredFields) > iEntry 
                      THEN DO:
                          ENTRY(iBrowseLoop,cFieldSecurity) = ENTRY(iEntry + 1, cSecuredFields).
                          CASE ENTRY(iBrowseLoop,cFieldSecurity):
                              WHEN "HIDDEN":U    THEN ASSIGN hColumn:VISIBLE = NO.
                              WHEN "READ ONLY":U THEN ASSIGN hColumn:COLUMN-READ-ONLY = YES NO-ERROR.
                          END CASE.
                      END.
    ...
    DO:
        ...
        DO:
            ...
            THEN DO:
                ...
                RUN increaseFrameforPopup IN TARGET-PROCEDURE
                                          (INPUT phObject,
                                           INPUT phFrame,
                                           INPUT hLookup,
                                           INPUT hWidget).
...
------------------------------------------------------------------------------*/
    ...
    REPEAT WHILE iPosn <> 0 AND iPosn <> ?:
        ...
        DO:
            ...
            IF DYNAMIC-FUNCTION("isObjQuoted":U IN TARGET-PROCEDURE, pcQueryString, iPosn) = FALSE THEN
