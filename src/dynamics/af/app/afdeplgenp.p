...
------------------------------------------------------------------------------*/
    ...
    publish 'ICFDA_logMessage':u ({&LOG-LEVEL-INFO}, '{&*LOG-CATEGORY}':u,
                'Generation options:~n'
               + "Target location         : " + pcTarget
               + "Plugin procedure        : " + plPluginProcedure
               + "Generate thin rendering : " + string(plThinRendering)
               + "Super procedure loc     : " + pcSuperProcLocation
               + "Result codes            : " + pcResultCodeList
               + "Generate translations   : " + STRING(LOOKUP("generatetranslations":U, cPgenOptions) > 0)
               + (IF (LOOKUP("generatetranslations":U, cPgenOptions) > 0) THEN
                   "Languages               : " + pcLanguageList 
                  ELSE
                  "")
               + (IF plCompile THEN "Rcode location          : " + pcCompileTarget ELSE "")
               + (IF plCompile THEN "Compile options         : " + pcCompileOptions ELSE "") ).
