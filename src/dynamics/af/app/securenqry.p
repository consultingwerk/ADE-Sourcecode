...
------------------------------------------------------------------------------*/
    ...
    REPEAT WHILE iPosn <> 0 AND iPosn <> ?:
        ...
        IF  cBefore >= "0":U AND cBefore <= "9":U 
        AND cAfter  >= "0":U AND cAfter  <= "9":U 
        THEN DO:
            /* See if it is not quoted and thus needs to be changed */
            IF DYNAMIC-FUNCTION("isObjQuoted":U IN gshSessionManager, pcQueryString, iPosn) = FALSE THEN
...
FUNCTION newWhereClause RETURNS CHARACTER
  ...
  ------------------------------------------------------------------------------*/
   ...
   ASSIGN pcWhere = DYNAMIC-FUNCTION("fixQueryString":U IN TARGET-PROCEDURE, INPUT pcWhere)
          cString = pcWhere
          iStart  = 1.
