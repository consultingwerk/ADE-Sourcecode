...
/*---------------------------------------------------------------------------------
  ...
  Description:  Service Type Combo SmartDataField

  Purpose:      Service Type Static Combo SmartDataField
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 15.8 COLON-ALIGNED
...
ON VALUE-CHANGED OF coCombo IN FRAME frMain /* Service Type */
DO:
  DEFINE VARIABLE hContainer    AS HANDLE       NO-UNDO.

  {set DataModified TRUE}.

  /* Run recreateSDF in the viewer so that the SmartDataField will change
     appropriate based on the newly changed service type  */
  hContainer = DYNAMIC-FUNCTION('getContainerSource':U).
  RUN recreateSDF IN hContainer NO-ERROR.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH gsc_service_type NO-LOCK BY gsc_service_type.service_type_code":U
    ttComboData.cBufferList = "gsc_service_type":U
    ttComboData.cKeyFieldName = "gsc_service_type.service_type_obj":U
    ttComboData.cDescFieldNames = "gsc_service_type.service_type_code,gsc_service_type.service_type_description":U
