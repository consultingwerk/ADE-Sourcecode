...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsc_nationality Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsc_nationality table
...
&GLOB DATA-LOGIC-TABLE gsc_nationality
&GLOB DATA-FIELD-DEFS  "af/obj2/gscnafullo.i"
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsc_nationality.nationality_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_nationality' 'nationality_code' "'Nationality code'"}.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
    IF (RowObjUpd.RowMod = 'U':U AND
      CAN-FIND(FIRST gsc_nationality
        WHERE gsc_nationality.nationality_code = rowObjUpd.nationality_code
        AND ROWID(gsc_nationality) <> TO-ROWID(ENTRY(1,RowObjUpd.RowIDent))))
    OR (RowObjUpd.RowMod <> 'U':U AND
      CAN-FIND(FIRST gsc_nationality
        WHERE gsc_nationality.nationality_code = rowObjUpd.nationality_code))
    THEN
      ASSIGN
        cValueList   = STRING(RowObjUpd.nationality_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'gsc_nationality' '' "'nationality_code, '" cValueList }.
  END.
