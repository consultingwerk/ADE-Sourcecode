...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_filter_set Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_filter_set table
...
&GLOB DATA-LOGIC-TABLE gsm_filter_set
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmfifullo.i"
 
/* Error handling definitions */
{checkerr.i &define-only = YES}

DEFINE VARIABLE gdCopyFilterSetObj  AS DECIMAL    NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsm_filter_set.filter_set_code) THEN
    ASSIGN
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U)
                     + {aferrortxt.i 'AF' '1' 'gsm_filter_set' 'filter_set_code' "'Filter set code'"}.
...
/*------------------------------------------------------------------------------
  Purpose:     Procedure used to validate records server-side after the transaction scope upon write
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE cMessageList    AS CHARACTER    NO-UNDO.
  ...
  DEFINE BUFFER o_gsm_filter_data FOR gsm_filter_data.  /* Original */
  DEFINE BUFFER n_gsm_filter_data FOR gsm_filter_data.  /* New */

  /* Check if we need to copy any child records */
  IF b_gsm_filter_set.rowMod = "C":U THEN
  DO:
    /* Read through all child records of the filter set we are copying from */
    copy_blk:
    FOR EACH o_gsm_filter_data NO-LOCK
       WHERE o_gsm_filter_data.filter_set_obj = gdCopyFilterSetObj:

      /* Create the new filter data record */
      CREATE n_gsm_filter_data NO-ERROR.

      /* Check if the CREATE trigger returned any errors */
      IF ERROR-STATUS:ERROR THEN
      DO:
        cMessageList = (IF RETURN-VALUE <> "":U AND RETURN-VALUE <> ? THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).

        LEAVE copy_blk.
      END.

      /* If we did not get any errors, copy the data across */
      BUFFER-COPY o_gsm_filter_data
           EXCEPT o_gsm_filter_data.filter_data_obj
                  o_gsm_filter_data.filter_set_obj
               TO n_gsm_filter_data
           ASSIGN n_gsm_filter_data.filter_set_obj = b_gsm_filter_set.filter_set_obj NO-ERROR.

      /* Check if the ASSIGN returned any errors */
      IF ERROR-STATUS:ERROR THEN
      DO:
        cMessageList = (IF RETURN-VALUE <> "":U AND RETURN-VALUE <> ? THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).
      ...
      VALIDATE n_gsm_filter_data NO-ERROR.

      /* Check if the ASSIGN trigger returned any errors */
      IF ERROR-STATUS:ERROR THEN
      DO:
        cMessageList = (IF RETURN-VALUE <> "":U AND RETURN-VALUE <> ? THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).
        ...
        LEAVE copy_blk.
      END.
    END. /* FOR EACH o_gsm_filter_data */
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Check that we don't create duplicates when copying, adding or updating */
  IF (LOOKUP(b_gsm_filter_set.rowMod, "A,C":U) <> 0                                                   AND
      CAN-FIND(FIRST gsm_filter_set NO-LOCK
               WHERE gsm_filter_set.filter_set_code = b_gsm_filter_set.filter_set_code))
  OR (b_gsm_filter_set.rowMod = "U":U                                                                  AND
      CAN-FIND(FIRST gsm_filter_set NO-LOCK
               WHERE gsm_filter_set.filter_set_code = b_gsm_filter_set.filter_set_code
                 AND ROWID(gsm_filter_set)         <> TO-ROWID(ENTRY(1, b_gsm_filter_set.rowIdent)))) THEN
    ASSIGN
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U)
                     + {aferrortxt.i 'AF' '8' 'gsm_filter_set' 'filter_set_code' "'Filter set code'" QUOTER(b_gsm_filter_set.filter_set_code)}.

  /* This object number is required when copying to copy all the child records. At this point, the buffer does not have the new obj yet */
  IF b_gsm_filter_set.rowMod = "C":U THEN
    gdCopyFilterSetObj = b_gsm_filter_set.filter_set_obj.
