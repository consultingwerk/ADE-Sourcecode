...
&GLOB DATA-FIELD-DEFS  "af/obj2/gscotfullo.i"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD customizedLabel DataLogicProcedure  _DB-REQUIRED
FUNCTION customizedLabel RETURNS CHARACTER
  ( INPUT pcClassCode       AS CHARACTER,
    INPUT pdCustomClassObj  AS DECIMAL      )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD findDataTags DataLogicProcedure  _DB-REQUIRED
FUNCTION findDataTags RETURNS CHARACTER 
        ( input pdClassObj as decimal ) FORWARD.
...
/* Settings for THIS-PROCEDURE
   ...
   Other Settings: CODE-ONLY APPSERVER DB-AWARE
 */
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST gsc_object_type 
              WHERE gsc_object_type.object_type_code = b_gsc_object_type.object_type_code) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gsc_object_type.object_type_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {aferrortxt.i 'AF' '8' 'gsc_object_type' '' "'object_type_code, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "gsc_object_type Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsc_object_type.object_type_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_object_type' 'object_type_code' "'Object Type Code'"}.

  IF isFieldBlank(b_gsc_object_type.object_type_description) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_object_type' 'object_type_description' "'Object Type Description'"}.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST gsc_object_type 
              WHERE gsc_object_type.object_type_code = b_gsc_object_type.object_type_code
                AND ROWID(gsc_object_type) <> TO-ROWID(ENTRY(1,b_gsc_object_type.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gsc_object_type.object_type_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
  ...
  IF  b_gsc_object_type.deployment_type = ? THEN
    b_gsc_object_type.deployment_type = "":U.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION customizedLabel DataLogicProcedure  _DB-REQUIRED
FUNCTION customizedLabel RETURNS CHARACTER
  ( INPUT pcClassCode       AS CHARACTER,
    INPUT pdCustomClassObj  AS DECIMAL      ) :
/*------------------------------------------------------------------------------
  Purpose:  Returns the name of a class with the customizing class appended,
  			if any.
    Notes:  
------------------------------------------------------------------------------*/
    DEFINE buffer gscot    for gsc_object_type.
    
    if pdCustomClassObj ne 0 then
    do:
        find first gscot where
                   gscot.object_type_obj = pdCustomClassObj
                   no-lock no-error.
        if available gscot then
            assign pcClassCode = pcClassCode + ' [' + gscot.object_type_code + ']'.
    end.    /* there is a custom class. */
    
    RETURN pcClassCode.
END FUNCTION.    /* customizedLabel */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION findDataTags DataLogicProcedure  _DB-REQUIRED
FUNCTION findDataTags RETURNS CHARACTER
        ( input pdClassObj as decimal ):
/*------------------------------------------------------------------------------
ACCESS_LEVEL=PROTECTED
  Purpose: Returns a CSV list of tags applicable to this class.
    Notes: * An access level of protected means that this function is only
             accessible from within this SDO.
------------------------------------------------------------------------------*/
    DEFINE variable cDataTags            as character                 no-undo.
    
    DEFINE buffer gsctg        for gsc_data_tag.
    DEFINE buffer gsmdt        for gsm_tagged_data.
    
    for each gsmdt where
             gsmdt.owning_entity_mnemonic = 'GSCOT' and
             gsmdt.owning_reference = string(pdClassObj)
             no-lock,
        first gsctg where
              gsctg.data_tag_obj = gsmdt.data_tag_obj
              no-lock:
        assign cDataTags = cDataTags + ',' +  gsctg.data_tag_code.                  
    end.    /* all tag data */
    
    return left-trim(cDataTags, ',').
END FUNCTION.    /* find data tags */
