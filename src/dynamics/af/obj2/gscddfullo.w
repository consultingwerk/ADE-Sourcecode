...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  IF LENGTH(RowObject.dataset_code) = 0 OR LENGTH(RowObject.dataset_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsc_deploy_dataset' 'dataset_code' "'Dataset Code'"}.

  IF LENGTH(RowObject.dataset_description) = 0 OR LENGTH(RowObject.dataset_description) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsc_deploy_dataset' 'dataset_description' "'Dataset Description'"}.
