...
&Scoped-define INTERNAL-TABLES gsm_login_company

/* Definitions for QUERY Query-Main                                     */
&Scoped-Define ENABLED-FIELDS  login_company_code login_company_short_name login_company_name~
 login_company_email login_company_disabled
&Scoped-define ENABLED-FIELDS-IN-gsm_login_company login_company_code ~
login_company_short_name login_company_name login_company_email ~
login_company_disabled 
&Scoped-Define DATA-FIELDS  login_company_obj login_company_code login_company_short_name~
 login_company_name login_company_email login_company_disabled
&Scoped-define DATA-FIELDS-IN-gsm_login_company login_company_obj ~
login_company_code login_company_short_name login_company_name ~
login_company_email login_company_disabled 
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH gsm_login_company NO-LOCK ~
    BY gsm_login_company.login_company_obj INDEXED-REPOSITION.
{&DB-REQUIRED-END}
&Scoped-define TABLES-IN-QUERY-Query-Main gsm_login_company
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main gsm_login_company
...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
    IF (RowObjUpd.RowMod = 'U':U AND
      CAN-FIND(FIRST gsm_login_company
        WHERE gsm_login_company.login_company_code = rowObjUpd.login_company_code
        AND ROWID(gsm_login_company) <> TO-ROWID(ENTRY(1,RowObjUpd.RowIDent))))
    OR (RowObjUpd.RowMod <> 'U':U AND
      CAN-FIND(FIRST gsm_login_company
        WHERE gsm_login_company.login_company_code = rowObjUpd.login_company_code))
    THEN
      ASSIGN
        cValueList   = STRING(RowObjUpd.login_company_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'gsm_login_company' 'login_company_code' "'login_company_code, '" cValueList }.
  END.
...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  IF LENGTH(RowObject.login_company_code) = 0 OR LENGTH(RowObject.login_company_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_login_company' 'login_company_code' "'Login Company Code'"}.

  IF LENGTH(RowObject.login_company_short_name) = 0 OR LENGTH(RowObject.login_company_short_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_login_company' 'login_company_short_name' "'Login Company Short Name'"}.

  IF LENGTH(RowObject.login_company_name) = 0 OR LENGTH(RowObject.login_company_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_login_company' 'login_company_name' "'Login Company Name'"}.
