...
/*---------------------------------------------------------------------------------
  ...
  Description:  Logical Service Combo SmartDataField

  Purpose:      Logical Service Static Combo SmartDataField
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getFieldHandle sObject 
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 26.8 COLON-ALIGNED
...
ON VALUE-CHANGED OF coCombo IN FRAME frMain /* Default Logical Service */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH gsc_logical_service NO-LOCK BY gsc_logical_service.logical_service_code":U
    ttComboData.cBufferList = "gsc_logical_service":U
    ttComboData.cKeyFieldName = "gsc_logical_service.logical_service_obj":U
    ttComboData.cDescFieldNames = "gsc_logical_service.logical_service_code,gsc_logical_service.logical_service_description":U
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getFilteredComboQuery sObject 
PROCEDURE getFilteredComboQuery :
/*------------------------------------------------------------------------------
  Purpose:     This routine is published from the viewer and is used to pass the query
               required by this combo back to the viewer for building. Once built,
               the query will be returned into the procedure buildCombo.
  ...
  Notes:       This is designed to facilitate all combo queries being built with
               a single appserver hit.
               This query needs to filter on service type and the viewer publishes
               this from displayFields itself (rather than standard viewer code
               that publishes getComboQuery during intialization). 
------------------------------------------------------------------------------*/

DEFINE INPUT-OUTPUT PARAMETER TABLE FOR ttComboData.

DEFINE VARIABLE dServiceType      AS DECIMAL    NO-UNDO.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  DO:
    ...
    dServiceType = DYNAMIC-FUNCTION('columnStringValue':U IN hDataSource, INPUT 'service_type_obj':U).
  END.  /* if valid-handle */
  ...
  ASSIGN
    ...
    ttComboData.cWidgetType = "decimal":U
    ttComboData.hWidget = coCombo:HANDLE
    ttComboData.cForEach = "FOR EACH gsc_logical_service WHERE gsc_logical_service.service_type_obj = ":U +
      STRING(dServiceType) + " NO-LOCK BY gsc_logical_service.logical_service_code":U
    ttComboData.cBufferList = "gsc_logical_service":U
    ttComboData.cKeyFieldName = "gsc_logical_service.logical_service_obj":U
    ttComboData.cDescFieldNames = "gsc_logical_service.logical_service_code,gsc_logical_service.logical_service_description":U
    ttComboData.cDescSubstitute = "&1 / &2":U
...
------------------------------------------------------------------------------*/
  ...
  DO:
    SUBSCRIBE TO "getFilteredComboQuery":U IN ghSDV.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getFieldHandle sObject 
FUNCTION getFieldHandle RETURNS HANDLE
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  Returns the handle of the SmartDataField object 
    Notes:  
------------------------------------------------------------------------------*/

  RETURN coCombo:HANDLE IN FRAME {&FRAME-NAME}.   /* Function return value. */
