...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_country Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_country table
...
&GLOB DATA-LOGIC-TABLE gsm_country
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmcyfullo.i"
...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
    IF (RowObjUpd.RowMod = 'U':U AND
      CAN-FIND(FIRST gsm_country
        WHERE gsm_country.country_code = rowObjUpd.country_code
        AND ROWID(gsm_country) <> TO-ROWID(ENTRY(1,RowObjUpd.RowIDent))))
    OR (RowObjUpd.RowMod <> 'U':U AND
      CAN-FIND(FIRST gsm_country
        WHERE gsm_country.country_code = rowObjUpd.country_code))
    THEN
      ASSIGN
        cValueList   = STRING(RowObjUpd.country_code)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'gsm_country' 'country_code' "'country_code, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsm_country.country_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_country' 'country_code' "'Country code'"}.
                    
  IF isFieldBlank(b_gsm_country.country_name) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_country' 'country_name' "'Country name'"}.
