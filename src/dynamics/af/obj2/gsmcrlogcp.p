...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_currency Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_currency table
...
&GLOB DATA-LOGIC-TABLE gsm_currency  
 &GLOB DATA-FIELD-DEFS  "af/obj2/gsmcrfullo.i" 
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST gsm_currency 
              WHERE gsm_currency.currency_code = b_gsm_currency.currency_code) THEN
  DO:
     ASSIGN cValueList   = STRING(b_gsm_currency.currency_code)
            cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) 
                         + {aferrortxt.i 'AF' '8' 'gsm_currency' '' "'currency_code, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "gsm_currency Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE dDecimalFormat           AS DECIMAL                     NO-UNDO.
  DEFINE VARIABLE cDecimalCharacter        AS CHARACTER                   NO-UNDO.
  DEFINE VARIABLE iDecimals                AS INTEGER                     NO-UNDO.
  DEFINE VARIABLE cFormatValidList         AS CHARACTER INITIAL "9,<,Z"   NO-UNDO.
  DEFINE VARIABLE iFormatEnd               AS INTEGER                     NO-UNDO.
  DEFINE VARIABLE iFormatPosition          AS INTEGER                     NO-UNDO.

  IF isFieldBlank(b_gsm_currency.currency_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_currency' 'currency_code' "'Currency Code'"}.

  IF isFieldBlank(b_gsm_currency.currency_description) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_currency' 'currency_description' "'Currency Description'"}.

  /* Check if the format is valid */
  ASSIGN dDecimalFormat = DECIMAL(STRING(9,b_gsm_currency.symbol_format_mask)) NO-ERROR.
  IF ERROR-STATUS:ERROR
       THEN ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U)
             + {af/sup2/aferrortxt.i 'AF' '5' 'gsm_currency' 'symbol_format_mask' "'symbol format mask (containing non-numeric format characters)'"}.

  /* Check if the number of decimals specified are the same as in the format mask *
   * This makes provision for any characters put at the end of the format         */
  DO iFormatPosition = LENGTH(b_gsm_currency.symbol_format_mask) TO 1 BY -1:
     IF LOOKUP(SUBSTRING(b_gsm_currency.symbol_format_mask,iFormatPosition,1),cFormatValidList) <> 0
         THEN ASSIGN iFormatEnd      = iFormatPosition
                     iFormatPosition = 1.
  END.
  ASSIGN cDecimalCharacter = SESSION:NUMERIC-DECIMAL-POINT 
         iDecimals         = iFormatEnd - INDEX(b_gsm_currency.symbol_format_mask,cDecimalCharacter).

  /* If an error was raised in the format, then the cFormat field is blank and the following error also occurs */
  /* So we only want to do the following validation if the format did not raise an error */
  IF  dDecimalFormat <> 0 
  AND iDecimals      <> b_gsm_currency.number_of_decimals   
     THEN ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U)
             + {af/sup2/aferrortxt.i 'AF' '18' 'gsm_currency' 'symbol_format_mask' "'symbol format mask'" "'the number of decimals specified'"}.
 
  /* Add the sign to the format if the user did not  */
  IF INDEX(b_gsm_currency.symbol_format_mask,"-") = 0 THEN 
      ASSIGN b_gsm_currency.symbol_format_mask = "-" + b_gsm_currency.symbol_format_mask.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST gsm_currency 
              WHERE gsm_currency.currency_code = b_gsm_currency.currency_code
                AND ROWID(gsm_currency) <> TO-ROWID(ENTRY(1,b_gsm_currency.RowIDent))) THEN
  DO:
     ASSIGN cValueList   = STRING(b_gsm_currency.currency_code)
            cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U)  
                         + {aferrortxt.i 'AF' '8' 'gsm_currency' '' "'currency_code, '" cValueList }.
