...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_field Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_field table
...
&GLOB DATA-LOGIC-TABLE gsm_field
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmffful2o.i"
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsm_field.field_name) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_field' 'field_name' "'Field name'"}.
