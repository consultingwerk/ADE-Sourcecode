...
/* Name widget-pool so that widgets created by this viewer are  
   in their own widget-pool and do not interfere with AppBuilder 
   widgets when this tools is invoked by the AppBuilder */
CREATE WIDGET-POOL "DataFieldPool":U + STRING(THIS-PROCEDURE).
...
DEFINE VARIABLE gcDynPropOpenedFor    AS CHARACTER  NO-UNDO.
...
/* This temp-table stores information about the datafield object (ryc_smartobject) */
...
/* This temp-table stores attribute values for attributes directly updatable on this viewer */
DEFINE TEMP-TABLE ttDataFieldAttr NO-UNDO
       ...
       FIELD original_attr_value  AS CHARACTER FORMAT "x(50)"
       INDEX idxOne attr_name.

/* This temp-table stores any attributes against the datafield not directly updatable on this viewer. *
 * The user will have to use the dynamic property sheet to update these.                              */
DEFINE TEMP-TABLE ttPropSheetInitialValues
       FIELD tRecordIdentifier  AS DECIMAL
       FIELD attr_name          AS CHARACTER
       FIELD updated_attr_value AS CHARACTER.

/* This temp-table stores original values for attributes against the datafield not directly
  updateable on the viewer */
DEFINE TEMP-TABLE ttPropSheetOrigValues
       FIELD tRecordIdentifier   AS DECIMAL
       FIELD attr_name           AS CHARACTER
       FIELD original_attr_value AS CHARACTER.

/* If the user updates any attributes in the dynamic property sheet, their values are stored here. */
...
&Scoped-Define ENABLED-OBJECTS btnAdd btnCancel btnDel btnProp btnReset ~
btnSave fiProductModule fiClass fiData-Type fiFormat fiLabel fiColumnLabel ~
toIncludeInDefaultListView fiDefaultValue fiOrder toIncludeInDefaultView ~
fiSchema-Format fiSchema-Label fiSchema-Column-Label fiSchema-Initial ~
fiSchema-Help fiSchema-view-as fiSchema-Validate-Expression ~
fiSchema-Validate-Message fiLabelIncludeInDefaultListView fiViewAsEdLabel ~
fiValExprEdLabel fiHelpEdLabel fiValMsgEdLabel RECT-1 RECT-2 
...
DEFINE BUTTON btnReset 
     ...
     LABEL "&Reset" 
     CONTEXT-HELP-ID 0
     SIZE 5 BY 1 TOOLTIP "Reset dataField record".

DEFINE BUTTON btnSave 
     IMAGE-UP FILE "src/adeicon/save.bmp":U NO-FOCUS FLAT-BUTTON
     LABEL "&Save" 
     CONTEXT-HELP-ID 0
     SIZE 5 BY 1 TOOLTIP "Save updated data field information".
...
DEFINE VARIABLE iWhereIsAttrInList AS INTEGER    NO-UNDO.
...
/* Check if the property sheet PLIPP is running.  If not, launch it */
ASSIGN hProcLib = SESSION:FIRST-PROCEDURE.

DO WHILE VALID-HANDLE(hProcLib) AND hProcLib:FILE-NAME <> "ry/prc/ryvobplipp.p":
    ASSIGN hProcLib = hProcLib:NEXT-SIBLING.
END.

IF NOT VALID-HANDLE(hProcLib) 
THEN DO:
    RUN ry/prc/ryvobplipp PERSISTENT SET hProcLib NO-ERROR.
    IF NOT VALID-HANDLE(hProcLib) 
    THEN DO:
        RUN dispErrMessage IN TARGET-PROCEDURE (INPUT "An error occured while launching the Dynamic Property Sheet.").
        RETURN.
    END.
END.

/* Now launch the dynamic property sheet */
RUN launchPropertyWindow IN hProcLib.
ASSIGN ghPropSheet = DYNAMIC-FUNCTION("getPropSheet":U IN hProcLib).
...
THEN DO:
    ...
    ON 'WINDOW-CLOSE':U OF hWindow PERSISTENT RUN dataFieldPropSheetClose IN TARGET-PROCEDURE.
    SUBSCRIBE TO "dataFieldPropSheetClose":U IN SESSION:FIRST-PROCEDURE.
    SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "PropertyChangedAttribute":U IN hProcLib.

    /* Build a list of attribute values - these are the attributes settable on this viewer */
    DO iFieldCnt = 1 TO NUM-ENTRIES(gcValidAttributes):
        ...
        ASSIGN cAttributeList = cAttributeList 
                              + cAttributeLabel + CHR(3) 
                              + "":U + CHR(3) 
                              + cAttributeValue + CHR(3).
    END.

    /* And then add all attributes not updatable on the viewer that have been updated by the user. */
    FOR EACH ttPropSheetInitialValues
       WHERE ttPropSheetInitialValues.tRecordIdentifier = ttDataField.tRecordIdentifier:

        ASSIGN cAttributeList = cAttributeList 
                              + ttPropSheetInitialValues.attr_name + CHR(3)           /* Attribute Name */
                              + "":U + CHR(3)                                         /* Result Code */
                              + ttPropSheetInitialValues.updated_attr_value + CHR(3). /* Attribute Value */
    END.

    /* If the user relaunches the property sheet while he's still on the same field, we need to make sure  *
     * we don't lose any changes he's already made.  So we check the ttUpdatedPropSheetAttrs temp-table to *
     * see if the user has updated the attribute we're processing.                                         */
    FOR EACH ttUpdatedPropSheetAttrs:
        CASE ttUpdatedPropSheetAttrs.data_type:
            WHEN "CHARACTER":U THEN ASSIGN cAttributeValue = ttUpdatedPropSheetAttrs.character_attribute_value.
            WHEN "DECIMAL":U THEN ASSIGN cAttributeValue = STRING(ttUpdatedPropSheetAttrs.decimal_attribute_value).
            WHEN "INTEGER":U THEN ASSIGN cAttributeValue = STRING(ttUpdatedPropSheetAttrs.integer_attribute_value).
            WHEN "LOGICAL":U THEN ASSIGN cAttributeValue = STRING(ttUpdatedPropSheetAttrs.logical_attribute_value).
            WHEN "DATE":U THEN ASSIGN cAttributeValue = STRING(ttUpdatedPropSheetAttrs.date_attribute_value).
            OTHERWISE
                ASSIGN cAttributeValue = "":U.
        END CASE.

        /* If the attribute is already in the list, override its value with the value entered by the user. *
         * Note this only applies when the user stays on the same field and relaunches the property sheet. */
        ASSIGN iWhereIsAttrInList = LOOKUP(ttUpdatedPropSheetAttrs.attribute_name, cAttributeList, CHR(3)) NO-ERROR.
        IF iWhereIsAttrInList = ?
        OR iWhereIsAttrInList = 0 THEN
            ASSIGN cAttributeList = cAttributeList 
                                  + ttUpdatedPropSheetAttrs.attribute_name + CHR(3) 
                                  + "":U + CHR(3) 
                                  + cAttributeValue + CHR(3).
        ELSE
            ASSIGN ENTRY(iWhereIsAttrInList + 2, cAttributeList, CHR(3)) = cAttributeValue.
    END.
    ASSIGN cAttributeList = RIGHT-TRIM(cAttributeList, CHR(3)) NO-ERROR.

    /* If we're on a different datafield, empty any updated properties we've stored */
    IF gcDynPropOpenedFor <> ttDataField.object_filename THEN
        EMPTY TEMP-TABLE ttUpdatedPropSheetAttrs.

    /* Make sure the datafield is not registered already */
    ...
    /* Make sure we don't have a row selected in the browse.  We don't want to confuse the user. */
    RUN disableAllFields IN TARGET-PROCEDURE.
    ASSIGN ghBrowser:SENSITIVE = FALSE
           gcOperation         = "Prop":U
           hWidget             = WIDGET-HANDLE(ENTRY(LOOKUP("btnProp":U, gcAllFieldNames), gcAllFieldHandles))
           hWidget:SENSITIVE   = TRUE
           gcDynPropOpenedFor  = ttDataField.object_filename.
END.
...
DEFINE VARIABLE cOriginalValues       AS CHARACTER  NO-UNDO.
DEFINE VARIABLE cUpdatedAttrs         AS CHARACTER  NO-UNDO.
DEFINE VARIABLE cUpdatedValues        AS CHARACTER  NO-UNDO.
...
DO ON ERROR UNDO, LEAVE:
    ...
    DO iFieldCnt = 1 TO NUM-ENTRIES(gcValidAttributes):
        ...
        ELSE DO:
            ...
            /* Create lists of updated attributes and their original and new values to pass to
               DataFieldMaster changed */
            FIND FIRST ttDataFieldAttr WHERE ttDataFieldAttr.tRecordIdentifier = gdSaveToRecordID AND
                ttDataFieldAttr.attr_name = cAttributeName NO-ERROR.
            IF AVAILABLE ttDataFieldAttr AND ttDataFieldAttr.updated_attr_value NE cValue THEN
              ASSIGN
                cUpdatedAttrs   = cUpdatedAttrs + (IF NUM-ENTRIES(cUpdatedAttrs) > 0 THEN ",":U ELSE "":U) +
                                  cAttributeName
                cUpdatedValues  = cUpdatedValues + (IF NUM-ENTRIES(cUpdatedAttrs) > 1 THEN CHR(3) ELSE "":U) + 
                                  IF cValue = ? THEN "?":U ELSE cValue
                cOriginalValues = cOriginalValues + (IF NUM-ENTRIES(cUpdatedAttrs) > 1 THEN CHR(3) ELSE "":U) +
                                  IF ttDataFieldAttr.updated_attr_value = ? THEN "?":U ELSE ttDataFieldAttr.updated_attr_value.
        END.
    END.
    ...
    FOR EACH ttUpdatedPropSheetAttrs:
        ...
        ASSIGN 
          cUpdatedAttrs = cUpdatedAttrs + (IF NUM-ENTRIES(cUpdatedAttrs) > 0 THEN ",":U ELSE "":U) +
                          ttUpdatedPropSheetAttrs.attribute_name
          cUpdatedValues = cUpdatedValues + IF NUM-ENTRIES(cUpdatedAttrs) > 1 THEN CHR(3) ELSE "":U.
          CASE ttUpdatedPropSheetAttrs.data_type:
            WHEN "CHARACTER":U THEN cUpdatedValues = cUpdatedValues + 
                                                     IF ttUpdatedPropSheetAttrs.character_attribute_value = ? THEN "?":U 
                                                     ELSE ttUpdatedPropSheetAttrs.character_attribute_value.
            WHEN "DECIMAL":U   THEN cUpdatedValues = cUpdatedValues + 
                                                     IF ttUpdatedPropSheetAttrs.decimal_attribute_value = ? THEN "?":U
                                                     ELSE STRING(ttUpdatedPropSheetAttrs.decimal_attribute_value).
            WHEN "INTEGER":U   THEN cUpdatedValues = cUpdatedValues + 
                                                     IF ttUpdatedPropSheetAttrs.integer_attribute_value = ? THEN "?":U 
                                                     ELSE STRING(ttUpdatedPropSheetAttrs.integer_attribute_value).
            WHEN "DATE":U      THEN cUpdatedValues = cUpdatedValues +
                                                     IF ttUpdatedPropSheetAttrs.date_attribute_value = ? THEN "?":U 
                                                     ELSE STRING(ttUpdatedPropSheetAttrs.date_attribute_value).
            WHEN "LOGICAL":U   THEN cUpdatedValues = cUpdatedValues + 
                                                     IF ttUpdatedPropSheetAttrs.logical_attribute_value = ? THEN "?":U 
                                                     ELSE STRING(ttUpdatedPropSheetAttrs.logical_attribute_value).
          END CASE.
        FIND ttPropSheetOrigValues WHERE ttPropSheetOrigValues.tRecordIdentifier = gdSaveToRecordID AND
             ttPropSheetOrigValues.attr_name = ttUpdatedPropSheetAttrs.attribute_name NO-ERROR.
        IF AVAILABLE ttPropSheetOrigValues THEN cOrigValue = ttPropSheetOrigValues.original_attr_value.
        ELSE DO:
          FIND FIRST ttDataFieldAttr WHERE ttDataFieldAttr.tRecordIdentifier = gdSaveToRecordID AND
            ttDataFieldAttr.attr_name = ttUpdatedPropSheetAttrs.attribute_name NO-ERROR.
          IF AVAILABLE ttDataFieldAttr THEN cOrigValue = ttDataFieldAttr.original_attr_value.
          ELSE cOrigValue = "":U.
        END.  /* else do */
        cOriginalValues = cOriginalValues + (IF NUM-ENTRIES(cUpdatedAttrs) > 1 THEN CHR(3) ELSE "":U) + 
                          IF cOrigValue = ? THEN "?":U ELSE cOrigValue.
    ...
    DO TRANSACTION ON ERROR UNDO trn-blk, LEAVE trn-blk:
        ...
        RUN insertObjectMaster IN hDesignManager (INPUT ttDataField.object_filename,
                                                  ...
                                                  ) NO-ERROR. /* Don't remove the NO-ERROR */
        IF RETURN-VALUE <> "":U 
        THEN DO:
            ASSIGN cMessage = RETURN-VALUE.
            UNDO trn-blk, LEAVE trn-blk.
        ...
        IF gcOperation = "ADD":U THEN
            ASSIGN ttDataField.smartobject_obj = dDataFieldObj.
        ...
        RUN insertObjectInstance IN hDesignManager (INPUT ttDataField.container_smartobject_obj,
                                                    ...
                                                    INPUT ttDataField.instance_order, 		/* Object sequence  */
        ...
        RUN validateDataFieldAttrs IN hDesignManager (INPUT ttDataField.object_filename, 
                                                      OUTPUT cInvalidAttrs,
                                                      OUTPUT cMessage).
        IF cMessage <> "":U THEN UNDO trn-blk, LEAVE trn-blk.
...
/* When DataField maintenance is invoked from the AppBuilder, the ABDesignField user
   property is set with information about the design field.  This design field information
   is then passed to DataFieldMasterChanged so that DataFieldMasterChanged can find the 
   design field and update it with updated attribute values.  */
cABDesignField = DYNAMIC-FUNCTION("getUserProperty":U IN TARGET-PROCEDURE, 
                                INPUT "ABDesignField":U). 
PUBLISH "DataFieldMasterChanged":U 
  (INPUT cABDesignField,
   INPUT cUpdatedAttrs,
   INPUT cUpdatedValues,
   INPUT cOriginalValues).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE VARIABLE dUniqueIdentifier AS DECIMAL    NO-UNDO.

EMPTY TEMP-TABLE ttDataField.
EMPTY TEMP-TABLE ttDataFieldAttr.

/* Retrieve the master object from the repsoitory */
...
FOR EACH ttObject WHERE ttObject.tContainerSmartObjectObj > 0:
  ...
  THEN DO:
      IF gcFilterField > "":U AND ttObject.tLogicalObjectName <> gcFilterField THEN
         NEXT Instance-LOOP.


      /* Create our dataField record */
      dUniqueIdentifier  = dUniqueIdentifier + 1.0.
      CREATE ttDataField.
      ASSIGN ttDataField.tRecordIdentifier         = dUniqueIdentifier 
             ttDataField.container_smartobject_obj = ttObject.tContainerSmartObjectObj
             ttDataField.container_object_filename = gcContainerName
             ttDataField.smartobject_obj           = ttObject.tSmartObjectObj
             ttDataField.object_filename           = ttObject.tLogicalObjectname
             ttDataField.object_type_code          = ttObject.tClassName
             ttDataField.product_module            = ttObject.tProductModuleCode
             ttDataField.instance_order            = ttObject.tPageObjectSequence
             NO-ERROR.
      ...
      FOR EACH ttClassAttribute: 
         ...
         * The rest have to be updated using the Dynamics property sheet.                    */                             
         IF LOOKUP(ttClassAttribute.tAttributeLabel, gcValidAttributes) = 0
            OR LOOKUP(ttClassAttribute.tAttributeLabel, gcValidAttributes) = ? 
         THEN DO:  
            IF AVAIL ttObjectAttribute AND ttObjectAttribute.tAttributeValue <> ttClassAttribute.tAttributeValue THEN
            DO:
                CREATE ttPropSheetInitialValues.
                ASSIGN ttPropSheetInitialValues.tRecordIdentifier  = dUniqueIdentifier
                       ttPropSheetInitialValues.attr_name          = ttObjectAttribute.tAttributeLabel
                       ttPropSheetInitialValues.updated_attr_value = ttObjectAttribute.tAttributeValue.
            END.
            
            CREATE ttPropSheetOrigValues.
            ASSIGN ttPropSheetOrigValues.tRecordIdentifier   = dUniqueIdentifier
                   ttPropSheetOrigValues.attr_name           = ttClassAttribute.tAttributeLabel
                   ttPropSheetOrigValues.original_attr_value = IF AVAIL ttObjectAttribute 
                                                               THEN ttObjectAttribute.tAttributeValue
                                                               ELSE ttClassAttribute.tAttributeValue.
         END.
         ELSE DO:    
            CREATE ttDataFieldAttr.
            ASSIGN ttDataFieldAttr.tRecordIdentifier   = dUniqueIdentifier
                   ttDataFieldAttr.attr_name           = ttClassAttribute.tAttributeLabel
                   ttDataFieldAttr.updated_attr_value  = IF AVAIL ttObjectAttribute 
                                                         THEN ttObjectAttribute.tAttributeValue
                                                         ELSE ttClassAttribute.tAttributeValue
                   ttDataFieldAttr.original_attr_value = ttDataFieldAttr.updated_attr_value.
         END.
      END.
  END. /* If datafield */
END.  /* Instance Looop  for each ttobject */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dataFieldPropSheetClose sObject 
PROCEDURE dataFieldPropSheetClose :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* Let any other datafield viewers know the property sheet is being closed */
UNSUBSCRIBE TO "dataFieldPropSheetClose":U IN SESSION:FIRST-PROCEDURE.
PUBLISH "dataFieldPropSheetClose":U FROM SESSION:FIRST-PROCEDURE.
    ...
    DO iFieldCnt = 1 TO NUM-ENTRIES(gcAllFieldNames):
        ...
        CASE cFieldName:
            WHEN "fiProductModule":U THEN
                ASSIGN hField:SCREEN-VALUE = ttDataField.product_module NO-ERROR.

            WHEN "fiClass":U THEN
                ASSIGN hField:SCREEN-VALUE = ttDataField.object_type_code NO-ERROR.

            WHEN "fiOrder":U THEN
                ASSIGN hField:SCREEN-VALUE = STRING(ttDataField.instance_order) NO-ERROR.

            WHEN "fiName":U THEN
                ASSIGN hField:SCREEN-VALUE = ENTRY(2, ttDataField.object_filename, ".":U) NO-ERROR.
...
/* If we have a parent record link, subscribe to any TableIO events in it */
{get containerSource hContainer}.
IF VALID-HANDLE(hContainer) 
THEN DO:
    {get containerToolbarSource hToolbar hContainer}.
    IF VALID-HANDLE(hToolbar) 
    THEN DO:
        {get tableIOTarget hTableIOTarget hToolbar}.
        IF VALID-HANDLE(hTableIOTarget) 
        THEN DO:
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "updateMode":U IN hToolbar.
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "addRecord":U IN hTableIOTarget RUN-PROCEDURE "TableIOInParent":U.
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "copyRecord":U IN hTableIOTarget RUN-PROCEDURE "TableIOInParent":U.
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "deleteRecord":U IN hTableIOTarget RUN-PROCEDURE "TableIOInParent":U.
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "updateRecord":U IN hToolbar RUN-PROCEDURE "TableIOInParent":U.
            SUBSCRIBE PROCEDURE TARGET-PROCEDURE TO "resetRecord":U IN hToolbar RUN-PROCEDURE "TableIOInParent":U.
        END.
    END.
END.
...
DO iFieldCnt = 1 TO NUM-ENTRIES(gcAllFieldHandles):
    ...
    END.        
END.
/* Not necessary as it will be launched from viewObject on initialization  *
 *   RUN rebuildViewer IN TARGET-PROCEDURE.                                */
...
ASSIGN hComboHandle            = WIDGET-HANDLE(ENTRY(LOOKUP("fiClass":U, gcAllFieldNames), gcAllFieldHandles))
       hComboHandle:LIST-ITEMS = DYNAMIC-FUNCTION("getClassChildrenFromDB":U IN gshRepositoryManager, INPUT "DataField").

/* Data Types */
ASSIGN hComboHandle            = WIDGET-HANDLE(ENTRY(LOOKUP("fiData-Type":U, gcAllFieldNames), gcAllFieldHandles))
       hComboHandle:LIST-ITEMS = "Character,Decimal,Integer,Date,Logical,Raw":U.
...
/* If the dynamic property sheet is open for another object, we don't want to *
 * store its updated attributes for our object.                               */
IF phHandle <> THIS-PROCEDURE 
OR pcContainer <> ttDataField.container_object_filename
OR pcObject <> ttDataField.object_filename THEN
    RETURN.
...
EMPTY TEMP-TABLE ttPropSheetOrigValues.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE TableIOInParent sObject 
PROCEDURE TableIOInParent :
/*------------------------------------------------------------------------------
  Purpose:     Example: When addRecord is published in the parent record maintenance viewer,
               select the tab folder it's on.
               IOW when ADD is pressed for the entity, reposition to the Entity tab.               
               (Launch the Entity Maintenance Folder, go to tab 2 and press add to see
               what this API does)
  Parameters:  <none>
  Notes:       Runs for Add, Copy, Delete, Reset and Update.
------------------------------------------------------------------------------*/
DEFINE VARIABLE hContainer     AS HANDLE     NO-UNDO.
DEFINE VARIABLE hToolbar       AS HANDLE     NO-UNDO.
DEFINE VARIABLE hTableIOTarget AS HANDLE     NO-UNDO.
...
DEFINE VARIABLE iTableIOPage   AS INTEGER    NO-UNDO.
...
THEN DO:
    ...
    THEN DO:
        ...
        IF VALID-HANDLE(hTableIOTarget) 
        THEN DO:
            {get objectPage iTableIOPage hTableIOTarget}.
            {get currentPage iCurrentPage hContainer}.
            IF iCurrentPage <> iTableIOPage THEN
                RUN selectPage IN hContainer (INPUT iTableIOPage).
        END.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE updateMode sObject 
PROCEDURE updateMode :
/*------------------------------------------------------------------------------
  Purpose:     When updateMode is published in the parent record maintenance viewer,
               select the tab folder it's on.
               IOW when "Modify" or "View" is pressed for the entity, reposition to the Entity tab.               
               (Launch the Entity Maintenance Folder, go to tab 2 and press "Modify" or "View" 
               to see what this API does)
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
