...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_filter_data Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_filter_data table
...
&GLOB DATA-LOGIC-TABLE gsm_filter_data
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmfdfullo.i"
...
------------------------------------------------------------------------------*/
  ...
  DO:
    cEnabledTables = ENTRY(1, {fn getEnabledTables hDataSource}).

    CASE cEnabledTables:
      WHEN "gsc_entity_mnemonic":U THEN
        {set ForeignFields 'gsm_filter_data.owning_entity_mnemonic,entity_mnemonic':U}.

      WHEN "gsm_filter_set":U THEN
        {set ForeignFields 'gsm_filter_data.filter_set_obj,filter_set_obj':U}.
    END CASE.
...
------------------------------------------------------------------------------*/
  ...
  /* When a value of blank ("") is saved into the owning_reference field from
     the Filter Set Maintenance window, a value of "0" is assigned instead,
     since the associated dynlookup has a decimal keyfield.
     The code below ensures that a blank is stored as expected in places where
     the filters are used.
   */
  FOR EACH RowObjUpd: 
      IF RowObjUpd.owning_reference = "0":U THEN
         RowObjUpd.owning_reference = "":U.
  END.
  ASSIGN
    ERROR-STATUS:ERROR = NO.
  RETURN cMessageList.
