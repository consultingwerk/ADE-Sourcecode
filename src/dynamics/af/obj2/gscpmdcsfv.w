...
/*---------------------------------------------------------------------------------
  ...
  Description:  Product Module combo SDF

  Purpose:      Product Module combo smartdatafield
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 15.2 COLON-ALIGNED
...
/*------------------------------------------------------------------------------
  Purpose:     Run from value changed of parent SDF
...
IF pdKey <> 0 AND pdKey <> ? AND gdSaveKey = pdKey THEN RETURN.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  */
  ...
  IF pdKey = 0 OR pdKey = ? THEN
  DO:
    coCombo:LIST-ITEM-PAIRS = ",":U.
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH gsc_product NO-LOCK WHERE gsc_product.product_obj = '":U + STRING(pdKey) + "',":U +
