...
/*---------------------------------------------------------------------------------
  ...
  Description:  Menu Item Parent Menu Combo SDF

  Purpose:      Menu Item Parent Menu Combo SDF
  ...
  (v:010000)    Task:    90000019   UserRef:    Posse
                Date:   22/03/2001  Author:     John Palazzo
...
/* Specific menu structure object to which a set of menu items belong. */
DEFINE VARIABLE gdMenu_Structure_Obj    AS DECIMAL      NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD hasMenuStructureChanged sObject 
FUNCTION hasMenuStructureChanged RETURNS LOGICAL
...
DEFINE FRAME frMain
     coCombo AT ROW 1 COL 18 COLON-ALIGNED
...
ON VALUE-CHANGED OF coCombo IN FRAME frMain /* Parent Menu Item */
DO:
...
DO WITH FRAME {&FRAME-NAME}:
  FIND FIRST ttComboData WHERE ttComboData.hWidget = coCombo:HANDLE NO-ERROR.
...
/* Must filter the combo for a specific menu structure. So get it and use it in the query. */
ASSIGN gdMenu_Structure_Obj = {fnarg columnValue '"menu_structure_obj"' ghSDO}.

IF gdMenu_Structure_Obj = ? OR 
   gdMenu_Structure_Obj = 0 THEN
  RETURN.
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  ASSIGN
    ...
    ttComboData.cForEach = "FOR EACH gsm_menu_item NO-LOCK ":U +
                           "    WHERE gsm_menu_item.menu_structure_obj = ":U + STRING(gdMenu_Structure_Obj) +
                           "      AND gsm_menu_item.sub_menu_item":U +
                           "    BY gsm_menu_item.menu_item_sequence":U
    ttComboData.cBufferList = "gsm_menu_item":U
    ttComboData.cKeyFieldName = "gsm_menu_item.menu_item_obj":U
    ttComboData.cDescFieldNames = "gsm_menu_item.menu_item_label,gsm_menu_item.menu_item_description":U
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION hasMenuStructureChanged sObject 
FUNCTION hasMenuStructureChanged RETURNS LOGICAL
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  Return Yes if the Menu Structure Object has changed. Otherwise No.
    Notes:  
------------------------------------------------------------------------------*/

  DEFINE VARIABLE dMenu_Structure_Obj    AS DECIMAL      NO-UNDO.

  ASSIGN dMenu_Structure_Obj = {fnarg columnValue '"menu_structure_obj"' ghSDO}.

  IF (gdMenu_Structure_Obj <> dMenu_Structure_Obj) THEN
      RETURN TRUE.
  ELSE
...
IF hasMenuStructureChanged() THEN
DO:
  EMPTY TEMP-TABLE ttComboData.
  RUN getComboQuery (INPUT-OUTPUT TABLE ttComboData).
