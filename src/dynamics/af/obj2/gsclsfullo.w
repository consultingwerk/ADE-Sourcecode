...
&Scoped-define INTERNAL-TABLES gsc_logical_service gsc_service_type

/* Definitions for QUERY Query-Main                                     */
&Scoped-Define ENABLED-FIELDS  logical_service_code logical_service_description service_type_obj~
 can_run_locally system_owned write_to_config connect_at_startup
&Scoped-define ENABLED-FIELDS-IN-gsc_logical_service logical_service_code ~
logical_service_description service_type_obj can_run_locally system_owned ~
write_to_config connect_at_startup 
&Scoped-Define DATA-FIELDS  logical_service_obj logical_service_code logical_service_description~
 service_type_obj service_type_code can_run_locally system_owned~
 write_to_config connect_at_startup
&Scoped-define DATA-FIELDS-IN-gsc_logical_service logical_service_obj ~
logical_service_code logical_service_description service_type_obj ~
can_run_locally system_owned write_to_config connect_at_startup 
...
gsc_service_type
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main gsc_logical_service
...
DEFINE QUERY Query-Main FOR 
      gsc_logical_service, 
      gsc_service_type
    FIELDS(gsc_service_type.service_type_code) SCROLLING.
&ANALYZE-RESUME
{&DB-REQUIRED-END}
...
"write_to_config" "write_to_config" ? ? "logical" ? ? ? ? ? ? yes ? no 1 yes
     _FldNameList[9]   > ICFDB.gsc_logical_service.connect_at_startup
"connect_at_startup" "connect_at_startup" ? ? "logical" ? ? ? ? ? ? yes ? no 17.4 no
...
FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
  IF (RowObjUpd.RowMod = 'U':U AND
    CAN-FIND(FIRST gsc_logical_service
      WHERE gsc_logical_service.logical_service_code = rowObjUpd.logical_service_code
      AND ROWID(gsc_logical_service) <> TO-ROWID(ENTRY(1,RowObjUpd.RowIDent))))
  OR (RowObjUpd.RowMod <> 'U':U AND
    CAN-FIND(FIRST gsc_logical_service
      WHERE gsc_logical_service.logical_service_code = rowObjUpd.logical_service_code))
  THEN
    ASSIGN
      cValueList   = STRING(RowObjUpd.logical_service_code)
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '8' 'gsc_logical_service' '' "'logical_service_code, '" cValueList }.
END.
...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  IF LENGTH(RowObject.logical_service_code) = 0 OR LENGTH(RowObject.logical_service_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsc_logical_service' 'logical_service_code' "'Logical Service Code'"}.

  IF LENGTH(RowObject.logical_service_description) = 0 OR LENGTH(RowObject.logical_service_description) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsc_logical_service' 'logical_service_description' "'Logical Service Description'"}.

  IF RowObject.service_type_obj = 0 OR RowObject.service_type_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsc_logical_service' 'service_type_obj' "'Service Type Obj'"}.
