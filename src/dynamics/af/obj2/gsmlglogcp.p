...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_login_company Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_login_company table
...
&GLOB DATA-LOGIC-TABLE gsm_login_company
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmlgful2o.i"
...
/*------------------------------------------------------------------------------
  Purpose:     Procedure used to validate b_gsm_login_company record client-side
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
...
DEFINE VARIABLE cValueList      AS CHARACTER    NO-UNDO.
  ...
  IF LENGTH(b_gsm_login_company.login_company_code) = 0 OR LENGTH(b_gsm_login_company.login_company_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_login_company' 'login_company_code' "'Login Company Code'"}.

  IF LENGTH(b_gsm_login_company.login_company_short_name) = 0 OR LENGTH(b_gsm_login_company.login_company_short_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_login_company' 'login_company_short_name' "'Login Company Short Name'"}.

  IF LENGTH(b_gsm_login_company.login_company_name) = 0 OR LENGTH(b_gsm_login_company.login_company_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
