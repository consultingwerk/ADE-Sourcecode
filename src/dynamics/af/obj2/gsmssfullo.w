...
------------------------------------------------------------------------------*/
  ...
  FIND rowObject WHERE rowObject.rowNum = gdRowNum NO-ERROR.

  IF  AVAILABLE rowObjUpd 
  AND AVAILABLE rowObject 
  THEN DO:
      ...
      IF AVAILABLE gsm_security_structure THEN
          ASSIGN gsm_security_structure.owning_obj             = gdOwningObj
                 ...
                 rowObject.owning_obj                          = gdOwningObj
                 rowObject.product_module_obj                  = gdProductModuleObj
                 rowObject.object_obj                          = gdObjectObj       
                 rowObject.instance_attribute_obj              = gdAttributeObj.
  END.
