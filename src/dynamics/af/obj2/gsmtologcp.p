...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsm_token Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsm_token table
...
&GLOB DATA-LOGIC-TABLE gsm_token
&GLOB DATA-FIELD-DEFS  "af/obj2/gsmtoful2o.i"
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsm_token.token_code) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsm_token' 'token_code' "'Token code'"}.
