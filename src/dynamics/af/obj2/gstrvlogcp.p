...
/*---------------------------------------------------------------------------------
  ...
  Description:  gst_record_version Data Logic Procedure

  Purpose:      A procedure library (PLIP) to support the maintenance of the gst_record_version table
  ...
  (v:010001)    Task:    90000114   UserRef:    posse
                Date:   03/05/2001  Author:     Tammy St Pierre

  Update Notes: Re-generated SDO

  (v:010002)    Task:    90000119   UserRef:    posse
                Date:   06/05/2001  Author:     Haavard Danielsen

  Update Notes: Changed method library ref to src/adm2/logic.i
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST gst_record_version 
              WHERE gst_record_version.entity_mnemonic = b_gst_record_version.entity_mnemonic
                AND gst_record_version.key_field_value = b_gst_record_version.key_field_value) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gst_record_version.entity_mnemonic) + ', ' + STRING(b_gst_record_version.key_field_value)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'gst_record_version' '' "'entity_mnemonic, key_field_value, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "Dynamics gst_record_version Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF LENGTH(b_gst_record_version.entity_mnemonic) = 0 OR LENGTH(b_gst_record_version.entity_mnemonic) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_record_version' 'entity_mnemonic' "'Entity Mnemonic'"}.

  IF LENGTH(b_gst_record_version.key_field_value) = 0 OR LENGTH(b_gst_record_version.key_field_value) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_record_version' 'key_field_value' "'Key Field Value'"}.

  IF b_gst_record_version.version_date = ? OR b_gst_record_version.version_date = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_record_version' 'version_date' "'Version Date'"}.

  IF b_gst_record_version.version_time = 0 OR b_gst_record_version.version_time = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_record_version' 'version_time' "'Version Time'"}.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST gst_record_version 
              WHERE gst_record_version.entity_mnemonic = b_gst_record_version.entity_mnemonic
                AND gst_record_version.key_field_value = b_gst_record_version.key_field_value
                AND ROWID(gst_record_version) <> TO-ROWID(ENTRY(1,b_gst_record_version.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_gst_record_version.entity_mnemonic) + ', ' + STRING(b_gst_record_version.key_field_value)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
