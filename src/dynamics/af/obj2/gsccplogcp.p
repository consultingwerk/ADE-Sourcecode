...
/*---------------------------------------------------------------------------------
  ...
  Description:  gsc_custom_procedure Data Logic Procedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the gsc_custom_procedure table
...
&GLOB DATA-LOGIC-TABLE gsc_custom_procedure
&GLOB DATA-FIELD-DEFS  "af/obj2/gsccpfullo.i"
...
FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
  ...
  IF (RowObjUpd.RowMod = 'U':U AND
    CAN-FIND(FIRST gsc_custom_procedure
      WHERE gsc_custom_procedure.procedure_name = rowObjUpd.procedure_name
      AND ROWID(gsc_custom_procedure) <> TO-ROWID(ENTRY(1,RowObjUpd.RowIDent))))
  OR (RowObjUpd.RowMod <> 'U':U AND
    CAN-FIND(FIRST gsc_custom_procedure
      WHERE gsc_custom_procedure.procedure_name = rowObjUpd.procedure_name))
  THEN
    ASSIGN
      cValueList   = STRING(RowObjUpd.procedure_name)
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                     {af/sup2/aferrortxt.i 'AF' '8' 'gsc_custom_procedure' 'procedure_name' "'procedure_name'" cValueList} .
            
 /*                    
 IF NOT(CAN-FIND(FIRST ryc_smartobject NO-LOCK WHERE ryc_smartobject.object_filename = rowObjUpd.procedure_name)) THEN
  ASSIGN
    cValueList   = STRING(RowObjUpd.procedure_name)
    cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                   {af/sup2/aferrortxt.i 'AF' '5' 'gsc_custom_procedure' 'procedure_name' "'procedure_name'" cValueList}.
 */
...
------------------------------------------------------------------------------*/
  ...
  IF isFieldBlank(b_gsc_custom_procedure.procedure_name) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_custom_procedure' 'procedure_name' "'Procedure name'"}.
 
  IF isFieldBlank(STRING(b_gsc_custom_procedure.category_obj)) THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'gsc_custom_procedure' 'category_obj' "'category_obj'"}.
