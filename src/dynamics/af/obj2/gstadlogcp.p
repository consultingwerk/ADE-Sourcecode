...
/*---------------------------------------------------------------------------------
  ...
  Description:  gst_audit Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the gst_audit table
...
&GLOB DATA-LOGIC-TABLE gst_audit
&GLOB DATA-FIELD-DEFS  "af/obj2/gstadfullo.i"
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "Astra 2 gst_audit Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    b_gst_audit.owning_obj              = DECIMAL( ENTRY( 1 , DYNAMIC-FUNCTION("getForeignValues":U IN TARGET-PROCEDURE) , CHR(1) ) ).
    b_gst_audit.owning_entity_mnemonic  = DYNAMIC-FUNCTION("getUserProperty":U  IN TARGET-PROCEDURE  , "OwningEntityMnemonic":U).
    .

  IF LENGTH(b_gst_audit.owning_entity_mnemonic) = 0 OR LENGTH(b_gst_audit.owning_entity_mnemonic) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_audit' 'owning_entity_mnemonic' "'Owning Entity Mnemonic'"}.

  IF b_gst_audit.owning_obj = 0 OR b_gst_audit.owning_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_audit' 'owning_obj' "'Owning Obj'"}.

  IF b_gst_audit.audit_date = ? OR b_gst_audit.audit_date = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_audit' 'audit_date' "'Audit Date'"}.

  IF b_gst_audit.audit_time = 0 OR b_gst_audit.audit_time = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_audit' 'audit_time' "'Audit Time'"}.

  IF b_gst_audit.audit_user_obj = 0 OR b_gst_audit.audit_user_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gst_audit' 'audit_user_obj' "'Audit User Obj'"}.
