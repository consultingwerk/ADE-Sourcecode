...
&scop object-name       dcusitedatadump.p
...
DEFINE TEMP-TABLE ttSiteNo NO-UNDO
  FIELD iSiteNo   AS INTEGER
  FIELD dRevSite  AS DECIMAL DECIMALS 9 
...
&IF DEFINED(EXCLUDE-reverseSiteNo) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD reverseSiteNo Procedure 
FUNCTION reverseSiteNo RETURNS DECIMAL
  ( INPUT piSiteNo AS INTEGER )  FORWARD.
...
/* ***************************  Main Block  *************************** */
PUBLISH "DCU_WriteLog":U ("Exporting site data...").
RUN initializeFixFramework.

IF NOT TEMP-TABLE ttSiteNo:HAS-RECORDS THEN
DO:
  PUBLISH "DCU_WriteLog":U ("WARNING: No site data was exported.").
  RETURN.
END.

IF NOT TEMP-TABLE ttFixProgram:HAS-RECORDS THEN
DO:
  PUBLISH "DCU_WriteLog":U ("WARNING: No export programs found.").
  RETURN.
END.

RUN exportSiteData. 
PUBLISH "DCU_WriteLog":U ("Site data export complete.").
...
&IF DEFINED(EXCLUDE-exportSiteData) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE exportSiteData Procedure 
PROCEDURE exportSiteData :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  FOR EACH ttFixProgram:
    ...
    FOR EACH ttDataFile 
      ...
      DO:
        PUBLISH "DCU_WriteLog":U ("Deleting old data file: " + ttDataFile.cDataFilePath).
        OS-DELETE VALUE(ttDataFile.cDataFilePath).
      END.
    ...
    FOR EACH ttSiteNo:
      PUBLISH "DCU_WriteLog":U ("Running dump program " + ttFixProgram.cProgName + " for site " + STRING(ttSiteNo.iSiteNo)).
      RUN VALUE(ttFixProgram.cFullProgPath) 
        (INPUT ttSiteNo.dRevSite,
...
------------------------------------------------------------------------------*/
  ...
  REPEAT:
    ...
    CASE cRecType:
      WHEN "S":U THEN
      DO:
        /* If the string is &S, determine what the current site number is */
        IF cString = "&S":U THEN 
        DO:
          RUN getSiteNumber 
            IN gshGenManager
        ...
        ELSE
          iSiteNo = iNumber.
        dRevSite = reverseSiteNo(iSiteNo). 
        FIND ttSiteNo 
          WHERE ttSiteNo.iSiteNo = iSiteNo
          NO-ERROR.
        IF NOT AVAILABLE(ttSiteNo) THEN
        DO:
          CREATE ttSiteNo.
          ASSIGN
            ttSiteNo.iSiteNo = iSiteNo
            ttSiteNo.dRevSite = dRevSite
          .
        END.
      END.
      WHEN "O":U THEN
      DO:
        ...
        ELSE
        DO:
          PUBLISH "DCU_WriteLog":U ("WARNING: Export program " + cString + " not found.").
        END.
      END.
      WHEN "D":U THEN
      DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION reverseSiteNo Procedure 
FUNCTION reverseSiteNo RETURNS DECIMAL
  ( INPUT piSiteNo AS INTEGER ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE cSiteReverse      AS CHARACTER   NO-UNDO.
  DEFINE VARIABLE cSiteDivision     AS CHARACTER   NO-UNDO.
  DEFINE VARIABLE dSiteReverse      AS DECIMAL     NO-UNDO.
  
  DO iLoop = LENGTH(STRING(piSiteNo)) TO 1 BY -1:
    cCalReverse = cCalReverse + SUBSTRING(STRING(piSiteNo),iLoop,1).
  END.

  ASSIGN
    cSiteReverse    = cCalReverse
    cSiteDivision   = "1":U + FILL("0":U,LENGTH(STRING(piSiteNo)))
    dSiteReverse    =  DECIMAL(INTEGER(cSiteReverse) / INTEGER(cSiteDivision)).

  RETURN dSiteReverse.   /* Function return value. */
