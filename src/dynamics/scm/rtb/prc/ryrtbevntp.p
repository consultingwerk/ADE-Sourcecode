...
/*---------------------------------------------------------------------------------
  ...
  (v:030000)    Task:          42   UserRef:    
                Date:   05/04/2003  Author:     Thomas Hansen

  Update Notes: Issue 8346:
                Fixed removal of actionUnderWay record to correctly remove the record after custom variants have been created.

  (v:040000)    Task:          50   UserRef:    
                Date:   05/27/2003  Author:     Thomas Hansen

  Update Notes: 05/27/2003 thomas:
                Issue 10741: Updated manager API call to afmessagep

  (v:040001)    Task:          52   UserRef:    
                Date:   06/03/2003  Author:     Thomas Hansen

  Update Notes: Delete should only apply to PCODE objects
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING value of RECID of the rtb_object table
  / Meaning :  pcOther         : Object name
               pcErrorMessage : ignored
...
------------------------------------------------------------------------------*/
  ...
  /*   We only want to handle events for PCODE objects, if the         */
  /*   object is not a PCODE object, then cancel the extension to this */
  /*   event by returning.                                             */
  FIND FIRST rtb_object WHERE RECID(rtb_object) = INTEGER(pcContext) NO-LOCK NO-ERROR. 
  IF rtb_object.obj-type <> "PCODE":U THEN 
  DO:
    /* Remove and actionUnderWay records that may have been created for this event.  */
  ...
  IF VALID-HANDLE(Grtb-p-stat) THEN 
    RUN show_status_line IN Grtb-p-stat ("Processing Dynamics Assign extensions for " + lv_assign_object_name + " ...").
  ...
  */     
  ...
  IF lImportUnderWay THEN DO:                                      
    PUBLISH "logEvent" (INPUT pcEvent, 
                       INPUT "[":U + STRING(TIME, "HH:MM:SS":U) + "] ":U + pcEvent + " - ":U + pcOther, 
                       INPUT "INF":U, 
                       INPUT Grtb-wspace-id, 
                       INPUT Grtb-userid, 
                       INPUT "LOG":U,
    ...
    IF pcErrorMessage <> "":U THEN
      PUBLISH "logEvent" (INPUT pcEvent, 
                         INPUT "ERROR / ":U + pcEvent + " (":U + pcOther + ") : ":U + pcErrorMessage, 
                         INPUT "ERROR":U, 
                         INPUT Grtb-wspace-id, 
                         INPUT Grtb-userid, 
                         INPUT "LOG,STATUS":U,
    ...
    */    
    ...
    IF cReturnCharacter = "YES" THEN 
      ASSIGN lSuppressMessages = TRUE. 
    ELSE 
      ASSIGN lSuppressMessages = FALSE. 
        
    IF lSuppressMessages THEN 
      ASSIGN pcErrorMessage = "":U.
  ...
  ASSIGN pcErrorMessage = "":U NO-ERROR.
  RETURN.
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : ""
  / Meaning :  pcOther         : Object name, Object type, Product Module, Version
               pcErrorMessage : non blank will cancel
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    lv_assign_object_name     = ENTRY(1, pcOther)
    lv_assign_object_type     = ENTRY(2, pcOther)
    lv_assign_product_module  = ENTRY(3, pcOther)
    lv_assign_object_version  = ENTRY(4, pcOther)
    .
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING(Grtb-proc-handle) (main Roundtable procedure handle)
  / Meaning :  pcOther         : String value of the name of the workspace that was selected.
               pcErrorMessage : ignored
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN lv_recid = INTEGER(pcContext) NO-ERROR.

  FIND FIRST rtb_object NO-LOCK
    WHERE RECID(rtb_object) = lv_recid
    NO-ERROR.
  IF NOT AVAILABLE rtb_object OR 
     rtb_object.object <> pcOther THEN 
  DO:
    MESSAGE
      "The update of data failed as the Roundtable object (rtb_object) could not be found."
      VIEW-AS ALERT-BOX WARNING BUTTONS OK.
    ASSIGN pcErrorMessage = "error":U.
    RETURN.
  END.
  ELSE
  /*   We only want to handle events for PCODE objects, only if the    */
  /*   object is a PCODE object do we want to call the Dynamics hook  */  
  IF rtb_object.obj-type = "PCODE":U THEN
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING value of RECID of the rtb_site table
  / Meaning :  pcOther         : STRING value of RECID of the rtb_deploy table
               pcErrorMessage : ignored
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
    ...
    THEN DO:
      ...
      FIND FIRST rtb_site NO-LOCK
        WHERE STRING(RECID(rtb_site)) = pcContext
        NO-ERROR.
      IF NOT AVAILABLE rtb_site
      THEN DO:
        ASSIGN
          pcErrorMessage = "Roundtable Site RECID value not found":U
          lDeployQuestion  = NO
          .
        RETURN.
      END.
      ...
      FIND FIRST rtb_deploy NO-LOCK
        WHERE STRING(RECID(rtb_deploy)) = pcOther
        NO-ERROR.
      IF NOT AVAILABLE rtb_deploy
      THEN DO:
        ASSIGN
          pcErrorMessage = "Roundtable Deployment RECID value not found":U
          lDeployQuestion  = NO
          .
        RETURN.
      END.
      ...
      THEN DO:
        ASSIGN
          pcErrorMessage = "Roundtable Site and Deployment value discrepencies":U
          lDeployQuestion  = NO
          .
        RETURN.
      END.
  ...
  THEN DO:
    ...
    RUN rtb/uib/rtbdeployw.w (INPUT pcContext ,INPUT pcOther).
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING value of RECID of the rtb_site table
  / Meaning :  pcOther         : STRING value of RECID of the rtb_deploy table
               pcErrorMessage : non blank will cancel deployment creation
...
/*------------------------------------------------------------------------------
  Purpose:     After deployment created
  Parameter :  pcContext       : ""
  / Meaning :  pcOther         : Workspace, License Type, Site Code
               pcErrorMessage : logical - false will cancel site creation
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH rtb_import NO-LOCK
    WHERE rtb_import.wspace-id  = pcOther
    AND   rtb_import.done       = YES:

    IF INDEX(rtb_import.object,".df":U) = 0 THEN 
       NEXT import-loop.
  ...
  PUBLISH "logEvent" (INPUT pcEvent, 
   ...
   IF TRIM(cButton, "&":U) = "YES":U THEN
      IF SEARCH("rtb/uib/rtb_rnpw.w":U) <> ? OR 
         SEARCH("rtb/uib/rtb_rnpw.r":U) <> ? THEN 
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : ""
  / Meaning :  pcOther         : Workspace id importing into
               pcErrorMessage : non blank will cancel
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING value of RECID of rtb.rtb_object
  / Meaning :  pcOther         : INPUT Object, Obj-Type, Pmod, WS Module
               pcErrorMessage : ignored
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST rtb_object WHERE RECID(rtb_object) = lv_recid 
    NO-LOCK NO-ERROR.
  ...
  IF AVAILABLE rtb_object AND rtb_object.obj-type = "PCODE":U THEN 
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:   pcEvent          :   "OBJECT-CHANGE-SHARE-STATUS",
                pcContext        :   String value of RECID for rtb.rtb_object
                pcOther          :   Object, Old status, New Status, Workspace Path, Task Path
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters: pcEvent     :   "OBJECT-CHANGE-SHARE-STATUS-BEFORE",
              pcContext   :   String value of RECID for rtb.rtb_object
              pcOther     :   Object, Old status, New Status, Workspace Path, Task Path
...
/*------------------------------------------------------------------------------
  ...
  FIND FIRST rtb_object WHERE RECID(rtb_object) = lv_recid NO-LOCK NO-ERROR.
  IF NOT AVAILABLE rtb_object
  OR rtb_object.object <> pcOther THEN 
  DO:
    IF VALID-HANDLE(gshSessionManager) THEN     
      RUN showMessages IN gshSessionManager
                      (INPUT {af/sup2/aferrortxt.i 'AF' '36' '?' '?' "'Progress Dynamics'" "'because the rtb_object record could not be found or the wrong object was found from the recid'"},
  ...
  IF AVAILABLE rtb_object AND 
     rtb_object.obj-type <> "PCODE":U
  THEN RETURN. /* only interested in pcode objects */

  FIND FIRST rtb_ver WHERE rtb_ver.obj-type  = rtb_object.obj-type AND   
                           rtb_ver.object    = rtb_object.OBJECT AND   
                           rtb_ver.pmod      = rtb_object.pmod AND   
                           rtb_ver.version   = rtb_object.version NO-LOCK NO-ERROR.
...
Purpose:     Before object is checked in
  Parameter :  pcContext       : STRING value of RECID of the rtb_object table
  / Meaning :  pcOther         : Object name
               pcErrorMessage : non blank will cancel check in
  ...
  DO:
    ...
    DO:
      ...
      CASE cMesWinButtonPressed:
        ...
        DO:
            ASSIGN
              pcErrorMessage = "Object Check-In Cancelled".
            RETURN.
        END.
        WHEN "Export":U THEN
            RUN setActionUnderway IN gshSessionManager
...
------------------------------------------------------------------------------*/
  ...
  THEN DO:
    RUN rtb/prc/afrtbappsp.p (INPUT INTEGER(pcContext)
                             ,INPUT pcOther
                             ,OUTPUT pcErrorMessage).
  END.
  */
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING value of RECID of rtb_object table
  / Meaning :  pcOther         : Object name
               pcErrorMessage : non blank will cancel
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
    lv_assign_object_name = pcOther.

  /*   We only want to handle delete events for PCODE objects, if the  */
  /*   object is nor a PCODE object, then cancel the extension to this */
  /*   event by returning.                                             */
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : String value of RECID of rtb.rtb_wspace loaded
  / Meaning :  pcOther         : Workspace Path, /* entry 1 is the root path */
               pcErrorMessage : ignored
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : Potential Workspace ID,   /* no records are created yet */
  / Meaning :  pcOther         : Workspace Path,  /* entry 1 is the root path */
               pcErrorMessage : Cancel load if FALSE
...
------------------------------------------------------------------------------*/
  ...
  /* Subscribe to the shutdown event - so that we can shut down this procedxure 
...
/*------------------------------------------------------------------------------
  ...
  Parameters:   pcEvent = name of event
                pcContext = event dependant
                pcOther = event dependant
                pcErrorMessage = An error message to display in the event of failure
...
------------------------------------------------------------------------------*/
  ...
  /*
  IF pcEvent = "ASSIGN-OBJECT-BEFORE":U OR pcEvent = "ASSIGN-OBJECT":U
  OR pcEvent = "CREATE-CV-BEFORE":U     OR pcEvent = "CREATE-CV":U
  OR pcEvent = "OBJECT-ADD-BEFORE":U    OR pcEvent = "OBJECT-ADD":U
  OR pcEvent = "OBJECT-DELETE-BEFORE":U OR pcEvent = "OBJECT-DELETE":U
  THEN DO:

    DEFINE VARIABLE lv_choice AS LOGICAL NO-UNDO.
    MESSAGE
        SKIP "Event: "   pcEvent
        SKIP "Context: " pcContext
        SKIP "Other: "   pcOther
        VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE lv_choice.
    IF lv_choice = NO
    THEN DO:
      ASSIGN
        pcErrorMessage  = "Error".
      RETURN.
    END.
  ...
  */
  ...
  CASE pcEvent:
    WHEN "ASSIGN-OBJECT":U                      THEN RUN assign-object              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "ASSIGN-OBJECT-BEFORE":U               THEN RUN assign-object-before       (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "BEFORE-CHANGE-WORKSPACE":U            THEN RUN before-change-workspace    (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "CHANGE-WORKSPACE":U                   THEN RUN change-workspace           (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "CREATE-CV":U                          THEN RUN create-cv                  (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "CREATE-CV-BEFORE":U                   THEN RUN create-cv-before           (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "DEPLOY":U                             THEN RUN deploy                     (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "DEPLOY-BEFORE":U                      THEN RUN deploy-before              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "DEPLOY-SITE-CREATE":U                 THEN RUN deploy-site-create         (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "DEPLOY-SITE-CREATE-BEFORE":U          THEN RUN deploy-site-create-before  (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "IMPORT":U                             THEN RUN import                     (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "IMPORT-BEFORE":U                      THEN RUN import-before              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "MOVE-TO-WEB":U                        THEN RUN move-to-web                (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "MOVE-TO-WEB-BEFORE":U                 THEN RUN move-to-web-before         (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-ADD":U                         THEN RUN object-add                 (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-ADD-BEFORE":U                  THEN RUN object-add-before          (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHANGE-SHARE-STATUS-BEFORE":U  THEN RUN object-change-share-status-before (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHANGE-SHARE-STATUS":U         THEN RUN object-change-share-status (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHECK-IN":U                    THEN RUN object-check-in            (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHECK-IN-BEFORE":U             THEN RUN object-check-in-before     (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHECK-OUT":U                   THEN RUN object-check-out           (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-CHECK-OUT-BEFORE":U            THEN RUN object-check-out-before    (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-COMPILE":U                     THEN RUN object-compile             (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-COMPILE-BEFORE":U              THEN RUN object-compile-before      (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-DELETE":U                      THEN RUN object-delete              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "OBJECT-DELETE-BEFORE":U               THEN RUN object-delete-before       (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "PARTNER-LOAD":U                       THEN RUN partner-load               (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "PARTNER-LOAD-BEFORE":U                THEN RUN partner-load-before        (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "PROPATH-CHANGE":U                     THEN RUN propath-change             (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "PROPATH-CHANGE-BEFORE":U              THEN RUN propath-change-before      (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "RELEASE-CREATE":U                     THEN RUN release-create             (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "RELEASE-CREATE-BEFORE":U              THEN RUN release-create-before      (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "ROUNDTABLE-STARTUP":U                 THEN RUN roundtable-startup         (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "ROUNDTABLE-SHUTDOWN":U                THEN RUN roundtable-shutdown        (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "SCHEMA-UPDATE":U                      THEN RUN schema-update              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "SCHEMA-UPDATE-BEFORE":U               THEN RUN schema-update-before       (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "SESSION-SHUTDOWN":U                   THEN RUN session-shutdown           (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-CHANGE":U                        THEN RUN task-change                (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-COMPLETE":U                      THEN RUN task-complete              (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-COMPLETE-BEFORE":U               THEN RUN task-complete-before       (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-CREATE":U                        THEN RUN task-create                (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-CREATE-BEFORE":U                 THEN RUN task-create-before         (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
    WHEN "TASK-CREATE-DURING":U                 THEN RUN task-create-during         (INPUT pcEvent, INPUT pcContext, INPUT pcOther, OUTPUT pcErrorMessage).
  END CASE.
  ...
  DO:
    ...
    DO:
      ...
      ASSIGN pcErrorMessage = cFullMessages.
    END.
  END.
...
/*------------------------------------------------------------------------------
  ...
  Parameter : pcContext       : RECID (table depends on context)
  / Meaning : pcOther         : context (see note)
              pcErrorMessage : Cancel load if FALSE
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : ""
  / Meaning :  pcOther         : Workspace id, release number
               pcErrorMessage : ignored
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:   pcEvent          :   "ROUNDTABLE-SHUTDOWN",
                pcContext        :   
                pcOther          :   
                ip_error_message  :   Ignored 
  Notes:       
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:   pcEvent          :   "ROUNDTABLE-STARTUP",
                pcContext        :   
                pcOther          :   
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : "" 
               pcOther         : Workspace id 
               pcErrorMessage : non blank will cancel (logical - false will cancel schema update)
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:   pcEvent          :   "SESSION-SHUTDOWN",
                pcContext        :   
                pcOther          :   
                ip_error_message  :   Ignored 
  Notes:       
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : STRING(Grtb-proc-handle) (main Roundtable procedure handle)
  / Meaning :  pcOther         : String value of the task that was selected.
               pcErrorMessage : ignored
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION ("setSessionParam":U IN THIS-PROCEDURE, "_scm_current_task_number":U, pcOther ).  
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST rtb_task NO-LOCK
    WHERE rtb_task.task-num = INTEGER(pcOther)
    NO-ERROR.
  ...
  THEN DO:
    ...
    CASE cMesWinButtonPressed:
      WHEN cMesWinButtonCancel /* Cancel */
        THEN DO:
          ASSIGN
            pcErrorMessage = "Task Completion Cancelled".
          RETURN.
        END.
      WHEN "Export All":U
        THEN
...
/*------------------------------------------------------------------------------
  ...
  Parameter :  pcContext       : String value of RECID of rtb.rtb_task 
               pcOther         : Task number in GUI - Wspace-id in TTY 
               pcErrorMessage : Ignored
