...
FIND FIRST Brtb_object WHERE Brtb_object.wspace-id = Grtb-wspace-id AND
                             Brtb_object.obj-type  = "PCODE" AND 
    ...
    */
    ...
    DO:
      ASSIGN 
        ...
        pdRepoXrefObj  = DECIMAL(REPLACE(gsm_scm_xref.owning_reference, ".":U, SESSION:NUMERIC-DECIMAL-POINT)).
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO:
    FIND FIRST b_gsmsx WHERE 
      ...
      b_gsmsx.owning_reference       = REPLACE(STRING(pdOwningObj),
                                               SESSION:NUMERIC-DECIMAL-POINT, ".":U)
      NO-LOCK NO-ERROR.
    ...
    DO:
      ...
      DO:
        FIND FIRST b_gsmsx WHERE 
          b_gsmsx.scm_tool_obj           = dSCMToolObj     AND  
          b_gsmsx.owning_entity_mnemonic = pcOwningEntity  AND 
          b_gsmsx.owning_reference       = REPLACE(STRING(dObjValue),
                                                   SESSION:NUMERIC-DECIMAL-POINT, ".":U)
          NO-LOCK NO-ERROR.
        IF AVAILABLE b_gsmsx THEN        
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN pcRoot = RIGHT-TRIM(REPLACE(grtb-wsroot,"~\":U,"/":U),"/":U).
