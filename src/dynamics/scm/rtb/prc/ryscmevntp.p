...
------------------------------------------------------------------------------*/
  ...
  ELSE
  DO:  
    IF cRepoProductModuleCode = "":U THEN
    DO:
        ASSIGN pcError = "Error checking in object: " + pcObjectName + ".":U + "~n":U
                       + "Either no SCM xref record exists for the Dynamics Product Module, or the Xref record does not point to a valid SCM Product Module." + "~n":U.
        RETURN.
    END.
