...
/*---------------------------------------------------------------------------------
  ...
  (v:020000)    Task:          46   UserRef:    
                Date:   05/06/2003  Author:     Thomas Hansen

  Update Notes: Issue 10557:
                Fixed typo in startPersistentprocs which was not looking for the
                r-code version of the RTB super procedure.

  (v:030000)    Task:          49   UserRef:    
                Date:   06/11/2003  Author:     Thomas Hansen

  Update Notes: Create maintenace tool for SCM tool

  (v:030001)    Task:          49   UserRef:    
                Date:   06/24/2003  Author:     Thomas Hansen
...
------------------------------------------------------------------------------*/
  ...
  IF lScmObjectExists THEN 
  DO:
    /* a couple of checks first */
    IF NOT lScmObjectExistsInWs THEN 
      RETURN ERROR {af/sup2/aferrortxt.i 'RV' '26' '?' '?' cObjectName cWorkspaceCode cArguments}.
          
    IF NOT lScmCheckedOutInWorkspace THEN 
      RETURN ERROR {af/sup2/aferrortxt.i 'RV' '27' '?' '?' cObjectName cWorkspaceCode cArguments}. 
         
    IF iScmVersionTaskNumber <> iTaskNumber THEN 
      RETURN ERROR {af/sup2/aferrortxt.i 'RV' '28' '?' '?' cObjectName iTaskNumber iScmVersionTaskNumber}.
  END.
  ELSE 
  DO:
    ...
    FIND FIRST rtb_object NO-LOCK
      WHERE rtb_object.wspace-id  = cWorkspaceCode AND   
            rtb_object.obj-type   = "PCODE":U      AND   
            rtb_object.object     = cObjectName_ado 
            NO-ERROR.
    IF NOT AVAILABLE rtb_object THEN 
    DO:
      /* get product module */
      IF cProductModuleFieldname <> "":U THEN 
      DO:
      ...
      /* get object type */
      IF cObjectTypeFieldname <> "":U THEN 
      DO:
        /* determine the product module from the primary table buffer */
      ...
      IF cDescriptionFieldname <> "":U THEN 
      DO:
        /* determine the product module from the primary table buffer */
        ...
        IF cObjectDescription = "":U THEN
          ASSIGN cObjectDescription = getFieldValue(phTableBufferHandle, cScmIdentifyingFieldname).
      END.
      ELSE
        RETURN ERROR {af/sup2/aferrortxt.i 'RV' '14' '?' '?' cConfigurationType iTaskNumber cArguments}.
      ...
      DO:
        RUN scmCreateObjectControl IN hScmTool
                                  (INPUT cObjectName,
                                   INPUT "PCODE":U,
                                   INPUT (IF AVAILABLE gsc_object_type    THEN gsc_object_type.object_type_code       ELSE "":U),   /* Code Subtype */
                                   INPUT (IF AVAILABLE gsc_product_module THEN gsc_product_module.product_module_code ELSE "":U),
                                   INPUT (IF AVAILABLE gsc_object_type    THEN gsc_object_type.object_type_code       ELSE "":U), /* Group */
                                   INPUT "00000":U,
                                   INPUT cObjectDescription,
                                   INPUT "":U,       /* ip_options                */
                                   INPUT iTaskNumber,
                                   INPUT NO,         /* ip_ui_on                  */
                                   INPUT "CENTRAL":U,  /* ip_share_status           */
...
------------------------------------------------------------------------------*/
  ...
  IF pcTableFla = pcPrimaryFla THEN 
  DO:
    /*
    ...
    */
    ...
    IF VALID-HANDLE(phOldBufferHandle)            AND 
       CAN-QUERY(phOldBufferHandle,"AVAILABLE":U) AND 
       phOldBufferHandle:AVAILABLE = TRUE         THEN        
      ASSIGN cOldObjectName = getFieldValue(phOldBufferHandle, cScmIdentifyingFieldname).
    ELSE 
      ASSIGN cOldObjectName = cNewObjectName.

    IF cOldObjectName = "":U OR 
       cOldObjectName = ?    THEN
      ASSIGN cOldObjectName = cNewObjectName.

    /* Product Modules - Should be renamed in RTB  */
    IF  cNewObjectName <> ?               AND 
        cNewObjectName <> "":U            AND 
        cNewObjectName <> cOldObjectName THEN
        RETURN ERROR {af/sup2/aferrortxt.i 'AF' '110' '?' '?' cOldObjectName cNewObjectName cArguments}.

    IF cProductModuleFieldname <> "":U THEN 
    DO:     
      ASSIGN dNewProductModule = DECIMAL(getFieldValue(phTableBufferHandle, cProductModuleFieldname)).
      ...
      ELSE
        ASSIGN dOldProductModule = dNewProductModule.
    ...
    IF  dNewProductModule <> ?                 AND 
        dNewProductModule <> 0                 AND 
        dOldProductModule <> 0                 AND 
        dOldProductModule <> ?                 AND 
        dNewProductModule <> dOldProductModule THEN 
    DO:
        ...
        IF AVAILABLE gsc_product_module THEN
            ASSIGN cNewProductModuleCode = "product module " + gsc_product_module.product_module_code
                                         + "(" + STRING(dNewProductModule) + ")".

        FIND FIRST gsc_product_module WHERE product_module_obj = dOldProductModule NO-LOCK NO-ERROR.
        IF AVAILABLE gsc_product_module THEN
            ASSIGN cOldProductModuleCode = "product module " + gsc_product_module.product_module_code
  ...
  */
  IF NOT lDeletingItem THEN 
  DO:
    CREATE BUFFER hBufferHandle FOR TABLE cTypeTableName.
    ASSIGN gcDBName = hBufferHandle:DBNAME.
  
    ASSIGN cWhereString = "WHERE ":U.
  
    DO iIdx = 1 TO NUM-ENTRIES(pcPrimaryKeyFields):
      ASSIGN lNullsInPrimaryKey = LOOKUP(getQuotedFieldValue(phTableBufferHandle, ENTRY(iIdx,pcPrimaryKeyFields)),',?,0,"0"') > 0.
      
      IF iIdx > 1 THEN
        ASSIGN cWhereString = cWhereString + " AND ".
        
      ASSIGN cWhereString = cWhereString + cTypeTableName + "." + ENTRY(iIdx,cScmPrimaryKeyFields) + " = " + getQuotedFieldValue(phTableBufferHandle, ENTRY(iIdx,pcPrimaryKeyFields)) + " ". 
    END.
    ...
    IF NOT hBufferHandle:AVAILABLE THEN 
    DO:
  ...
  */
  ...
  DO:    
    /* First of all, we only want to check if the current buffer is for the 
       ...
       other buffers.  */
    IF phTableBufferHandle:TABLE = cQueryTableName THEN 
    DO:
      /* Make sure the buffer is available. */
      IF phTableBufferHandle:AVAILABLE THEN 
      DO:
        ASSIGN
        ...
        IF cQueryObjectExt <> "":U THEN 
          ASSIGN cQueryObjectFullName = cQueryObjectName + "." + cQueryObjectExt.
        ELSE 
          ASSIGN cQueryObjectFullName = cQueryObjectName.
  ...
  DO:
    ...
    ASSIGN cWhereString = "WHERE ":U  + cQueryTableName + ".":U + cQueryObjectFieldName + " = ":U + QUOTER(cQueryObjectFullName).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lScmTaskOpen                AS LOGICAL    NO-UNDO.
  ...
  ASSIGN
    ...
    lScmTaskOpen   = (cScmTaskstatus = "W"). /* W = Work in progress */

  IF iTaskNumber = ? OR 
     iTaskNumber = 0 THEN 
    RETURN ERROR {af/sup2/aferrortxt.i 'RV' '5' '?' '?' cArguments}.

  IF NOT lScmTaskOpen THEN 
    RETURN ERROR {af/sup2/aferrortxt.i 'RV' '25' '?' '?' cArguments}.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cWorkspaceCode = Grtb-wspace-id.

  IF cWorkspaceCode = ?     OR 
     cWorkspaceCode = "":U  THEN 
     RETURN ERROR {af/sup2/aferrortxt.i 'RV' '2' '?' '?' cArguments}.

  ASSIGN cArguments = cArguments + "~nWORKSPACE='" + cWorkspaceCode + "'":U.
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, RETURN ERROR RETURN-VALUE:
    ...
    ASSIGN    
      lCreatingItem = (pcAction = "WRITE":U  AND pcTableFla = pcPrimaryFla AND lNew )
      lDeletingItem = (pcAction = "DELETE":U AND pcTableFla = pcPrimaryFla).
    ...
    IF assignmentUnderway(FALSE) THEN 
      RETURN.
    ...
    ASSIGN cObjectName_ado = cObjectName.
    IF VALID-HANDLE(hScmTool) THEN
...
*/  
    ...
    DO:
      ...
      IF NOT AVAILABLE rtb_ver THEN 
        RETURN ERROR  {af/sup2/aferrortxt.i 'RV' '7' '?' '?' pcPrimaryFla rtb_object.object cArguments}.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  ASSIGN pcTablePkFields = REPLACE(REPLACE(REPLACE(pcTablePkFields," ":U,"":U),CHR(13),"":U),CHR(10),"":U).
...
------------------------------------------------------------------------------*/
  ...
  IF CONNECTED("rtb":U)                       AND 
     NOT VALID-HANDLE(hScmTool)               AND 
     (SEARCH("rtb/prc/afrtbprocp.p":U) <> ?   OR 
      SEARCH("rtb/prc/afrtbprocp.r":U) <> ?)  THEN
  DO:
    RUN rtb/prc/afrtbprocp.p PERSISTENT SET hScmTool.
    ASSIGN hScmTool:PRIVATE-DATA = "SCMTool":U.     
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF lScmCheckedOutInWorkspace THEN 
  DO:
    /* the item is checked out explicitly */
    IF iScmVersionTaskNumber <> iTaskNumber  THEN 
      RETURN ERROR {af/sup2/aferrortxt.i 'RV' '12' '?' '?' pcPrimaryFla cObjectName iTaskNumber iScmVersionTaskNumber cArguments}.
    ...
    IF cDescriptionFieldname <> "":U THEN 
    DO:
      ASSIGN cNewObjectDescription = getFieldValue(phTableBufferHandle, cDescriptionFieldname).                       
    ...
    IF cNewObjectDescription <> cOldObjectDescription THEN 
    DO:    
