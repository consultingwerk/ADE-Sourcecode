...
------------------------------------------------------------------------------*/
 ...
 below &3 that have not already been loaded into Roundtable.",
     (IF RS-scan-by:SCREEN-VALUE = "Subtype":U THEN
        "Subtype":U
      ELSE ""),
     (IF RS-scan-by:SCREEN-VALUE = "Filespec":U THEN
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE Brtb_subtype THEN DO:
    ...
    Mdot =  R-INDEX(Pfile-name,".").
            IF Mdot = 0 THEN 
              Mdot = LENGTH(Pfile-name) + 1. /* add one here for the location of '.' if one actually existed.   */

    Msuffix = IF Mdot > LENGTH(Brtb_subtype.part-sufix[1]) THEN
                SUBSTRING(Pfile-name,Mdot - (LENGTH(Brtb_subtype.part-sufix[1])),LENGTH(Brtb_subtype.part-sufix[1]))
              ELSE "".

    Mbasename = IF Mdot > LENGTH(Brtb_subtype.part-sufix[1]) THEN
                  SUBSTRING(Pfile-name,1,Mdot - LENGTH(Brtb_subtype.part-sufix[1]) - 1)
                ELSE
                  Pfile-name.
