...
&SCOPED-DEFINE CAPITAL-EVENTS "Startup,Before-Open,Open,Close,Before-Save,Save"
...
DEFINE VARIABLE Merror AS CHARACTER   NO-UNDO INITIAL "".
...
IF p_product = "UIB" AND 
  ...
  IF VALID-HANDLE(ghScmTool) THEN DO:
    ...
    IF ENTRY(1,p_other) <> "":U THEN
      ASSIGN p_other = ENTRY(1,p_other).                                                 
  END. 
END.
...
IF CAN-DO({&CAPITAL-EVENTS},p_event) THEN DO:
  PUBLISH 'evRtbAdeEvent':U (INPUT p_product,
                             INPUT p_event,
                             INPUT p_context,
                             INPUT p_other,
                             OUTPUT Merror).
  ...
  IF Merror <> "" THEN DO:
    p_ok = FALSE.
    IF Merror <> "Cancel" THEN
      MESSAGE Merror VIEW-AS ALERT-BOX ERROR TITLE "Roundtable".
  END.
