...
------------------------------------------------------------------------------*/
  ...
  DO iRow = 1 TO 10:
    ...
    REPEAT i1 = 1 TO MINIMUM(8,hb:NUM-FIELDS):
      ...
      ELSE 
        c1 = c1 + '|':U 
           + html-encode(IF hf:BUFFER-VALUE <> ? 
                         THEN hf:BUFFER-VALUE
                         ELSE IF hf:DATA-TYPE BEGINS 'Date' 
                              THEN '' 
                              ELSE '?').
    ...
    FOR EACH DICTDB._index-field NO-LOCK OF DICTDB._index
      ...
      IF DICTDB._index._index-name = cIndex AND get-value('w' + STRING(i1)) > '' THEN DO:
        ...
        CASE DICTDB._field._data-type:
          ...
          WHEN 'datetime'  THEN ASSIGN
            cWhere = cWhere + ' = datetime("' + get-value('w' + STRING(i1)) + '")'.
          WHEN 'datetime-tz' THEN ASSIGN
            cWhere = cWhere + ' = datetime-tz("' + get-value('w' + STRING(i1)) + '")'.
        END CASE.
      END.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO i1 = 0 TO iNum:
    ...
    CASE hField:DATA-TYPE:
      ...
      WHEN "recid":U       OR  
      WHEN "rowid":U   THEN  
        {&OUT} html-encode(RIGHT-TRIM(hField:STRING-VALUE(i1))).
      OTHERWISE /* clob,blob */
        {&OUT} 'Cannot output ' + hField:DATA-TYPE + ' datatype'.
    END CASE.           
    {&OUT} 
      "</TD></TR>" + CHR(10).
  END.
...
------------------------------------------------------------------------------*/
  ...
  DO i1 = 0 TO iNum:
    ...
    CASE hField:DATA-TYPE:
      ...
      WHEN "datetime"    THEN hField:BUFFER-VALUE(i1) = DATETIME(cValue) NO-ERROR.
      WHEN "datetime-tz" THEN hField:BUFFER-VALUE(i1) = DATETIME-TZ(cValue) NO-ERROR.
