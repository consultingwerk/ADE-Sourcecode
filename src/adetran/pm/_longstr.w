...
&Scoped-Define DISPLAYED-OBJECTS src trg Src-Label Trg-Label 
...
DEFINE FRAME DEFAULT-FRAME
     BtnOK AT ROW 1.48 COL 40
     src AT ROW 1.81 COL 4 NO-LABEL
     BtnClear AT ROW 2.71 COL 40
     BtnUndo AT ROW 3.95 COL 40
     BtnDone AT ROW 5.19 COL 40
     BtnHelp AT ROW 6.48 COL 40
...
OR help of frame {&frame-name} do:
  run adecomm/_adehelp.p ("TRAN":U, "CONTEXT":U, {&TMLong_String_Translation_DB}, ?). 
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  ASSIGN LongStrWin:HIDDEN  = TRUE
         LongStrWin:PARENT  = _MainWindow:HANDLE.
...
------------------------------------------------------------------------------*/
  ...
  IF _MainWindow:X < LongStrWin:X AND _MainWindow:Y < LongStrWin:Y AND
     _MainWindow:X + _MainWindow:WIDTH-PIXELS > 
                LongStrWin:X + LongStrWin:WIDTH-PIXELS AND
     _MainWindow:Y + _MainWindow:HEIGHT-PIXELS >
                LongStrWin:Y + LongStrWin:HEIGHT-PIXELS THEN DO:
    IF SESSION:HEIGHT-PIXELS - _MainWindow:Y < 370 THEN
      LongStrWin:Y = _MainWindow:Y - 40.
    ELSE LongStrWin:Y = _MainWindow:Y +  100.
  END.
  VIEW LongStrWin.
