...
define variable KitList as char no-undo.
define variable GlossaryList as char no-undo.
...
&Scoped-Define ENABLED-OBJECTS Rect1 Rect2 Rect3 BtnOK GlossaryName ~
BtnCancel BtnHelp FileName BtnFile BtnOptions CodePage FieldOrder Delim ~
...
DEFINE FRAME DIALOG-1
     ...
     BtnCancel AT ROW 2.81 COL 62
     BtnHelp AT ROW 4.14 COL 62
     FileName AT ROW 5.38 COL 4.4 NO-LABEL
     BtnFile AT ROW 5.38 COL 45
     BtnOptions AT ROW 5.38 COL 62
...
OR HELP OF FRAME {&FRAME-NAME} DO:
  run adecomm/_adehelp.p ("TRAN":U, "CONTEXT":U, {&Import_DB}, ?).      
END.
...
DO:      
  ...
  */
  ...
  repeat:
    ...
    run SetBar in _hMeter (FileSize, seek(ThisStream), Fld1). 
          
    if FieldOrder:screen-value = string(1) then assign
      NewFld1 = Fld1
      NewFld2 = Fld2.
    else assign
      NewFld1 = Fld2
    ...
    find xlatedb.XL_GlossDet where
      xlatedb.XL_GlossDet.GlossaryName = GlossaryName:screen-value and
      xlatedb.XL_GlossDet.ShortSrc     BEGINS SUBSTRING(NewFld1, 1, 63, "RAW":U) and
      xlatedb.XL_GlossDet.ShortTarg    BEGINS SUBSTRING(NewFld2, 1, 63, "RAW":U) and
      xlatedb.XL_GlossDet.SourcePhrase MATCHES NewFld1 and
      xlatedb.XL_GlossDet.TargetPhrase MATCHES NewFld2 no-error.
       
    if not available xlatedb.XL_GlossDet then do:             
      create xlatedb.XL_GlossDet no-error.
      assign NumRecs                                   = NumRecs + 1
             xlatedb.XL_GlossDet.GlossaryName          = GlossaryName:screen-value
             xlatedb.XL_GlossDet.GlossaryType          = "D":u
             xlatedb.XL_GlossDet.ModifiedByTranslator  = false
             xlatedb.XL_GlossDet.ShortSrc              = SUBSTRING(NewFld1, 1, 63, "RAW":U)
             xlatedb.XL_GlossDet.ShortTarg             = SUBSTRING(NewFld2, 1, 63, "RAW":U)
             xlatedb.XL_GlossDet.SourcePhrase          = NewFld1
             xlatedb.XL_GlossDet.TargetPhrase          = NewFld2.
    end.
  end.
  output close.
  run HideMe in _hMeter. 
  ...
  find xlatedb.XL_Glossary 
    where xlatedb.XL_Glossary.GlossaryName = GlossaryName:screen-value no-error.
                     
  if available xlatedb.XL_Glossary then assign
    xlatedb.XL_Glossary.GlossaryCount = xlatedb.XL_Glossary.GlossaryCount + NumRecs. 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign       
    GlossaryName              = if GlossaryList <> "" then GlossaryList else "None":u
    GlossaryName:list-items   = GlossaryName
    GlossaryName:screen-value = s_Glossary
