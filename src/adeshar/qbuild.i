...
/* -----------------------------------------------------------------------
  qbuild.i:  Create a 4GL query based on
     &TblList    - A &SEP1 delimited list of tables each elements of the
   ...
   Used by:
      UIB: importing code and validating queries (in adeuib/_qbuild.p)
      Query Builder: rebuilding query after each change in tables
     
   Created: 1/19/94 Wm.T. Wood 
  ------------------------------------------------------------------------- */
 ...
 ELSE 
   query_options_qbi = " NO-LOCK":U.  /* this is the best default */
  ...
  */
  ...
  IF NUM-ENTRIES ({&TblList}, {&Sep1}) > 0 THEN DO:
      ...
      IF NUM-ENTRIES(cLine_qbi," ") eq 1 OR 
        ...
        */                         
        ...
        THEN DO:
          ...
          =
         /* ( 
          IF CAN-FIND(_FILE WHERE _FILE._FILE-NAME = ENTRY (1, cLine_qbi, " ":U)) THEN 
          LDBNAME (1) + ".":U ELSE "" 
          )
          + */ 
          ENTRY (1, cLine_qbi, " ":U).
    ...
    IF {&SortBy} ne "" THEN 
      {&4GLQury} =  {&4GLQury} + {&EOL} + "    ":U + TRIM ({&SortBy}).
 
    /* Add INDEXED-REPOSITIONED if In the list */
    IF LOOKUP("INDEXED-REPOSITION":U, {&OptionList}, " ":U) > 0 
