...
[
  ...
  &ENDIF
   /*Admin     */ "FCX---",
   /*Dump      */ "FCX---,F-----,FCX---,FCX---,FCX---,FCX---,FCX---,F-----,------,FCX---",
...
&IF "{&WINDOW-SYSTEM}" = "TTY" /*###*/
 &THEN
  &global-define AS400V7_IX     11 /* tools 12 */
  &global-define MSSQL_IX       13 /* tools 14 */  
  &global-define ODBC_IX        15 /* tools 16 */
  &global-define ORACLE_IX      17 /* tools 18 */
 &ELSE                         /*###*/                       
  &global-define AS400V7_IX     10  /* tools 11 */
  &global-define MSSQL_IX       12 /* tools 13 */
  &global-define ODBC_IX        14 /* tools 15 */
  &global-define ORACLE_IX      16 /* tools 17 */
&ENDIF
...
/*== Pop-up menus for DB2/400 Schema Migration */

Define sub-menu mnu_as4_tools
   menu-item mi_as4_as4inc    label "Load &DB2/400 Incremental .df..."
   MENU-ITEM mi_as4_loaddf    LABEL "Load &Data Definitions (.df file)..."
   .
...
Define sub-menu mnu_AS400V7
   ...
   RULE
   SUB-MENU mnu_as4_tools     LABEL "Schema &Migration Tools"
   .
   ...
   */
   ...
   do g_all_ix = 1 to NUM-ENTRIES(all_gates):
      ...
      if NOT CAN-DO(supp_gates, gate) then
      	 case gate:
      	    when "AS400" then DO:
               Gray_Table[{&AS400V7_IX}] = /* DB2/400 Menu */
                  "------,------,------,------,------,------,------". 
               Gray_Table[{&AS400V7_IX} + 1] = /* Migrate Menu */
                  "------,------".
            END.
            when "MSS" then do:
...
/*----- "Migration Tools" -----*/
on choose of menu-item mi_as4_as4inc in menu mnu_as4_tools
  run Perform_Func ("@AS400,33=as4inc,_as4_lod").
on choose of menu-item mi_as4_loaddf in menu mnu_as4_tools
  run Perform_Func ("@AS400,33=loaddf,_as4_lod").
/*-------------------------------DataServer/MSSQL--------------------------*/
