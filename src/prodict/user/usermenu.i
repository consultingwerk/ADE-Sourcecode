...
[
   ...
   /*Auditing Reports */ "FCXQR-A,FCXQR-A,FCXQR-A,FCXQR-A,------,FCXQR-A,FCXQR-A,FCXQR-A,FCXQR-A,------,FCXQR-A,FCXQR-A,FCXQR-A,------,FCXQR-A",
...
&IF "{&WINDOW-SYSTEM}" = "TTY" /*###*/
 &THEN
 &global-define MSSQL_IX       16 /* tools 17 */ 
 &global-define ODBC_IX        18 /* tools 19 */
 &global-define ORACLE_IX      20 /* tools 21 */
 &ELSE                         /*###*/                       
  &global-define MSSQL_IX       15 /* tools 16 */
  &global-define ODBC_IX        17 /* tools 18 */
  &global-define ORACLE_IX      19 /* tools 20 */
&ENDIF
...
Procedure Do_Menu_Item_Initial:
   ...
   IF NUM-DBS > 0 AND
      (INDEX(code,"Y") > 0 OR
       INDEX(code,"Z") > 0 OR
       INDEX(code,"A") > 0) THEN DO:
     CREATE BUFFER hBuffer FOR TABLE "DICTDB._file".
     
     IF INDEX(code,"Z") > 0 THEN DO:
       /* Check to see whether this menu item requires the _db-details 
     ...
     IF INDEX(code,"Y") > 0 THEN DO:
       /* Check to see whether this menu item requires any of the security
          tables to be in the database */
       hBuffer:FIND-FIRST("WHERE _file._file-name BEGINS ~'_sec~'",
                          NO-LOCK) NO-ERROR.
