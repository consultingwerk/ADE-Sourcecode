...
DO WHILE TRUE TRANSACTION:
  ...
  CLOSE STORED-PROC send-sql-statement h2 = PROC-STATUS WHERE PROC-HANDLE = h1. 
  ...
  REPEAT FOR DICTDB2.{1}
    ...
    IF RETRY THEN DO:
      ...
      IF (err% = 0) OR
         ...
         RUN STORED-PROC send-sql-statement h1 = PROC-HANDLE ("--Bulk-insert Abort").
         CLOSE STORED-PROC send-sql-statement h2 = PROC-STATUS WHERE PROC-HANDLE = h1.                	 
         UNDO TOP, LEAVE TOP.
      END.
      NEXT.
  ...
  */   
  RUN STORED-PROC send-sql-statement h1 = PROC-HANDLE ("--Bulk-insert End").
  CLOSE STORED-PROC send-sql-statement h2 = PROC-STATUS WHERE PROC-HANDLE = h1. 
