...
DEFINE {1} VARIABLE modf AS CHARACTER FORMAT "X(12)":U 
     ...
     LIST-ITEMS "NONE","CASCADE","SET NULL" 
     DROP-DOWN-LIST
...
PROCEDURE Fill_Par_Table_Index:
   ...
   FOR EACH DICTDB._file WHERE DICTDB._File._Db-Recid = DbRecid:
              FOR EACH DICTDB._index WHERE DICTDB._index._file-recid = RECID(DICTDB._file) NO-LOCK:
               ...
               (DICTDB._Constraint._Con-Type = "P" OR DICTDB._Constraint._Con-Type = "PC" 
                   OR DICTDB._Constraint._Con-Type = "MP" ) NO-LOCK NO-ERROR.
...
PROCEDURE Fill_Parent_Record:
  ...
  ELSE MESSAGE " No Primary constraint(s) have been defined, Foreign Key cannot be created " view-as alert-box warning.
