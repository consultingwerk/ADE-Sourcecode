...
ON HELP OF FRAME DEFAULT-FRAME OR CHOOSE OF HELP_BUT IN FRAME DEFAULT-FRAME
 DO:   
    RUN adecomm/_adehelp.p (INPUT "admn", INPUT "CONTEXT", 
                              INPUT {&MS_SQL_Server_DataServer_Delete_Constraints_Dialog_Box}, 
      	       	     	      INPUT ?).
 END.
&ENDIF
...
OUTPUT STREAM constlog TO VALUE(cnstrpt_name) UNBUFFERED APPEND NO-ECHO NO-MAP.
  ...
  IF  Constrainttype:SCREEN-VALUE IN FRAME DEFAULT-FRAME = "CLUSTERED"
  THEN DO :
    MESSAGE "Are you sure you want to delete all Clustered constraint(s) from database ?"
      VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO  update ans.
      IF ans = YES THEN
  ...
  IF  Constrainttype:SCREEN-VALUE IN FRAME DEFAULT-FRAME = "DEFAULT"
  THEN DO:
    MESSAGE "Are you sure you want to delete all Default constraint(s) from database ?"
      VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO  update ans.
      IF ans = YES THEN
