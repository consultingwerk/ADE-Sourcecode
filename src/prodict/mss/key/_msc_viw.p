...
Description:   
   This file contains the form for View Maintain Foreign Constraints in MSS.

HISTORY
Author: Uttam Shrivastava 

Date Created: 11/25/2011
----------------------------------------------------------------------------*/
{prodict/user/uservar.i }
{prodict/mss/key/viewmnt.i NEW}
...
ON CHOOSE OF btnProps IN FRAME frame_const 
   DO:
      IF NOT is_error THEN DO:
          ...
          ELSE RUN prodict/mss/key/modcons.p.
          RUN fill_constraint.
      END. 
   ...
   DO :
     ...
     FOR EACH _constraint WHERE _File-recid = TblRecId AND _Constraint._Db-Recid = DbRecId NO-LOCK:
        IF _constraint._Con-Status = "N" OR _constraint._Con-Status = "C" OR _constraint._Con-Status = "M" 
            THEN c_lst_Cnsts:ADD-LAST(_con-name) IN FRAME frame_const.
        END.	
     END.
