...
IF mss_pdbname = "" OR mss_pdbname = ? THEN DO:
    IF  LDBNAME("DICTDBG") <> ? THEN DO:
        ...
        IF DBTYPE("DICTDBG") <> "MSS" THEN DO:
            IF not batch_mode THEN DO:
              &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
                  PUT STREAM logfile UNFORMATTED "Database type for 
		  Logical Database" mss_pdbname "is not MSS." skip(1).
              &ELSE
                  MESSAGE "Database type for Logical Database " mss_pdbname " 
		  is not MSS." VIEW-AS ALERT-BOX ERROR.
              &ENDIF.
            END.      
            ELSE DO:
                 PUT STREAM logfile UNFORMATTED "Database type for Logical 
		 Database " mss_pdbname " is not MSS." skip(1).
                 UNDO, RETURN error.
            END.
        END.
	...
	IF mss_dbname <> "" AND mss_dbname <> ? THEN DO:
        ...
        IF NOT batch_mode THEN DO:
          &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
                MESSAGE msg_prefix + ": should not be set without
                       specifying a logical database name." SKIP(1).
          &ELSE
                MESSAGE msg_prefix + ": should not be set without
                     specifying a logical database name." VIEW-AS ALERT-BOX ERROR.
          &ENDIF.
        END.
        ELSE DO:
            PUT STREAM logfile UNFORMATTED msg_prefix + ": should not be set
            without specifying a logical database name." SKIP(1).
            UNDO, RETURN error.
        END.
    END.
    ELSE DO:
        IF NOT batch_mode THEN DO:
          &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
              MESSAGE "Logical database name must be specified if logical 
	      database is not the current working database." skip(1).
          &ELSE
              MESSAGE "Logical database name must be specified if logical database 
	      is not the current working database." VIEW-AS ALERT-BOX ERROR.
          &ENDIF.
        END. 
        ELSE DO: 
            PUT STREAM logfile UNFORMATTED "Logical database name must be 
	    specified if logical database is not the current working database." skip(1).
            UNDO, RETURN error.
        END.
    END.
END. /* END of unspecified logical database */
  ...
  IF osh_dbname = "" OR osh_dbname = ? THEN DO:
       IF LDBNAME("DICTDB") <> ? THEN DO:
          IF osh_conparms <> "" AND osh_conparms <> ? THEN DO:
              ...
              IF NOT batch_mode THEN DO:
                  &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
                     PUT STREAM logfile UNFORMATTED "Schema holder connection parameters 
		     should not be set without specifying a schema holder database name." skip(1).
                  &ELSE
                     MESSAGE "Schema holder connection parameters should not be set without 
		     specifying a schema holder database name." VIEW-AS ALERT-BOX ERROR.
                  &ENDIF.
              END.
              ELSE DO:
                  PUT STREAM logfile UNFORMATTED "Schema holder connection parameters should 
		  not be set without specifying a schema holder database name." skip(1).
                  UNDO, RETURN error.
              END.
          END. 
      END.
      ELSE DO:
          ...
          IF NOT batch_mode THEN DO:
              &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
                   PUT STREAM logfile UNFORMATTED "Schema holder database name 
		   must be specified if there is no current working database." skip(1).
                &ELSE
                   MESSAGE "Schema holder database name must be specified if there 
		   is no current working database." VIEW-AS ALERT-BOX ERROR.
              &ENDIF.
          END.
          ELSE DO:
               PUT STREAM logfile UNFORMATTED "Schema holder database name must be
	       specified if there is no current working database." skip(1).
               UNDO, RETURN error.
          END.
      END.
END.
...
/* At this point there is no connected schema holder nor can one be chosen from connected dbs */
     IF LDBNAME("DICTDB") = ? THEN DO:
         ...
         IF osh_dbname = "" OR osh_dbname = ? THEN DO:
             IF NOT batch_mode THEN DO:
               &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN      
                   MESSAGE "Schema holder database must be specified if no existing connections can be used for it"
               &ELSE
                   MESSAGE "Schema holder database must be specified if no existing connections can be used for it"
             ...
             ELSE 
                 PUT STREAM logfile UNFORMATTED 
                     "Schema holder database must be specified if no existing connections can be used for it"
                       skip(2).
             UNDO, RETURN error.
         END.
		...
		old-dictdb = osh_dbname + ".db".
	     ...
	     IF SEARCH(old-dictdb) = ? THEN DO:
	        CREATE DATABASE osh_dbname FROM "empty".
                PUT STREAM logfile UNFORMATTED "Empty schema holder database created " osh_dbname skip(1).
             END.
         END.
END.
...
IF osh_dbname = "" OR osh_dbname = ? THEN DO:
   ...
   IF batch_mode THEN 
       PUT STREAM logfile UNFORMATTED 
              "Schema holder Database Name is required."
               skip(2).
   ELSE DO:
       &IF "{&WINDOW-SYSTEM}" = "TTY" &THEN 
...
/* validity check on schema holder */
     IF DBTYPE(osh_dbname) <> "PROGRESS" THEN DO:
        IF "{&WINDOW-SYSTEM}" = "TTY" THEN 
         MESSAGE "Schema holder database " osh_dbname " should be PROGRESS database type, not " DBTYPE(osh_dbname)  
                     VIEW-AS ALERT-BOX ERROR.
      END.
     /* Value to post to the screen input if batch process is run but not in batch mode */
/*     IF osh_conparms = ? OR osh_conparms = "" THEN 
         ASSIGN osh_conparms = "<current working database>".   /* This is in case NOT batch_mode and value is displayed */
*/
/* Name conflict */       
