...
define variable field_position   as integer no-undo.
    ...
    */
    ...
    assign
      ...
      /* OE00210200: This code is specific to DB2 database, which has the special columns with 'GENERATED AS IDENTITY' property.
         ...
         un-updatable columns. And if the column if of type, 'GENERATED BY DEFAULT AS IDENTITY', append the 'rpos' value as a
         negative value, to diversify it from the 'GENERATED ALWAYS AS IDENTITY' columns. */
	  IF (INDEX(UPPER(_Db._Db-misc2[5]), "UDB") <> 0 OR INDEX(UPPER(_Db._Db-misc2[5]), "UDB DB") <> 0) THEN DO:
		  IF (s_ttb_fld.ds_msc24 EQ "ALWAYS") THEN DO:
			if s_ttb_tbl.ds_msc22 = ? then 
			   assign s_ttb_tbl.ds_msc22 = "".
			field_position = s_ttb_fld.ds_stoff.
			s_ttb_tbl.ds_msc22 = s_ttb_tbl.ds_msc22 + string(field_position) + ",".
		  end.
 		  ELSE IF (s_ttb_fld.ds_msc24 EQ "BY_DEFAULT") THEN DO:
			if s_ttb_tbl.ds_msc22 = ? then 
			   assign s_ttb_tbl.ds_msc22 = "".
			field_position = s_ttb_fld.ds_stoff * -1.
			s_ttb_tbl.ds_msc22 = s_ttb_tbl.ds_msc22 + string(field_position) + ",".
		  end.
	  END.
