...
/*************************************************************/
  ...
  History: 
    ...
    tmasood  08/19/2025    Replace ENCRYPT-AUDIT-MAC-KEY method with ENCODE-PASSWORD method                   
------------------------------------------------------------------------*/
...
ON GO OF FRAME {&FRAME-NAME} DO:
  ...
  DO:
      cMacKey = SECURITY-POLICY:ENCODE-PASSWORD(("_db-id-mnt_initial_"), "aedh0").
  ...
  IF glMacMod AND
    ...
    ELSE DO:
      IF phOldBuff:AVAILABLE THEN DO:
          ...
          IF NOT SECURITY-POLICY:VALIDATE-PASSWORD(fiMacKey:SCREEN-VALUE, cOldMacKey) THEN
            cMacKey = SECURITY-POLICY:ENCODE-PASSWORD(fiMacKey:SCREEN-VALUE, "aedh0").
