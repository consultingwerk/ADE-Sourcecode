...
------------------------------------------------------------------------------*/ 
  phFillIn:SCREEN-VALUE = IF phFillIn:DATA-TYPE NE "LOGICAL":U 
                         THEN "":U 
                         ELSE "no":U.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME X ch da de i lo WITH STREAM-IO.
...
------------------------------------------------------------------------------*/
  ASSIGN ghProp = WIDGET-HANDLE(ENTRY(1, TARGET-PROCEDURE:ADM-DATA, CHR(1)))
         ghProp = ghProp:BUFFER-FIELD('FilterTarget':U).
...
------------------------------------------------------------------------------*/
  ...
  DO:    
    phField:FORMAT = cFormat.
    IF phField:DATA-TYPE <> "DECIMAL" THEN
    DO:       
