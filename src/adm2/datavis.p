...
------------------------------------------------------------------------------*/
  ...
  /* Used to check field security on editors */
  ...
  DO iCnt = 1 TO NUM-ENTRIES(cFieldHandles):
    ...
    then-blk:
    DO:
      /* If this editor has been disabled by field security, its SCREEN-VALUE won't have been set.     *
       * If this is the case, we don't want to set its MODIFIED property to YES, because we don't want *
       * to assign a blank SCREEN-VALUE back to the database.                                          */
      IF cAllFieldHandles = "":U THEN 
      DO:
          /* cFieldSecurity maps to cAllFieldHandles */
          &SCOPED-DEFINE xp-assign
          {get allFieldHandles cAllFieldHandles}
          {get fieldSecurity cFieldSecurity}.
          &UNDEFINE xp-assign
      END.
      ASSIGN iFieldPos = LOOKUP(STRING(hFrameField),cAllFieldHandles) NO-ERROR.

      IF  iFieldPos <> 0
      AND iFieldPos <> ?
      AND NUM-ENTRIES(cFieldSecurity) >= iFieldPos
      AND ENTRY(iFieldPos,cFieldSecurity) = "HIDDEN":U THEN
          LEAVE then-blk.

      /* This field has not been cleared by field security. 
