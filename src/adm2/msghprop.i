...
&IF DEFINED(ADM-EXCLUDE-STATIC) = 0 &THEN
  ...
  {src/adm2/custom/msghandlerdefscustom.i}
&ENDIF
...
&ENDIF
  ...
  &GLOBAL-DEFINE xpDestination
  &GLOBAL-DEFINE xpInMessageSource
  &GLOBAL-DEFINE xpOutMessageTarget
  &GLOBAL-DEFINE xpReplyRequired
  &GLOBAL-DEFINE xpReplySelector
  &GLOBAL-DEFINE xpCurrentMessageId
  {src/adm2/smrtprop.i}
...
&IF "{&ADMSuper}":U = "":U &THEN
  ...
  ghADMProps:ADD-NEW-FIELD('Destination':U, 'CHARACTER':U, 0, ?, '':U).
  ghADMProps:ADD-NEW-FIELD('InMessageSource':U, 'HANDLE':U).
  ghADMProps:ADD-NEW-FIELD('OutMessageTarget':U, 'HANDLE':U).
  ghADMProps:ADD-NEW-FIELD('ReplyRequired':U, 'LOGICAL':U, 0, ?, NO).
  ghADMProps:ADD-NEW-FIELD('ReplySelector':U, 'CHARACTER':U, 0, ?, '':U).
  ghADMProps:ADD-NEW-FIELD('CurrentMessageId':U, 'CHARACTER':U, 0, ?, ?).
&ENDIF
