...
/*------------------------------------------------------------------------
  ...
  Description: List all the SmartObject properties.
...
/* Define a temp table to hold the properties and their values. */
...
{ src/adm2/support/admhlp.i } /* help definitions */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_attr f _FREEFORM
  QUERY br_attr NO-LOCK DISPLAY
      tt.attr-name  LABEL "Property" WIDTH 29
...
DEFINE FRAME f
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "SmartObject Properties":L
...
ASSIGN admVersion = IF ERROR-STATUS:ERROR OR cValue = ? THEN "ADM1":U
                                                        ELSE "ADM2":U.
...
/* -----------------------------------------------------------
  Purpose:    Populate a list with the session attributes
              ...
              be called for SmartObjects version > ADM1
  Parameters:  <none>
  Notes:       
-------------------------------------------------------------*/
  ...
  /* Get the special attributes in the SmartObjects internal storage. 
     ...
     Note that delimiter = CHR(4) (replaces '=' )
                           CHR(3) (replaces carriage return)
   */

  attr-list = DYNAMIC-FUNCTION("InstancePropertyList":U IN p_hSMO, "*":U) NO-ERROR.
  IF ERROR-STATUS:ERROR eq NO THEN DO:
    ASSIGN iCnt = NUM-ENTRIES (attr-list,CHR(3)).
    DO i = 1 TO iCnt:
      /* Each line in the attribute list is of the form "attr<CHR(4)>value". Parse
         this line. Note that there can be values that are QUOTED (if they
         contain a comma. So any line without a CHR(4) can be appended to the
         last tt record.*/
      ASSIGN cLine = ENTRY(i, attr-list,CHR(3))
             iPos  = INDEX(cLine, CHR(4)).
      IF iPos > 1 THEN DO:
        CREATE tt.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get_attributes_adm0 f 
PROCEDURE get_attributes_adm0 :
/* -----------------------------------------------------------
  Purpose:    Populate a list with the session attributes
              in the form "Attribute = value". This should only
...
-------------------------------------------------------------*/
  ...
  IF ERROR-STATUS:ERROR eq NO THEN DO:
    ...
    END.
  END.
END PROCEDURE. /* get_attributes_adm0 */
