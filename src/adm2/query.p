...
------------------------------------------------------------------------------*/
  ...
  /* This will get the list of Updatable Columns with {&adm-tabledelimiter} delimiters between
...
------------------------------------------------------------------------------*/
 ...
 /* If the same row is being resent because of an update, we don't want to react 
  ...
  DO:
    ...
    IF lOneToOne AND cASDivision <> "CLIENT":U THEN
    DO:
      &SCOPED-DEFINE xp-assign
...
------------------------------------------------------------------------------*/
  ...
  {get UpdateFromSource lOneToOne}.
  &UNDEFINE xp-assign
  IF cAsDivision = '':U AND NOT lOneToOne THEN
  DO:
...
/*------------------------------------------------------------------------------
  ...
  **/
  ...
  /* retrieve entitydetails on the first call on server */ 
  ...
  DO:
    ...
    DO:
      ...
      END.
    END.
    /* We currently call DataAvailable to assign ForeignFields also 
       in the case where fetch is pending  */
...
------------------------------------------------------------------------------*/  
  ...
  DO iTable = 1 TO NUM-ENTRIES(cBuffers):   
    ...
    DO:         
      ...
      IF NOT lFound THEN 
        RETURN ERROR 
          (IF hBuffer:LOCKED THEN {fnarg messageNumber 18} ELSE RETURN-VALUE).
  ...
  DO iColumn= 1 TO NUM-ENTRIES(cSortEntries):
    ...
    DO:
      ASSIGN
        ...
        lOldDescending = INDEX(cOldSortEntry + ' ':U,' DESCENDING ':U) > 0.
...
------------------------------------------------------------------------------*/
  ...
  IF {fn getUpdateFromSource} AND {fn getASDivision} <> "CLIENT":U THEN 
  DO:
    &SCOPED-DEFINE xp-assign
    {set QueryString '':U}
    {set FirstRowNum ?}
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ...
    cExpression = REPLACE(TRIM(SUBSTR(pcQueryString,iByPos)," .":U),
                          ' INDEXED-REPOSITION':U,
                          '':U).
    /* Remove extra blanks (this may be used to comapre new and old 
