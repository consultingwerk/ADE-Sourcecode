...
IF NOT lFieldIsSecured AND lDisplayField THEN DO:
  ...
  IF iEntry > 0 AND
    ...
    ASSIGN hCombo:LIST-ITEM-PAIRS = hCombo:LIST-ITEM-PAIRS
           hCombo:SCREEN-VALUE = "":U.
...
------------------------------------------------------------------------------*/
  ...
  /* Now we need to see if we can set the combo back to the original
     set value */
  /* We only want to do this if called from somewhere other than VALUE-CHANGED */
  IF INDEX(PROGRAM-NAME(2),"valueChanged":U) = 0 AND 
     INDEX(PROGRAM-NAME(2),"combo.p":U) = 0 THEN DO:
    FIND FIRST ttDCombo
         WHERE ttDCombo.hWidget = TARGET-PROCEDURE
         NO-LOCK NO-ERROR.
    IF AVAILABLE ttDCombo THEN DO:
