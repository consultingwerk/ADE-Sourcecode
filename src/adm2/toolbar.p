...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE hContainer               AS HANDLE     NO-UNDO.
  DEFINE VARIABLE lObjectHasMenus          AS LOGICAL    NO-UNDO INITIAL YES.
  ...
  DO:
    ASSIGN
       cToolbar = {fn getObjectName}.
    {set LogicalObjectName cToolbar}.
    IF cToolbar = ? THEN ASSIGN cToolbar = "":U.
  END.

  /* If this toolbar is in the cache, don't extract it */
  IF CAN-FIND(FIRST ttToolbarBand
              WHERE ttToolbarBand.Toolbar = cToolbar) THEN
    ASSIGN cToolbar = "":U.

  /* Check if we should extract object bands */
  &SCOPED-DEFINE xp-assign
  {get UIBmode cUIBmode}.
  &UNDEFINE xp-assign
  ...
  DO:
    ...
    &SCOPED-DEFINE xp-assign
    {get hasObjectMenu lObjectHasMenus hContainer}.
    &UNDEFINE xp-assign
  END.
  ELSE
    ASSIGN lObjectHasMenus = YES.

  /* Work out which object bands to extract from the repository */
  IF lObjectHasMenus <> NO THEN /* could be ? */
  DO:
    ASSIGN cObjectList = {fnarg supportedObjects NO}. /* no=Only return unloaded bands */
    IF cObjectList = ? THEN ASSIGN cObjectList = "":U.

    /* Remove any objects already cached from the list */
    DO iCnt = 1 TO NUM-ENTRIES(cObjectList):
      IF CAN-FIND(FIRST ttObjectBand
                  WHERE ttObjectBand.OBJECT = ENTRY(iCnt, cObjectList)) THEN
        ASSIGN ENTRY(iCnt, cObjectList) = "":U.
    END.
    ASSIGN cObjectList = TRIM(cObjectList, ",":U).
  END.

  /* If all the requested toolbars and objects have been cached, don't extract again */
  IF  cToolbar = "":U
  AND cObjectList = "":U THEN.
  ELSE DO:
      /* Extract the toolbar and object bands */
      RUN loadToolbarBands IN TARGET-PROCEDURE (cToolbar,
                                                cObjectList).
      ...
      FOR EACH ttBand 
         WHERE ttBand.ProcedureHandle = ?:
          ...
          ASSIGN lDelete = CAN-FIND(FIRST bttBand 
  ...
  /* If we're in design mode, update category lists */
  IF cUIBmode = 'Design':U THEN
  DO:
      /* We could have cleared the toolbar name because it's already been cached. */
      IF cToolbar = "":U THEN
        {get LogicalObjectName cToolbar}.
...
IF pcToolbarList = ? THEN ASSIGN pcToolbarList = "":U.
IF pcObjectList  = ? THEN ASSIGN pcObjectList = "":U.
...
THEN DO:
    ...
    DO iCnt = 1 TO NUM-ENTRIES(pcObjectList):
        IF CAN-FIND(FIRST ttObjectBand
                    WHERE ttObjectBand.OBJECT = ENTRY(iCnt, pcObjectList)) THEN
            ASSIGN ENTRY(iCnt, pcObjectList) = "":U.
    END.
    ASSIGN pcObjectList = TRIM(pcObjectList, ",":U).
END.

/* If all the requested toolbars and objects have been cached, return */
IF  pcToolbarList = "":U
AND pcObjectList = "":U THEN
  RETURN.
...
-----------------------------------------------------------------------------*/
  ...
  FOR EACH btMenu WHERE btMenu.MenubarHdl    = hMenuBar
    ...
    DO:
      ...
      IF lNoAccelerator AND NOT lcontainerLink AND btMenu.Hdl:ACCELERATOR <> '' THEN
