...
/*---------------------------------------------------------------------------------
  ...
  Description:  rym_wizard_tree Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the rym_wizard_tree table
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST rym_wizard_tree 
              WHERE rym_wizard_tree.object_name = b_rym_wizard_tree.object_name) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_wizard_tree.object_name)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'rym_wizard_tree' '' "'object_name, '" cValueList }.
  END.


  IF CAN-FIND(FIRST rym_wizard_tree 
              WHERE rym_wizard_tree.product_code = b_rym_wizard_tree.product_code
                AND rym_wizard_tree.product_module_code = b_rym_wizard_tree.product_module_code
                AND rym_wizard_tree.object_name = b_rym_wizard_tree.object_name) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_wizard_tree.product_code) + ', ' + STRING(b_rym_wizard_tree.product_module_code) + ', ' + STRING(b_rym_wizard_tree.object_name)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'rym_wizard_tree' '' "'product_code, product_module_code, object_name, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "Astra 2 rym_wizard_tree Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF LENGTH(b_rym_wizard_tree.product_code) = 0 OR LENGTH(b_rym_wizard_tree.product_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_tree' 'product_code' "'Product Code'"}.

  IF LENGTH(b_rym_wizard_tree.product_module_code) = 0 OR LENGTH(b_rym_wizard_tree.product_module_code) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_tree' 'product_module_code' "'Product Module Code'"}.

  IF LENGTH(b_rym_wizard_tree.object_name) = 0 OR LENGTH(b_rym_wizard_tree.object_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_tree' 'object_name' "'Object Name'"}.
  
  IF LENGTH(b_rym_wizard_tree.object_description) = 0 OR LENGTH(b_rym_wizard_tree.object_description) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_tree' 'object_description' "'Object Description'"}.

  IF LENGTH(b_rym_wizard_tree.root_node_code) = 0 OR LENGTH(b_rym_wizard_tree.root_node_code) = ? THEN
      ASSIGN
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_tree' 'root_node_code' "'Root Node Code'"}.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST rym_wizard_tree 
              WHERE rym_wizard_tree.object_name = b_rym_wizard_tree.object_name
                AND ROWID(rym_wizard_tree) <> TO-ROWID(ENTRY(1,b_rym_wizard_tree.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_wizard_tree.object_name)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST rym_wizard_tree 
              WHERE rym_wizard_tree.product_code = b_rym_wizard_tree.product_code
                AND rym_wizard_tree.product_module_code = b_rym_wizard_tree.product_module_code
                AND rym_wizard_tree.object_name = b_rym_wizard_tree.object_name
                AND ROWID(rym_wizard_tree) <> TO-ROWID(ENTRY(1,b_rym_wizard_tree.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rym_wizard_tree.product_code) + ', ' + STRING(b_rym_wizard_tree.product_module_code) + ', ' + STRING(b_rym_wizard_tree.object_name)
