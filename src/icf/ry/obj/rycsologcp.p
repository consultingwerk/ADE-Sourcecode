...
/*---------------------------------------------------------------------------------
  ...
  Description:  ryc_smartobject Data Logicocedure Library Template
 
  Purpose:      A procedure library (PLIP) to support the maintenance of the ryc_smartobject
...
&GLOB DATA-FIELD-DEFS  "ry/obj/rycsofullo.i"
...
------------------------------------------------------------------------------*/
  ...
  DO:
     ASSIGN cValueList   = STRING(b_ryc_smartobject.object_filename) + ', ' + STRING(b_ryc_smartobject.customization_result_obj)
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST ryc_smartobject 
              WHERE ryc_smartobject.product_module_obj = b_ryc_smartobject.product_module_obj
                ...
                AND ROWID(ryc_smartobject) <> TO-ROWID(ENTRY(1,b_ryc_smartobject.RowIDent))) THEN
  DO:
     ASSIGN cValueList   = STRING(b_ryc_smartobject.product_module_obj) + ', ' + STRING(b_ryc_smartobject.object_filename) + ', ' + STRING(b_ryc_smartobject.customization_result_obj)
            cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U)  
...
------------------------------------------------------------------------------*/
  ...
  IF b_ryc_smartobject.customization_result_obj = 0 OR b_ryc_smartobject.customization_result_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'ryc_smartobject' 'customization_result_obj' "'Customization Result Obj'"}.
  ...
  IF b_ryc_smartobject.runnable_from_menu = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
  ...
  IF b_ryc_smartobject.sdo_smartobject_obj = 0 OR b_ryc_smartobject.sdo_smartobject_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'ryc_smartobject' 'sdo_smartobject_obj' "'Sdo SmartObject Obj'"}.
  ...
  IF b_ryc_smartobject.custom_smartobject_obj = 0 OR b_ryc_smartobject.custom_smartobject_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'ryc_smartobject' 'custom_smartobject_obj' "'Custom SmartObject Obj'"}.
 
  IF b_ryc_smartobject.extends_smartobject_obj = 0 OR b_ryc_smartobject.extends_smartobject_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'ryc_smartobject' 'extends_smartobject_obj' "'Extends SmartObject Obj'"}.
 
  IF b_ryc_smartobject.security_smartobject_obj = 0 OR b_ryc_smartobject.security_smartobject_obj = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {aferrortxt.i 'AF' '1' 'ryc_smartobject' 'security_smartobject_obj' "'Security SmartObject Obj'"}.
