    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             INPUT  'DisplayedFieldryc_smartobject.object_filenameKeyFieldryc_smartobject.smartobject_objFieldLabelContainerFieldTooltipPress F4 for LookupKeyFormat->>>>>>>>>>>>>>>>>9.999999999KeyDatatypedecimalDisplayFormatX(70)DisplayDatatypecharacterBaseQueryStringFOR EACH gsc_object_type NO-LOCK
                     ...
                     AND [EXCLUDE_REPOSITORY_PRODUCT_MODULES],
       ...
       RUN constructObject (
             ...
             OUTPUT hResultCode ).
       RUN repositionObject IN hResultCode ( 2.05 , 20.00 ) NO-ERROR.
       ...
       RUN constructObject (
             ...
             INPUT  'DisplayedFieldgsc_product_module.product_module_description,gsc_product_module.product_module_codeKeyFieldgsc_product_module.product_module_objFieldLabelProduct ModuleFieldTooltipSelect option from listKeyFormat->>>>>>>>>>>>>>>>>9.999999999KeyDatatypedecimalDisplayFormatX(256)DisplayDatatypeCHARACTERBaseQueryStringFOR EACH gsc_product_module NO-LOCK
                     WHERE [EXCLUDE_REPOSITORY_PRODUCT_MODULES]
                     BY gsc_product_module.product_module_codeQueryTablesgsc_product_moduleSDFFileName<Local4>SDFTemplateParentFieldParentFilterQueryDescSubstitute&2 / &1ComboDelimiterListItemPairsInnerLines5ComboFlagFlagValueBuildSequence1SecurednoCustomSuperProcPhysicalTableNamesTempTablesQueryBuilderJoinCodeQueryBuilderOptionListQueryBuilderOrderListQueryBuilderTableOptionListQueryBuilderTuneOptionsQueryBuilderWhereClausesFieldName<Local4>DisplayFieldyesEnableFieldyesLocalFieldyesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT hProductModule ).
       RUN repositionObject IN hProductModule ( 4.14 , 95.80 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    RUN adjustTabOrder (hCustomSuperProcedure,    fiObjectDescription:HANDLE, 'BEFORE':U).
    RUN adjustTabOrder (hObjectType,              hCustomSuperProcedure,      'BEFORE':U).
    RUN adjustTabOrder (hResultCode,              hObjectType,                'BEFORE':U).
    RUN adjustTabOrder (hObjectFilename,          hResultCode,                'BEFORE':U).
    RUN adjustTabOrder (hContainerTemplate,       fiObjectDescription:HANDLE, 'AFTER':U).
    RUN adjustTabOrder (hProductModule,           buCreate:HANDLE,            'AFTER':U).
  END.
...
------------------------------------------------------------------------------*/
  ...
  CASE phObject:
    ...
    DO:
      ghSmartObject:BUFFER-FIELD("d_custom_smartobject_obj":U):BUFFER-VALUE = DECIMAL(pcKeyFieldValue).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
        ...
        toTemplateObject:COLUMN          = dNewColumn + 0.75
      /*coDefaultMode:WIDTH-CHARS        = dNewWidth - 4.80*/
        rctContainerTemplate:COLUMN      = dNewColumn - 15.50 /*20.00*/
        rctContainerTemplate:WIDTH-CHARS = dNewWidth  + 15.50 /*20.00*/
    ...
    RUN repositionObject  IN hContainerTemplate    (1.71, dNewColumn).
    ...
    RUN repositionObject  IN hProductModule        (4.14, dNewColumn).
