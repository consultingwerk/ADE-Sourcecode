...
DEFINE VARIABLE ghttPageObject      AS HANDLE     NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CREATE BUFFER ghttPageObject     FOR TABLE ghttPageObject.
  CREATE BUFFER ghttObjectType     FOR TABLE ghttObjectType.
  CREATE QUERY  ghQuery.

  ghQuery:SET-BUFFERS(ghttObjectInstance, ghttPageObject, ghttObjectType).
  ...
  DO WITH FRAME {&FRAME-NAME}:
    gcBaseQuery    = "FOR EACH ttObjectInstance":U
                   ...
                   + "   WHERE ttPageObject.d_customization_result_obj = '&1'":U
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      ghttPageObject     = WIDGET-HANDLE(DYNAMIC-FUNCTION("getUserProperty":U IN ghParentContainer, "ttPageObject":U))
...
------------------------------------------------------------------------------*/
  ...
  ghContainerHandle:TITLE = gcTitle + " - ":U + TRIM(ENTRY(2, hContainerHandle:TITLE, "-":U)).
...
------------------------------------------------------------------------------*/
  ...
  iSequenceNumber = ghttPageObject:BUFFER-FIELD("i_page_object_sequence":U):BUFFER-VALUE.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    DO:
      lSuccess = ghQuery:REPOSITION-TO-ROWID(rRowID) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    httPageObject:FIND-FIRST(" WHERE d_container_smartobject_obj = ":U + QUOTER(httObjectInstance:BUFFER-FIELD("d_container_smartobject_obj":U):BUFFER-VALUE)
                             + " AND d_object_instance_obj       = ":U + QUOTER(httObjectInstance:BUFFER-FIELD("d_object_instance_obj":U):BUFFER-VALUE)
                             + " AND c_action                   <> 'D'":U).
