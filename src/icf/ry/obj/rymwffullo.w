...
/*---------------------------------------------------------------------------------
  ...
  Description:  Wizard Folder Window SDO

  Purpose:      Wizard Folder Window SDO
  ...
  (v:010000)    Task:        6199   UserRef:    
                Date:   03/07/2000  Author:     Anthony Swindells
...
&Scoped-define INTERNAL-TABLES rym_wizard_fold

/* Definitions for QUERY Query-Main                                     */
&Scoped-Define ENABLED-FIELDS  product_code product_module_code object_name object_description~
 window_title default_mode no_sdo page_layout viewer_link_name
&Scoped-define ENABLED-FIELDS-IN-rym_wizard_fold product_code ~
product_module_code object_name object_description window_title ~
default_mode no_sdo page_layout viewer_link_name 
&Scoped-Define DATA-FIELDS  product_code product_module_code object_name object_description~
 window_title default_mode no_sdo page_layout viewer_link_name~
 generated_date generated_time generated_time_str wizard_fold_obj
&Scoped-define DATA-FIELDS-IN-rym_wizard_fold product_code ~
product_module_code object_name object_description window_title ~
default_mode no_sdo page_layout viewer_link_name generated_date ~
generated_time wizard_fold_obj 
&Scoped-Define MANDATORY-FIELDS 
&Scoped-Define APPLICATION-SERVICE 
&Scoped-Define ASSIGN-LIST 
&Scoped-Define DATA-FIELD-DEFS "ry/obj/rymwffullo.i"
{&DB-REQUIRED-START}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH rym_wizard_fold NO-LOCK ~
    BY rym_wizard_fold.product_code ~
       BY rym_wizard_fold.product_module_code ~
        BY rym_wizard_fold.object_name INDEXED-REPOSITION.
{&DB-REQUIRED-END}
&Scoped-define TABLES-IN-QUERY-Query-Main rym_wizard_fold
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main rym_wizard_fold
...
/* Query rebuild information for SmartDataObject Query-Main
     _TblList          = "RYDB.rym_wizard_fold"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _OrdList          = "RYDB.rym_wizard_fold.product_code|yes,RYDB.rym_wizard_fold.product_module_code|yes,RYDB.rym_wizard_fold.object_name|yes"
...
"object_description" "object_description" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[5]   > RYDB.rym_wizard_fold.window_title
"window_title" "window_title" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[6]   > RYDB.rym_wizard_fold.default_mode
"default_mode" "default_mode" ? ? "character" ? ? ? ? ? ? yes ? no 28 yes
     _FldNameList[7]   > RYDB.rym_wizard_fold.no_sdo
"no_sdo" "no_sdo" "No Data Object" ? "logical" ? ? ? ? ? ? yes ? no 7.2 yes
     _FldNameList[8]   > RYDB.rym_wizard_fold.page_layout
"page_layout" "page_layout" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[9]   > RYDB.rym_wizard_fold.viewer_link_name
"viewer_link_name" "viewer_link_name" ? ? "character" ? ? ? ? ? ? yes ? no 28 yes
     _FldNameList[10]   > RYDB.rym_wizard_fold.generated_date
"generated_date" "generated_date" ? ? "date" ? ? ? ? ? ? no ? no 15.2 yes
     _FldNameList[11]   > RYDB.rym_wizard_fold.generated_time
...
"STRING (RowObject.generated_time,""HH:MM:SS"")" "generated_time_str" "Time" "x(8)" "character" ? ? ? ? ? ? no ? no 8 no
     _FldNameList[13]   > RYDB.rym_wizard_fold.wizard_fold_obj
"wizard_fold_obj" "wizard_fold_obj" ? ? "decimal" ? ? ? ? ? ? no ? no 21.6 yes
...
FOR EACH RowObjUpd WHERE LOOKUP(RowObjUpd.RowMod,"A,C,U":U) <> 0:
  ...
  IF (RowObjUpd.RowMod = "U":U AND
      CAN-FIND(FIRST rym_wizard_fold
               WHERE rym_wizard_fold.object_name = RowObjUpd.object_name
                 AND ROWID(rym_wizard_fold) <> TO-ROWID(RowObjUpd.ROWIDent))) OR 
     (RowObjUpd.RowMod <> "U":U AND
      CAN-FIND(FIRST rym_wizard_fold
               WHERE rym_wizard_fold.object_name = RowObjUpd.object_name))  THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '8' 'rym_wizard_fold' 'object_name' "'object name'" RowObjUpd.object_name "'. Please use a different object name'"}
           .
  ...
  DO:
    ...
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_fold' 'product_code' "'product code'" cText}
           .
  ...
  DO:
    ...
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_fold' 'product_module_code' "'product module code'" cText}
           .
  ...
  DO:
    ...
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_fold' 'PAGE_layout' "'page layout'" cText}
           .
  END.
...
IF LENGTH(RowObject.product_code) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_fold' 'product_code' "'product code'"}
                        .
IF LENGTH(RowObject.product_module_code) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_fold' 'product_module_code' "'product module code'"}
                        .
IF LENGTH(RowObject.OBJECT_name) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_fold' 'object_name' "'object name'"}
                        .
IF INDEX(RowObject.OBJECT_name,".":U) > 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_fold' 'object_name' "'object name'" "'This is a logical object and so should not contain a file extension.'"}
                        .
IF LENGTH(RowObject.OBJECT_description) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_fold' 'object_description' "'object description'"}
                        .
IF LENGTH(RowObject.WINDOW_title) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_fold' 'window_title' "'window title'"}
                        .
