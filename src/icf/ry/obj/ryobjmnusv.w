...
&Scoped-Define ENABLED-OBJECTS toSubMenu buAdd fiMenuStructureTitle 
...
DEFINE BUTTON buAdd 
     IMAGE-UP FILE "src/adeicon/new.bmp":U NO-FOCUS FLAT-BUTTON
     LABEL "Add" 
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  {get ToolbarSource ghToolbar ghContainerSource}.
...
------------------------------------------------------------------------------*/
  ...
  ghContainerHandle:TITLE = gcTitle + TRIM(ENTRY(2, ghParentCHandle:TITLE, "-":U)).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
        ...
        fiLabel:SIDE-LABEL-HANDLE:ROW       = pdHeight - 4.33.
    ...
    IF VALID-HANDLE(ghBrowse) THEN
      ASSIGN
          ...
          ghBrowse:HEIGHT-CHARS = pdHeight - ghBrowse:ROW    - 7.15.
    ...
    ASSIGN
        ...
        rctSeperator2:X = buTransfer:X - 2.
  END.
