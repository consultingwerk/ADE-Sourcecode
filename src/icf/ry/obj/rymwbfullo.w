...
/*---------------------------------------------------------------------------------
  ...
  Description:  Wizard Browser SDO

  Purpose:      Wizard Browser SDO
...
&scop object-name       rymwbfullo.w
DEFINE VARIABLE lv_this_object_name AS CHARACTER INITIAL "{&object-name}":U NO-UNDO.
&scop object-version    010103
...
&Scoped-define INTERNAL-TABLES rym_wizard_brow

/* Definitions for QUERY Query-Main                                     */
&Scoped-Define ENABLED-FIELDS  product_code product_module_code object_name object_description~
 launch_container custom_super_procedure window_title_field sdo_name~
 selected_fields sdo_filename sdo_foreign_fields
&Scoped-define ENABLED-FIELDS-IN-rym_wizard_brow product_code ~
product_module_code object_name object_description launch_container ~
custom_super_procedure window_title_field sdo_name selected_fields ~
sdo_foreign_fields 
&Scoped-Define DATA-FIELDS  product_code product_module_code object_name object_description~
 launch_container custom_super_procedure window_title_field sdo_name~
 selected_fields generated_date generated_time generated_time_str~
 sdo_filename sdo_foreign_fields wizard_brow_obj
&Scoped-define DATA-FIELDS-IN-rym_wizard_brow product_code ~
product_module_code object_name object_description launch_container ~
custom_super_procedure window_title_field sdo_name selected_fields ~
generated_date generated_time sdo_foreign_fields wizard_brow_obj 
&Scoped-Define MANDATORY-FIELDS 
&Scoped-Define APPLICATION-SERVICE 
&Scoped-Define ASSIGN-LIST 
&Scoped-Define DATA-FIELD-DEFS "ry/obj/rymwbfullo.i"
{&DB-REQUIRED-START}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH rym_wizard_brow NO-LOCK ~
    BY rym_wizard_brow.product_code ~
       BY rym_wizard_brow.product_module_code ~
        BY rym_wizard_brow.object_name INDEXED-REPOSITION.
{&DB-REQUIRED-END}
&Scoped-define TABLES-IN-QUERY-Query-Main rym_wizard_brow
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main rym_wizard_brow
...
/* Query rebuild information for SmartDataObject Query-Main
     _TblList          = "RYDB.rym_wizard_brow"
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _OrdList          = "RYDB.rym_wizard_brow.product_code|yes,RYDB.rym_wizard_brow.product_module_code|yes,RYDB.rym_wizard_brow.object_name|yes"
     _FldNameList[1]   > RYDB.rym_wizard_brow.product_code
"product_code" "product_code" ? ? "character" ? ? ? ? ? ? yes ? no 13 yes
     _FldNameList[2]   > RYDB.rym_wizard_brow.product_module_code
"product_module_code" "product_module_code" ? ? "character" ? ? ? ? ? ? yes ? no 20.6 yes
     _FldNameList[3]   > RYDB.rym_wizard_brow.object_name
"object_name" "object_name" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[4]   > RYDB.rym_wizard_brow.object_description
"object_description" "object_description" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[5]   > RYDB.rym_wizard_brow.launch_container
"launch_container" "launch_container" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[6]   > RYDB.rym_wizard_brow.custom_super_procedure
"custom_super_procedure" "custom_super_procedure" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[7]   > RYDB.rym_wizard_brow.window_title_field
"window_title_field" "window_title_field" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[8]   > RYDB.rym_wizard_brow.sdo_name
"sdo_name" "sdo_name" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[9]   > RYDB.rym_wizard_brow.selected_fields
"selected_fields" "selected_fields" ? ? "character" ? ? ? ? ? ? yes ? no 70 yes
     _FldNameList[10]   > RYDB.rym_wizard_brow.generated_date
"generated_date" "generated_date" ? ? "date" ? ? ? ? ? ? no ? no 15.2 yes
     _FldNameList[11]   > RYDB.rym_wizard_brow.generated_time
...
"""""" "sdo_filename" ? "x(1)" "character" ? ? ? ? ? ? yes ? no 1 no
     _FldNameList[14]   > RYDB.rym_wizard_brow.sdo_foreign_fields
"sdo_foreign_fields" "sdo_foreign_fields" ? ? "character" ? ? ? ? ? ? yes ? no 70 yes
     _FldNameList[15]   > RYDB.rym_wizard_brow.wizard_brow_obj
"wizard_brow_obj" "wizard_brow_obj" ? ? "decimal" ? ? ? ? ? ? no ? no 21.6 yes
...
FOR EACH RowObjUpd WHERE LOOKUP(RowObjUpd.RowMod,"A,C,U":U) <> 0:
  ...
  IF (RowObjUpd.RowMod = "U":U AND
      CAN-FIND(FIRST rym_wizard_brow
               WHERE rym_wizard_brow.OBJECT_name = RowObjUpd.object_name
                 AND ROWID(rym_wizard_brow) <> TO-ROWID(RowObjUpd.ROWIDent))) OR 
     (RowObjUpd.RowMod <> "U":U AND
      CAN-FIND(FIRST rym_wizard_brow
               WHERE rym_wizard_brow.object_name = RowObjUpd.object_name))  THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '8' 'rym_wizard_brow' 'object_name' "'object name'" RowObjUpd.object_name "'. Please use a different object name'"}
           .
  ...
  DO:
    ...
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_brow' 'product_code' "'product code'" cText}
           .
  ...
  DO:
    ...
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_brow' 'product_module_code' "'product module code'" cText}
           .
...
IF LENGTH(RowObject.product_code) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_brow' 'product_code' "'product code'"}
                        .
IF LENGTH(RowObject.product_module_code) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_brow' 'product_module_code' "'product module code'"}
                        .
IF LENGTH(RowObject.OBJECT_name) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_brow' 'object_name' "'object name'"}
                        .
IF INDEX(RowObject.OBJECT_name,".":U) > 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_brow' 'object_name' "'object name'" "'This is a logical object and so should not contain a file extension.'"}
                        .
IF LENGTH(RowObject.OBJECT_description) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_brow' 'object_description' "'object description'"}
                        .
IF  LENGTH(RowObject.sdo_name) = 0 
OR  LENGTH(RowObject.sdo_name) = ? THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                           {af/sup2/aferrortxt.i 'AF' '1' 'rym_wizard_brow' 'sdo_name' "'sdo name'"}
                           .

IF LENGTH(RowObject.sdo_foreign_fields) > 0 AND
   NUM-ENTRIES(RowObject.sdo_foreign_fields) MOD 2 <> 0 THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_brow' 'sdo_foreign_fields' "'sdo foreign fields'" "'The SDO Foreign Fields must be a comma delimited list of field pair, child, parent, etc.'"}
                           .
IF LENGTH(RowObject.custom_super_procedure) > 0 AND
   NUM-ENTRIES(RowObject.custom_super_procedure,".":U) < 2 THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                           {af/sup2/aferrortxt.i 'AF' '5' 'rym_wizard_brow' 'custom_super_procedure' "'custom super procedure'" "'The custom super procedure specified should contain a path and extension'"}
                           .
