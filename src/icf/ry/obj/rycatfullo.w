...
/*---------------------------------------------------------------------------------
  ...
  Description:  Attribute Full SDO

  Purpose:      Attribute Full SDO
  ...
  (v:010003)    Task:        7754   UserRef:    
                Date:   29/01/2001  Author:     Anthony Swindells

  Update Notes: Attribute Maintenance
...
&Scoped-define INTERNAL-TABLES ryc_attribute ryc_attribute_type ~
ryc_attribute_group

/* Definitions for QUERY Query-Main                                     */
&Scoped-Define ENABLED-FIELDS  attribute_label attribute_type_tla system_owned attribute_narrative~
 attribute_group_obj
&Scoped-define ENABLED-FIELDS-IN-ryc_attribute attribute_label ~
attribute_type_tla system_owned attribute_narrative attribute_group_obj 
&Scoped-Define DATA-FIELDS  attribute_group_name attribute_label attribute_type_tla system_owned~
 attribute_type_description attribute_narrative attribute_group_obj~
 attribute_obj
&Scoped-define DATA-FIELDS-IN-ryc_attribute attribute_label ~
attribute_type_tla system_owned attribute_narrative attribute_group_obj ~
attribute_obj 
&Scoped-define DATA-FIELDS-IN-ryc_attribute_type attribute_type_description 
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH ryc_attribute NO-LOCK, ~
      FIRST ryc_attribute_type WHERE ryc_attribute_type.attribute_type_tla = ryc_attribute.attribute_type_tla NO-LOCK, ~
...
{&DB-REQUIRED-END}
&Scoped-define TABLES-IN-QUERY-Query-Main ryc_attribute ryc_attribute_type ~
ryc_attribute_group
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main ryc_attribute
&Scoped-define SECOND-TABLE-IN-QUERY-Query-Main ryc_attribute_type
...
/* Query rebuild information for SmartDataObject Query-Main
     _TblList          = "RYDB.ryc_attribute,RYDB.ryc_attribute_type WHERE RYDB.ryc_attribute ...,RYDB.ryc_attribute_group WHERE RYDB.ryc_attribute ..."
     _Options          = "NO-LOCK INDEXED-REPOSITION"
     _TblOptList       = ", FIRST, FIRST"
     _OrdList          = "RYDB.ryc_attribute.attribute_label|yes"
     _JoinCode[2]      = "RYDB.ryc_attribute_type.attribute_type_tla = RYDB.ryc_attribute.attribute_type_tla"
     _JoinCode[3]      = "RYDB.ryc_attribute_group.attribute_group_obj = RYDB.ryc_attribute.attribute_group_obj"
...
"attribute_group_name" "attribute_group_name" ? ? "character" ? ? ? ? ? ? no ? no 28 yes
     _FldNameList[2]   > RYDB.ryc_attribute.attribute_label
"attribute_label" "attribute_label" ? ? "character" ? ? ? ? ? ? yes ? no 35 yes
     _FldNameList[3]   > RYDB.ryc_attribute.attribute_type_tla
"attribute_type_tla" "attribute_type_tla" ? ? "character" ? ? ? ? ? ? yes ? no 17.8 yes
     _FldNameList[4]   > RYDB.ryc_attribute.system_owned
"system_owned" "system_owned" ? ? "logical" ? ? ? ? ? ? yes ? no 14.2 yes
     _FldNameList[5]   > RYDB.ryc_attribute_type.attribute_type_description
"attribute_type_description" "attribute_type_description" ? ? "character" ? ? ? ? ? ? no ? no 35 yes
     _FldNameList[6]   > RYDB.ryc_attribute.attribute_narrative
"attribute_narrative" "attribute_narrative" ? ? "character" ? ? ? ? ? ? yes ? no 500 yes
     _FldNameList[7]   > RYDB.ryc_attribute.attribute_group_obj
"attribute_group_obj" "attribute_group_obj" ? ? "decimal" ? ? ? ? ? ? yes ? no 21.6 yes
     _FldNameList[8]   > RYDB.ryc_attribute.attribute_obj
"attribute_obj" "attribute_obj" ? ? "decimal" ? ? ? ? ? ? no ? no 21.6 yes
...
FOR EACH RowObjUpd WHERE LOOKUP(RowObjUpd.RowMod,"A,C,U":U) <> 0:
  ...
  IF (RowObjUpd.RowMod = "U":U AND
      CAN-FIND(FIRST ryc_attribute
               WHERE ryc_attribute.attribute_label = RowObjUpd.attribute_label
                 AND ROWID(ryc_attribute) <> TO-ROWID(ENTRY(1,RowObjUpd.ROWIDent)))) OR 
     (RowObjUpd.RowMod <> "U":U AND
      CAN-FIND(FIRST ryc_attribute
               WHERE ryc_attribute.attribute_label = RowObjUpd.attribute_label))  THEN
    ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
           {af/sup2/aferrortxt.i 'AF' '8' 'ryc_attribute' 'attribute_label' "'attribute label'" RowObjUpd.attribute_label "'. Please use a different attribute label'"}
           .
...
IF LENGTH(RowObject.attribute_type_tla) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'ryc_attribute' 'attribute_type_tla' "'attribute type tla'"}
                        .
IF LENGTH(RowObject.attribute_label) = 0 THEN
  ASSIGN cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE "":U) +
                        {af/sup2/aferrortxt.i 'AF' '1' 'ryc_attribute' 'attribute_label' "'attribute label'"}
                        .
