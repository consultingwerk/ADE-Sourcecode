...
DEFINE FRAME frMain
     coObject AT ROW 2 COL 10.6 COLON-ALIGNED
     bSelect AT ROW 3.05 COL 12.6
     cHelpFilename AT ROW 1 COL 63.8 COLON-ALIGNED
     bUpdateFileName AT ROW 1 COL 116.2
     cHelpContext AT ROW 2 COL 63.8 COLON-ALIGNED
     bUpdateContext AT ROW 2 COL 116.2
      ...
      THEN DO:
          ...
          */
          ...
          THEN DO:
              ...
              IF LOOKUP("getFieldName":U, hNewObject:INTERNAL-ENTRIES) <> 0 
              THEN DO WITH FRAME {&FRAME-NAME}:
                  ASSIGN cSDFContainer  = pcObjectName
                         ...
                         cSDFObject     = ENTRY(NUM-ENTRIES(hNewObject:FILE-NAME, "/":U),  hNewObject:FILE-NAME, "/":U)
                         cSDFFieldName  = DYNAMIC-FUNCTION('getFieldName' IN hNewObject).
                         hLabelHandle   = DYNAMIC-FUNCTION('getLabelHandle' IN hNewObject) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF  VALID-HANDLE(ghCallerHandle) 
  AND LOOKUP("getLogicalObjectName":U, ghCallerHandle:INTERNAL-ENTRIES) > 0 THEN
