...
------------------------------------------------------------------------------*/
  ...
  /* Delete all the page objects associated with the page */
  FOR EACH ryc_page_object EXCLUSIVE-LOCK
     WHERE ryc_page_object.container_smartobject_obj = ryc_page.container_smartobject_obj
       AND ryc_page_object.page_obj                  = ryc_page.page_obj:
    
    /* Delete all the object instances associated with the page object */
    FOR EACH ryc_object_instance EXCLUSIVE-LOCK
       WHERE ryc_object_instance.container_smartobject_obj = ryc_page_object.container_smartobject_obj
    ...
    DO:
      DELETE ryc_page_object NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  DO iCounter = 1 TO (IF pdCustomizationResultObj = 0.00 THEN 1 ELSE 2):
    ...
    FOR EACH ttObjectInstance
      ...
      /* This will read the attributes specified against the master instance */
      ...
      DO:
        FOR EACH ryc_attribute_value NO-LOCK
           WHERE ryc_attribute_value.primary_smartobject_obj   = ttObjectInstance.d_smartobject_obj
             AND ryc_attribute_value.container_smartobject_obj = 0.00
             AND ryc_attribute_value.object_instance_obj       = 0.00
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ...
    RUN getPageObjectTT          (INPUT pcObjectFilename, INPUT pdCustomizationResultObj).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttObjectInstance:
    ttObjectInstance.i_page = DYNAMIC-FUNCTION("getObjectPage":U, ttObjectInstance.c_instance_name,
                                                                  ttObjectInstance.d_object_instance_obj,
                                                                  ttObjectInstance.d_smartobject_obj,
                                                                  ttObjectInstance.d_customization_result_obj).
  END.
  ...
  FOR EACH ttObjectInstance
    ...
    FIND FIRST ttPageObject
         WHERE ttPageObject.d_object_instance_obj = ttObjectInstance.d_object_instance_obj NO-ERROR.

    IF AVAILABLE ttPageObject THEN
      FIND FIRST ttPage
           WHERE ttPage.d_page_obj = ttPageObject.d_page_obj.
    ELSE
      IF AVAILABLE ttPage THEN RELEASE ttPage.
  ...
  DO:
    FOR FIRST ryc_smartobject NO-LOCK
      ...
      FOR EACH ryc_ui_event NO-LOCK
         WHERE ryc_ui_event.primary_smartobject_obj   = ryc_smartobject.smartobject_obj
           AND ryc_ui_event.container_smartobject_obj = 0.00
           AND ryc_ui_event.object_instance_obj       = 0.00
           AND ryc_ui_event.smartobject_obj           = ryc_smartobject.smartobject_obj
           AND ryc_ui_event.object_type_obj           = ryc_smartobject.object_type_obj:
    ...
    FOR EACH ryc_object_instance NO-LOCK
      ...
      DO iObjectInstance = 1 TO iIterations:
        ...
        ASSIGN ttObjectInstance.d_container_smartobject_obj = IF iObjectInstance = 1 THEN dContainerSmartObjectObj                  ELSE dCustomizedSmartObjectObj
               ...
               ttObjectInstance.d_custom_smartobject_obj    = ryc_smartobject.custom_smartobject_obj
...
&IF DEFINED(EXCLUDE-getPageObjectTT) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE getPageObjectTT Procedure 
PROCEDURE getPageObjectTT :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER  ryc_page_object  FOR ryc_page_object.
  DEFINE BUFFER  ttSmartObject    FOR ttSmartObject.
  DEFINE BUFFER bttPageObject     FOR ttPageObject.
  DEFINE BUFFER  ttPageObject     FOR ttPageObject.
  ...
  FOR EACH ryc_page_object NO-LOCK
     WHERE ryc_page_object.container_smartobject_obj = dMainSmartObjectObj:
      
    CREATE ttPageObject.
    ASSIGN ttPageObject.d_container_smartobject_obj = dMainSmartObjectObj
           ttPageObject.d_page_object_obj           = ryc_page_object.page_object_obj
           ttPageObject.d_customization_result_obj  = 0.00
           ttPageObject.d_page_obj                  = ryc_page_object.page_obj
           ttPageObject.i_page_object_sequence      = ryc_page_object.page_object_sequence
           ttPageObject.d_object_instance_obj       = ryc_page_object.object_instance_obj
           .
  END.
  
  FOR EACH ryc_page_object NO-LOCK
     WHERE ryc_page_object.container_smartobject_obj = dCustomizedSmartObjectObj:

    CREATE ttPageObject.
    ASSIGN ttPageObject.d_container_smartobject_obj = dCustomizedSmartObjectObj
           ttPageObject.d_page_object_obj           = ryc_page_object.page_object_obj
           ttPageObject.d_customization_result_obj  = pdCustomizationResultObj
           ttPageObject.d_page_obj                  = ryc_page_object.page_obj
           ttPageObject.i_page_object_sequence      = ryc_page_object.page_object_sequence
           ttPageObject.d_object_instance_obj       = ryc_page_object.object_instance_obj.
  END.
  ...
  DO:
    FOR EACH ttObjectInstance
       WHERE ttObjectInstance.d_customization_result_obj <> 0.00
         AND ttObjectInstance.d_object_instance_obj      <  0.00,
       FIRST bttObjectInstance
       WHERE bttObjectInstance.d_customization_result_obj = 0.00
         AND bttObjectInstance.c_instance_name            = ttObjectInstance.c_instance_name,
       FIRST ttPageObject
       WHERE ttPageObject.d_object_instance_obj = bttObjectInstance.d_object_instance_obj,
       FIRST ttPage
       WHERE ttPage.d_page_obj = ttPageObject.d_page_obj,
       FIRST bttPage
       WHERE bttPage.c_page_reference            = ttPage.c_page_reference
         AND bttPage.d_customization_result_obj <> 0.00:

      CREATE bttPageObject.
      ASSIGN bttPageObject.d_container_smartobject_obj = ttObjectInstance.d_container_smartobject_obj /*dCustomizedSmartObjectObj*/
             bttPageObject.d_page_object_obj           = DYNAMIC-FUNCTION("getTemporaryObj":U)
             bttPageObject.d_customization_result_obj  = pdCustomizationResultObj
             bttPageObject.d_page_obj                  = bttPage.d_page_obj
             bttPageObject.i_page_object_sequence      = ttPageObject.i_page_object_sequence
             bttPageObject.d_object_instance_obj       = ttObjectInstance.d_object_instance_obj.
    END.
  END.
  ...
  DO iCounter = 1 TO (IF pdCustomizationResultObj = 0.00 THEN 1 ELSE 2):
    ...
    FOR FIRST ryc_smartobject NO-LOCK
      ...
      FIND FIRST b_ryc_smartobject NO-LOCK
           WHERE b_ryc_smartobject.smartobject_obj = ryc_smartobject.custom_smartobject_obj NO-ERROR.
      ...
      ASSIGN ttSmartObject.c_object_description       = ryc_smartobject.object_description
             ...
             ttSmartObject.c_custom_super_procedure   = IF AVAILABLE b_ryc_smartobject THEN b_ryc_smartobject.object_filename ELSE "":U
             ttSmartObject.d_custom_smartobject_obj   = ryc_smartobject.custom_smartobject_obj
...
------------------------------------------------------------------------------*/
  ...
  DO iCounter = 1 TO (IF pdCustomizationResultObj = 0.00 THEN 1 ELSE 2):
    ...
    FOR EACH ttObjectInstance
      ...
      DO:
        FOR EACH ryc_ui_event NO-LOCK
           WHERE ryc_ui_event.primary_smartobject_obj   = ttObjectInstance.d_smartobject_obj
             AND ryc_ui_event.container_smartobject_obj = 0.00
             AND ryc_ui_event.object_instance_obj       = 0.00
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO:
    ...
    DO:
      ...
      /* Manage the details of the page objects */
      RUN setPageObject (INPUT pdCustomizationResultObj) NO-ERROR.
  ...
  FOR EACH ttObjectInstance
    ...
    DO:
      ...
      DO:
        FOR EACH ttPageObject
           WHERE ttPageObject.d_object_instance_obj      = ttObjectInstance.d_object_instance_obj
             AND ttPageObject.d_customization_result_obj = pdCustomizationResultObj:
      ...
      END.
      ELSE
      IF ttObjectInstance.i_page <> 0 THEN
        DO:
          FIND FIRST ttPageObject
               WHERE ttPageObject.d_customization_result_obj = ttObjectInstance.d_customization_result_obj
                 AND ttPageObject.d_object_instance_obj      = ttObjectInstance.d_object_instance_obj
                 AND ttPageObject.c_action                  <> "D":U NO-ERROR.

          IF AVAILABLE ttPageObject AND ttPageObject.c_action = "":U THEN
            ttPageObject.c_action = (IF ttPageObject.d_page_object_obj < 0.00 THEN "A":U ELSE "M":U).
        END.
    END.
    ...
    DO:
      ...
      FOR EACH ttPageObject
        ...
        ttPageObject.d_object_instance_obj = ryc_object_instance.object_instance_obj.
      END.
...
------------------------------------------------------------------------------*/
  ...
  /* Resequence the pages to assign the updated sequence numbers to avoid index clashes */
  IF dContainerSmartObjectObj <> 0.00 THEN
    RUN updatePageSequence (INPUT dContainerSmartObjectObj, TRUE).
/*  DO:
    FOR EACH ryc_page EXCLUSIVE-LOCK
       WHERE ryc_page.container_smartobject_obj = dContainerSmartObjectObj:
...
*/
  ...
  FOR EACH ttPage
    ...
    DO:
      ...
      DO:
        FOR EACH ttPageObject
           WHERE ttPageObject.d_page_obj = ttPage.d_page_obj:

          FOR EACH ttObjectInstance
             WHERE ttObjectInstance.d_object_instance_obj = ttPageObject.d_object_instance_obj:
    ...
    DO:
      ...
      FOR EACH ttPageObject
        ...
        ASSIGN
            ttPageObject.d_page_obj = ryc_page.page_obj
            ttPageObject.c_action   = (IF ttPageObject.d_page_object_obj > 0 THEN "M":U ELSE "A":U).
      END.
...
&IF DEFINED(EXCLUDE-setPageObject) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setPageObject Procedure 
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER cryc_page FOR ryc_page.
  DEFINE BUFFER  ryc_page FOR ryc_page.

  /* Update the sequence numbers of modified records, seeing that it could have been resequenced - Set it to ? to avoid index clashes */
  FOR EACH ttPageObject
     WHERE ttPageObject.c_action                  <> "":U
       AND ttPageObject.d_customization_result_obj = pdCustomizationResultObj,
     FIRST ryc_page_object EXCLUSIVE-LOCK
     WHERE ryc_page_object.page_object_obj = ttPageObject.d_page_object_obj:
    
    ryc_page_object.page_object_sequence = ? NO-ERROR.

    IF ERROR-STATUS:ERROR THEN
      RETURN ERROR (IF RETURN-VALUE <> "":U THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).
  END.

  /* ttPageObjects that should be deleted have been deleted by deleteObjectInstanceData */
  FOR EACH ttPageObject
     WHERE ttPageObject.c_action                  <> "":U
       AND ttPageObject.d_customization_result_obj = pdCustomizationResultObj
        BY ttPageObject.c_action DESCENDING:

    /* ObjectInstances and Pages were already created. For customizations, if ObjectInstance or Pages were not to be
       created, the PageObject records would be deleted. The ones here, should be created */
    
    /* Find the object exclusively as we would need to need to update or delete it */
    FIND FIRST ryc_page_object EXCLUSIVE-LOCK
         WHERE ryc_page_object.page_object_obj = ttPageObject.d_page_object_obj NO-ERROR.

    /* Delete objects that should be deleted */
    IF ttPageObject.c_action = "D":U THEN
    DO:
      /* If it doesn't exist, then it never did (object instance added and deleted in the same session) */
      IF AVAILABLE ryc_page_object THEN
      DO:
    ...
    /* Create smartobjects that should be created */
    IF NOT AVAILABLE ryc_page_object THEN
    DO:
      CREATE ryc_page_object NO-ERROR.
    ...
    /* Assign the new values */
    ASSIGN
        ryc_page_object.container_smartobject_obj = ttPageObject.d_container_smartobject_obj
        ryc_page_object.page_obj                  = ttPageObject.d_page_obj
        ryc_page_object.page_object_sequence      = ttPageObject.i_page_object_sequence
        ryc_page_object.object_instance_obj       = ttPageObject.d_object_instance_obj NO-ERROR.

    IF ERROR-STATUS:ERROR THEN
      RETURN ERROR (IF RETURN-VALUE <> "":U THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).

    VALIDATE ryc_page_object.

    IF ERROR-STATUS:ERROR THEN
      RETURN ERROR (IF RETURN-VALUE <> "":U THEN RETURN-VALUE ELSE ERROR-STATUS:GET-MESSAGE(1)).
  END.

  /* Now that the page object records have been updated, delete the pages and any data they might have left on them */
  FOR EACH ttPage
     WHERE ttPage.i_page_sequence           <> 0
    ...
    IF pdCustomizationResultObj = 0.00 THEN
    DO:
      /* Delete all customizations too. The users have been informed about it, and they had to agree in order to get the page deleted. */
      FOR FIRST ttSmartObject
          WHERE ttSmartObject.d_smartobject_obj <> 0.00,
           EACH ryc_smartobject NO-LOCK
          WHERE ryc_smartobject.object_filename           = ttSmartObject.c_object_filename
            AND ryc_smartobject.customization_result_obj <> 0.00,
          FIRST cryc_page EXCLUSIVE-LOCK
          WHERE cryc_page.container_smartobject_obj = ryc_smartobject.smartobject_obj
            AND cryc_page.page_reference            = ttPage.c_page_reference:

        RUN deletePageData (BUFFER cryc_page) NO-ERROR.
  ...
  /* If we did not delete any pages, we would not have the obj number */
  IF dContainerSmartObjectObj = 0.00 THEN
  DO:
    FIND FIRST ttPage
         WHERE ttPage.i_page_sequence           <> 0
  ...
  RUN updatePageSequence (INPUT dContainerSmartObjectObj, TRUE) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  FOR FIRST ttSmartObject
    ...
    DO:
      FIND FIRST gsc_object_type NO-LOCK
           WHERE gsc_object_type.object_type_obj = ttSmartObject.d_object_type_obj NO-ERROR.
      
      /* Check to see if we are creating a Dynamic Frame */
      IF {fnarg ClassIsA "gsc_object_type.object_type_code, 'DynFrame':U" gshRepositoryManager} THEN
        FIND FIRST rycso NO-LOCK
             WHERE rycso.object_filename = "rydynframw":U NO-ERROR.
      ELSE
        /* Check to see if we are creating a Dynamic SBO */
        IF {fnarg ClassIsA "gsc_object_type.object_type_code, 'DynSBO':U" gshRepositoryManager} THEN
          FIND FIRST rycso NO-LOCK
               WHERE rycso.object_filename = "dynsbo.w":U NO-ERROR.
        ELSE
          /* If none of the above, we are creating a normal Dynamic Container */
          FIND FIRST rycso NO-LOCK
               WHERE rycso.object_filename = "rydyncontw.w":U NO-ERROR.
      ...
      ASSIGN
          ...
          ryc_smartobject.custom_smartobject_obj   = ttSmartObject.d_custom_smartobject_obj
          ...
          /* The PhysicalSmartObjectObj must ONLY be assigned when 0.00 (upon creation), as someone could have modified it subsequently in the ROM tool */
          ryc_smartobject.physical_smartobject_obj = (IF ryc_smartobject.physical_smartobject_obj = 0.00 THEN rycso.smartObject_obj
                                                                                                         ELSE ryc_smartobject.physical_smartobject_obj) NO-ERROR.
    ...
    /* ttPageObject */
    FOR EACH ttPageObject
       WHERE ttPageObject.d_container_smartobject_obj = ttSmartObject.d_smartobject_obj:
      
      ttPageObject.d_container_smartobject_obj = ryc_smartobject.smartobject_obj.
    END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ryc_ui_event.constant_value  = ttUiEvent.l_constant_value
      ryc_ui_event.action_type     = ttUiEvent.c_action_type
      ryc_ui_event.action_target   = ttUiEvent.c_action_target
      ryc_ui_event.event_name      = ttUiEvent.c_event_name
      ryc_ui_event.event_action    = ttUiEvent.c_event_action
      ryc_ui_event.event_parameter = ttUiEvent.c_event_parameter
      ryc_ui_event.event_disabled  = ttUiEvent.l_event_disabled.
  ...
  FOR FIRST ryc_smartobject EXCLUSIVE-LOCK
    ...
    DO:
      ...
      FOR EACH ryc_object_instance EXCLUSIVE-LOCK
        ...
        IF CAN-FIND(FIRST ryc_page_object
                    WHERE ryc_page_object.container_smartobject_obj = ryc_object_instance.container_smartobject_obj
                      AND ryc_page_object.object_instance_obj       = ryc_object_instance.object_instance_obj) THEN NEXT.
    ...
    FOR EACH ryc_page EXCLUSIVE-LOCK
      ...
      FOR EACH ryc_page_object NO-LOCK
         WHERE ryc_page_object.container_smartobject_obj = ryc_page.container_smartobject_obj
           AND ryc_page_object.page_obj                  = ryc_page.page_obj,
         FIRST ryc_object_instance EXCLUSIVE-LOCK
         WHERE ryc_object_instance.object_instance_obj = ryc_page_object.object_instance_obj,
         FIRST ttObjectInstance
         WHERE ttObjectInstance.d_object_instance_obj = ryc_object_instance.object_instance_obj:
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER ttCPageObject     FOR ttPageObject.     /* Customization Page Object     */
  ...
  FOR EACH ttCObjectInstance
    ...
    IF CAN-FIND(FIRST ttCPageObject
                WHERE ttCPageObject.d_object_instance_obj = ttCObjectInstance.d_object_instance_obj
                  AND ttCPageObject.c_action             <> "D":U
                  AND ttCPageObject.c_action             <> "":U) THEN
      lInstanceChanged = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttCPage
    ...
    IF CAN-FIND(FIRST ttPageObject
                WHERE ttPageObject.d_page_obj = ttCPage.d_page_obj
                  AND ttPageObject.c_action  <> "D":U
                  AND ttPageObject.c_action  <> "":U) THEN
      lPageChanged = TRUE.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE iObjectPage AS INTEGER    NO-UNDO INITIAL 0.
  
  FOR FIRST ttObjectInstance
      WHERE ttObjectInstance.c_instance_name            = pcInstanceName
        AND ttObjectInstance.d_smartobject_obj          = pdSmartObjectObj
        AND ttObjectInstance.d_object_instance_obj      = pdObjectInstanceObj
        AND ttObjectInstance.d_customization_result_obj = pdCustomizationResultObj,
      FIRST ttPageObject
      WHERE ttPageObject.d_object_instance_obj = ttObjectInstance.d_object_instance_obj,
      FIRST ttPage
    ...
    iObjectPage = ttPage.i_page_sequence.
  END.

  RETURN iObjectPage.   /* Function return value. */
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE dExtendsOTObj     AS DECIMAL    NO-UNDO.
  DEFINE VARIABLE iCounter          AS INTEGER    NO-UNDO.
  ...
  DO WHILE AVAILABLE gsc_object_type:
    ASSIGN
        cReversedOTString = cReversedOTString
                          + (IF cReversedOTString = "":U THEN "":U ELSE ",":U)
                          + gsc_object_type.object_type_code
        dExtendsOTObj     = gsc_object_type.extends_object_type_obj.

    FIND FIRST gsc_object_type NO-LOCK
         WHERE gsc_object_type.object_type_obj = dExtendsOTObj NO-ERROR.
  END.
  ...
  RETURN cInheritsFromOT.   /* Function return value. */
