...
/*---------------------------------------------------------------------------------
  ...
  Description:  Page Object Data Extraction Plip

  Purpose:      Contains procedures for reading/updating ryc_Page_object into a temp-table and
                passing temp-table back and forth to calling procedure.

  Parameters:   input-output table for ryc_page_object defined like rowobjupd
...
DEFINE INPUT-OUTPUT PARAMETER TABLE-HANDLE phttRycPageObject.
...
DO: 
    RUN ry/obj/rycpofullo.w PERSISTENT SET hSDO ON gshAstraAppserver NO-ERROR.
    ...
    ENTRY(1,cQuery) = "FOR EACH ryc_page_object NO-LOCK ":u +
                         "WHERE ryc_page_object.container_smartobject_obj = DECIMAL(":u + STRING(pdSmartObjectObj) + ") ":u +
                         "AND ryc_page_object.object_instance_obj = DECIMAL(":u + STRING(pdObjectInstanceObj) + ")":u.
    hTtHandle = phttRycPageObject.
    RUN ry/app/rydynqrytt.p (INPUT cQuery, INPUT (IF pcAction EQ "C":U THEN pcAction ELSE "U":U), INPUT-OUTPUT hTtHandle).
    phttRycPageObject = hTtHandle.
...
ELSE DO:
  ...
  hTTBuffer = phttRycPageObject:DEFAULT-BUFFER-HANDLE.
  ...
  phttRycPageObject = hTTBuffer:TABLE-HANDLE.
...
ASSIGN cDescription = "Dynamics ryc_page_bject PLIP".
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT-OUTPUT PARAMETER TABLE FOR ttRycPageObject.
...
FIND FIRST ttRycPageObject NO-ERROR.
RUN serverCommit IN hSDO (INPUT-OUTPUT TABLE ttRycPageObject, OUTPUT cErrorMessage, OUTPUT cUndoRowIds) NO-ERROR.
IF  ERROR-STATUS:ERROR OR RETURN-VALUE <> "":u  THEN DO:
    cErrorMessage = IF cErrorMessage = "":u THEN RETURN-VALUE ELSE cErrorMessage + CHR(3) + RETURN-VALUE.
...
FIND ttRycPageObject WHERE ttRycPageObject.rowMod = "U" NO-ERROR.
FIND FIRST ryc_page_object NO-LOCK
     WHERE ryc_page_object.container_smartobject_obj = ttRycPageObject.container_smartobject_obj
     AND   ryc_page_object.object_instance_obj       = ttRycPageObject.object_instance_obj NO-ERROR.

/* Assign object obj to temp-table, to be passed back to calling procedure */
/* Only necessary if adding (smartobject_obj = 0) */
FOR EACH ttRycPageObject 
    WHERE ttRycPageObject.object_instance_obj = 0:

    ASSIGN
        ttRycPageObject.object_instance_obj = IF AVAILABLE ryc_page_object THEN ryc_page_object.object_instance_obj ELSE 0.
END.
