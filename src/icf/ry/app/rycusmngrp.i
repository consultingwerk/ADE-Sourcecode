...
PROCEDURE rycusrr2rp: { ry/app/rycusrr2rp.p } . END PROCEDURE.
PROCEDURE rycusfapip: { ry/app/rycusfapip.p } . END PROCEDURE.
&ENDIF
...
------------------------------------------------------------------------------*/
    ...
    ELSE DO:
        ...
        ASSIGN gcCustomisationTypesPrioritised  = DYNAMIC-FUNCTION("getSessionParam":U IN THIS-PROCEDURE, INPUT "CustomizationTypePriority":U)
               gcSessionResultCodes             = "":U
               gcSessionCustomisationReferences = "":U.
        ...
        RUN ry/app/rycusfapip.p ON gshAstraAppServer ( INPUT gcCustomisationTypesPrioritised, OUTPUT cTypeAPI ) NO-ERROR.
        &ELSE
        RUN rycusfapip ( INPUT gcCustomisationTypesPrioritised, OUTPUT cTypeAPI ) NO-ERROR.
        &ENDIF
    END.
    ...
    DO iTypeLoop = 1 TO NUM-ENTRIES(cTypeApi):
        ASSIGN cReferenceCode = "":U
               cReferenceCode = DYNAMIC-FUNCTION(ENTRY(iTypeLoop, cTypeApi) IN THIS-PROCEDURE)
               NO-ERROR.
    ...
    ELSE DO:
        ...
        &IF DEFINED(Server-Side) EQ 0 &THEN              
        RUN ry/app/rycusrr2rp.p ON gshAstraAppServer ( INPUT        gcCustomisationTypesPrioritised,
                                                       INPUT        cTypeApi,                                                   
                                                       INPUT-OUTPUT gcSessionCustomisationReferences,
                                                       OUTPUT       gcSessionResultCodes              ) NO-ERROR.
        &ELSE
        RUN rycusrr2rp ( INPUT        gcCustomisationTypesPrioritised,
                         INPUT        cTypeApi,                                                   
                         INPUT-OUTPUT gcSessionCustomisationReferences,
                         OUTPUT       gcSessionResultCodes              ) NO-ERROR.
        &ENDIF
    ...
    RUN setSessionResultCodes IN THIS-PROCEDURE (INPUT gcSessionResultCodes,
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    SUBSCRIBE PROCEDURE THIS-PROCEDURE TO "ICFCFM_LoginComplete":U ANYWHERE.
...
THEN DO:
    DYNAMIC-FUNCTION("setSessionParam":U, INPUT "SessionResultCodes", INPUT pcResultCodes).
...
IF NOT (SESSION:REMOTE OR SESSION:CLIENT-TYPE = "WEBSPEED":U) THEN /* we're on the client */
    RUN setClientResultCodes IN THIS-PROCEDURE (INPUT pcResultCodes,
...
/*------------------------------------------------------------------------------
  Purpose:  Returns the current reference for the login company.
    Notes:  
------------------------------------------------------------------------------*/
    RETURN DYNAMIC-FUNCTION("getPropertyList" IN gshSessionManager, "CurrentOrganisationName", YES).
END FUNCTION.
...
/*------------------------------------------------------------------------------
  Purpose:  Returns the current reference for the session (UI) type.
    Notes:  
------------------------------------------------------------------------------*/
    RETURN DYNAMIC-FUNCTION("getPhysicalSessionType":U IN THIS-PROCEDURE).
END FUNCTION.
