...
filter-btn Btn-rebuild coProductModule ToBox-Rep BROWSE-1 Btn_Sort Btn_Add ~
...
coProductModule ToBox-Rep TBReg LogFile 
...
DEFINE VARIABLE ToBox-Rep AS LOGICAL INITIAL no 
     LABEL "Repository &Modules" 
     VIEW-AS TOGGLE-BOX
     SIZE 23 BY .81 TOOLTIP "Include framework product modules in the combo box above." NO-UNDO.
...
/* BROWSE-TAB BROWSE-1 ToBox-Rep DEFAULT-FRAME */
...
DO:
  ...
  IF NOT AVAILABLE gsc_product_module THEN DO WITH FRAME DEFAULT-FRAME:
    ASSIGN coProductModule.
    FIND FIRST gsc_product_module 
                 WHERE gsc_product_module.product_module_obj = coProductModule  NO-LOCK NO-ERROR.
  END.
  IF NOT AVAILABLE gsc_product_module THEN DO:
    ...
    RETURN NO-APPLY.
  END.
  cPMValue = gsc_product_module.product_module_code.


  /* Set the current value of Product Module in the Profile Data */
...
DO:
  ...
  /* Set the current value of Product Module in the Profile Data */
  RUN UpdatePMinProfile.  /* This has the side effect of refreshing gcProfileData */
  ...
  DO i = 1 TO NUM-ENTRIES(cPM_LIST):
    ...
    IF cPM NE "":U THEN DO:
      FIND gsc_product_module NO-LOCK WHERE gsc_product_module.product_module_code = cPM.
      cDirName = cRootDirectory + "~/":U + gsc_product_module.relative_path.
      ...
      IF FILE-INFO:FILE-SIZE < 50 THEN DO:  /* Too small to contain any useful id information */
        ...
        IF (tmpFile MATCHES "*~.p" OR  tmpFile MATCHES "*~.w") AND TBReg:CHECKED THEN DO:
          ...
          RUN af/sup2/afsdocrdbp.p
              (INPUT FILE-INFO:FULL-PATHNAME,  /* Fully qualified path name of object to register  */
               INPUT "Procedure":U,            /* Register with object_type "Procedure"            */
               INPUT gsc_product_module.Product_module_code,  /* Register with this product module */
               INPUT "",                       /* Blank description                                */
               INPUT "":U,                     /* Deployment Type                                  */
               INPUT NO,                       /* Design only                                      */
               INPUT NO,                       /* Don't prompt for product module                  */
               OUTPUT pError).                 /* Error text occurred                              */
        ...
        ELSE DO:  /* Go ahead and try to covert this object */
          ...
          IF CantConvert THEN DO: /* Register this procedure */
            ...
            IF TBReg:CHECKED THEN DO:
              ...
              IF NOT CAN-FIND(gsc_object_type 
                   WHERE gsc_object_type.object_type_code = ProcType) THEN
                ProcType = "Procedure":U.  /* Last resort */

              RUN af/sup2/afsdocrdbp.p
                  (INPUT FILE-INFO:FULL-PATHNAME,  /* Fully qualified path name of object to register  */
                   INPUT ProcType,                 /* Register with object_type ProcType               */
                   INPUT gsc_product_module.Product_module_code,  /* Register with this product module */
                   INPUT "",                       /* Blank description                                */
                   INPUT "":U, /* Deployment Type */
                   INPUT NO,   /* Design Only */
              ...
              END.  /* If an error occurred */
              ELSE DO:
                PUT STREAM log-file UNFORMATTED "Registered as '" + ProcType + "'." + CHR(10).
                ASSIGN Procedure.Stts = "Registered".
              END.  /* Else no error and properly registered */
          ...
          ELSE DO: /* Migrate here */
            ...
            IF pError NE ""  AND 
               NOT pError BEGINS "Associated SDO" THEN DO:
              IF pError = "Error":U THEN pError = "":U.  /* A flag ... no message */
              ASSIGN lErrorFound = YES
                     pError = pError +  Procedure.Name + " not migrated." + CHR(10).
            ...
            DO iLine = 1 TO NUM-ENTRIES(pError,CHR(10)):
              PUT STREAM log-file UNFORMATTED
                         "    " +
                        (IF pError = "" THEN "No errors." ELSE 
                                             ENTRY(iLine,pError,CHR(10)))
                         FORMAT "X(75)" SKIP.
            END.
...
DO:
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    FIND gsc_product_module NO-LOCK
        WHERE gsc_product_module.product_module_obj = pm_obj NO-ERROR.
    IF AVAILABLE gsc_product_module THEN
       hTBReg:TOOLTIP = 
                'To be registered, non migrated static objects must be in the "' +
                gsc_product_module.relative_path + '" directory'.
...
&Scoped-define SELF-NAME ToBox-Rep
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ToBox-Rep V89dcnv
ON VALUE-CHANGED OF ToBox-Rep IN FRAME DEFAULT-FRAME /* Repository Modules */
DO:
  rRowid = ?.
  RUN setProfileData IN gshProfileManager (INPUT "General":U,
                                           ...
                                           INPUT "DispRepos":U,
                                           INPUT ?,
                                           INPUT IF self:CHECKED THEN "YES":U ELSE "NO":U,
                                           INPUT NO,
                                           INPUT "SES":U).
...
rRowid = ?.
/* Establish proper value for toggle-box */
RUN getProfileData IN gshProfileManager (INPUT "General":U,     /* Profile type code       */
...
ToBox-Rep:CHECKED = (cRepModules = "YES":U).
...
------------------------------------------------------------------------------*/
    ...
    FIND gsc_product_module WHERE gsc_product_module.product_module_code = _P.product_module_code NO-LOCK.

    ASSIGN  _P.object_filename       = pcObjName
            _P._SAVE-AS-FILE         = pcObjName
            _P.product_module_code   = gsc_product_module.product_module_code
            _P.object_type_code      = 
   ...
   FIND FIRST gsc_product_module NO-LOCK
              WHERE gsc_product_module.product_module_code = cSuperProdMod NO-ERROR.

   /* Load preferred procname */
   IF LOOKUP(pcType,"SDO,SBO":U) > 0 THEN _C._DATA-LOGIC-PROC = gsc_product_module.relative_path + 
                                                                       "~/":U + pcSuperName + ".p".
                                     ELSE _C._CUSTOM-SUPER-PROC = gsc_product_module.relative_path + 
                                                                       "~/":U + pcSuperName + ".p".

   IF AVAILABLE gsc_product_module THEN DO:
     IF LOOKUP(pcType,"SDO,SBO":U) > 0 
       THEN _C._DATA-LOGIC-PROC-PMOD = gsc_product_module.product_module_code.
       ELSE _C._CUSTOM-SUPER-PROC-PMOD  = gsc_product_module.product_module_code.
   ...
   IF NOT lSkip THEN
     RUN ry/prc/rygendynp.p (INPUT RECID(_P), OUTPUT pcError).
   ELSE
...
------------------------------------------------------------------------------*/
  DISPLAY Directory tog_subdir file-filter coProductModule ToBox-Rep TBReg 
          LogFile 
      WITH FRAME DEFAULT-FRAME IN WINDOW V89dcnv.
  ENABLE Directory btn_browse tog_subdir file-filter filter-btn Btn-rebuild 
         coProductModule ToBox-Rep BROWSE-1 Btn_Sort Btn_Add Btn_Remove TBReg 
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH gsc_product_module BY gsc_product_module.product_module_code:
     ...
     IF NOT ToBox-Rep:CHECKED IN FRAME {&FRAME-NAME} THEN DO:
       DO i = 1 TO NUM-ENTRIES("{&REPOSITORY-MODULES}"):
         IF gsc_product_module.product_module_code BEGINS ENTRY(i, "{&REPOSITORY-MODULES}") THEN
           NEXT PopLoop.
       END.  /* Do i to Num-entries */
     END. /* If toggle is not checked */
      
     ASSIGN listItemPairs = ListItemPairs + CHR(4) +
                             product_module_code + " // " + product_module_description + CHR(4) +
                            STRING(gsc_product_module.product_module_obj,">>>>>>>>>>>>>>>>>>9.9999999999":U).
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO NUM-ENTRIES(cSDOList):
    ...
    ELSE DO:  /* Must register it */
      RUN af/sup2/afsdocrdbp.p
         (INPUT SEARCH(TmpFile),          /* Fully qualified path name of object to register  */
          INPUT "SDO":U,                  /* Register with object_type "Static SDO"           */
          INPUT gsc_product_module.Product_module_code,  /* Register with this product module */
          INPUT "",                       /* Blank description                                */
          INPUT "":U,                     /* Deployment Type                                  */
          INPUT NO,                       /* Design only                                      */
...
------------------------------------------------------------------------------*/
   ...
   /* Get current value of product module default, Convert once, we will use many times */
   IF NOT AVAILABLE gsc_product_module THEN
     FIND gsc_product_module NO-LOCK WHERE gsc_product_module.product_module_obj = coProductModule.
   ASSIGN cPMValue = gsc_product_module.product_module_code.
