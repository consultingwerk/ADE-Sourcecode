...
{src/icf/af/sup2/afxmlreplctrl.i}

/* ttNode table and manipulation include */
{src/icf/af/sup2/afttnode.i}
...
------------------------------------------------------------------------------*/
  ...
  DO:
    cMessage = {src/icf/af/sup2/aferrortxt.i 'ICF' '4' '?' '?'}.
    lError = YES.
  END.
  ...
  DO TRANSACTION ON ERROR UNDO, LEAVE:
    ...
    FOR EACH bttRelate:
      ...
      REPEAT WHILE iCount < NUM-ENTRIES(bttRelate.JoinFields):
        ...
        DO:
          lError = YES.
          cMessage = {src/icf/af/sup2/aferrortxt.i 'ICF' '5' '?' '?' 'Parent' cParentField}.
          UNDO tran-block, LEAVE tran-block.
        END.
        IF NUM-ENTRIES(cChildField,".":U) <> 2 THEN
        DO:
          lError = YES.
          cMessage = {src/icf/af/sup2/aferrortxt.i 'ICF' '5' '?' '?' 'Child' cChildField}.
          UNDO tran-block, LEAVE tran-block.
        END.
