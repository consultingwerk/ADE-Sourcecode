...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      hfiOutputDestination:SCREEN-VALUE = FILE-INFO:FULL-PATHNAME + "~\temp" + STRING(ETIME) + ".txt":U
      hfiScanDirectory:SCREEN-VALUE     = FILE-INFO:FULL-PATHNAME
  ...
  RUN trgFillInValueChanged   IN TARGET-PROCEDURE ("fiDumpDirectory":U).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE hchkRecurse     AS HANDLE     NO-UNDO.
  DEFINE VARIABLE hbuStart        AS HANDLE     NO-UNDO.

  hfiFieldHandle = DYNAMIC-FUNCTION("getFieldHandle":U IN TARGET-PROCEDURE, pcFieldName).

  CASE pcFieldName:
    WHEN "fiScanDirectory":U     THEN gcScanDirectory  = hfiFieldHandle:SCREEN-VALUE.
    WHEN "fiDumpDirectory":U     THEN gcDumpDirectory  = hfiFieldHandle:SCREEN-VALUE.
    WHEN "fiOutputDestination":U THEN gcOutputFilename = hfiFieldHandle:SCREEN-VALUE.
  END CASE.
