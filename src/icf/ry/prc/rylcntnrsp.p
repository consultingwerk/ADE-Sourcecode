...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
/*RUN addLink IN ghTargetProcedure (ghContainerToolbar, "Toolbar":U, ghTargetProcedure).*/
/*  RUN addLink IN ghTargetProcedure (ghBrowseToolbar,    "Toolbar":U, ghTargetProcedure).*/
/*  
  
  DYNAMIC-FUNCTION("enableActions":U IN ghBrowseToolbar,    "cbShowFilter":U).
*/
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      cContainerMode = DYNAMIC-FUNCTION("getUserProperty":U IN ghTargetProcedure, "ContainerMode":U)
      lSuccess       = DYNAMIC-FUNCTION("disableActions":U  IN ghBrowseToolbar, "New,cbCopy,cbDelete,cbModify,cbSave,cbUndo,cbCancel,Export":U)
  ...
  cEnabledActions = cEnabledActions  + (IF cEnabledActions = "":U THEN "":U ELSE ",":U)
                  + "cbShowFilter":U + (IF lHasData THEN ",Export":U ELSE "":U).
