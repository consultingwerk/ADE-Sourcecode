...
&IF DEFINED(EXCLUDE-isViewerOrStaticSO) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD isViewerOrStaticSO Procedure 
FUNCTION isViewerOrStaticSO RETURNS LOGICAL
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ASSIGN
        plContinue = FALSE
        cMessage   = {aferrortxt.i 'AF' '5' '?' '?' "'object filename'"}.
  ...
  DO:
    ASSIGN
        plContinue = FALSE
        cMessage   = {aferrortxt.i 'AF' '5' '?' '?' "'product module'"}.
  ...
  IF hLookupFillIn:SCREEN-VALUE <> "":U AND
     ...
     {fn getDataValue hLookup}   = 0.00 THEN
  DO:
    ASSIGN
        plContinue = FALSE
        cMessage   = {aferrortxt.i 'AF' '5' '?' '?' "'super procedure'"}.
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION("setUserProperty":U IN TARGET-PROCEDURE, "ttPageObject":U,          STRING(TEMP-TABLE ttPageObject:DEFAULT-BUFFER-HANDLE)).
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttObjectType
     WHERE LOOKUP(ttObjectType.c_object_type_code, gcFolderClasses) <> 0:
      ...
      FOR EACH  ttTmpltObjectInstance
         WHERE  ttTmpltObjectInstance.i_page           <> piPageSequenceToFetch
            OR (ttTmpltObjectInstance.d_object_type_obj = ttObjectType.d_object_type_obj
           AND  piPageSequenceToReplace                <> 0):
    
        /* PageObjects */
        FOR EACH ttTmpltPageObject
           WHERE ttTmpltPageObject.d_object_instance_obj = ttTmpltObjectInstance.d_object_instance_obj:
    
          DELETE ttTmpltPageObject.
        END.
  ...
  /* Pages */
  FOR EACH ttTmpltPage
     WHERE ttTmpltPage.i_page_sequence <> piPageSequenceToFetch:
  ...
  DO:
    FOR FIRST ttTmpltPage
      ...
      IF piPageSequenceToFetch <> 0 THEN
        BUFFER-COPY ttTmpltPage
      ...
      /* --------- PageObject --------- */
      /* If piPageSequenceToFetch is zero, then PageObject records will not exist and this block will not be iterated */
      FOR EACH ttTmpltPageObject
         WHERE ttTmpltPageObject.d_page_obj = ttTmpltPage.d_page_obj:
  ...
  FOR EACH ttTmpltObjectInstance
     WHERE ttTmpltObjectInstance.d_smartobject_obj <> 0.00
       AND ttTmpltObjectInstance.i_page             = piPageSequenceToFetch:
    ...
    /* If this is the case then no page object details would have been retrieved, so we have to create it */
    IF piPageSequenceToReplace <> 0 AND
       piPageSequenceToFetch    = 0 THEN
    DO:
      CREATE ttPageObject.
      ASSIGN ttPageObject.d_container_smartobject_obj = ttSmartObject.d_smartobject_obj
             ttPageObject.d_customization_result_obj  = dCustomizationResultObj
    ...
    FOR EACH ttSmartObject
      ...
      FOR EACH ttAttributeValue
        ...
        CASE ttAttributeValue.i_data_type:
          WHEN {&DECIMAL-DATA-TYPE}   THEN pcAttributeList = pcAttributeList + STRING(ttAttributeValue.d_decimal_value).
          WHEN {&INTEGER-DATA-TYPE}   THEN pcAttributeList = pcAttributeList + STRING(ttAttributeValue.i_integer_value).
          WHEN {&DATE-DATA-TYPE}      THEN pcAttributeList = pcAttributeList + STRING(ttAttributeValue.t_date_value).
          WHEN {&RAW-DATA-TYPE}       THEN.
          WHEN {&LOGICAL-DATA-TYPE}   THEN pcAttributeList = pcAttributeList + STRING(ttAttributeValue.l_logical_value).
          WHEN {&CHARACTER-DATA-TYPE} THEN pcAttributeList = pcAttributeList + ttAttributeValue.c_character_value.
        END CASE.
      END.
    END.
  END.
    ...
    FOR EACH ttObjectInstance
      ...
      FOR EACH ttAttributeValue
        ...
        DO:
          ...
          CASE ttAttributeValue.i_data_type:
            WHEN {&DECIMAL-DATA-TYPE}   THEN cAttributeValues = cAttributeValues + STRING(ttAttributeValue.d_decimal_value).
            WHEN {&INTEGER-DATA-TYPE}   THEN cAttributeValues = cAttributeValues + STRING(ttAttributeValue.i_integer_value).
            WHEN {&DATE-DATA-TYPE}      THEN cAttributeValues = cAttributeValues + STRING(ttAttributeValue.t_date_value).
            WHEN {&RAW-DATA-TYPE}       THEN.
            WHEN {&LOGICAL-DATA-TYPE}   THEN cAttributeValues = cAttributeValues + STRING(ttAttributeValue.l_logical_value).
            WHEN {&CHARACTER-DATA-TYPE} THEN cAttributeValues = cAttributeValues + ttAttributeValue.c_character_value.
          END CASE.
        END.
      END.
      ...
      IF LAST-OF(ttObjectType.c_object_type_code) THEN
        IF LOOKUP(ttObjectType.c_object_type_code, DYNAMIC-FUNCTION("getClassChildrenFromDB":U IN gshRepositoryManager, "Panel":U)) = 0 THEN
        DO:
...
------------------------------------------------------------------------------*/
  ...
  DO iAttribute = 1 TO NUM-ENTRIES(cDenormalizedAttributes):
    ...
    /* Check if there is an attribute value on the m */
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
      ...
      ghToolbar          = DYNAMIC-FUNCTION("linkHandles":U        IN TARGET-PROCEDURE, "Toolbar-Source":U)
  ...
  DO:
    ASSIGN
        ...
        cHiddenActions      = cHiddenActions + (IF cHiddenActions = "":U THEN "":U ELSE ",":U)
                            + "New,cbCancel,cbCopy,cbDelete,cbFind,txtExit,nodeMaintenance,cbSaveAs".
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION("evaluateActions":U    IN ghPageMaintenance, "MODIFY":U).
...
------------------------------------------------------------------------------*/
  ...
  IF NOT DYNAMIC-FUNCTION("isViewerOrStaticSO":U IN TARGET-PROCEDURE, ttObjectType.c_object_type_code) THEN
  DO:
...
------------------------------------------------------------------------------*/
  ...
  IF {fn getFolderLabels ghPageSource} <> cFolderLabels OR piPageSequence = ? THEN
  DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE pcAction:
    ...
    DO:
      ...
      RUN editRyObjectInAB IN ghDesignManager (INPUT "":U,
...
------------------------------------------------------------------------------*/
  ...
  CASE phWidget:TYPE:
    WHEN "COMBO-BOX":U  OR
    WHEN "RADIO-SET":U  OR
    WHEN "FILL-IN":U    THEN phWidget:SCREEN-VALUE = cAttributeValue.

    WHEN "TOGGLE-BOX":U THEN phWidget:CHECKED      = (cAttributeValue = "yes":U).
  END CASE.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE ttSmartObject AND ttSmartObject.d_custom_smartobject_obj <> 0.00 THEN
    lHasSuper = TRUE.
    ...
    WHEN "ADD":U THEN
      ASSIGN
          cDisabledActions = "New,cbDelete,cbFind,cbUndo,CntainerPreview,cbGenDataLogic":U
                           + (IF lDataContainer THEN ",Pages,objMenuStruct":U
          ...
          gcEnabledActions = "cbSave,cbCancel,Links,cbObjectLocator,Properties,cbPageSequence,Pref":U
                           + (IF lDataContainer THEN "":U
                                                ELSE (IF lFrameContainer THEN ",Pages":U ELSE ",Pages,objMenuStruct":U))
    ...
    WHEN "MODIFY":U THEN
      ASSIGN
          cDisabledActions = "cbCancel,cbSave,cbUndo":U
                           + (IF lDataContainer THEN ",Pages,objMenuStruct,CntainerPreview":U
                                                ELSE (IF lFrameContainer THEN ",objMenuStruct,CntainerPreview":U ELSE ",cbGenDataLogic":U))
                           + (IF lHasSuper      THEN "":U ELSE ",cbEditCustomObj":U)
                           + (IF lDataLogicProc THEN "":U ELSE ",cbEditDataLogic":U)
          gcEnabledActions = "New,cbDelete,cbFind,Links,Properties,cbPageSequence,cbObjectLocator,Pref":U
                           + (IF lDataContainer THEN ",cbGenDataLogic":U  ELSE (IF lFrameContainer THEN ",Pages":U ELSE ",Pages,objMenuStruct,CntainerPreview":U))
                           + (IF lHasSuper      THEN ",cbEditCustomObj":U ELSE "":U)
                           + (IF lDataLogicProc THEN ",cbEditDataLogic":U ELSE "":U)
          lSuccess         = DYNAMIC-FUNCTION("setDisabledActions":U IN ghToolbar, cDisabledActions)
          lSuccess         = DYNAMIC-FUNCTION("disableActions":U     IN ghToolbar, cDisabledActions)
    ...
    WHEN "UPDATE":U THEN
      ASSIGN
          cDisabledActions = "New,cbDelete,cbFind,cbCancel,CntainerPreview":U
                           ...
                           + (IF lHasSuper      THEN "":U ELSE ",cbEditCustomObj":U)
                           + (IF lDataLogicProc THEN "":U ELSE ",cbEditDataLogic":U)
          gcEnabledActions = "cbSave,cbUndo,Links,cbObjectLocator,PageSequence,Properties,Pref":U
                           + (IF lDataContainer THEN ",cbGenDataLogic":U  ELSE (IF lFrameContainer THEN ",Pages":U ELSE ",Pages,objMenuStruct":U))
                           + (IF lHasSuper      THEN ",cbEditCustomObj":U ELSE "":U)
                           + (IF lDataLogicProc THEN ",cbEditDataLogic":U ELSE "":U)
          lSuccess         = DYNAMIC-FUNCTION("setDisabledActions":U IN ghToolbar, cDisabledActions)
          lSuccess         = DYNAMIC-FUNCTION("disableActions":U     IN ghToolbar, cDisabledActions)
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION isViewerOrStaticSO Procedure 
FUNCTION isViewerOrStaticSO RETURNS LOGICAL
...
------------------------------------------------------------------------------*/
  IF gcClassChildren = "":U THEN
    gcClassChildren = REPLACE(DYNAMIC-FUNCTION("getClassChildrenFromDB":U IN gshRepositoryManager, "Viewer,StaticSO":U), CHR(3), ",":U).
