...
/*---------------------------------------------------------------------------------
  ...
  Description:  rvc_configuration_type Data Logic Procedure Library Template

  Purpose:      A procedure library (PLIP) to support the maintenance of the rvc_configuration_type table
...
&GLOB DATA-LOGIC-TABLE rvc_configuration_type
&GLOB DATA-FIELD-DEFS  "rv\obj\rvcctfullo.i"
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN cDescription = "Dynamics rvc_configuration_type Data Logic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST rvc_configuration_type 
              WHERE rvc_configuration_type.type_table_name = b_rvc_configuration_type.type_table_name) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rvc_configuration_type.type_table_name)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'rvc_configuration_type' '' "'type_table_name, '" cValueList }.
  END.


  IF CAN-FIND(FIRST rvc_configuration_type 
              WHERE rvc_configuration_type.configuration_type = b_rvc_configuration_type.configuration_type) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rvc_configuration_type.configuration_type)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                      {af/sup2/aferrortxt.i 'AF' '8' 'rvc_configuration_type' '' "'configuration_type, '" cValueList }.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST rvc_configuration_type 
              WHERE rvc_configuration_type.type_table_name = b_rvc_configuration_type.type_table_name
                AND ROWID(rvc_configuration_type) <> TO-ROWID(ENTRY(1,b_rvc_configuration_type.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rvc_configuration_type.type_table_name)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
  ...
  IF NOT isCreate() AND CAN-FIND(FIRST rvc_configuration_type 
              WHERE rvc_configuration_type.configuration_type = b_rvc_configuration_type.configuration_type
                AND ROWID(rvc_configuration_type) <> TO-ROWID(ENTRY(1,b_rvc_configuration_type.RowIDent))) THEN
  DO:
     ASSIGN
        cValueList   = STRING(b_rvc_configuration_type.configuration_type)
...
------------------------------------------------------------------------------*/
  ...
  IF LENGTH(b_rvc_configuration_type.type_table_name) = 0 OR LENGTH(b_rvc_configuration_type.type_table_name) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rvc_configuration_type' 'type_table_name' "'Type Table Name'"}.

  IF LENGTH(b_rvc_configuration_type.type_description) = 0 OR LENGTH(b_rvc_configuration_type.type_description) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rvc_configuration_type' 'type_description' "'Type Description'"}.

  IF LENGTH(b_rvc_configuration_type.configuration_type) = 0 OR LENGTH(b_rvc_configuration_type.configuration_type) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'rvc_configuration_type' 'configuration_type' "'Configuration Type'"}.
