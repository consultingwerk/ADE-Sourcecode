...
TRIGGER PROCEDURE FOR DELETE OF rvt_action .
...
DEFINE BUFFER o_rvt_action FOR rvt_action.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = rvt_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = rvt_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtactrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = rvt_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = rvt_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtactrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = rvt_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = rvt_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtactrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rvtac":U, INPUT BUFFER rvt_action:HANDLE, INPUT BUFFER o_rvt_action:HANDLE).
