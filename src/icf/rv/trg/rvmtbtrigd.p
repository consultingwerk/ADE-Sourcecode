...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   rvm_task_object           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE rvm_task_object
&SCOPED-DEFINE TRIGGER_FLA rvmtb
&SCOPED-DEFINE TRIGGER_OBJ task_object_obj


DEFINE BUFFER lb_table FOR rvm_task_object.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR rvm_task_object.     /* Used for lock upgrades */

DEFINE BUFFER o_rvm_task_object FOR rvm_task_object.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = rvm_task_object.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = rvm_task_object.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvmtbtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = rvm_task_object.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = rvm_task_object.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvmtbtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = rvm_task_object.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = rvm_task_object.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvmtbtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rvmtb':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rvmtb":U, INPUT BUFFER rvm_task_object:HANDLE, INPUT BUFFER o_rvm_task_object:HANDLE).
