...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   rvt_deleted_item           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE rvt_deleted_item
&SCOPED-DEFINE TRIGGER_FLA rvtdi
&SCOPED-DEFINE TRIGGER_OBJ deleted_item_obj


DEFINE BUFFER lb_table FOR rvt_deleted_item.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR rvt_deleted_item.     /* Used for lock upgrades */

DEFINE BUFFER o_rvt_deleted_item FOR rvt_deleted_item.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtditrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtditrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = rvt_deleted_item.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rvtditrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rvtdi':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rvtdi":U, INPUT BUFFER rvt_deleted_item:HANDLE, INPUT BUFFER o_rvt_deleted_item:HANDLE).
