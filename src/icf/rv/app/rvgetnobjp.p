...
/*---------------------------------------------------------------------------------
  ...
  Description:  Get next object number from RVDB

  Purpose:      Get next object number from RVDB
...
ASSIGN
    piSeqObj1    = IF plIncrement THEN NEXT-VALUE(seq_obj1,RVDB) ELSE CURRENT-VALUE(seq_obj1,RVDB)
    piSeqObj2    = CURRENT-VALUE(seq_obj2,RVDB)
    piSeqSiteDiv = CURRENT-VALUE(seq_site_division,RVDB)
    piSeqSiteRev = CURRENT-VALUE(seq_site_reverse,RVDB)
    piSessnId    = CURRENT-VALUE(seq_session_id,RVDB)
    .

IF piSeqObj1 = 0 AND plIncrement
THEN
    ASSIGN
        piSeqObj2 = NEXT-VALUE(seq_obj2,RVDB).
