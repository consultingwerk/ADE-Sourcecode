...
TRIGGER PROCEDURE FOR WRITE OF ryc_smartlink_type OLD BUFFER o_ryc_smartlink_type.

/* generic trigger override include file to disable trigger if required */
{af/sup2/aftrigover.i &DB-NAME      = "ICFDB"
                      &TABLE-NAME   = "ryc_smartlink_type"
                      &TRIGGER-TYPE = "WRITE"}
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER ryc_smartlink_type:HANDLE).
...
IF NOT NEW ryc_smartlink_type AND ryc_smartlink_type.{&TRIGGER_OBJ} <> o_ryc_smartlink_type.{&TRIGGER_OBJ} THEN
    DO:
...
{icf/trg/rycsttrigw.i}

/* Update Audit Log */
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rycst':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "rycst":U, INPUT BUFFER ryc_smartlink_type:HANDLE, INPUT BUFFER o_ryc_smartlink_type:HANDLE).
