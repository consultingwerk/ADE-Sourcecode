...
TRIGGER PROCEDURE FOR DELETE OF gsc_profile_code .
...
DEFINE BUFFER o_gsc_profile_code FOR gsc_profile_code.
...
/* gsc_profile_code is used by gsm_profile_data ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST gsm_profile_data WHERE
    ...
    DO:
      ...
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "gsc_profile_code|gsm_profile_data":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpctrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpctrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_profile_code.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpctrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gscpc":U, INPUT BUFFER gsc_profile_code:HANDLE, INPUT BUFFER o_gsc_profile_code:HANDLE).
