...
/* gsm_multi_media is represented by gsc_object ON PARENT DELETE SET NULL */

&IF DEFINED(lbe_object) = 0 &THEN
  DEFINE BUFFER lbe_object FOR gsc_object.
  &GLOBAL-DEFINE lbe_object yes
&ENDIF
FOR EACH gsc_object NO-LOCK
   WHERE gsc_object.toolbar_multi_media_obj = gsm_multi_media.multi_media_obj
   ON STOP UNDO, RETURN ERROR "AF^104^gsmmmtrigd.p^update gsc_object":U:
    FIND FIRST lbe_object EXCLUSIVE-LOCK
         WHERE ROWID(lbe_object) = ROWID(gsc_object)
         NO-ERROR.
    IF AVAILABLE lbe_object THEN
      DO:
        
        ASSIGN lbe_object.toolbar_multi_media_obj = 0 .
      END.
END.
