...
TRIGGER PROCEDURE FOR DELETE OF ryc_action .
...
DEFINE BUFFER o_ryc_action FOR ryc_action.
...
/* ryc_action is included in ryc_band_action ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST ryc_band_action WHERE
    ryc_band_action.action_obj = ryc_action.action_obj) THEN
    DO:
      /* Cannot delete parent because child exists! */
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "ryc_action|ryc_band_action":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = ryc_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = ryc_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycactrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = ryc_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = ryc_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycactrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = ryc_action.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = ryc_action.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycactrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rycac":U, INPUT BUFFER ryc_action:HANDLE, INPUT BUFFER o_ryc_action:HANDLE).
