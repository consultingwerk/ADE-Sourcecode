...
/* Generated by ICF ERwin Template */
/* gsc_deploy_dataset was deployed gsm_dataset_deployment ON PARENT DELETE CASCADE */
&IF DEFINED(lbe_dataset_deployment) = 0 &THEN
  DEFINE BUFFER lbe_dataset_deployment FOR gsm_dataset_deployment.
  &GLOBAL-DEFINE lbe_dataset_deployment yes
&ENDIF
FOR EACH gsm_dataset_deployment NO-LOCK
   WHERE gsm_dataset_deployment.deploy_dataset_obj = gsc_deploy_dataset.deploy_dataset_obj
   ON STOP UNDO, RETURN ERROR "AF^104^gscddtrigd.p^delete gsm_dataset_deployment":U:
    FIND FIRST lbe_dataset_deployment EXCLUSIVE-LOCK
         WHERE ROWID(lbe_dataset_deployment) = ROWID(gsm_dataset_deployment)
         NO-ERROR.
    IF AVAILABLE lbe_dataset_deployment THEN
      DO:
        {af/sup/afvalidtrg.i &action = "DELETE" &table = "lbe_dataset_deployment"}
      END.
END.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscddtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscddtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_deploy_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscddtrigd.p^delete gsm_multi_media":U:
