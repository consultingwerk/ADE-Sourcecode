...
TRIGGER PROCEDURE FOR DELETE OF gsc_sequence .
...
/* gsc_sequence has gsc_next_sequence ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST gsc_next_sequence WHERE
    gsc_next_sequence.sequence_obj = gsc_sequence.sequence_obj) THEN
    DO:
      /* Cannot delete parent because child exists! */
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "gsc_sequence|gsc_next_sequence":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsqtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsqtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsqtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gscsq":U, INPUT BUFFER gsc_sequence:HANDLE, INPUT BUFFER o_gsc_sequence:HANDLE).
