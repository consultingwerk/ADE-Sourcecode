...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gsm_comment           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gsm_comment
&SCOPED-DEFINE TRIGGER_FLA gsmcm
&SCOPED-DEFINE TRIGGER_OBJ comment_obj


DEFINE BUFFER lb_table FOR gsm_comment.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gsm_comment.     /* Used for lock upgrades */

DEFINE BUFFER o_gsm_comment FOR gsm_comment.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_comment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_comment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmcmtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_comment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_comment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmcmtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_comment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_comment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmcmtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmcm':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmcm":U, INPUT BUFFER gsm_comment:HANDLE, INPUT BUFFER o_gsm_comment:HANDLE).
