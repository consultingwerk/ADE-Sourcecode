...
TRIGGER PROCEDURE FOR DELETE OF gsm_site_specific .
...
DEFINE BUFFER o_gsm_site_specific FOR gsm_site_specific.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsptrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsptrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_site_specific.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsptrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmsp":U, INPUT BUFFER gsm_site_specific:HANDLE, INPUT BUFFER o_gsm_site_specific:HANDLE).
