...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   ryc_subscribe           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE ryc_subscribe
&SCOPED-DEFINE TRIGGER_FLA rycsu
&SCOPED-DEFINE TRIGGER_OBJ subscribe_obj


DEFINE BUFFER lb_table FOR ryc_subscribe.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR ryc_subscribe.     /* Used for lock upgrades */

DEFINE BUFFER o_ryc_subscribe FOR ryc_subscribe.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsutrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsutrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = ryc_subscribe.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsutrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rycsu':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rycsu":U, INPUT BUFFER ryc_subscribe:HANDLE, INPUT BUFFER o_ryc_subscribe:HANDLE).
