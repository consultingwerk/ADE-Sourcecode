...
{af/sup2/aftrigover.i &DB-NAME      = "ICFDB"
                      &TABLE-NAME   = "gsm_user_allocation"
                      &TRIGGER-TYPE = "DELETE"}
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmultrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmultrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_user_allocation.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmultrigd.p^delete gsm_multi_media":U:
