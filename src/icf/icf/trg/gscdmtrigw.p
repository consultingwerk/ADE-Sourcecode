...
TRIGGER PROCEDURE FOR WRITE OF gsc_delivery_method OLD BUFFER o_gsc_delivery_method.
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER gsc_delivery_method:HANDLE).
...
IF NOT NEW gsc_delivery_method AND gsc_delivery_method.{&TRIGGER_OBJ} <> o_gsc_delivery_method.{&TRIGGER_OBJ} THEN
    DO:
...
{icf/trg/gscdmtrigw.i}

/* Update Audit Log */
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gscdm':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "gscdm":U, INPUT BUFFER gsc_delivery_method:HANDLE, INPUT BUFFER o_gsc_delivery_method:HANDLE).
