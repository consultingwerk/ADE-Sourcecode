...
TRIGGER PROCEDURE FOR DELETE OF ryc_smartobject_field .
...
DEFINE BUFFER o_ryc_smartobject_field FOR ryc_smartobject_field.
...
/* ryc_smartobject_field has ryc_custom_ui_trigger ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST ryc_custom_ui_trigger WHERE
    ...
    DO:
      ...
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "ryc_smartobject_field|ryc_custom_ui_trigger":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsftrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsftrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = ryc_smartobject_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycsftrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rycsf":U, INPUT BUFFER ryc_smartobject_field:HANDLE, INPUT BUFFER o_ryc_smartobject_field:HANDLE).
