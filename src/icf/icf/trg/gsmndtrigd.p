...
TRIGGER PROCEDURE FOR DELETE OF gsm_node .
...
/* gsm_node is the parent of gsm_node ON PARENT DELETE SET NULL */


FOR EACH lb_table NO-LOCK
   WHERE lb_table.parent_node_obj = gsm_node.node_obj
   ON STOP UNDO, RETURN ERROR "AF^104^gsmndtrigd.p^update gsm_node":U:
    ...
    IF AVAILABLE lb1_table THEN
      DO:
        ...
        ASSIGN lb1_table.parent_node_obj = 0 .
      END.
END.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_node.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_node.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmndtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_node.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_node.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmndtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_node.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_node.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmndtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmnd":U, INPUT BUFFER gsm_node:HANDLE, INPUT BUFFER o_gsm_node:HANDLE).
