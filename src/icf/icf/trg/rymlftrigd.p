...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   rym_lookup_field           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE rym_lookup_field
&SCOPED-DEFINE TRIGGER_FLA rymlf
&SCOPED-DEFINE TRIGGER_OBJ lookup_field_obj


DEFINE BUFFER lb_table FOR rym_lookup_field.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR rym_lookup_field.     /* Used for lock upgrades */

DEFINE BUFFER o_rym_lookup_field FOR rym_lookup_field.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymlftrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymlftrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = rym_lookup_field.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymlftrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rymlf':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rymlf":U, INPUT BUFFER rym_lookup_field:HANDLE, INPUT BUFFER o_rym_lookup_field:HANDLE).
