...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gsc_package_dataset           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gsc_package_dataset
&SCOPED-DEFINE TRIGGER_FLA gscpd
&SCOPED-DEFINE TRIGGER_OBJ package_dataset_obj


DEFINE BUFFER lb_table FOR gsc_package_dataset.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gsc_package_dataset.     /* Used for lock upgrades */

DEFINE BUFFER o_gsc_package_dataset FOR gsc_package_dataset.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpdtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpdtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_package_dataset.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpdtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gscpd':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gscpd":U, INPUT BUFFER gsc_package_dataset:HANDLE, INPUT BUFFER o_gsc_package_dataset:HANDLE).
