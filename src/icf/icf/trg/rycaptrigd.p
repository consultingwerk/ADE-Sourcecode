...
TRIGGER PROCEDURE FOR DELETE OF ryc_attribute_group .
...
/* ryc_attribute_group has ryc_attribute ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST ryc_attribute WHERE
    ryc_attribute.attribute_group_obj = ryc_attribute_group.attribute_group_obj) THEN
    DO:
      /* Cannot delete parent because child exists! */
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "ryc_attribute_group|ryc_attribute":U.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycaptrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycaptrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = ryc_attribute_group.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rycaptrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rycap":U, INPUT BUFFER ryc_attribute_group:HANDLE, INPUT BUFFER o_ryc_attribute_group:HANDLE).
