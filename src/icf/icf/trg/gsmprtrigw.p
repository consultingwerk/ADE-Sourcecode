...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER gsm_profile:HANDLE).
  



/* Generated by ICF ERwin Template */
/* gsm_category of gsm_profile ON CHILD UPDATE RESTRICT */
IF NEW gsm_profile OR  gsm_profile.category_obj <> o_gsm_profile.category_obj  THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsm_category WHERE
        gsm_profile.category_obj = gsm_category.category_obj)) THEN
              DO:
                /* Cannot update child because parent does not exist ! */
                ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 103 lv-include = "gsm_profile|gsm_category":U.
...
IF NOT NEW gsm_profile AND gsm_profile.{&TRIGGER_OBJ} <> o_gsm_profile.{&TRIGGER_OBJ} THEN
    DO:
...
{icf/trg/gsmprtrigw.i}

/* Update Audit Log */
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmpr':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "gsmpr":U, INPUT BUFFER gsm_profile:HANDLE, INPUT BUFFER o_gsm_profile:HANDLE).
