...
DEFINE BUFFER o_gsc_next_sequence FOR gsc_next_sequence.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsntrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsntrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_next_sequence.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscsntrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gscsn":U, INPUT BUFFER gsc_next_sequence:HANDLE, INPUT BUFFER o_gsc_next_sequence:HANDLE).
