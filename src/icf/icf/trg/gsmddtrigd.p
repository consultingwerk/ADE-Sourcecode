...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gsm_dataset_deployment           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gsm_dataset_deployment
&SCOPED-DEFINE TRIGGER_FLA gsmdd
&SCOPED-DEFINE TRIGGER_OBJ dataset_deployment_obj


DEFINE BUFFER lb_table FOR gsm_dataset_deployment.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gsm_dataset_deployment.     /* Used for lock upgrades */

DEFINE BUFFER o_gsm_dataset_deployment FOR gsm_dataset_deployment.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmddtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmddtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_dataset_deployment.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmddtrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmdd':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmdd":U, INPUT BUFFER gsm_dataset_deployment:HANDLE, INPUT BUFFER o_gsm_dataset_deployment:HANDLE).
