...
/* Generated by ICF ERwin Template */
/* gsc_product_module  gsc_object ON PARENT DELETE RESTRICT */
IF CAN-FIND(FIRST gsc_object WHERE
    gsc_object.product_module_obj = gsc_product_module.product_module_obj) THEN
    DO:
      /* Cannot delete parent because child exists! */
      ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 101 lv-include = "gsc_product_module|gsc_object":U.
      RUN error-message (lv-errgrp, lv-errnum, lv-include).
    END.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsc_product_module.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsc_product_module.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpmtrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsc_product_module.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsc_product_module.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpmtrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsc_product_module.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsc_product_module.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gscpmtrigd.p^delete gsm_multi_media":U:
