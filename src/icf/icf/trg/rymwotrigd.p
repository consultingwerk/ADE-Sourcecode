...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   rym_wizard_objc           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE rym_wizard_objc
&SCOPED-DEFINE TRIGGER_FLA rymwo
&SCOPED-DEFINE TRIGGER_OBJ wizard_objc_obj


DEFINE BUFFER lb_table FOR rym_wizard_objc.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR rym_wizard_objc.     /* Used for lock upgrades */

DEFINE BUFFER o_rym_wizard_objc FOR rym_wizard_objc.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymwotrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymwotrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = rym_wizard_objc.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^rymwotrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'rymwo':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "rymwo":U, INPUT BUFFER rym_wizard_objc:HANDLE, INPUT BUFFER o_rym_wizard_objc:HANDLE).
