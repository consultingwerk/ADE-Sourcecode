...
TRIGGER PROCEDURE FOR WRITE OF gsm_session_type OLD BUFFER o_gsm_session_type.

/* generic trigger override include file to disable trigger if required */
{af/sup2/aftrigover.i &DB-NAME      = "ICFDB"
                      &TABLE-NAME   = "gsm_session_type"
                      &TRIGGER-TYPE = "WRITE"}
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER gsm_session_type:HANDLE).
...
IF NOT NEW gsm_session_type AND gsm_session_type.{&TRIGGER_OBJ} <> o_gsm_session_type.{&TRIGGER_OBJ} THEN
    DO:
...
{icf/trg/gsmsetrigw.i}

/* Update Audit Log */
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmse':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "gsmse":U, INPUT BUFFER gsm_session_type:HANDLE, INPUT BUFFER o_gsm_session_type:HANDLE).
