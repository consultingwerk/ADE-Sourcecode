...
IF CAN-FIND(FIRST gsc_entity_mnemonic
  ...
  RUN af/app/afpropfrmp.p (INPUT BUFFER gsc_language:HANDLE).
...
IF NOT NEW gsc_language AND gsc_language.{&TRIGGER_OBJ} <> o_gsc_language.{&TRIGGER_OBJ} THEN
    DO:
...
{icf/trg/gsclgtrigw.i}

/* Update Audit Log */
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsclg':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "WRITE":U, INPUT "gsclg":U, INPUT BUFFER gsc_language:HANDLE, INPUT BUFFER o_gsc_language:HANDLE).
