...
TRIGGER PROCEDURE FOR DELETE OF gst_trigger_override .
...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gst_trigger_override           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gst_trigger_override
&SCOPED-DEFINE TRIGGER_FLA gstto
&SCOPED-DEFINE TRIGGER_OBJ trigger_override_obj


DEFINE BUFFER lb_table FOR gst_trigger_override.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gst_trigger_override.     /* Used for lock upgrades */

DEFINE BUFFER o_gst_trigger_override FOR gst_trigger_override.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsttotrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsttotrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gst_trigger_override.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsttotrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gstto':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gstto":U, INPUT BUFFER gst_trigger_override:HANDLE, INPUT BUFFER o_gst_trigger_override:HANDLE).
