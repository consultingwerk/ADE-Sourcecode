...
/* Created automatically using ERwin ICF Trigger template db/af/erw/afercustrg.i
   ...
   gsm_security_structure           Expands to full table name, e.g. gsm_user
...
&SCOPED-DEFINE TRIGGER_TABLE gsm_security_structure
&SCOPED-DEFINE TRIGGER_FLA gsmss
&SCOPED-DEFINE TRIGGER_OBJ security_structure_obj


DEFINE BUFFER lb_table FOR gsm_security_structure.      /* Used for recursive relationships */
DEFINE BUFFER lb1_table FOR gsm_security_structure.     /* Used for lock upgrades */

DEFINE BUFFER o_gsm_security_structure FOR gsm_security_structure.
...
IF CAN-FIND(FIRST lbx_gsm_comment 
            WHERE lbx_gsm_comment.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_comment NO-LOCK
       WHERE lbx_gsm_comment.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsstrigd.p^delete gsm_comment":U:
...
IF CAN-FIND(FIRST lbx_gsm_user_allocation 
            WHERE lbx_gsm_user_allocation.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_user_allocation NO-LOCK
       WHERE lbx_gsm_user_allocation.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsstrigd.p^delete gsm_user_allocation":U:
...
IF CAN-FIND(FIRST lbx_gsm_multi_media 
            WHERE lbx_gsm_multi_media.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}) THEN
    FOR EACH lbx_gsm_multi_media NO-LOCK
       WHERE lbx_gsm_multi_media.owning_obj = gsm_security_structure.{&TRIGGER_OBJ}
       ON STOP UNDO, RETURN ERROR "AF^104^gsmsstrigd.p^delete gsm_multi_media":U:
...
IF CAN-FIND(FIRST gsc_entity_mnemonic
            WHERE gsc_entity_mnemonic.entity_mnemonic = 'gsmss':U
              AND gsc_entity_mnemonic.auditing_enabled = YES) THEN
  RUN af/app/afauditlgp.p (INPUT "DELETE":U, INPUT "gsmss":U, INPUT BUFFER gsm_security_structure:HANDLE, INPUT BUFFER o_gsm_security_structure:HANDLE).
