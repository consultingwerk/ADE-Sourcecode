...
/* Generated by ICF ERwin Template */
/* gsc_object is a ryc_smartobject ON CHILD INSERT RESTRICT */
IF 
    ( ryc_smartobject.object_obj <> 0 ) THEN
  DO:
    IF NOT(CAN-FIND(FIRST gsc_object WHERE
        ryc_smartobject.object_obj = gsc_object.object_obj)) THEN
        DO:
          /* Cannot create child because parent does not exist ! */
          ASSIGN lv-error = YES lv-errgrp = "AF ":U lv-errnum = 102 lv-include = "ryc_smartobject|gsc_object":U.
          RUN error-message (lv-errgrp, lv-errnum, lv-include).
        END.
...
ASSIGN ryc_smartobject.{&TRIGGER_OBJ} = getNextObj().
