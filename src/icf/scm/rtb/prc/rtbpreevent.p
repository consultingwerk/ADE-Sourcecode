...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE cCompileModules         AS CHARACTER  NO-UNDO.
  DEFINE VARIABLE iCompileLoop            AS INTEGER    NO-UNDO.
  
  

  cFullName = findFile(ipFilename).

  IF cFullName <> "":U
  THEN DO:
    /* Step 4 - Do a forced compile of the DCU Code to make sure we are always running the latest code. 
                This is being done with the selective compile API for RTB - as it let's RTb do the work 
                of compiling, saving r-code etc.
    */
    IF VALID-HANDLE(gshSessionManager) THEN DO:
      cMessage = "Before running the DCU it is necessary to compile all of the objects in the DCU and RTB modules." + "~n":U + 
      ...
      RUN showMessages IN gshSessionManager (INPUT  cMessage,    /* message to display */
                                             ...
                                             INPUT  "DCU Compilation":U,             /* Message window title */
  ...
  /* Do a forced selective compile of all of the objects in the specified modules */
    /* - *DCU* : The DCU specific modules */
    /* - *RTB* : The RTB and SCMRTB specific modules */ 
    ASSIGN
      cCompileModules = "*dcu*,*rtb*".

    DO iCompileLoop = 1 TO NUM-ENTRIES(cCompileModules,","):  
      /* Do a forced selective compile of all of the objects in the *DCU* modules */
      ...
      RUN selective_compile IN hRtbApi
                           ...
                           INPUT ENTRY(iCompileLoop, cCompileModules),            /* Pmodule    - module to select objects from or MATCHES pattern */
    ...
    */
    ...
    AND NOT (VALID-HANDLE(hRtbevntp))
    :
      IF hHandle:PRIVATE-DATA = "ryrtbevntp.p":U
      THEN
        ASSIGN
          hRtbevntp = hHandle.
      ASSIGN
    ...
    END.
    IF VALID-HANDLE(hRtbevntp)
    THEN
      RUN killPlip IN hRtbevntp.
    ...
    /* Step 7 */
    ASSIGN
      lOverrideCreated = deleteFile(ipFilename).
    ...
    */
    ...
    IF VALID-HANDLE(gshSessionManager) THEN DO:
      ASSIGN
        cMesWinTitle            = "Post Import Selective Compile (Dynamics Enhancement)":U
        cMesWinButtonList       = "Compile Now,Compile Later":U
        cMesWinButtonDefault    = "Compile Now":U
        cMesWinButtonCancel     = "Compile Later":U
        cMesWinAnswerValue      = "":U
        cMesWinAnswerDataType   = "":U
        cMesWinAnswerFormat     = "":U
        .
  
      /* Step 9 - Re-compile Workspace. 
         This has to be done after the framework has been restarted, as database aliases and other 
         settings need to be made before a compile is started. 
         
         Ask the user first if they want to do this ? If not, they can do the compile themselves later.
      */
      
      ASSIGN 
         cMesWinMessage = "The import procesing has completed successfully. " + "~n":U + 
                     "Do you want to do a Roundtable Selective Compile for this workspace ? " + "~n":U + "~n":U + 
                     "If you choose to do so, all objects that require compilation because they have changed " + 
                     "or because they are affected by changes to objects in the workspace will be compiled." + "~n":U  + "~n":U + 
                     "NOTE: The compile processing will not show any output during the compilation.  " + "~n":U + "~n":U + 
                     "If you do not want to run the compile now or wish to have the status displayed during " +
                     "compilation, manually start the Selective Compile from Roundtable later.".                   
      ...
      IF cMesWinButtonPressed = "Compile Now" THEN 
      ASSIGN 
        lCompile = TRUE.     
    END. /* IF VALID-HANDLE() ...*/
    ...
    IF lCompile THEN  DO:    
      ...
      PUBLISH "logEvent" (INPUT "IMPORT PRE-EVENT", 
                          INPUT "[":U + STRING(TIME , "HH:MM:SS") + "] Post Import Selective Compile." + "~n":U, 
                          INPUT "":U, 
                          INPUT Grtb-wspace-id, 
                          INPUT Grtb-userid, 
                          INPUT "LOG,STATUS":U,
      ...
      RUN selective_compile IN hRtbApi
                           (INPUT 0,               /* Ptask-num  - task number to select objects from or 0 to ignore tasks */
                           INPUT "*":U,            /* Pmodule    - module to select objects from or MATCHES pattern */
                           INPUT "*":U,            /* Pobj-group - Object group to select objects from or MATCHES pattern */
                           INPUT "PCODE":U,       /* Pobj-type  - Object types to select (PCODE, PFILE, etc.) or MATCHES pattern */
                           INPUT "*":U,           /* Pobject    - Object to select or MATCHES pattern */
                           INPUT NO,              /* Pforce     - Logical YES to force compile of (current) objects */
                           INPUT NO,              /* Plistings  - Logical YES to generate compile listings */
                           INPUT YES,              /* Pxref      - Logical YES to perform xref of selected object(s) */
                           OUTPUT cCompileError  /* Perror     - This will be non-blank if the process could no be performed */
                           ).
      ...
      */
      IF cCompileError <> "":U THEN DO:
        ...
        */
        ...
        IF VALID-HANDLE(gshSessionManager) THEN DO:
         ASSIGN
            cMesWinTitle            = "Selective Compile Errors (Dynamics Enhancement)":U
            cMesWinButtonList       = "OK,Cancel":U
            cMesWinButtonDefault    = "OK":U
            cMesWinButtonCancel     = "Cancel":U
            cMesWinAnswerValue      = "":U
            cMesWinAnswerDataType   = "":U
            cMesWinAnswerFormat     = "":U
            cMesWinMessage          = "There were compilation errors / warnings during the compile." + "~n":U + 
                                      "Do you want to see the errors ? ". 
          ...
          IF cMesWinButtonPressed = "OK":U THEN        
          ASSIGN 
            lViewCompileErrors = TRUE.
          ELSE 
            lViewCompileErrors = FALSE.   
        END. /* IF VALID-HANDLE ... */
        
        IF lViewcompileErrors THEN
        RUN rtb/p/rtb_rnpw.p (INPUT ENTRY(2,cCompileError), INPUT 0).                       
      END.
    END. /* if Compile now ...*/
  ...
  PUBLISH "logEvent" (INPUT "IMPORT PRE-EVENT":U, 
                      INPUT cMessage, 
                      INPUT "":U, 
                      INPUT Grtb-wspace-id, 
                      INPUT Grtb-userid, 
...
------------------------------------------------------------------------------*/
  ...
  IF CAN-FIND(FIRST rtb_import NO-LOCK
              WHERE rtb_import.wspace-id  = grtb-wspace-id
              AND   rtb_import.obj-type   = "PCODE"
              AND   rtb_import.object     = "icfsetup.xml":U
             )
  THEN ASSIGN lUpgrade = YES.
  ELSE ASSIGN lUpgrade = NO.

  IF lUpgrade AND VALID-HANDLE(gshSessionManager)
  THEN DO:
      ASSIGN
        ...
        cMesWinAnswerDataType   = "":U
        cMesWinAnswerFormat     = "":U
        cMesWinMessage          = "The IMPORT contains object(s) that are used by the Progress Dynamics Configuration Utility (DCU)." + 
  ...
  THEN DO:
    ...
    RUN showMessages IN gshSessionManager (INPUT  cMesWinMessage,    /* message to display */
                                           INPUT  "INF":U,     /* error type */
                                           INPUT  "OK":U,    /* button list */
                                           INPUT  "OK":U,    /* default button */ 
                                           INPUT  "":U,       /* cancel button */
                                           INPUT  "Roundtable / DCU Extensions",             /* Message window title */
                                           INPUT  NO,              /* display if empty */ 
                                           INPUT  ?,                /* container handle */ 
                                           OUTPUT cMesWinButtonPressed           /* button pressed */
                                          ).
    ...
    IF cMesWinButtonPressed = "YES":U
    THEN
      ASSIGN
        opErrorMessage = "":U.
    ELSE DO:
      ASSIGN
        opErrorMessage = "Cancelled":U.
      RETURN.
    END.
  ...
  THEN DO:
    /* Step 2 */
    ASSIGN
      lOverrideCreated = createFile(ipFilename).
  ...
  ASSIGN 
    cMessage = "[":U + STRING(TIME, "HH:MM:SS":U) + "] Running RTB / DCU Integration Upgrade : " + STRING(lUpgrade) + ".":U + "~n":U
    .
...
------------------------------------------------------------------------------*/
 ...
 /* Set default properties if they have not been passed in */
  ...
  IF pcLogFile = "":U THEN
    ASSIGN 
      pcLogFile = "dynrtb_log.log"
      .
      
  IF pcLogOutput = "":U THEN
    ASSIGN 
      pcLogOutput = "MESSAGE":U
      .
  ...
  IF LOOKUP("MESSAGE":U, pcLogOutPut) > 0 THEN DO:
    IF VALID-HANDLE(gshSessionManager) THEN DO:
      RUN showMessages IN gshSessionManager (INPUT  pcEventMessage,    /* message to display */
                                             INPUT  "INF":U,          /* error type */
                                             INPUT  "&OK":U,    /* button list */
                                             INPUT  "&OK":U,           /* default button */ 
                                             INPUT  "":U,       /* cancel button */
...
------------------------------------------------------------------------------*/
  ...
  CASE cCheckType:
    WHEN "VALIDATE":U
      THEN DO:
        IF SEARCH(cFullFileName) <> ?
        THEN DO:
          ASSIGN
            opReturnMessage = "FOUND":U.
        END.
        ELSE
          ASSIGN
            opReturnMessage = "":U.
...
------------------------------------------------------------------------------*/
  ...
  IF opScmErrorMessage = "":U
  THEN
    ASSIGN
      opScmErrorMessage = cFullName.
