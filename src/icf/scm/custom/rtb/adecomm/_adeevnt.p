...
IF p_event = "Startup" AND (p_product = "UIB" OR p_product = "Editor") THEN DO:
  ...
  IF SESSION:FIRST-CHILD <> ? THEN DO:
    ASSIGN
       Mwindow-handle = SESSION:FIRST-CHILD.
    /* --- add menu item "Roundtable" to the tools menu --- */
    RUN rtb/p/rtb_addm.p (INPUT Mwindow-handle).  
  END.  /* if session:first-child <> ? */
...
IF p_event = "Shutdown"
  ...
  /* We need to check if the Desktop is not running before we can attempt
     ...
     More investigation and ADETOOL changes required.
  */
     )
THEN DO:
