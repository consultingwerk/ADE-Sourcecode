...
THEN DO:
  ...
  THEN DO iLoopNum = 1 TO iLoopFields:
    ...
    THEN DO:
      ...
      IF LOOKUP(cBufferFieldName,cKeyFieldValue) > 0 THEN
        ASSIGN
          ENTRY(LOOKUP(cBufferFieldName,cKeyFieldValue),cKeyFieldValue) = STRING(hNewBufferField:BUFFER-VALUE).
    END.
    ELSE
      ASSIGN
        hNewBufferField = ?
    ...
    THEN DO:
      ...
      IF LOOKUP(cBufferFieldName,cKeyFieldValue) > 0 THEN
        ASSIGN
          ENTRY(LOOKUP(cBufferFieldName,cKeyFieldValue),cKeyFieldValue) = STRING(hOldBufferField:BUFFER-VALUE).
    END.
    ELSE
      ASSIGN
        hOldBufferField = ?
