...
------------------------------------------------------------------------------*/
    ...
    FOR EACH gsc_product_module NO-LOCK:
        ...
        IF VALID-HANDLE(ghScmTool) THEN DO:
           RUN scmGetModuleDir IN ghScmtool (INPUT gsc_product_module.product_module_code, OUTPUT cScmModulePath).
           ASSIGN 
        ...
        DO iProdModLoop = 1 TO NUM-ENTRIES("{&REPOSITORY-MODULES}":U):
          cPrMod = TRIM(ENTRY(iProdModLoop,"{&REPOSITORY-MODULES}":U)).
          IF cPrMod = "":U THEN
            NEXT.
          IF gsc_product_module.product_module_code BEGINS cPrMod THEN
            ASSIGN ttHeaderInfo.tIsRepository = YES.
        END.
