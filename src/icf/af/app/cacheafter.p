...
THEN DO:
    ...
    RUN af/app/afgetseccp.p (OUTPUT TABLE ttSecurityControl).
    ASSIGN phSecurityData = TEMP-TABLE ttSecurityControl:HANDLE.

    /* Get translation data */

    RUN af/app/afbldtrncp.p (INPUT pdLanguageObj,
                             OUTPUT TABLE ttTranslation).
    ASSIGN phTranslationData = TEMP-TABLE ttTranslation:HANDLE.

    /* Extract result code stuff */

    RUN ry/app/rycusfapip.p (INPUT pcCustTypesPrioritised, OUTPUT pcTypeAPI) NO-ERROR.
    ...
    RUN ry/app/rycusrr2rp.p (INPUT        pcCustTypesPrioritised,
                             INPUT        pcTypeAPI,
