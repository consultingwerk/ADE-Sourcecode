...
DEFINE VARIABLE hDDOProc AS HANDLE     NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  RUN startProcedure IN THIS-PROCEDURE ("ONCE|af/app/afddo.p":U, OUTPUT hDDOProc) NO-ERROR.

  /* If the handle to hDDOProc is invalid, then start it. */
  IF NOT VALID-HANDLE(hDDOProc) THEN
    RUN "af/app/afddo.p":U PERSISTENT SET hDDOProc.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION:
    ...
    lAns = DYNAMIC-FUNCTION("findUnique":U IN hDDOProc,
                             hbSTTable,
                             hbSTTable:NAME + ".cServiceName = '":U + cServiceName + "'":U,
                             "EXCLUSIVE-LOCK":U,
                             NO).
    ...
    lAns = DYNAMIC-FUNCTION("findCurrent":U IN hDDOProc,
                             hbSTTable,
                             "NO-LOCK":U,
                             NO).
  END.
...
------------------------------------------------------------------------------*/
  ...
  lAns = DYNAMIC-FUNCTION("findUnique":U IN hDDOProc,
                           hbSTTable,
                           hbSTTable:NAME + ".cServiceName = '":U + pcServiceName + "'":U,
                           "":U,
                           NO).
...
------------------------------------------------------------------------------*/
  ...
  /* Open a query */
  hQuery = DYNAMIC-FUNCTION("createQuery":U IN hDDOProc,
                            STRING(hServiceType),
                            "FOR EACH ":U + hServiceType:NAME + " NO-LOCK":U).
  ...
  hQuery:QUERY-CLOSE().
  DYNAMIC-FUNCTION("deleteQuery":U IN hDDOProc, hQuery, YES).
