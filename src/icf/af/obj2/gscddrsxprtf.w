...
------------------------------------------------------------------------------*/
  ...
  IF pcFilter <> "":U THEN
    cForEach = REPLACE(gcEntityForEach, "  ":U, " WHERE ":U + pcFilter + " ":U).
  ELSE
    cForEach = gcEntityForEach.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    openQuery(ghEntityBuffer, INPUT-OUTPUT ghQryAvailable, gcEntityForEach).
...
------------------------------------------------------------------------------*/
  ...
  gcEntityForEach = "FOR EACH ":U + ghEntityBuffer:NAME + " NO-LOCK":U.
  IF gcDSKey <> "":U THEN
  DO:
    gcEntityForEach = gcEntityForEach + "  ":U.
