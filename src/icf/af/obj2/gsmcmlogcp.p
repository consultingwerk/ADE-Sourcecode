...
------------------------------------------------------------------------------*/
  ...
  AND LOOKUP("setUserProperty":U, ghDataSource:INTERNAL-ENTRIES) <> 0 THEN
      DYNAMIC-FUNCTION("setUserProperty":U IN ghDatasource, "CommentsContainer":U, "":U).
...
------------------------------------------------------------------------------*/
  ...
  IF LENGTH(b_gsm_comment.owning_reference) = 0 OR LENGTH(b_gsm_comment.owning_reference) = ? THEN
    ASSIGN
      cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
                    {af/sup2/aferrortxt.i 'AF' '1' 'gsm_comment' 'owning_reference' "'Owning Reference'"}.
...
------------------------------------------------------------------------------*/
  ...
  DO:
    ...
    RUN dataAvailable IN ghDataSource (INPUT "DIFFERENT":U).
