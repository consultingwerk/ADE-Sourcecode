...
&Scoped-define INTERNAL-TABLES gsm_menu_item
...
image_alternate_rule item_narration 
...
{&DB-REQUIRED-START}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH gsm_menu_item NO-LOCK INDEXED-REPOSITION.
{&DB-REQUIRED-END}
&Scoped-define TABLES-IN-QUERY-Query-Main gsm_menu_item
...
"item_category_obj" "item_category_obj" ? "->>>>>>>>>>>>>>>>>9.999999999" "character" ? ? ? ? ? ? yes ? no 29 yes
...
"menu_item_obj" "menu_item_obj" ? "->>>>>>>>>>>>>>>>>9.999999999" "decimal" ? ? ? ? ? ? no ? no 33.6 yes
...
FOR EACH rowObjUpd WHERE CAN-DO("A,C,U":U, rowObjUpd.RowMod):
  IF rowObjUpd.menu_item_reference = "AutoGenerate":U THEN
  DO:
  ...
  DO:
     ...
     ASSIGN  RowObjUpd.Item_select_type       = ""    
             ...
             RowObjUpd.item_toolbar_label     = IF RowObjUpd.item_toolbar_label = "Label":U
                                                THEN RowObjUpd.item_toolbar_label
                                                ELSE ""
             RowObjUpd.tooltip_text           = IF RowObjUpd.item_toolbar_label = "Label":U
                                                THEN RowObjUpd.tooltip_text
                                                ELSE ""
             RowObjUpd.ITEM_control_style     = ""
             RowObjUpd.OBJECT_obj             = 0
             RowObjUpd.instance_attribute_obj = 0
             RowObjUpd.image1_up_filename     = ""
             RowObjUpd.image2_up_filename     = ""
             NO-ERROR.
     IF RowObjUpd.item_control_type = "Separator":U  THEN
...
FOR EACH RowObjUpd WHERE CAN-DO('A,C,U':U,RowObjUpd.RowMod): 
  ...
  IF rowObjUpd.object_obj > 0 OR rowObjUpd.object_obj = ? THEN
  DO:
     IF NOT CAN-FIND(FIRST gsc_object
                     WHERE gsc_object.object_obj = rowObjUpd.object_obj)  THEN
      ASSIGN
        cValueList   = STRING(RowObjUpd.ITEM_category_obj)
        cMessageList = cMessageList + (IF NUM-ENTRIES(cMessageList,CHR(3)) > 0 THEN CHR(3) ELSE '':U) + 
