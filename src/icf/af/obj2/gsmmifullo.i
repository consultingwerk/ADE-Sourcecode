  ...
  FIELD instance_attribute_obj LIKE gsm_menu_item.instance_attribute_obj VALIDATE  FORMAT "->>>>>>>>>>>>>>>>>9.999999999"~
  FIELD item_category_obj LIKE gsm_menu_item.item_category_obj VALIDATE  FORMAT "->>>>>>>>>>>>>>>>>9.999999999"~
  ...
  FIELD menu_item_obj LIKE gsm_menu_item.menu_item_obj VALIDATE  FORMAT "->>>>>>>>>>>>>>>>>9.999999999"~
  FIELD menu_item_reference LIKE gsm_menu_item.menu_item_reference VALIDATE  LABEL "Item Reference*"~
  FIELD object_obj LIKE gsm_menu_item.object_obj VALIDATE  FORMAT "->>>>>>>>>>>>>>>>>9.999999999"~
  FIELD on_create_publish_event LIKE gsm_menu_item.on_create_publish_event VALIDATE  LABEL "Create Event"~
  FIELD product_module_obj LIKE gsm_menu_item.product_module_obj VALIDATE  FORMAT "->>>>>>>>>>>>>>>>>9.999999999"~
