  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN constructObject (
             ...
             INPUT  FRAME frMain:HANDLE ,
             INPUT  'DisplayedFieldgsc_product_module.product_module_code,gsc_product_module.product_module_descriptionKeyFieldgsc_product_module.product_module_codeFieldLabelProduct ModuleFieldTooltipSelect the preferred Product Module for Entity ObjectsKeyFormatX(10)KeyDatatypecharacterDisplayFormatX(10)DisplayDatatypecharacterBaseQueryStringFOR EACH gsc_product_module
                     WHERE [EXCLUDE_REPOSITORY_PRODUCT_MODULES] NO-LOCK BY gsc_product_module.product_module_code INDEXED-REPOSITIONQueryTablesgsc_product_moduleSDFFileNamedcSDOProdModSDFTemplateParentFieldParentFilterQueryDescSubstitute&1 / &2ComboDelimiterListItemPairsInnerLines5ComboFlagFlagValueBuildSequence1SecurednoCustomSuperProcPhysicalTableNamesTempTablesQueryBuilderJoinCodeQueryBuilderOptionListQueryBuilderOrderListQueryBuilderTableOptionListQueryBuilderTuneOptionsQueryBuilderWhereClausesFieldNameEntityObjectProductModuleDisplayFieldyesEnableFieldyesHideOnInitnoDisableOnInitnoObjectLayout':U ,
             OUTPUT h_dyncombo ).
...
------------------------------------------------------------------------------*/
    ...
    DYNAMIC-FUNCTION("cacheObjectOnClient":U IN gshRepositoryManager,
                     INPUT rowObject.entity_mnemonic_description:INPUT-VALUE IN FRAME {&FRAME-NAME}, INPUT ?, INPUT ?, INPUT NO).
    ...
    DYNAMIC-FUNCTION("setUserProperty":U IN TARGET-PROCEDURE,
                     ...
                     INPUT STRING(hObjectBuffer:AVAILABLE) ).

    IF hObjectBuffer:AVAILABLE THEN
    DO WITH FRAME {&FRAME-NAME}:
        DYNAMIC-FUNCTION("setWidgetState":U IN TARGET-PROCEDURE, INPUT NO).

        ASSIGN rowObject.EntityObjectClass:SCREEN-VALUE = hObjectBuffer:BUFFER-FIELD("tClassName":U):BUFFER-VALUE NO-ERROR.
        ...
        IF VALID-HANDLE(hEntityClassCombo) THEN
            DYNAMIC-FUNCTION("setDataValue":U IN hEntityClassCombo,
                             INPUT hObjectBuffer:BUFFER-FIELD("tProductModuleCode":U):BUFFER-VALUE).
    END.    /* not yet created. */
    ELSE
        DYNAMIC-FUNCTION("setWidgetState":U IN TARGET-PROCEDURE, INPUT YES).    
