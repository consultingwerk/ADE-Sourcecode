...
&Scoped-Define ENABLED-OBJECTS buCancel buGenerate toGenerateDataObjects ~
...
------------------------------------------------------------------------------*/
  ...
  cMessage = "The following Data Object(s) already exist:~n~n" + cSDONames + "~n~n":U
           + "Do you wish to override these Data Objects?":U.

  RUN askQuestion IN gshSessionManager ( INPUT cMessage,          /* messages */
                                         INPUT "&Yes,&No":U,      /* button list */
                                         INPUT "&No":U,           /* default */
                                         INPUT "&No":U,           /* cancel */
                                         INPUT "Override Data Objects":U, /* title */
                                         INPUT "":U,              /* datatype */
                                         INPUT "":U,              /* format */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    THEN DO:
      RUN showMessages IN gshSessionManager
                      (INPUT  "At least one table or object should be selected."    /* message to display */
                      ,INPUT  "ERR"                                                 /* error type         */
                      ,INPUT  "&OK"                                                 /* button list        */
                      ,INPUT  "&OK"                                                 /* default button     */ 
                      ,INPUT  "&OK"                                                 /* cancel button      */
    ...
    ELSE DO:
      ...
      DO:
        cMessage = "You have chosen not to generate Data Fields. However, certain object types like SDO's and Viewers require data fields to function. "
                 + "If you are sure the data fields exist, continue with the object generation." + CHR(10) + CHR(10)
                 + "Would you like to continue with the object generation?":U.
        
        RUN askQuestion IN gshSessionManager ( INPUT cMessage,          /* messages */
                                               INPUT "&Yes,&No":U,      /* button list */
                                               INPUT "&No":U,           /* default */
                                               INPUT "&No":U,           /* cancel */
                                               INPUT "Continue object generation":U, /* title */
                                               INPUT "":U,              /* datatype */
                                               INPUT "":U,              /* format */
                                               INPUT-OUTPUT cAnswer,    /* answer */
                                               OUTPUT cButtonPressed).  /* button pressed */
          ...
          THEN DO:
            ASSIGN
              cValidationError = cValidationError
                               + (IF cValidationError <> "":U THEN CHR(10) ELSE "":U)
                               + "  TABLE : ":U  + cTTName
                               + "  ENTITY : ":U + cTTEntity
                               + CHR(10)
                               + "Please do an Entity Import for table " + cTTName + " or add it manually from the Entity Control Maintenance Suite.  "
                               + "The entity information is needed to generate the required objects correctly.".
             NEXT blkBrowseRows.
          END.
        END.
        ELSE DO:
    ...
    THEN DO:
      ...
      THEN DO:
        ASSIGN
          cValidationError = cValidationError + CHR(10)
                           + CHR(10)
  ...
  */
  ...
  IF VALID-HANDLE(gshSessionManager) THEN
    RUN showMessages IN gshSessionManager (INPUT IF lErrorOccured = NO
                                                 THEN "Object generation completed successfully."
                                                 ELSE "Object generation completed with errors.",
                                           INPUT "MES":U,
                                           INPUT "OK",
                                           INPUT "OK",
                                           INPUT "OK",
...
------------------------------------------------------------------------------*/
    ...
    IF cProfileData EQ "":U OR cProfileData EQ ? THEN
        ASSIGN cProfileData = STRING(FALSE). /* Default to NO */
            ...
            DO:
                IF VALID-HANDLE(ghScmTool) THEN
                    RUN scmModuleInWorkspace IN ghScmTool ( INPUT  ttHeaderInfo.tName,
                                                            ...
                                                            OUTPUT ttHeaderInfo.tDisplayRecord ).
                ELSE
                    ASSIGN ttHeaderInfo.tDisplayRecord = YES.
...
------------------------------------------------------------------------------*/
  ...
  OR cBrowseSuffix = "":U
  THEN
    RETURN "A suffix must be specified for the Browse".

  IF iBrowseNumFields = ?
  OR iBrowseNumFields = 0
  THEN
    RETURN "The number of fields in the Browse must be greater than 0".
...
------------------------------------------------------------------------------*/
  ...
  OR cSdoSuffix = "":U
  THEN
    RETURN "A suffix must be specified for the DataObject".

  IF cLogicProcedureSuffix = ?
  OR cLogicProcedureSuffix = "":U
  THEN
    RETURN "A suffix must be specified for the DataLogic Procedure".
...
------------------------------------------------------------------------------*/
  ...
  OR cViewerSuffix = "":U
  THEN
    RETURN "A suffix must be specified for the Viewer".

  IF iViewerNumFields = ?
  OR iViewerNumFields = 0
  THEN
    RETURN "The number of fields in the Viewer must be greater than 0".

  IF iViewerFieldsPerColumn = ?
  OR iViewerFieldsPerColumn = 0
  THEN
    RETURN "The number of fields per column in the Viewer must be greater than 0".
