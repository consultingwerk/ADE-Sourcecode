...
------------------------------------------------------------------------------*/
  ...
  IF gcMode = "View":U THEN DO:
    ...
    IF VALID-HANDLE(h_connectParam) AND
       LOOKUP("disableField":U,h_connectParam:INTERNAL-ENTRIES) > 0 THEN
      RUN disableField IN h_connectParam.
  END.
  ELSE DO:
    RUN enableFields.
    IF VALID-HANDLE(h_connectParam) AND
       LOOKUP("enableField":U,h_connectParam:INTERNAL-ENTRIES) > 0 THEN
