...
&Scoped-Define DISPLAYED-OBJECTS fiMenuType fiBandDesc fiModule 
...
DEFINE FRAME frMain
     fiMenuType AT ROW 1.95 COL 74 COLON-ALIGNED
     fiBandDesc AT ROW 3 COL 23 COLON-ALIGNED NO-LABEL
     RowObject.menu_structure_sequence AT ROW 4.33 COL 23 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9 BY 1
     RowObject.menu_structure_spacing AT ROW 5.52 COL 23.2 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9 BY 1
     RowObject.menu_structure_alignment AT ROW 6.71 COL 9
          VIEW-AS COMBO-BOX 
          LIST-ITEMS "LEFT","RIGHT","CENTER" 
          DROP-DOWN-LIST
          SIZE 17 BY 1
     RowObject.insert_rule AT ROW 8.14 COL 25
          LABEL "Insert separator before band"
          VIEW-AS TOGGLE-BOX
          SIZE 32 BY .81
     fiModule AT ROW 10.29 COL 73 COLON-ALIGNED NO-LABEL BLANK 
     RowObject.toolbar_menu_structure_obj AT ROW 11.48 COL 62 COLON-ALIGNED
           VIEW-AS TEXT 
          SIZE 37 BY .62
     RECT-7 AT ROW 1.29 COL 1
     "Toolbar Bands" VIEW-AS TEXT
          SIZE 14 BY .62 AT ROW 1 COL 4
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_dynlookup ).
       RUN repositionObject IN h_dynlookup ( 1.95 , 25.00 ) NO-ERROR.
