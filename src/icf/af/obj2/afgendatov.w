...
fiFollowDepth coDataLogicObjectType coLPProductModule fiLPRelativePath ~
fiLPSuffix fiLPTemplateName fiDOLabel fiLPLabel toIndex RECT-12 RECT-13 ~
RECT-15 
...
DEFINE VARIABLE toFollowJoins AS LOGICAL INITIAL yes 
     LABEL "Follow &Joins" 
     VIEW-AS TOGGLE-BOX
...
DO:
    ...
    IF gcOldProdMod = coLPProductModule:SCREEN-VALUE OR
      ...
      PUBLISH "ProductModuleChanged":U FROM ghContainerSource (INPUT coProductModule:SCREEN-VALUE).
...
------------------------------------------------------------------------------*/
  ...
  IF cPrefData <> ? AND
    ...
    DO WITH FRAME {&FRAME-NAME}:
      ...
      IF cSDOPM <> ? AND
         cSDOPM <> "":U THEN DO:
        coProductModule:SCREEN-VALUE = cSDOPM.
      ...
      IF cDLPPM <> ? AND
         cDLPPM <> "":U THEN DO:
        coLPProductModule:SCREEN-VALUE = cDLPPM.
        APPLY "VALUE-CHANGED":U TO coLPProductModule.
      END.
