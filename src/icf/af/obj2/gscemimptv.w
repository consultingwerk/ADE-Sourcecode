...
&Scoped-define QUERY-STRING-BrBrowse FOR EACH ttTable     WHERE ttTable.cDatabase = coDatabase:SCREEN-VALUE
...
DEFINE VARIABLE toOverrideWithSchema AS LOGICAL INITIAL no 
     LABEL "Override all attributes from schema" 
     VIEW-AS TOGGLE-BOX
     SIZE 42.8 BY .81 TOOLTIP "Override all local attributes such as Format, Label and Help with schema values" NO-UNDO.
...
DEFINE FRAME frDetails
     ...
     coEntityProductModule AT ROW 6.71 COL 20.6 COLON-ALIGNED
     coEntityClass AT ROW 7.76 COL 20.6 COLON-ALIGNED
     coProductModule AT ROW 9.38 COL 21 COLON-ALIGNED
     coObjectType AT ROW 10.43 COL 21 COLON-ALIGNED
     toOverrideWithSchema AT ROW 11.57 COL 23.4
     buImport AT ROW 12.91 COL 71.8
     RECT-1 AT ROW 9 COL 3
     "Auditing Enabled:" VIEW-AS TEXT
          SIZE 17.6 BY .76 AT ROW 2.95 COL 48.2
     "  DataFields" VIEW-AS TEXT
          SIZE 12 BY .62 AT ROW 8.81 COL 7.4
...
DO:
    ...
    THEN DO:
      RUN showMessages IN gshSessionManager (INPUT {af/sup2/aferrortxt.i 'AF' '5' '?' '?' '"entity to import"'},
                                             INPUT "ERR":U,
                                             INPUT "OK":U,
                                             INPUT "OK":U,
                                             INPUT "OK":U,
    ...
    DO WITH FRAME frDetails:
        ...
        DO:
            RUN generateDataFields IN hRDM ( INPUT coDatabase:SCREEN-VALUE IN FRAME frMain,             /* pcDataBaseName */
                                             ...
                                             INPUT TRIM(ENTRY(1,coProductModule:SCREEN-VALUE, "/":U)),  /* pcProductModuleCode */
        ...
        IF cError EQ "":U THEN
        GENERATE-ENTITY-LOOP:
        FOR EACH ttTable WHERE ttTable.lImport = YES:
           RUN generateEntityObject IN hRDM ( INPUT (ttTable.cDatabase + CHR(3) + ttTable.cTable),
                                              INPUT coEntityClass:SCREEN-VALUE,
                                              INPUT TRIM(ENTRY(1, coEntityProductModule:SCREEN-VALUE, "/":U)),
                                              ...
                                              INPUT toGenerateDataFields:CHECKED         /* plAssociateDataFields   */  ) NO-ERROR.
            IF ERROR-STATUS:ERROR OR RETURN-VALUE NE "":U THEN
            DO:
                ASSIGN cError = RETURN-VALUE.                
                LEAVE GENERATE-ENTITY-LOOP.
            END.    /* error. */
        END.    /* GENERATE-ENTITY-LOOP: each ttImport. */
...
------------------------------------------------------------------------------*/
    ...
    RUN af/sup2/gscemrddbp.p ( INPUT  lDisplayRepository,
                               OUTPUT cDBList,
    ...
    DO WITH FRAME frMain:
        ...
        ASSIGN hRdm = DYNAMIC-FUNCTION("getManagerHandle":U IN THIS-PROCEDURE, INPUT "RepositoryDesignManager":U)
               ...
               coProductModule:LIST-ITEMS         = DYNAMIC-FUNCTION("productModuleList":U IN hRDM)
               coProductModule:SCREEN-VALUE       = coProductModule:ENTRY(1)
               coEntityProductModule:LIST-ITEMS   = DYNAMIC-FUNCTION("productModuleList":U IN hRDM)
               coEntityProductModule:SCREEN-VALUE = coProductModule:ENTRY(1)
