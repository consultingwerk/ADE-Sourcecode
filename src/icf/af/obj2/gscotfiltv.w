...
------------------------------------------------------------------------------*/
  ...
  DO iLoop = NUM-ENTRIES(cParentClasses) TO 1 BY -1:
    ...
    IF iLoop <= NUM-ENTRIES(cParentClasses) THEN
      RUN tvNodeEvent IN hContainer (INPUT "EXPAND":U, INPUT cClassNodeKey).
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(hContainer) THEN DO:
    ...
    DYNAMIC-FUNCTION("setLastLaunchedNode":U IN hContainer, "":U).
  END.
END PROCEDURE.
