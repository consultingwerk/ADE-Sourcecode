...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 RECT-3 selTables selFields ~
selIndices bTables bFields bIndex bClose bHelp 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 RECT-3 selTables selFields selIndices bTables bFields 
         bIndex bClose bHelp 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
PROCEDURE SetTable :
/*------------------------------------------------------------------------------
  Purpose:     Can be used to reposition the Table and Field selection list 
               values
               
  Parameters:  pcTable   Name of Table
               pcField  Name of FIeld
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT  PARAMETER pcTable AS CHARACTER  NO-UNDO.
DEFINE INPUT  PARAMETER pcField AS CHARACTER  NO-UNDO.

IF pcTable > "" AND LOOKUP(pcTable,selTables:LIST-ITEMS IN FRAME {&FRAME-NAME}) > 0 THEN
DO:
  ASSIGN selTables:SCREEN-VALUE  = pcTable NO-ERROR.
  APPLY "VALUE-CHANGED":U TO selTables.
END.

IF pcField > "" AND LOOKUP(pcTable,selTables:LIST-ITEMS IN FRAME {&FRAME-NAME}) > 0 THEN
DO:
  ASSIGN selFields:SCREEN-VALUE  = pcField NO-ERROR.
  APPLY "VALUE-CHANGED":U TO selFields.
END.
