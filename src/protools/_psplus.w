  ...
  DO WITH FRAME F-Main:
    ...
    IF AVAILABLE tSpyInfo THEN
      ASSIGN edParms:SCREEN-VALUE       = tSpyInfo.tParms
             ...
             fiStatus:SCREEN-VALUE      = tSpyInfo.tDescrip NO-ERROR.
    ELSE
      ASSIGN edParms:SCREEN-VALUE       = ''
             ...
             fiStatus:SCREEN-VALUE      = '' NO-ERROR.
  END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  MESSAGE 'Pro*Spy Plus Help is not available at this time.'
...
------------------------------------------------------------------------------*/
  ...
  DO iColumn = 1 TO (IF pcColumns EQ '*' THEN hBuffer:NUM-FIELDS 
    ...
    IF pcColumns EQ '*' THEN
      phBrowse:ADD-LIKE-COLUMN(hBuffer:BUFFER-FIELD(iColumn)).
    ELSE
      phBrowse:ADD-LIKE-COLUMN(ENTRY(iColumn,pcColumns)).
  END.
