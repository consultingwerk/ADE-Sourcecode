...
{adeuib/uniwidg.i}      /* Universal Widget TEMP-TABLE definition            */
 ...
 {defrescd.i}            /* Defines the DEFAULT-RESULT-CODE result codes.     */
...
/* Temp-table definition for TT used to store Object Links { ry/inc/ryreplnset.i } */
...
define temp-table ttCleanupAttributeValue no-undo like ttStoreAttribute.
 ...
 DEFINE VARIABLE glNewCalcField       AS LOGICAL    NO-UNDO.
   ...
   DO:
    ...
    cMasterObjectName = IF b_U._OBJECT-NAME > "" 
     ...
     DO:
       ...
       cError = {errortxt.i 'RY' '14' '?' '?' "gcObjectName"}.
     ...
     DO:
       ...
       dDynObject_Obj = dynamic-function('getObjectInstanceObj':u in ghRepDesignManager,
                                         dSmartViewer, cInstanceName).
...
------------------------------------------------------------------------------*/
 DEFINE VARIABLE cCalcMaster      AS CHARACTER  NO-UNDO.
 ...
 DO: 
   ...
   FOR EACH _BC WHERE _BC._x-recid = grQueryRecid:
     ...
     DO:
       ...
       IF NOT dynamic-function('objectExists':u in ghRepDesignManager,
                               (IF AVAIL b_TT THEN b_TT._LIKE-TABLE ELSE _BC._TABLE ) + ".":U + cName) then
       DO:
     ...
     ELSE DO:
       /* If it is a calculated field and the status is "STATIC" then it is a static SDO being saved
          as dynamic and the master name is the _BC._DISP-NAME (_BC._NAME is the expression for static
          SDO calc fields).  If the master cannot be found and this is not a new calculated field
          an error is raised. */
       cCalcMaster = IF _BC._STATUS = "STATIC":U THEN _BC._DISP-NAME ELSE _BC._NAME.
       IF NOT dynamic-function('ObjectExists':u in ghRepDesignManager, cCalcMaster) and
          NOT (_BC._STATUS BEGINS "NEWCALC":U) THEN
       DO:
         gpcError = "Calculated Field " + cCalcMaster + " has no datafield defined in the repository." + CHR(10) +
                     "You must create DataFields for all SDO calculated fields." + CHR(10) + CHR(10) +
                     "Aborting the saving of SDO " + _P.object_filename + ".":U.
         RETURN.
 ...
 DO: 
   ...
   IF gcSuperPref NE "None":U THEN DO:
     ...
     RUN CreateAttributeRow("MASTER":U, gdSmartObject_obj, "DataLogicProcedure":U, 
                            'CHARACTER':u, gcDLPFileName, ?, ?, ?, ?, ?) NO-ERROR.
     IF gpcError NE "" AND NOT gpcError BEGINS "Associated data" THEN RETURN.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER pcCalcFieldList             AS CHARACTER      NO-UNDO.
  ...
  FOR EACH _BC WHERE _BC._x-recid = pURecid 
    ...
    ASSIGN pcCalcFieldList = pcCalcFieldList + _BC._DISP-NAME + ",":U + _BC._NAME + ",":U.
  END. /* FOR EACH _BC */
...
------------------------------------------------------------------------------*/
  ...
  if dynamic-function('getSmartobjectObj':u in gshRepositoryManager, pcObjectFilename, 0) eq 0 then
  do:
    RUN AppendToPError ({errortxt.i 'RY' '14' '?' '?' "pcObjectFilename"}).
    RETURN.
  END.
  ...
  END.  /* If a radio-set, combo or Selection-list  */
    ...
    empty temp-table ttCleanupAttributeValue.
    
    /* Move records to clean to temp table. We'll clean them
       up by removing any values that duplicate the parent values
       and copy them back into the ttStoreAttribute table.
       
       We only want to clean a subset of the attribute value tables,
       and so we use a separate temp-table. */
    for each ttStoreAttribute where
             ttStoreAttribute.tAttributeParent = 'Instance':u and
             ttStoreAttribute.tAttributeParentObj = pdInstance_Obj and
             lookup(ttStoreAttribute.tAttributeLabel, 'Row,Column':u) = 0:
        create ttCleanupAttributeValue.
        buffer-copy ttStoreAttribute to ttCleanupAttributeValue.
        
        delete ttStoreAttribute.
    end.    /* each ttStoreAttribute */
    
    dynamic-function('cleanStoreAttributeValues':u in ghRepDesignManager,
                     pcObjectFilename,
                     '':u,    /*pcClassName not required for Master */
                     'Master',
                     buffer ttCleanupAttributeValue:handle). 
    
    /* Move cleand values back */
    for each ttCleanupAttributeValue:
        create ttStoreAttribute.
        buffer-copy ttCleanupAttributeValue to ttStoreAttribute.
    end.    /* each ttCleanupAttributeValue */
...
------------------------------------------------------------------------------*/
  ...
  ELSE DO:
    ...
    IF SEARCH(cRelPath + (IF cRelPath > "" THEN "~/":U ELSE "") + pcObjectName ) = ? THEN 
    DO:  /* File is not in the right directory */
      ASSIGN pcError = pcObjectName +
                       " is not in the '" + cRelPath +
  ...
  FOR EACH _BC WHERE _BC._x-recid = bRecid_U:
    ASSIGN fldList = fldList + "," + _BC._disp-NAME
           ...
           cLabels             = cLabels             + CHR(5) + IF _BC._LABEL = _BC._DEF-LABEL THEN "?":U
                                                                ELSE _BC._LABEL
           cWidths             = cWidths             + CHR(5) + IF _BC._DATA-TYPE BEGINS "DA" /* Date */
                                                                THEN (IF _BC._WIDTH NE FONT-TABLE:GET-TEXT-WIDTH-CHARS(_BC._FORMAT)
                                                                THEN STRING(_BC._WIDTH) ELSE STRING(FONT-TABLE:GET-TEXT-WIDTH-CHARS(_BC._FORMAT))) ELSE
                                                                STRING(DECIMAL(_BC._WIDTH))
...
/*------------------------------------------------------------------------------
  ...
  Parameters:
        ...
        INPUT pcDataType           - DataType of the attribute
...
------------------------------------------------------------------------------*/
  ...
  CASE pcDataType:
      WHEN 'CHARACTER' THEN ttStoreAttribute.tCharacterValue = pcValue.
      WHEN 'DECIMAL'   THEN ttStoreAttribute.tDecimalValue   = pdeValue.
      WHEN 'INTEGER'   THEN ttStoreAttribute.tIntegerValue   = piValue.
      WHEN 'LOGICAL'   THEN ttStoreAttribute.tLogicalValue   = plValue.
      WHEN 'DATE'      THEN ttStoreAttribute.tDateValue      = pdaValue.
      WHEN 'RAW'       THEN ttStoreAttribute.tRawValue       = prValue.
      OTHERWISE      ASSIGN ttStoreAttribute.tCharacterValue = pcValue.
  END CASE.
          ...
          DO:          
            CASE cDataType:
              WHEN "CHARACTER":U OR WHEN "CHAR":U THEN
                 RUN CreateAttributeRow(pclevel,pdObj,cLabel,'Character':u,cValue,?,?,?,?,?).
              WHEN "DECIMAL":U OR WHEN "DEC":U THEN
                 RUN CreateAttributeRow(pclevel,pdObj,cLabel,'Decimal':u,?,DEC(cValue),?,?,?,?).
              WHEN "INTEGER":U OR WHEN "INT":U THEN
                 RUN CreateAttributeRow(pclevel,pdObj,cLabel,'Integer':u,?,?,INT(cValue),?,?,?).
              WHEN "LOGICAL":U OR WHEN "LOG":U THEN
                RUN CreateAttributeRow(pclevel,pdObj,cLabel,'Logical':u,?,?,?,LOGICAL(cValue),?,?).
              WHEN "DATE":U THEN
                RUN CreateAttributeRow(pclevel, pdObj,cLabel,'Date':u, ?, ?, ?, ?, DATE(cValue), ?).
            END CASE.
...
------------------------------------------------------------------------------*/
 ...
 DEFINE VARIABLE lCalcExistsOnEntity   AS LOGICAL    NO-UNDO.
 DEFINE VARIABLE cCalcClass            AS CHARACTER  NO-UNDO.
 DEFINE VARIABLE dNewCalcObj           AS DECIMAL    NO-UNDO.
...
/* Default DLP is ValOnly */
 ...
 END.
 cEntityTable = IF cFirstEnabledTable NE '':U THEN cFirstEnabledTable
...
/* get the include file info */
 ...
 DO:
  /* Assign updatable fields for validation purposes */
   ...
   FOR EACH _TT WHERE _TT._p-recid = RECID(_P) :
     ...
     DO:
        ASSIGN cSourceFile = "".
        IF CONNECTED("TEMP-DB":U) AND _TT._LIKE-DB = "TEMP-DB":U THEN
        DO:
           ...
           DO:
              RUN adeuib/_tempdbfind.p (INPUT "TABLE":U, 
                                        ...
                                        OUTPUT cSourceFile).
              IF cSourceFile > "" AND LOOKUP(cSourceFile,cSourceFileList) > 0 THEN
                 NEXT.
              IF cSourceFile NE "" THEN
                  ASSIGN cTempTableDef   =  cTempTableDef  + "~{":U + cSourceFile + "}":U + CHR(10) 
                         cSourceFileList = cSourceFileList + (If cSourceFileList = "" THEN "" ELSE ",") + cSourceFile.
           END.
           
        END.    
        IF cSourceFile EQ "" THEN
   ...
   IF _C._DATA-LOGIC-PROC NE "":U THEN
     IF _C._DATA-LOGIC-PROC MATCHES "*~/":U + cDLPRelativeDir + "~/":U + gcDLPFileName THEN
       cDLPRootDir = SUBSTRING(_C._DATA-LOGIC-PROC, 1, LENGTH(_C._DATA-LOGIC-PROC, "CHARACTER") -
                               ...
                               "CHARACTER").
   IF cTempTableDef > "" THEN
      cTempTableDef = "/* ***********Included Temp-Table & Buffer definitions **************** */" 
                        + CHR(10) + cTempTableDef.
 ...
 DO:
   COMPILE VALUE(cDLPRelativeDir + "~/":U + gcDLPFileName) SAVE NO-ERROR.
   DO iErrorNum = 1 TO ERROR-STATUS:NUM-MESSAGES:
     cError = cError + ERROR-STATUS:GET-MESSAGE(iErrorNum) + CHR(10).
   END.
   IF cError = "":U THEN
   DO:
     COMPILE VALUE(cDLPRelativeDir + "~/":U +
                   REPLACE(gcDLPFileName,".p":U,"_cl.P":U)) SAVE NO-ERROR.
     DO iErrorNum = 1 TO ERROR-STATUS:NUM-MESSAGES:
       cError = cError + ERROR-STATUS:GET-MESSAGE(iErrorNum) + CHR(10).
     END.
   END.  /* if cError blank */
   gpcAssocError = cError.
 END.

 /* Create instances for calculated fields that do not exist on SDO entities and 
    create new calcualted fields */
 FOR EACH _BC WHERE _BC._x-recid = RECID(b_U) AND _BC._DBNAME = "_<CALC>":U:
   /* Create new calc field */
   IF _BC._STATUS BEGINS 'NEWCALC':U AND NUM-ENTRIES(_BC._STATUS, CHR(3)) > 2 THEN
   DO:
     ASSIGN
       cCalcClass      = ENTRY(2, _BC._STATUS, CHR(3))
       glNewCalcField  = TRUE
       hAttributeTable = ?.
     RUN insertObjectMaster IN ghRepDesignManager
         (INPUT _BC._NAME, INPUT '':U, INPUT ENTRY(3, _BC._STATUS, CHR(3)),
          INPUT cCalcClass, INPUT cCalcClass + ' ':U + _BC._NAME,
          INPUT '':U, INPUT '':U, INPUT '':U, INPUT NO, INPUT YES, INPUT '':U,
          INPUT NO, INPUT '':U, INPUT '':U, INPUT '':U, INPUT ?, 
          INPUT TABLE-HANDLE hAttributeTable, OUTPUT dNewCalcObj) NO-ERROR.
     IF ERROR-STATUS:ERROR OR RETURN-VALUE <> "" THEN 
     DO:
       RUN AppendToPError (RETURN-VALUE).
       RETURN.
     ...
     RUN retrieveDesignClass IN ghRepDesignManager
                       ( INPUT  cCalcClass, OUTPUT cInheritClasses,
                         OUTPUT TABLE ttClassAttribute, OUTPUT TABLE ttUiEvent, OUTPUT TABLE ttSupportedLink) NO-ERROR.  

     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'Data-Type':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE _BC._DATA-TYPE THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Data-Type':U,'Character':u,_BC._DATA-TYPE,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'Label':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE _BC._LABEL THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Label':U,'Character':u,_BC._LABEL,?,?,?,?,?).
     IF NUM-ENTRIES(_BC._STATUS, CHR(3)) > 3 THEN 
       cCalcColLabel = ENTRY(4, _BC._STATUS, CHR(3)).
     IF cCalcColLabel = '?':U THEN
       cCalcColLabel = ?.
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'ColumnLabel':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE cCalcColLabel THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'ColumnLabel':U,'Character':u,cCalcColLabel,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
       ttClassAttribute.tAttributeLabel = 'Format':U NO-ERROR.
     IF AVAIL ttClassAttribute AND
       ttClassAttribute.tAttributeValue NE _BC._FORMAT THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Format':U,'Character':u,_BC._FORMAT,?,?,?,?,?).
     FIND ttClassAttribute WHERE ttClassAttribute.tClassName = cCalcClass AND
         ttClassAttribute.tAttributeLabel = 'Help':U NO-ERROR.
     IF AVAIL ttClassAttribute AND 
         ttClassAttribute.tAttributeValue NE _BC._HELP THEN
       RUN CreateAttributeRow('MASTER':U,dNewCalcObj,'Help':U,'Character':u,_BC._HELP,?,?,?,?,?).
     
     hAttributeTable = TEMP-TABLE ttStoreAttribute:DEFAULT-BUFFER-HANDLE.
     RUN StoreAttributeValues IN gshRepositoryManager
         (INPUT hAttributeTable ,
          INPUT TABLE-HANDLE ghUnknown) NO-ERROR.  /* Compiler requires a variable with unknown */
     IF ERROR-STATUS:ERROR OR RETURN-VALUE <> "" THEN 
     DO:
       RUN AppendToPError (RETURN-VALUE).
       RETURN.
     END.
   END.  /* if new calc field */

   /* Check for calc field on SDO entities */
   lCalcExistsOnEntity = FALSE.
   EntityLoop:
   DO i = 1 TO NUM-ENTRIES(cTablesInSDO):

     RUN retrieveDesignObject IN ghRepDesignManager
       (INPUT ENTRY(i,cTablesInSDO), INPUT '':U,    /* Default result codes */
        OUTPUT TABLE ttObject, OUTPUT TABLE ttPage,
        OUTPUT TABLE ttLink, OUTPUT TABLE ttUIEvent,
        OUTPUT TABLE ttObjectAttribute).

     IF ENTRY(i,cTablesInSDO) = cEntityTable THEN
     DO:
       ...
       FOR EACH ttObject WHERE ttObject.tContainerSmartObjectObj > 0 :
         IF ttObject.tPageObjectSequence > iInstanceOrder THEN
           iInstanceOrder = ttObject.tPageObjectSequence.
       END.
     END.

     /* If a static SDO is being saved as dynamic the master name is
        _BC._DISP-NAME (_BC._NAME is the calc expression for static SDO
        calc fields). */
     cCalcMaster = IF _BC._STATUS = 'STATIC':U THEN _BC._DISP-NAME
                   ELSE _BC._NAME.
     FIND FIRST ttObject WHERE ttObject.tContainerSmartObjectObj > 0 AND
         ttObject.tLogicalObjectName = cCalcMaster AND
         ttObject.tObjectInstanceName = _BC._DISP-NAME NO-ERROR.
     IF AVAILABLE ttObject THEN
     DO:
       lCalcExistsOnEntity = TRUE.
       LEAVE EntityLoop.
     END.               
   END.  /* entityLoop */
   IF NOT lCalcExistsOnEntity THEN
   DO:
     RUN retrieveDesignObject IN ghRepDesignManager
       (INPUT cCalcMaster, INPUT '':U,              /* Default result codes */
        OUTPUT TABLE ttObject, OUTPUT TABLE ttPage,
        OUTPUT TABLE ttLink, OUTPUT TABLE ttUIEvent,
        OUTPUT TABLE ttObjectAttribute).
     ...
     IF AVAILABLE ttObject THEN
       cClassName = ttObject.tClassName.

     hAttributeTable = ?.
     RUN insertObjectInstance IN ghRepDesignManager
         (INPUT dEntityObj, INPUT cCalcMaster, INPUT '':U,
          INPUT _BC._DISP-NAME, INPUT cClassName + ' ':U + _BC._NAME,
          INPUT STRING(iInstanceOrder + 1), INPUT ?, INPUT iInstanceOrder + 1,
          INPUT NO, INPUT ?, INPUT TABLE-HANDLE hAttributeTable,
          OUTPUT dDataFieldObj, OUTPUT dInstanceObj)  NO-ERROR.
     ...
     DO:
       ...
       RETURN.
     END.
   END.  /* if calc doesn't exist on entity */
 END.  /* if calc fields */
     ...
     DO:
        IF b_U._TYPE NE "FRAME":U AND b_U._TYPE NE "WINDOW":U AND
          ...
          ASSIGN gdMinHeight = MAX(gdMinHeight, _L._ROW -
                                 (IF gcClassName = "DynBrow":U OR gcClassName = "DynView":U 
                                  THEN 1 ELSE 0)
                                     + _L._Height)
...
------------------------------------------------------------------------------*/
 ...
 DEFINE INPUT PARAMETER plRowObjectNoUndo        AS LOGICAL    NO-UNDO.
 ...
 RUN af/app/afgendlogp.p ( INPUT cRelativelyPathedName,
                           ...
                           INPUT NO,
                           INPUT pcTempTableDef,
                           INPUT plRowObjectNoUndo) NO-ERROR.
 ...
 THEN
   RETURN ERROR {errortxt.i 'AF' '19' '?' '?' "''" "'logic procedure: ' + cAbsolutePathedName"}.
 ...
 IF SEARCH(cAbsolutePathedName) EQ ?
 THEN
   RETURN ERROR {errortxt.i 'AF' '19' '?' '?' "''" "'logic procedure client proxy: ' + cAbsolutePathedName"}.        
...
------------------------------------------------------------------------------*/
 ...
 DO:  /* Give up */
   cError = {errortxt.i 'RY' '16' '?' '?' "gcSDOName"}.
   RUN AppendToPError (INPUT cError).
   RETURN.
...
------------------------------------------------------------------------------*/
 ...
 /* Look to see if the SDO is already registered */
    if dynamic-function('objectExists':u in ghRepDesignManager, ENTRY(1, cSDOFileName, ".":U)) then
        gcSDORepos = ENTRY(1, cSDOFileName, ".":U).
   IF gcSDORepos = "":U THEN
    if dynamic-function('objectExists':u in ghRepDesignManager, cSDOFileName) then
        gcSDORepos = cSDOFileName.
 ...
 DO:
   ...
   ELSE DO:
     ...
     DO ON  STOP UNDO, LEAVE ON ERROR UNDO, LEAVE:
       cError = {errortxt.i 'RY' '19' '?' '?' "_P._data-object"}.
       RUN AppendToPError (INPUT cError).
...
------------------------------------------------------------------------------*/
  ...
  ELSE IF _L._BGCOLOR NE b_L._BGCOLOR THEN
    RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "BGCOLOR":U,'Integer':u,?,?,_L._BGCOLOR,?,?,?).
  ...
  ELSE IF _L._FGCOLOR NE b_L._FGCOLOR THEN
    RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "FGCOLOR":U,'Integer':u,?,?,_L._FGCOLOR,?,?,?).    
  ...
  ELSE IF _L._NO-BOX NE b_L._NO-BOX  THEN
    RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "BOX":U,'Logical':u,?,?,?,(NOT _L._NO-BOX),?,?).
  ...
  ELSE IF _L._FONT NE b_L._FONT THEN
    RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "FONT":U, 'Integer':u,?,?,_L._FONT,?,?,?).
  ...
  ELSE IF _L._3-D NE b_L._3-D THEN
    RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "THREE-D":U,'Logical':u,?,?,?,_L._3-D,?,?).
  ...
  DO:
     ...
     ELSE IF _L._COL NE b_L._COL OR  b_L._REMOVE-FROM-LAYOUT THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "COLUMN":U,'Decimal':u,?,_L._COL,?,?,?,?).
     ...
     ELSE IF _L._CONVERT-3D-COLORS NE b_L._CONVERT-3D-COLORS THEN
       RUN CreateAttributeRow (pcObjectLevel,pdInputObj, "CONVERT-3D-COLORS":U,'Logical':u,?,?,?,_L._CONVERT-3D-COLORS,?,?).
     ...
     ELSE IF _L._EDGE-PIXELS NE b_L._EDGE-PIXELS THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "EDGE-PIXELS":U,'Integer':u,?,?,_L._EDGE-PIXELS,?,?,?).
     ...
     DO:
       ...
       ELSE IF _L._LABEL NE b_L._LABEL THEN
         RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "FieldLabel":U,'Character':u,_L._LABEL,?,?,?,?,?).
     END.
     ...
     ELSE IF _L._FILLED NE b_L._FILLED THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "FILLED":U, 'Logical':u,?,?,?,_L._FILLED,?,?).
     ...
     ELSE IF _L._GRAPHIC-EDGE NE b_L._GRAPHIC-EDGE THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "GRAPHIC-EDGE":U, 'Logical':u,?,?,?,_L._GRAPHIC-EDGE,?,?).
     ...
     ELSE IF _L._GROUP-BOX NE b_L._GROUP-BOX THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "GROUP-BOX":U, 'Logical':u,?,?,?,_L._GROUP-BOX,?,?).

     IF _L._LABEL = b_L._LABEL AND NOT m_L._REMOVE-FROM-LAYOUT THEN
       RUN DeleteAttributeRow (pcObjectLevel, pdInputObj, "LABEL":U).
     ELSE IF _L._LABEL NE b_L._LABEL THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "LABEL":U, 'Character':u, _L._LABEL,?,?,?,?,?).
     ...
     ELSE IF _L._NO-LABELS NE b_L._NO-LABELS THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "LABELS":U, 'Logical':u,?,?,?,(NOT _L._NO-LABELS),?,?).
     ...
     ELSE IF _L._NO-FOCUS NE b_L._NO-FOCUS THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "NO-FOCUS":U, 'Logical':u,?,?,?,_L._NO-FOCUS,?,?).
     ...
     ELSE IF _L._ROUNDED NE b_L._ROUNDED THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "ROUNDED":U, 'Logical':u,?,?,?,_L._ROUNDED,?,?).
     ...
     ELSE IF _L._ROW NE b_L._ROW OR b_L._REMOVE-FROM-LAYOUT THEN
       RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "ROW":U, 'Decimal':u, ?,_L._ROW,?,?,?,?).
     ...
     DO:
       ...
       ELSE 
         RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "VISIBLE":U,'Logical':u,?,?,?,NOT _L._REMOVE-FROM-LAYOUT,?,?).
     END.  
  END.
  ...
  DO:
    ...
    ELSE IF _L._SEPARATOR-FGCOLOR NE b_L._SEPARATOR-FGCOLOR THEN
      RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "SEPARATOR-FGCOLOR":U,'Integer':u,?,?,_L._SEPARATOR-FGCOLOR,?,?,?).

    IF _L._SEPARATORS = b_L._SEPARATORS AND NOT m_L._REMOVE-FROM-LAYOUT THEN 
      RUN DeleteAttributeRow (pcObjectLevel, pdInputObj, "SEPARATOR-FGCOLOR":U).
    ELSE IF _L._SEPARATORS NE b_L._SEPARATORS THEN
      RUN CreateAttributeRow (pcObjectLevel, pdInputObj, "SEPARATOR-FGCOLOR":U,'Logical':u,?,?,?,_L._SEPARATORS,?,?).
  END.
...
/* Create Object Instances of the contained SDOs */
 FOR EACH b_U WHERE b_U._WINDOW-HANDLE = _P._WINDOW-HANDLE AND 
  ...
  DO i = 1 TO NUM-ENTRIES(_S._settings, CHR(3)):
    ...
    ELSE IF cAttr = "ForeignFields":U AND cValue NE "":U THEN DO:
      ...
      RUN CreateAttributeRow(INPUT "INSTANCE":U, INPUT dObject_obj, INPUT cAttr, 'Character':u, cValue, ?, ?, ?, ?, ?).
    END.
    ELSE DO:  /* Generic Case */
        ...
        do:
            run adecomm/_datatyp.p (ttClassAttribute.tDataType, output cDataType).
            CASE cDataType:
              WHEN 'Logical':u THEN DO:
                IF cValue = "":U THEN cValue = "no":U.
                IF LOGICAL(cValue) NE LOGICAL(ttClassAttribute.tAttributeValue) THEN
                  RUN CreateAttributeRow(INPUT "INSTANCE":U, INPUT dObject_obj, INPUT cAttr, 'Logical':u, ?,?,?,LOGICAL(cValue),?,?).
              END.
              WHEN 'Integer':u THEN DO:
                ...
                IF NOT ERROR-STATUS:ERROR AND iValue NE INT(ttClassAttribute.tAttributeValue) THEN
                   RUN CreateAttributeRow(INPUT "INSTANCE":U, INPUT dObject_obj, INPUT cAttr, 'Integer':u, ?,?,iValue,?,?,?).
              END.
              WHEN 'Decimal':u THEN DO:
                ...
                IF NOT ERROR-STATUS:ERROR AND dValue NE DECIMAL(ttClassAttribute.tAttributeValue) THEN
                    RUN CreateAttributeRow(INPUT "INSTANCE":U, INPUT dObject_obj, INPUT cAttr, 'Decimal':u, ?,dValue,?,?,?,?).
              END.
 ...
 FOR EACH _admlinks WHERE _admlinks._P-RECID = RECID(_P).
    create ttSmartLink.
    assign ttSmartLink.tContainerObj = gdDynamicObj
           ttSmartLink.tLinkName     = _admlinks._link-type
           ttSmartLink.tUserLinkName = _admlinks._link-type.
    
    FIND b_U WHERE RECID(b_U) = INTEGER(_admlinks._link-source).
    ttSmartLink.tSourceObj = b_U._OBJECT-OBJ.
    
    FIND b_U WHERE RECID(b_U) = INTEGER(_admlinks._link-dest).
    ttSmartLink.tTargetObj = b_U._OBJECT-OBJ.
 END.
 
 if can-find(first ttSmartLink) then
 do:
     run insertObjectLinks in ghRepDesignManager (gdDynamicObj,
                                                 temp-table ttSmartLink:default-buffer-handle,
                                                 table-handle ghUnknown) no-error.
     if return-value ne '' or error-status:error then
         run appendToPError (return-value).                                                     
 end.    /* at least one links to save */
...
DEFINE VARIABLE cDataBasesInQuery         AS CHARACTER  NO-UNDO.
 ...
 /* Use cTables to build cAssignList, cUpdatableColumnsByTable, cDataColumnsByTable and cCalcFieldList */
 RUN BuildSDOFieldListsByTable(INPUT RECID(b_U),
                               INPUT  cTables,                      
                               OUTPUT cAssignList,
                               OUTPUT cUpdatableColumnsByTable,     
                               OUTPUT cDataColumnsByTable,
                               OUTPUT cCalcFieldList).
 ...
 FOR EACH ttClassAttribute:
    ...
    CASE cLabel:
      ...
      WHEN "CalcFieldList":U THEN
            setAttributeChar("MASTER":U,cCalcFieldList,cLabel, cValue,gdSmartObject_obj).
   ...
   FOR EACH ttClassAttribute:
     ...
     CASE cLabel:
       ...
       WHEN "Order":U THEN DO:
         ...
         DO:
            IF b_U._TAB-ORDER NE INT(cValue) THEN DO:
              ...
              dynamic-function('updateInstanceSequence':u in ghRepDesignManager,
                               pdObject_Obj, b_U._TAB-ORDER).
            END.
      ...
      WHEN "SHOWPOPUP":U THEN
       ...
       WHEN "TOOLTIP":U THEN
         setAttributeChar("INSTANCE":U,IF b_U._TOOLTIP = ? THEN "" ELSE b_U._TOOLTIP,cLabel, cValue,pdObject_obj).
       WHEN "THREE-D":U THEN
    ...
    CASE cLabel:
      ...
      DO:
        ...
        DO:
           ...
           ELSE IF cABValue <> cObjectValue AND pcLayout EQ "Master Layout":U THEN
              RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Character':u,cABValue,?,?,?,?,?).
        END.   
      ...
      DO:
         ...
         ELSE IF _L._COL <> DEC(cObjectValue) AND (pcLayout EQ "Master Layout":U OR _L._COL NE mstr_L._COL)THEN
           RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Decimal':u,?,_L._COL,?,?,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF pcFldname <> cObjectValue AND pcLayout EQ "Master Layout":U THEN
            RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Character':u,pcFldname,?,?,?,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF _L._HEIGHT <> DEC(cObjectValue) AND (pcLayout EQ "Master Layout":U OR _L._HEIGHT NE mstr_L._HEIGHT)THEN
           RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Decimal':u,?,_L._HEIGHT,?,?,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF _L._REMOVE-FROM-LAYOUT <> LOGICAL(cObjectValue) 
            ...
            RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Logical':u,?,?,?,_L._REMOVE-FROM-LAYOUT,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF _S._FILE-NAME <> cObjectValue AND pcLayout EQ "Master Layout":U THEN
            RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Character':u,_S._FILE-NAME,?,?,?,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF _L._ROW <> DEC(cObjectValue) AND (pcLayout EQ "Master Layout":U OR _L._ROW NE mstr_L._ROW)THEN
           RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Decimal':u,?,_L._ROW,?,?,?,?).
      END.   
      ...
      DO:
         ...
         ELSE IF _L._WIDTH <> DEC(cObjectValue) AND (pcLayout EQ "Master Layout":U OR _L._WIDTH NE mstr_L._WIDTH)THEN
           RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Decimal':u,?,_L._WIDTH,?,?,?,?).
      END.   
      ...
      DO:
        ...
        DO:
           ...
           ELSE IF b_U._TAB-ORDER <> INTEGER(cObjectValue) AND pcLayout EQ "Master Layout":U THEN
              RUN CreateAttributeRow("INSTANCE":U,pdObject_obj,cLabel,'Integer':u,?,?,b_U._TAB-ORDER,?,?,?).
        END.   
...
------------------------------------------------------------------------------*/
 ...
 CASE _S._FILE-NAME:
   ...
   DO:
      cError = {errortxt.i 'AF' '5' '?' '?' "'SmartSelect SDF for field ' + b_U._NAME "}.
      RUN AppendToPError (INPUT cError).
      RETURN.
   END.
   OTHERWISE DO:
     ...
     DO:
        cError = {errortxt.i 'RY' '15' '?' '?' "_S._FILE-NAME"}.
        RUN AppendToPError (INPUT cError).
        RETURN.
     END.
     /* Determine whether the object is registered with or without the extension */
     run getRecordDetail in gshGenManager (' for each ryc_smartobject where ':u
                                           + 'ryc_SmartObject.object_Filename = ':u
                                           + quoter(cFileNameNoPath)
                                           + ' no-lock ':u,
                                           output cValues).
    if cValues ne ? and cValues ne '':u then
        cOBJFilename = cFileNameNoPath.
     ELSE
     IF NUM-ENTRIES(cFileNameNoPath,".") > 1 then
     do:
         run getRecordDetail in gshGenManager (' for each ryc_smartobject where ':u
                                               + 'ryc_SmartObject.object_Filename = ':u
                                               + quoter(ENTRY(1,cFileNameNoPath,".":U))
                                               + ' no-lock ':u,
                                               output cValues).
        if cValues ne ? and cValues ne '':u then
 ...
 DO:
   ...
   DO:
     cError = {errortxt.i 'RY' '15' '?' '?' "cOBJFileName"}.
     RUN AppendToPError (INPUT cError).
     RETURN.
   END.
 END.
 ...
 DO:
   cAttrDiffs = CheckCustomChanges(RECID(_L), FALSE). /* Don't do these customizations now */
   /* Need to get the object_obj of the customization if it exists */
   dObject_obj = dynamic-function('getObjectInstanceObj':u in ghRepDesignManager,
                                  dViewer_obj, cObjFilename).
...
/*  CUSTOM LAYOUTS for viewers and browsers  */
 ...
 DO iResultCode = 2 TO NUM-ENTRIES(gcResultCodes):
   ...
   /* Need to get the object_obj of the customization if it exists.
      [PJ] Yes, getSmartObjectObj() exists, but it's probably best that this code mirrors the
      original code as closely as possible, to avoid introducing subtle changes
      in behaviour. */
   run getRecordDetail in gshGenManager (' for each ryc_smartobject where ':u
                                         + ' ryc_smartobject.object_filename = ':U
                                         + quoter(gcObjFilename) + ' and ':u 
                                         + ' ryc_smartobject.customization_result_obj = ':u                                                 
                                         + quoter(dynamic-function('getResultCodeObj':u in ghRepDesignManager, cResultCode))
                                         + ' no-lock ',
                                         output cValues).
    if cValues eq ? or cValues eq '':u then
        gdDynamicObj = 0.
    else
        gdDynamicObj = decimal(dynamic-function('mappedEntry':u in _h_func_lib,
                                                'ryc_smartobject.smartobject_obj':u,
                                                cValues,
                                                yes,
                                                chr(3))).                                                     
...
------------------------------------------------------------------------------*/
  ...
  IF _C._CUSTOM-SUPER-PROC-PMOD <> "":U AND _C._CUSTOM-SUPER-PROC-PMOD <> ? THEN
    run getRecordDetail in gshGenManager ('for each gsc_product_module where ':u
                                         + ' gsc_product_module.product_module_code = ':u
                                         + quoter(_C._CUSTOM-SUPER-PROC-PMOD)
                                         + ' no-lock ':u,
                                          output cValues). 
  if (cValues eq '':u or cValues eq ?) and _P.product_module_code <> "":U then
    run getRecordDetail in gshGenManager ('for each gsc_product_module where ':u
                                         + ' gsc_product_module.product_module_code = ':u
                                         + quoter(_P.product_module_code)
                                         + ' no-lock ':u,
                                          output cValues). 
  IF cValues ne ? and cValues ne '':u THEN DO:
    cCSPProductModule = dynamic-function('mappedEntry':u in _h_func_lib,
                        'gsc_product_module.product_module_code':u, cValues, yes, chr(3)).
    IF VALID-HANDLE(hSCMTool) THEN DO:
      ...
      RUN scmGetModuleDir IN hSCMTool (INPUT cCSPProductModule, INPUT "":U, OUTPUT cCSPRelativeDir) NO-ERROR.          
      ASSIGN cCSPRelativeDir = TRIM(REPLACE(cCSPRelativeDir,"~\":U,"~/":U),"~/":U).
    END.
    ELSE ASSIGN cCSPRelativeDir = TRIM(REPLACE(dynamic-function('mappedEntry':u in _h_func_lib,
                                              'gsc_product_module.relative_path':u, cValues, yes, chr(3))
                                              ,"~\":U,"~/":U),"~/":U).
  ...
  ASSIGN cCSPRootDir   = TRIM(cCSPRootDir,"~/":U)
         cCSPRootDir   = IF cCSPRelativeDir NE "":U 
                         ...
                         ELSE cCSPRootDir NO-ERROR.

  ASSIGN _save_file = cCSPFullName.
  IF VALID-HANDLE(hSCMTool) THEN
  DO:
     /* Make sure the the full path to the super procedure is correct using
        the relative path we just got from the SCM tool above
        - there may be differences between the SCM relative path and 
        the Dynamics path. */
     ASSIGN _save_file = DYNAMIC-FUNCTION('getSessionRootDirectory':U IN THIS-PROCEDURE)
                       + "/":U + cCSPRelativeDir
                       + (IF cCSPRelativeDir > "" THEN "/":U ELSE "":U)
                       + cCSPFileName NO-ERROR.
  END.
  ...
  DO:
     ...
     DO: 
        ...
        EMPTY TEMP-TABLE ttStoreAttribute.
        RUN CreateAttributeRow("MASTER":U,gdDynamicObj,"SuperProcedure":U ,'Character':u,_C._CUSTOM-SUPER-PROC,?,?,?,?,?).
        hTmp = TEMP-TABLE ttStoreAttribute:DEFAULT-BUFFER-HANDLE.
        RUN StoreAttributeValues IN gshRepositoryManager
            (INPUT hTmp ,
             INPUT TABLE-HANDLE ghUnknown)  NO-ERROR.  /* Compiler requires a variable with unknown */
  ...
  DO:
    ...
    DO:
       ...
       ELSE IF pcABValue <> pcValue THEN
          RUN CreateAttributeRow(pclevel,pdObj,pcLabel,'Character':u,pcABValue,?,?,?,?,?).
    END.   
...
------------------------------------------------------------------------------*/
  ...
  DO:
     ...
     ELSE IF pdABValue <> dValue THEN
         RUN CreateAttributeRow(pclevel,pdObj,pcLabel,'Decimal':u,?,pdABValue,?,?,?,?).  
  END.   
 ...
 /* If the attribute is defined on the master or instance*/
  ...
  DO:
     ...
     ELSE IF piABValue <> iValue THEN
         RUN CreateAttributeRow(pclevel,pdObj,pcLabel,'Integer':u,?,?,piABValue,?,?,?).    
  END.   
 ...
 /* If the attribute is defined on the master or instance*/
  ...
  DO:
     ...
     ELSE IF plABValue <> lValue THEN
         RUN CreateAttributeRow(pclevel,pdObj,pcLabel,'Logical':u,?,?,?,plABValue,?,?).   
  END.   
