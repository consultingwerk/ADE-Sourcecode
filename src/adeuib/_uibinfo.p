     ...
     p_name     - Name of the object in the form:
                  ...
                  * To refer to a window, frame, procedur or palette-item
                    ...
                    Special case:   
                       ...
                       TEMPLATE-ITEM To find the template item and return the
                                     template file             
...
IF pi_context eq ? AND p_name begins "TEMPLATE-ITEM" THEN DO:
  RUN parse-template-request.
  RETURN.
END.
          ...
          WHEN "HANDLE" THEN DO:
            ...
            IF VALID-HANDLE(h) THEN DO:
              ...
              ELSE DO:
                ...
                /* If h is a window and we don't have a current window, then
                   make the current window be h.  This situation comes up when
                   the current window has been minimized. IZ 11515 */
                IF AVAILABLE _U AND 
                   h:TYPE = "WINDOW":U AND 
                   NOT VALID-HANDLE(_h_win) THEN _h_win = h.
              END.  
            END.  
          END.
...
&IF DEFINED(EXCLUDE-parse-template-request) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE parse-template-request Procedure 
PROCEDURE parse-template-request :
/*------------------------------------------------------------------------------
  Purpose:     Returns template file name
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEF VAR cTemplateName AS CHAR NO-UNDO.
  DEF VAR cValidTypes  AS CHAR NO-UNDO INIT "ATTRIBUTES,TEMPLATE,LABEL,FILES,TYPE,PROPSHEET":U.
  IF NOT CAN-DO(cValidTypes,P_request) THEN
  DO:
    RUN error-msg ("Could not understand TEMPLATE-ITEM request: " + p_request
                    + Chr(10)
                    + "Use one of the following values: " + cValidTypes  ).
  END.
  IF NUM-ENTRIES(p_name,"":U) = 1 THEN
  DO:
    RUN error-msg ("Could not understand: " + p_name
                   + CHR(10)
                   + "TEMPLATE-ITEM must have an objectname and space as separator")
                  .
    RETURN.  
  END.
  
  ASSIGN cTemplateName = TRIM(SUBSTRING(p_name, INDEX(p_name," "),-1)).
  
  FIND _custom WHERE _custom._name = cTemplateName NO-ERROR.
  ...
  CASE p_request:
    WHEN "Attributes":U THEN
        p_info = _custom._attr.
    WHEN "Label":U THEN
        p_info = _custom._name.
    WHEN "PropSheet":U THEN
        p_info = _custom._design_propsheet_file.
    WHEN "template":U THEN
        p_info = _custom._design_template_file.
    WHEN "TYPE":U THEN
       p_info  = _custom._type.      
    WHEN "FILES":U THEN
       p_info  = _custom._files.      
...
------------------------------------------------------------------------------*/
  ...
  CASE token[1]:
    ...
    DO:
       ...
       END.
    END.
    WHEN "DATA-LOGIC-PROCEDURE-TEMPLATE" THEN 
    DO:
       IF AVAILABLE _U THEN 
       DO:
          FIND x_C WHERE RECID(x_C) = _U._x-recid.
          ASSIGN p_info = x_C._DATA-LOGIC-PROC-TEMPLATE.
       END.
    END.

    WHEN "DATA-LOGIC-PROCEDURE-NEW" THEN 
    DO:
       IF AVAILABLE _U THEN 
       DO:
          FIND x_C WHERE RECID(x_C) = _U._x-recid.
          ASSIGN p_info = STRING(x_C._DATA-LOGIC-PROC-NEW).
       END.
    END.
