...
/*--------------------------------------------------------------------------
    File        : _saveasdynobject
    Purpose     : AppBuilder's Save as a dynamic object dialog box.

    Syntax      : RUN adeuib/saveasdynobject.w 
                  (INPUT  phWindow,
...
DO:
    ...
    DO:        
        ...
        RUN scmGetModuleDir IN ghScmTool (INPUT coProductModule:screen-value,
                                          INPUT "":U, 
    ...
    do:
        run getRecordDetail in gshGenManager (
             'for each gsc_product_module where '
             + ' gsc_product_module.product_module_code = ' + quoter(coProductModule:screen-value)
             + ' no-lock ':u,
             output cFieldData ).
          
        ASSIGN cFieldData = entry(lookup("gsc_product_module.relative_path", cFieldData, chr(3)) + 1, cFieldData, chr(3))
               fiRelDirectory:SCREEN-VALUE = TRIM(REPLACE(cFieldData,"~\":U,"~/":U),"~/":U).    
    end.
END.
...
DO:
    ...
    DO:        
        /* Get the relative directory from SCM instead of from gsc_product_module  */
        RUN scmGetModuleDir IN ghScmTool (INPUT coSuperProductModule:screen-value,
                                          INPUT "":U, 
                                          OUTPUT fiSuperRelDirectory).
        ASSIGN fiSuperRelDirectory:SCREEN-VALUE = TRIM(REPLACE(fiSuperRelDirectory,"~\":U,"~/":U),"~/":U) NO-ERROR. 
    END.
    ELSE 
    do:
        run getRecordDetail in gshGenManager (
             'for each gsc_product_module where '
             + ' gsc_product_module.product_module_code = ' + quoter(coSuperProductModule:screen-value)
             + ' no-lock ':u,
             output cFieldData ).
        ASSIGN cFieldData = entry(lookup("gsc_product_module.relative_path", cFieldData, chr(3)) + 1, cFieldData, chr(3))
               fiSuperRelDirectory:SCREEN-VALUE = TRIM(REPLACE(cFieldData,"~\":U,"~/":U),"~/":U).    
    end.
...
DO:
 IF SELF:CHECKED THEN
    ASSIGN fiPrcFileName:SCREEN-VALUE = fiFileName:SCREEN-VALUE 
           ...
           coSuperProductModule:sensitive = yes
           coSuperType:sensitive = yes .
 ELSE
    ASSIGN fiPrcFileName:SCREEN-VALUE = ""
           ...
           coSuperProductModule:sensitive = no
           coSuperType:sensitive = no .
...
------------------------------------------------------------------------------*/
 ...
 DO WITH FRAME {&FRAME-NAME}:
  ...
  IF VALID-HANDLE(ghRepositoryDesignManager) AND 
     DYNAMIC-FUNCTION("ObjectExists":U IN ghRepositoryDesignManager, INPUT fiFilename )  THEN
  DO:
  ...
  END.
   ...
   DO:
       run getRecordDetail in gshGenManager (
             'for each gsc_product_module where '
             + ' gsc_product_module.product_module_code = ' + quoter(_RyObject.product_module_code)
             + ' no-lock ':u,
             output cFieldData ).
     IF gcObjectType = "DynView":U OR gcObjectType = "DynBrow":U /* For DynViewers and DynBrowsers */
        or gcObjectType eq 'DynDataView'
     THEN DO:
         if cFieldData gt '':u then
          ASSIGN
            _P._save-as-path      = entry(lookup("gsc_product_module.relative_path", cFieldData, chr(3)) + 1, cFieldData, chr(3))
            _P.product_module_obj = decimal(entry(lookup("gsc_product_module.product_module_obj", cFieldData, chr(3)) + 1, cFieldData, chr(3)))
            _P.object_path        = entry(lookup("gsc_product_module.relative_path", cFieldData, chr(3)) + 1, cFieldData, chr(3))
            NO-ERROR.
