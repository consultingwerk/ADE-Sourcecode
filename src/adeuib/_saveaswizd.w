...
/*---------------------------------------------------------------------------------
  ...
  Description:  Save-As repository object wizard
...
&scop object-name       _saveaswizd.w
...
{defrescd.i}   /* Result code preprocessor def'n */
...
def var cColValues as character no-undo.
...
DO:
   ASSIGN BUFF_P._save-as-path        = IF toRelative:CHECKED THEN fiRelDirectory:SCREEN-VALUE ELSE ""
    ...
    cQuery = 'for each gsc_product_module where ':u
          + 'gsc_product_module.product_module_code = ':u + quoter(BUFF_P.product_module_code)
          + 'no-lock'.
    run getRecordDetail IN gshGenManager (INPUT cQuery,OUTPUT cColValues) NO-ERROR.
    if cColValues ne ? and cColValues ne '':u then
        assign BUFF_P._save-as-path      =         entry(lookup('gsc_product_module.relative_path', cColValues, chr(3)) + 1, cColValues, chr(3))        
               BUFF_P.product_module_obj = decimal(entry(lookup('gsc_product_module.product_module_obj', cColValues, chr(3)) + 1, cColValues, chr(3)))
               BUFF_P.object_path        =         entry(lookup('gsc_product_module.relative_path', cColValues, chr(3)) + 1, cColValues, chr(3))
               no-error.
END.
...
------------------------------------------------------------------------------*/
  ...
  def var hEntityCache as handle no-undo.
  ...
  DO:
    ...
    DO:
        ...
        hEntityCache = dynamic-function('getEntityCacheBuffer':u in gshGenManager,
                                  '':u,    /*entity mnemonic*/
                                  cFirstTable).
        hEntityCache:Find-first(' where ':u 
                                + hEntityCache:Name + '.entity_mnemonic_description = ':u
                                + quoter(cFirstTable)) no-error.
        if hEntityCache:Available then
            cFLA = hEntityCache::entity_mnemonic.
        else
            cFLA = '':u.
