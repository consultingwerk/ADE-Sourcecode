...
RUN adjustColumn.
IF RETURN-VALUE = 'DataField':U THEN 
DO:
  /* Delete the running instance of the datasource object */
...
DO iFieldNum = 1 TO NUM-ENTRIES(cFields):
  ...
  cTable = DYNAMIC-FUNCTION('ColumnPhysicalTable':U IN hDataSource, cListField) NO-ERROR.

  cObjectName = IF LOOKUP(cListField, cCalculatedCols) > 0 THEN cListField
  ...
  /* get inherited values of the class */
     ...
     IF _L._HEIGHT = 0 OR _L._WIDTH = 0 THEN
     DO:
       dFormatWidth = DYNAMIC-FUNCTION('getWidgetSizeFromFormat':U IN hRepDesignManager,
                                        INPUT _F._FORMAT,
                                        INPUT 'CHARACTER':U,
                                        OUTPUT dFormatHeight).
       IF _L._HEIGHT = 0 THEN _L._HEIGHT = dFormatHeight.
       IF _L._WIDTH  = 0 THEN _L._WIDTH  = dFormatWidth.
     END.  /* if height or width is 0 */
     IF _L._HEIGHT = 0 THEN _L._HEIGHT = 1.
     IF _L._WIDTH = 0 THEN _L._WIDTH = 10.
   ...
   /* This should never happen since the datafield was cached in adjustColumn, 
      but just in case... */
   ELSE DO:
     MESSAGE cObjectName +
             ' datafield master object cannot be found in the repository cache.':U 
              VIEW-AS ALERT-BOX ERROR.
     RETURN 'NO DRAW':U.
  END.  /* else do - object buffer not available */
...
/*------------------------------------------------------------------------------
  Purpose:     Reads the label for each field and adjusts _frmx to accomodate 
               these labels
  Parameters:  <none>
  Notes:       The is the first time the datafield master object is cached 
               on the client, if it does not exist, an error is given to the 
               user and fields are not created on the viewer.
------------------------------------------------------------------------------*/
...
DO iFieldNum = 1 TO NUM-ENTRIES(cFields):
  ...
  /* A calculated field in a static SDO may not have a calculatedfield master
     object in the repository yet so it is created here. */
  IF NOT lObjectExists AND lCalculatedCol THEN 
  DO:
    ASSIGN 
      cDataType = DYNAMIC-FUNCTION('columnDataType':U IN hDataSource, INPUT cObjectName)
      cLabel    = DYNAMIC-FUNCTION('columnLabel':U IN hDataSource, INPUT cObjectName)
      cFormat   = DYNAMIC-FUNCTION('columnFormat':U IN hDataSource, INPUT cObjectName)
      cHelp     = DYNAMIC-FUNCTION('columnHelp':U IN hDataSource, INPUT cObjectName).

    RUN generateCalculatedField IN hRepDesignManager
            (INPUT cObjectName,             /* calcFieldName   */
             INPUT cDataType,               /* Data Type       */
             INPUT cFormat,                 /* Format          */
             INPUT cLabel,                  /* Label           */
             INPUT cHelp,                   /* Help            */
    ...
    DO:
      MESSAGE 'A calculated field for ':U + cObjectName +
              ' could not be created.  ':U + RETURN-VALUE VIEW-AS ALERT-BOX.
      ASSIGN cErrorFields = cErrorFields + (IF NUM-ENTRIES(cErrorFields) > 0 THEN ',':U ELSE '':U) +
                            cListField.
      NEXT FieldLoop. 
    END.  /* if error */
    lObjectExists = TRUE.
  END.  /* if calculated field and master does not exsit in repository */

  IF NOT lObjectExists THEN
  DO:
    MESSAGE cObjectName +
            ' datafield has no master object in the repository.':U + CHR(10) +
            'Run Entity Import to create it.':U VIEW-AS ALERT-BOX ERROR.
    RETURN 'DataField':U.
  ...
  DO:
    MESSAGE cObjectName + ' has invalid attribute values in its Datafield master object.  ':U + CHR(10) +
            ...
            'include this datafield in a viewer.':U VIEW-AS ALERT-BOX.
    ASSIGN cErrorFields = cErrorFields + (IF NUM-ENTRIES(cErrorFields) > 0 THEN ',':U ELSE '':U) +
                          cListField.
    NEXT FieldLoop. 
  ...
  IF AVAIL ttObject THEN 
  DO:
     /* If it is a calculated column and the object inherits from calculatedfield class
        then it is a valid calculated field and we can read its attributes.  */
     IF (lCalculatedCol AND 
         DYNAMIC-FUNCTION("ClassIsA":U IN gshRepositoryManager,ttObject.tClassName,"CalculatedField":U))
        OR NOT lCalculatedCol THEN
     DO:
     ...
     ELSE DO:
       MESSAGE 'An object exists in the repository with the same name as ':U +
                cObjectName + 
               ' but it is not a Calculated Field.':U + CHR(10) +  
               'Rename the calculated field in the SmartDataObject ':U +
               'so that it has a unique object name.':U VIEW-AS ALERT-BOX ERROR.
       ASSIGN cErrorFields = cErrorFields + (IF NUM-ENTRIES(cErrorFields) > 0 THEN ',':U ELSE '':U) +
                             cListField.
       NEXT FieldLoop.
     END. /* else do */
...
------------------------------------------------------------------------------*/
 ...
 CASE pcAttribute :
     ...
     WHEN 'Data-Type':U            THEN 
       IF pcValue NE ? THEN _F._DATA-TYPE = pcValue.                                          
     ...
     WHEN 'LARGE':U                THEN _F._LARGE            = LOGICAL(pcValue).
