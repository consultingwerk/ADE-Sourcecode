...
DEFINE INPUT PARAMETER pcTempTableStatement         AS CHARACTER   NO-UNDO.    /* Define statement for temp-table or buffer */
...
/*NOTE:  Since the Data logic in adm2/logic.i can only support one table we need to ensure that 
         validation is generated for the first enabled table only. */
  ...
  THEN DO:
    ...
    THEN DO:
       RUN adecomm/_oscpath.p (pcrootFolder,OUTPUT iError).
       IF iError > 0 THEN
              RETURN ERROR "Unable to create folder:" +  pcRootFolder.
    END.    /* create the folder */
    ELSE
      RETURN ERROR "The folder does not exist: " + pcRootFolder.
  END.    /* folder does not exist */
  ...
  /* Now compile both logic procedure and proxy file */
  IF SEARCH(cFullyPathedProcedure) <> ? THEN
     COMPILE VALUE(cFullyPathedProcedure) SAVE.
  IF SEARCH(cClientProxyName) <> ? THEN
     COMPILE VALUE(cClientProxyName) SAVE.
...
------------------------------------------------------------------------------*/
  ...
  END.
  ELSE
    RETURN ERROR "Invalid Data Logic Procedure Template: " + pcLogicProcedureTemplate.
  ...
  FOR EACH ttTemplate NO-LOCK:
    ...
    /* If Temp-Table/buffer Statement is passed, add it at the beginning of the definition section */
    IF pcTempTableStatement > ""  THEN 
    DO:
      IF INDEX(cLineValue,"&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DEFINITIONS":U) > 0 THEN
      DO:
          cLineValue = cLineValue + CHR(10) + pcTempTableStatement.
    ...
    THEN DO:
      RUN scanLine (INPUT-OUTPUT cLineValue
                   ...
                   ,INPUT        ""
                   ,INPUT        YES
                   ,INPUT        "Description :  " + "Generated Data Logic Procedure for table :" + pcTableNameList 
    ...
    THEN DO:
      ...
      RUN scanLine (INPUT-OUTPUT cLineValue
                   ,INPUT        "ASSIGN cDescription = "
                   ,INPUT        "PLIP"
                   ,INPUT        YES
                   ,INPUT        'ASSIGN cDescription = "' +  ENTRY(1,pcTableNameList) + ' Data Logic Procedure'
    ...
    THEN DO:
     ...
     /* Define Table-Name and truncate Data-Logic-Table to maximum 28 characters - Issuezilla 2132 */
      ...
      DO:
         ASSIGN cLineValue = "&GLOB TABLE-NAME "  + cTableName.
         DYNAMIC-FUNCTION("addDataLogicProcedure", INPUT cLineValue).
         ASSIGN cLineValue = "&GLOB DATA-LOGIC-TABLE "  + SUBSTRING(cTableName,1,28).
         DYNAMIC-FUNCTION("addDataLogicProcedure", INPUT cLineValue).
      END.
      ELSE 
      DO:
         ASSIGN cLineValue = "&GLOB DATA-LOGIC-TABLE "  + cTableName.
         DYNAMIC-FUNCTION("addDataLogicProcedure", INPUT cLineValue).
      END.

      ASSIGN
        cLineValue = '&GLOB DATA-FIELD-DEFS  "' + pcDefinitionIncludeName + '"':U.
      DYNAMIC-FUNCTION("addDataLogicProcedure", INPUT cLineValue).
...
------------------------------------------------------------------------------*/
  ...
  DO iLoop = 1 TO NUM-ENTRIES(pcProcedureName,CHR(1)):
    ...
    IF (cFunction = "buildRowObjectValidate":U AND pcValidationFields > "") THEN 
    DO:
      ...
      REPEAT WHILE cIndexInformation <> ?:
        ...
        THEN DO:
           ...
           ASSIGN cClauseFields = SUBSTITUTE( "  IF CAN-FIND(FIRST &1 ":U
                                      ...
                                      + CHR(10)
                                      + "                         + ' A record with &2,'" + cValueList + " already exists."
                                      + CHR(10)
                                      + "  END."
                                      + CHR(10)
                                      ,cTable
