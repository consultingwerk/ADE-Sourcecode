...
/* -----------------------------------------------------------------
  ...
  REPEAT:
    ...
    IF _inp_line[1] eq "END" and _inp_line[2] eq "CASE."
    THEN LEAVE MAIN-REPEAT.
    
    /* Look for a new page number.  If the page number is "?" then copy to
       the current page. */
    ...
    IF _inp_line[1] eq "RUN" AND
      ...
      IF _inp_line[1] eq "INPUT" THEN DO:
        /* Strip quotes off the file-name 'string'.
           Strip off DB-AWARE flag if there ENTRY(1,..,CHR(3)).
    ...
    THEN DO:
      ...
      IF AVAILABLE _U AND AVAILABLE anchor_U THEN DO:
        /* We're all set let adjust the tabs */
