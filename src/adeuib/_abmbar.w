...
/*------------------------------------------------------------------------------
  ...
  Parameters:  pcDesignField AS CHARACTER 
               ...
               pcOriginalValues AS CHARACTER
                 CHR(3)-delimited list of the original values for the modified attributes
                 
  Notes:       When Edit DataField master is invoked for a datafield of a 
               dynamic viewer, _h_menubar_proc is subscribed to the 
               DataFieldMasterChanged event in the datafield maintenance viewer.
------------------------------------------------------------------------------*/
...
DEFINE INPUT  PARAMETER pcOriginalValues AS CHARACTER  NO-UNDO.
...
DEFINE VARIABLE lListItems     AS LOGICAL    NO-UNDO.
  ...
  DO iNumAttrs = 1 TO NUM-ENTRIES(pcUpdatedAttrs):
    ...
    ASSIGN
      cUpdatedValue  = ENTRY(iNumAttrs, pcUpdatedValues, CHR(3))  
      cOriginalValue = ENTRY(iNumAttrs, pcOriginalValues, CHR(3)) NO-ERROR.
    IF NOT ERROR-STATUS:ERROR THEN
    DO:
  
      CASE cAttribute:
        WHEN 'AUTO-COMPLETION':U      THEN IF _F._AUTO-COMPLETION = LOGICAL(cOriginalValue) THEN
                                              _F._AUTO-COMPLETION = LOGICAL(cUpdatedValue).
        WHEN 'AUTO-INDENT':U          THEN IF _F._AUTO-INDENT = LOGICAL(cOriginalValue) THEN
                                              _F._AUTO-INDENT = LOGICAL(cUpdatedValue).
        WHEN 'AUTO-RESIZE':U          THEN IF _F._AUTO-RESIZE = LOGICAL(cOriginalValue) THEN
                                              _F._AUTO-RESIZE = LOGICAL(cUpdatedValue).
        WHEN 'AUTO-RETURN':U          THEN IF _F._AUTO-RETURN = LOGICAL(cOriginalValue) THEN
                                              _F._AUTO-RETURN = LOGICAL(cUpdatedValue).
        WHEN 'BGCOLOR':U              THEN IF _L._BGCOLOR = INTEGER(cOriginalValue) THEN
                                              _L._BGCOLOR = INTEGER(cUpdatedValue).
        WHEN 'BLANK':U                THEN IF _F._BLANK = LOGICAL(cOriginalValue) THEN
                                              _F._BLANK = LOGICAL(cUpdatedValue).
        WHEN 'BOX':U                  THEN IF _L._NO-BOX NE LOGICAL(cOriginalValue) THEN
                                              _L._NO-BOX = NOT LOGICAL(cUpdatedValue).
        WHEN 'CONTEXT-HELP-ID':U      THEN IF _U._CONTEXT-HELP-ID = INTEGER(cOriginalValue) THEN
                                              _U._CONTEXT-HELP-ID = INTEGER(cUpdatedValue).
        WHEN 'Data-Type':U            THEN IF cUpdatedValue NE ? AND 
                                              _F._DATA-TYPE = cOriginalValue THEN
                                              _F._DATA-TYPE = cUpdatedValue.
        WHEN 'DEBLANK':U              THEN IF _F._DEBLANK = LOGICAL(cOriginalValue) THEN
                                              _F._DEBLANK = LOGICAL(cUpdatedValue).
        WHEN 'DELIMITER':U            THEN IF _F._DELIMITER = cOriginalValue THEN
                                              _F._DELIMITER = cUpdatedValue.
        WHEN 'DISABLE-AUTO-ZAP':U     THEN IF _F._DISABLE-AUTO-ZAP = LOGICAL(cOriginalValue) THEN
                                              _F._DISABLE-AUTO-ZAP = LOGICAL(cUpdatedValue).
        WHEN 'DisplayField':U         THEN IF _U._DISPLAY = LOGICAL(cOriginalValue) THEN 
                                              _U._DISPLAY = LOGICAL(cUpdatedValue).
        WHEN 'DRAG-ENABLED':U         THEN IF _F._DRAG-ENABLED = LOGICAL(cOriginalValue) THEN 
                                              _F._DRAG-ENABLED = LOGICAL(cUpdatedValue).
        WHEN 'DROP-TARGET':U          THEN IF _U._DROP-TARGET = LOGICAL(cOriginalValue) THEN 
                                              _U._DROP-TARGET = LOGICAL(cUpdatedValue).
        WHEN 'Enabled':U              THEN IF _U._ENABLE = LOGICAL(cOriginalValue) THEN 
                                              _U._ENABLE = LOGICAL(cUpdatedValue).
        WHEN 'EXPAND':U               THEN IF _F._EXPAND = LOGICAL(cOriginalValue) THEN 
                                              _F._EXPAND = LOGICAL(cUpdatedValue).
        WHEN 'FGCOLOR':U              THEN IF _L._FGCOLOR = INTEGER(cOriginalValue) THEN
                                              _L._FGCOLOR = INTEGER(cUpdatedValue).
        WHEN 'FONT':U                 THEN IF _L._FONT = INTEGER(cOriginalValue) THEN
                                              _L._FONT = INTEGER(cUpdatedValue).
        WHEN 'FORMAT':U               THEN IF _F._FORMAT = cOriginalValue THEN
                                              _F._FORMAT = cUpdatedValue.
        WHEN 'HEIGHT-CHARS':U         THEN IF _L._HEIGHT = DECIMAL(cOriginalValue) THEN
                                              _L._HEIGHT = DECIMAL(cUpdatedValue).
        WHEN 'HELP':U                 THEN IF _U._HELP = cOriginalValue THEN
                                              _U._HELP = cUpdatedValue.
        WHEN 'Horizontal':U           THEN IF _F._HORIZONTAL = LOGICAL(cOriginalValue) THEN 
                                              _F._HORIZONTAL = LOGICAL(cUpdatedValue).
        WHEN 'InitialValue':U         THEN IF _F._INITIAL-DATA = cOriginalValue THEN
                                              _F._INITIAL-DATA = cUpdatedValue.
        WHEN 'INNER-LINES':U          THEN IF _F._INNER-LINES = INTEGER(cOriginalValue) THEN
                                              _F._INNER-LINES = INTEGER(cUpdatedValue).
        WHEN 'LABEL':U                THEN IF _U._LABEL = cOriginalValue THEN
                                              _U._LABEL = cUpdatedValue.
        WHEN 'LABELS':U               THEN IF _L._NO-LABEL NE LOGICAL(cOriginalValue) THEN
                                              _L._NO-LABEL = NOT LOGICAL(cUpdatedValue).
        WHEN 'LARGE':U                THEN IF _F._LARGE = LOGICAL(cOriginalValue) THEN 
                                              _F._LARGE = LOGICAL(cUpdatedValue). 
        WHEN 'LIST-ITEM-PAIRS'        THEN 
        DO:
          cTmp = REPLACE(_F._LIST-ITEM-PAIRS, CHR(10), '':U). 
          IF cTmp = cOriginalValue THEN
          DO:
        ...
        END.  /* when list-items */
        WHEN 'MAX-CHARS':U            THEN IF _F._MAX-CHARS = INTEGER(cOriginalValue) THEN
                                              _F._MAX-CHARS = INTEGER(cUpdatedValue).
        WHEN 'PRIVATE-DATA':U         THEN IF _U._PRIVATE-DATA = cOriginalValue THEN
                                              _U._PRIVATE-DATA = cUpdatedValue.
        ...
        DO:
          ...
          END.
          IF cTmp = cOriginalValue THEN _F._LIST-ITEMS = cUpdatedValue.
        ...
        WHEN 'READ-ONLY':U            THEN IF _F._READ-ONLY = LOGICAL(cOriginalValue) THEN 
                                              _F._READ-ONLY = LOGICAL(cUpdatedValue).   
        WHEN 'RETURN-INSERTED':U      THEN IF _F._RETURN-INSERTED = LOGICAL(cOriginalValue) THEN 
                                              _F._RETURN-INSERTED = LOGICAL(cUpdatedValue).   
        WHEN 'SCROLLBAR-HORIZONTAL':U THEN IF _F._SCROLLBAR-H = LOGICAL(cOriginalValue) THEN 
                                              _F._SCROLLBAR-H = LOGICAL(cUpdatedValue).   
        WHEN 'SCROLLBAR-VERTICAL':U   THEN IF _U._SCROLLBAR-V = LOGICAL(cOriginalValue) THEN 
                                              _U._SCROLLBAR-V = LOGICAL(cUpdatedValue).    
        WHEN 'ShowPopup':U            THEN IF _U._SHOW-POPUP = LOGICAL(cOriginalValue) THEN 
                                              _U._SHOW-POPUP = LOGICAL(cUpdatedValue).    
        WHEN 'SORT':U                 THEN IF _F._SORT = LOGICAL(cOriginalValue) THEN 
                                              _F._SORT = LOGICAL(cUpdatedValue).        
        WHEN 'SubType':U              THEN IF _U._SUBTYPE = cOriginalValue THEN
                                              _U._SUBTYPE = cUpdatedValue.
        WHEN 'TAB-STOP':U             THEN IF _U._NO-TAB-STOP NE LOGICAL(cOriginalValue) THEN
                                              _U._NO-TAB-STOP = NOT LOGICAL(cUpdatedValue). 
        WHEN 'TOOLTIP':U              THEN IF _U._TOOLTIP = cOriginalValue THEN
                                              _U._TOOLTIP = cUpdatedValue. 
        WHEN 'WIDTH-CHARS':U          THEN IF _L._WIDTH = DECIMAL(cOriginalValue) THEN
                                              _L._WIDTH = DECIMAL(cUpdatedValue).
        WHEN 'VisualizationType':U    THEN IF _U._TYPE = cOriginalValue THEN 
        DO:
        ...
        WHEN 'Visible':U              THEN IF _U._HIDDEN NE LOGICAL(cOriginalValue) THEN
                                              _U._HIDDEN = NOT LOGICAL(cUpdatedValue). 
      END CASE.
...
FOR EACH ttObject:    
   ...
   /* MASTER OBJECT */
   IF ttObject.tLogicalObjectName EQ B_P.OBJECT_FILENAME THEN
...
DO:
   CASE f_U._TYPE:
      WHEN "BROWSE":U THEN
        IF cObjectClassName <> "DynBrow":U THEN
           cNewObjectClass= "DynBrow":U.
      WHEN "BUTTON":U THEN
        IF cObjectClassName <> "DynButton":U THEN
           cNewObjectClass= "DynButton":U.
      WHEN "COMBO-BOX":U THEN
        IF cObjectClassName <> "DynComboBox":U THEN
           cNewObjectClass= "DynComboBox":U.
      WHEN "EDITOR":U THEN
         IF cObjectClassName <> "DynEditor":U THEN
            cNewObjectClass= "DynEditor":U.
      WHEN "FILL-IN":U THEN
         IF cObjectClassName <> "DynFillIn":U THEN
            cNewObjectClass= "DynFillIn":U.
      WHEN "IMAGE":U THEN
         IF cObjectClassName <> "DynImage":U THEN
            cNewObjectClass= "DynImage":U.
      WHEN "RADIO-SET":U THEN
         IF cObjectClassName <> "DynRadioSet":U THEN
            cNewObjectClass= "DynRadioSet":U.
      WHEN "RECTANGLE":U THEN
         IF cObjectClassName <> "DynRectangle":U THEN
            cNewObjectClass= "DynRectangle":U.
      WHEN "TEXT":U THEN
         IF cObjectClassName <> "DynText":U THEN
            cNewObjectClass = "DynText":U.
      WHEN "TOGGLE-BOX":U THEN
        IF cObjectClassName <> "DynToggle":U THEN
           cNewObjectClass = "DynToggle":U.
      WHEN "SELECTION-LIST":U THEN
        IF cObjectClassName <> "DynSelection":U THEN
