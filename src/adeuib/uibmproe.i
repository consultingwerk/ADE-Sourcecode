...
PROCEDURE editMasterDataField:
  ...
  DO iCnt = 1 TO NUM-ENTRIES(cTargets):
      ...
      IF VALID-HANDLE(hViewer) AND
         LOOKUP("setParentInfo":U, hViewer:INTERNAL-ENTRIES) > 0 THEN
      DO:
        RUN setParentInfo IN hViewer (INPUT _U._TABLE,
                                      INPUT _U._OBJECT-NAME).
        /* Set ABDesignField property to data that will be passed to
           DataFieldMasterChanged and used to find the field to 
           update it based on modifications made to datafield master 
           object.  The DataField maintenance viewer publishes 
           DataFieldMasterChanged when changes are saved. */
        DYNAMIC-FUNCTION("setUserProperty":U IN hViewer,
                         INPUT "ABDesignField":U,
                         INPUT _U._NAME + "|":U +
                               _U._TYPE + "|":U +
                               STRING(_U._WINDOW-HANDLE)).
        SUBSCRIBE PROCEDURE _h_menubar_proc 
          TO "DataFieldMasterChanged":U IN hViewer.
      END.
  END.
...
/* Note this is the historical name.  This will also identify if a window is
   to be converted to a dynamic window and save it accordingly           */
    ...
    ELSE DO ON ERROR UNDO, LEAVE:
      ...
      END.                  
      ELSE DO:  
         IF _P.object_type_code = "SmartDataObject":U THEN 
         ...
         RUN insertObjectMaster IN ghRepositoryDesignManager 
            ( INPUT  _P.object_filename,     /* pcObjectName         */
              ...
              INPUT  _P._TYPE,               /* pcObjectTypeCode     */
         ...
         IF ERROR-STATUS:ERROR AND NOT pError MATCHES "*directory*":U THEN 
         DO:
