  ...
  DO WITH FRAME f:
    ...
    ELSE DO:
      ...
      CASE tt.type:
        ...
        DO: 
          IF tt.type = "TYPE":U THEN
              valid-list = valid-types.
          ELSE DO:
              valid-list = valid-dtypes.
              IF AVAILABLE _F THEN
                  FIND _U WHERE _U._x-recid = RECID(_F) NO-LOCK NO-ERROR.
              IF AVAILABLE _U THEN
                  CASE _U._TYPE:
                      WHEN "EDITOR":U  THEN valid-list = valid-dtypes-editor.
                      WHEN "FILL-IN":U THEN valid-list = valid-dtypes-fillin.
                  END CASE.
          END.
          ASSIGN cb-options:SCREEN-VALUE = ?
                 cb-options:LIST-ITEMS   = valid-list
                 cb-options:SCREEN-VALUE = tt.attr-value
                 cb-options:HIDDEN       = FALSE
                 fi-integer:HIDDEN       = TRUE
                 fi-decimal:HIDDEN       = TRUE
      ...
      ELSE IF tt.attr-name eq "TYPE":U THEN DO:
        ...
        ELSE DO:
          IF _F._DATA-TYPE = "LongChar" THEN
              valid-types = "Editor".
          ELSE
              valid-types = (IF _F._DATA-TYPE BEGINS "DATETIME"
                             THEN "Fill-in":U ELSE "Combo-Box,Fill-in":U) +
