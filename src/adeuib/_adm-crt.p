...
DO:
 ...
 /* Generate tab order code */
  /* Temporarily make non-visible SO's negative tab-order */
  FOR EACH x_U WHERE x_U._WINDOW-HANDLE eq _U._HANDLE
                   AND x_U._TYPE eq "SmartObject"
                   AND x_U._STATUS eq p_Status,
          EACH x_S WHERE RECID(x_S) eq x_U._x-recid AND
                         x_S._page-number = 0 AND NOT x_S._VISUAL:
     IF x_U._TAB-ORDER > 0 THEN x_U._TAB-ORDER = (0 - x_U._TAB-ORDER).
  END.                    

  /* Temporarily make no-exported SO's negative tab-order */
  IF p_Status = "EXPORT":U THEN
  DO:
     /* Now restore the non-exported SO's tab-orders */
     FOR EACH x_U WHERE x_U._WINDOW-HANDLE eq _U._HANDLE
                      AND x_U._TYPE eq "SmartObject"
                      AND x_U._STATUS <> "EXPORT":U,
             EACH x_S WHERE RECID(x_S) eq x_U._x-recid AND
                            x_S._page-number = 0:
        IF x_U._TAB-ORDER > 0 THEN x_U._TAB-ORDER = (0 - x_U._TAB-ORDER).
     END.                    
  ...
  tab-order-code = "".
  IF this-page = 0 THEN DO:
    FOR EACH x_U WHERE x_U._WINDOW-HANDLE eq _U._HANDLE
                   AND x_U._TYPE eq "SmartObject"
                   AND x_U._STATUS eq p_Status,
          EACH x_S WHERE RECID(x_S) eq x_U._x-recid AND
                         x_S._page-number = 0 AND x_S._VISUAL
          BY x_U._tab-order:
       IF x_U._tab-order > 1 THEN DO:  /* This is not first, move after something */
         DO PRESELECT EACH xx_U WHERE
                      xx_U._WINDOW-HANDLE eq _U._HANDLE AND
                      xx_U._PARENT-RECID eq x_U._PARENT-RECID AND
            ...
            FIND LAST xx_U WHERE xx_U._WINDOW-HANDLE eq _U._HANDLE AND
                                 ...
                                 xx_U._TAB-ORDER < x_U._TAB-ORDER NO-ERROR.
         END. /* End preselect */
         IF AVAILABLE xx_U THEN DO:
           ...
           FIND FIRST xx_S WHERE RECID(xx_S) eq xx_U._x-recid AND
                                 xx_S._page-number = 0 NO-ERROR.
         END.  /* IF AVAIABLE xx_U */
       END.  /* Not first in the tab order */
       IF x_U._tab-order = 1 OR NOT AVAILABLE xx_U THEN DO:  /* move before something */
         DO PRESELECT EACH xx_U WHERE
                      xx_U._WINDOW-HANDLE eq _U._HANDLE AND
                      xx_U._PARENT-RECID eq x_U._PARENT-RECID AND
                      RECID(xx_U) NE xx_U._PARENT-RECID AND
       ...
       /* At this point if we have an xx_U record. It is the anchor that x_U
       ...
       IF AVAILABLE xx_U AND NOT xx_U._NO-TAB-STOP THEN DO:
         ...
         IF NOT _L._NO-FOCUS THEN DO:
           tab-order-code = tab-order-code + {&EOL} +
             ...
             ELSE
             "       RUN adjustTabOrder ( ") + x_U._NAME + " ,":U.
    ...
    END.  /* For each x_U smartobject */
  END.  /* Only do this for page 0 */
