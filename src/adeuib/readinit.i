...
ASSIGN _L._FGCOLOR        = IF {&AC_color-mode} = "7"   THEN INTEGER({&AC_FGcolor})
       ...
       _U._TABLE          = {&AC_table}
       _U._BUFFER         = IF {&AC_buffer} = ? THEN _U._TABLE
                                                ELSE {&AC_buffer}
       _U._LABEL-SOURCE   = IF _U._DBNAME = ? THEN "E" ELSE "D".
IF {&AC_buffer} NE ? THEN _NAME-REC._wTABLE = {&AC_buffer}.
...
ASSIGN _U._HELP           = IF {&AC_help}    ne ? THEN {&AC_help}    ELSE "" 
