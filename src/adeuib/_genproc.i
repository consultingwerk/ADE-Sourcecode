    ...
    DO:
      ...
      IF NOT skip_queries THEN DO:
         ...
         */
         ...
         /* P_4GLSDO is the SDO include file, P_4GL is the SDO object */
         ...
         IF ipC._Rowobject-NO-UNDO THEN 
           PUT STREAM P_4GL UNFORMATTED
             '&Scoped-Define DATA-TABLE-NO-UNDO NO-UNDO' SKIP.
              ...
              FOR EACH _TT WHERE _TT._p-recid = RECID(_P) :  
                 CASE _TT._TABLE-TYPE:        
                    ...
                    DO:
                        ...
                        DO:
                           RUN adeuib/_tempdbvalid.p (OUTPUT lvalid). /* Check that control file is present in TEMP-DB */
                           IF lvalid THEN
                           DO:
              ...
              DO:
                ...
                RUN adeuib/_gendatalog.p          
                   (INPUT ipC._DATA-LOGIC-PROC,           /* Rel pathed file name  */
                    ...
                    ipC._Rowobject-NO-UNDO                 /* Use NO-UNDO for RowObject */
                    ).
              END.
           END. 
...
PROCEDURE put_win_preproc_vars:
  ...
  ASSIGN tmp_line[{&MaxUserLists} + 1] = "&Scoped-Define ENABLED-FIELDS "
         ...
         tmp_line[{&MaxUserLists} + 5] = "&Scoped-Define ADM-DATAFIELD-MAPPING "
         .
  ...
  FOR EACH ipU WHERE ipU._STATUS ne "DELETED":U 
    ...
    DO i = 1 TO list-cnt:
      ...
      /* Displayed objects */
      ELSE IF i eq {&MaxUserLists} + 4 THEN
        add2list = (ipU._parent-recid eq frm-recid) AND 
      ...
      /* ADM Datafield mapping */
      ELSE
        add2list = ipU._parent-recid EQ frm-recid AND
                   ipU._LOCAL-NAME NE "":U AND
                   _F._SOURCE-DATA-TYPE = "CLOB":U.
                   
      IF add2list THEN DO:
        emptyList[i] = NO.
        /* ADM Datafield mapping - list of local fields and their mapping to 
           data fields, format is:
           localname,buffer.fieldname[,...] */
        IF i EQ {&MaxUserLists} + 5 THEN
        DO:
          IF LENGTH(tmp_line [i] + ipU._LOCAL-NAME + ",":U + ipU._BUFFER + ".":U + ipU._NAME + ",":U, "CHARACTER":U) > 75 THEN DO:
            tmp_code[i] = tmp_code[i] + tmp_line[i] + "~~" + CHR(10).
            tmp_line[i] = "".
          END.
          tmp_line[i] = tmp_line[i] + ipU._LOCAL-NAME + ",":U + ipU._BUFFER + ".":U + ipU._NAME + ",":U.
        END.
  ...
  IF frame_name_f ne "" AND ((emptyList[{&MaxUserLists} + 1] eq NO) OR 
                            ...
                            (emptyList[{&MaxUserLists} + 4] eq NO) OR
                            (emptyList[{&MaxUserLists} + 5] EQ NO))
  THEN DO:
    ...
    DO i = {&MaxUserLists} + 1 TO {&MaxLists}:
      IF NOT emptyList[i] THEN DO:
        IF i EQ {&MaxUserLists} + 5 THEN 
          tmp_code[i] = RIGHT-TRIM(tmp_code[i], ",":U).
