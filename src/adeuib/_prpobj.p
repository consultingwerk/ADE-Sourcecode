...
PROCEDURE complete_the_transaction:
  DO WITH FRAME prop_sht:
    ...
    IF h_self:TYPE = "FRAME":U AND _L._WIN-TYPE THEN
      ASSIGN h_self:BGCOLOR = _L._BGCOLOR.

    IF _U._TYPE = "FILL-IN":U THEN
      h_self:BGCOLOR = IF _L._WIN-TYPE THEN (IF _L._BGCOLOR = ? THEN 15 ELSE _L._BGCOLOR)
                                       ELSE _tty_bgcolor.
