...
DO:
  RUN adecomm/_relname.p (f_C._DATA-LOGIC-PROC, "",OUTPUT cTmp).
  IF AVAILABLE f_C AND hBuffer:BUFFER-FIELD("setValue":U):BUFFER-VALUE <> cTmp THEN
...
DO:
  ...
  END.  /* if not dynsdo */
  ELSE IF AVAILABLE q_U AND hBuffer:BUFFER-FIELD("setValue":U):BUFFER-VALUE = ? AND cSDOLabel NE "":U THEN
       cAttribute = cAttribute + CHR(3) + cLabel + CHR(3) + cResultCode + CHR(3) + cSDOLabel.  
END.  /* when label */
...
WHEN "SuperProcedure":U    THEN 
   IF AVAILABLE f_C AND hBuffer:BUFFER-FIELD("setValue":U):BUFFER-VALUE <> f_C._CUSTOM-SUPER-PROC  THEN
     cAttribute = cAttribute + CHR(3) + cLabel + CHR(3) + cResultCode + CHR(3) 
                             + IF f_C._CUSTOM-SUPER-PROC = ? THEN "" ELSE f_C._CUSTOM-SUPER-PROC.  
...
WHEN "WindowTitleField":U  THEN
  IF (DYNAMIC-FUNCTION("ClassIsA" IN gshRepositoryManager, _P.object_type_code, "DynView":U)
     ...
     AND AVAILABLE f_C AND hBuffer:BUFFER-FIELD("setValue":U):BUFFER-VALUE <> f_C._WINDOW-TITLE-FIELD AND f_C._WINDOW-TITLE-FIELD <> ? THEN
