...
Description: Undelete a SLIDER widget.
...
Modified by gfs on 03/08/96 Added support for Win95 slider
----------------------------------------------------------------------------*/
...
IF NOT _L._WIN-TYPE THEN 
 _L._HEIGHT = IF NOT _F._HORIZONTAL THEN MAX(2,INTEGER(_L._HEIGHT)) ELSE 2.
IF NOT _L._WIN-TYPE AND NOT _F._HORIZONTAL THEN _L._WIDTH = MAX(9,INTEGER(_L._WIDTH)).
...
CASE _F._TIC-MARKS:
  WHEN "TOP/LEFT" THEN
    IF _F._HORIZONTAL THEN _U._HANDLE:TIC-MARKS = "TOP".
    ELSE _U._HANDLE:TIC-MARKS = "LEFT".
  WHEN "BOTTOM/RIGHT" THEN
    IF _F._HORIZONTAL THEN _U._HANDLE:TIC-MARKS = "BOTTOM".
    ELSE _U._HANDLE:TIC-MARKS = "RIGHT".
  OTHERWISE _U._HANDLE:TIC-MARKS = _F._TIC-MARKS.
END CASE.                                                       
...
IF NOT _L._NO-LABELS THEN DO:  /* Note: we allow labels for dynamic sliders */
